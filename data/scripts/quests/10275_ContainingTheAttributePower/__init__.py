...
class Quest(JQuest):
    ...
    def onKill(self, npc, player, isPet):
        ...
        npcId = npc.getNpcId()
        if npcId == Air:
            if st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == YangSword and st.getInt("cond") in [8,
                                                                                                    10] and st.getQuestItemsCount(
                SoulPieceAir) < 6 and st.getRandom(100) < 30:
            ...
            if st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == YinSword and st.getInt("cond") in [3,
                                                                                                   5] and st.getQuestItemsCount(
                SoulPieceWater) < 6 and st.getRandom(100) < 30:
