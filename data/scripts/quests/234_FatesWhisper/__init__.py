...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        # if quest is accepted and in progress
            ...
            if npcId == NPC[0]:
                ...
                elif cond == 2 and (st.getQuestItemsCount(KERMONS_INFERNIUM_SCEPTER) + st.getQuestItemsCount(
                        GOLCONDAS_INFERNIUM_SCEPTER) + st.getQuestItemsCount(HALLATES_INFERNIUM_SCEPTER) < 3):
                ...
                # all is ready.  Now give a menu to trade the B weapon for the player's choice of A Weapon.
                    ...
                    if st.getInt("bypass"):  # weapon is set
                        if st.getQuestItemsCount(st.getInt("weaponId")) > 0:  # have weapon
                            htmltext = st.showHtmlFile("31002-AGradeList.htm").replace("%weaponname%",
                                                                                       Weapons[st.getInt("weaponId")])
                        else:  # dont have
                            htmltext = st.showHtmlFile("31002-15.htm").replace("%weaponname%",
                                                                               Weapons[st.getInt("weaponId")])
    ...
    def onAttack(self, npc, player, damage, isPet, skill):
        ...
        if st.getInt("cond") == 7 and npc.getNpcId() == 29020:
            if player.getActiveWeaponItem() and player.getActiveWeaponItem().getItemId() == PIPETTE_KNIFE and st.getQuestItemsCount(
                    RED_PIPETTE_KNIFE) == 0:
