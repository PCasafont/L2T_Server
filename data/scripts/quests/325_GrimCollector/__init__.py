...
def payback(st):
    ...
    amount = 0
    if count:
        amount = 30 * st.getQuestItemsCount(ZOMBIE_HEAD1_ID) + 20 * st.getQuestItemsCount(
            ZOMBIE_HEART1_ID) + 20 * st.getQuestItemsCount(ZOMBIE_LIVER1_ID) + 100 * st.getQuestItemsCount(
            SKULL1_ID) + 40 * st.getQuestItemsCount(RIB_BONE1_ID) + 14 * st.getQuestItemsCount(
            SPINE1_ID) + 14 * st.getQuestItemsCount(ARM_BONE1_ID) + 14 * st.getQuestItemsCount(
            THIGH_BONE1_ID) + 341 * st.getQuestItemsCount(COMPLETE_SKELETON_ID)
        if count > 10:
            amount += 1629
...
class Quest(JQuest):
    def __init__(self, id, name, descr):
        ...
        self.questItemIds = [ZOMBIE_HEAD1_ID, ZOMBIE_HEART1_ID, ZOMBIE_LIVER1_ID, SKULL1_ID, RIB_BONE1_ID, SPINE1_ID,
                             ARM_BONE1_ID, THIGH_BONE1_ID, COMPLETE_SKELETON_ID, ANATOMY_DIAGRAM_ID]
    ...
    def onEvent(self, event, st):
        ...
        if event == "30336-03.htm":
            ...
            if st.getQuestItemsCount(SPINE1_ID) and st.getQuestItemsCount(ARM_BONE1_ID) and st.getQuestItemsCount(
                    SKULL1_ID) and st.getQuestItemsCount(RIB_BONE1_ID) and st.getQuestItemsCount(THIGH_BONE1_ID):
                st.takeItems(SPINE1_ID, 1)
                st.takeItems(SKULL1_ID, 1)
                st.takeItems(ARM_BONE1_ID, 1)
                st.takeItems(RIB_BONE1_ID, 1)
    ...
    def onTalk(self, npc, player):
        ...
        elif npcId == 30434 and cond and st.getQuestItemsCount(ANATOMY_DIAGRAM_ID) and pieces(
                st) and not st.getQuestItemsCount(COMPLETE_SKELETON_ID):
            htmltext = "30434-05.htm"
        elif npcId == 30434 and cond and st.getQuestItemsCount(ANATOMY_DIAGRAM_ID) and pieces(
                st) and st.getQuestItemsCount(COMPLETE_SKELETON_ID):
