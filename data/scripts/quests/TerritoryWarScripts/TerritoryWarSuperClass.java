...
{
	private static Map<Integer, TerritoryWarSuperClass> _forTheSakeScripts = new HashMap<Integer, TerritoryWarSuperClass>();
	private static Map<Integer, TerritoryWarSuperClass> _protectTheScripts = new HashMap<Integer, TerritoryWarSuperClass>();
	private static Map<Integer, TerritoryWarSuperClass> _killTheScripts = new HashMap<Integer, TerritoryWarSuperClass>();
	...
	public String onSkillSee(L2Npc npc, L2PcInstance caster, L2Skill skill, L2Object[] targets, boolean isPet)
	{
		if (Util.contains(targets, npc))
		{
			...
			{
				if (TerritoryWarManager.getInstance().getTerritory(caster.getSiegeSide() - 80).getOwnedWardIds().size() >= 3)
				{
					caster.sendMessage("Your territory owns 3 wards already!");
					return super.onSkillSee(npc, caster, skill, targets, isPet);
				}
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (event.equalsIgnoreCase("setNextTWDate"))
		{
			...
			Log.info("Next TerritoryWarTime: " + startTWDate.getTime());
		}
	...
	public String onEnterWorld(L2PcInstance player)
	{
		...
		if (territoryId > 0)
		{
			...
			{
				TerritoryWarSuperClass killthe = _killTheScripts.get(player.getCurrentClass().getId());
				if (killthe != null)
				{
					...
					Log.warning("TerritoryWar: Missing Kill the quest for player " + player.getName() + " whose class id: " + player.getCurrentClass().getId());
			}
		}
		return null;
	}
