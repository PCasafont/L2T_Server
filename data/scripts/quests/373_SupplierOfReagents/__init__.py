...
FORMULAS = {
    ...
    NECROPLASM: [ROTTEN_BONE, 10, BLOOD_ROOT, 1], DEMONPLASM: [DEMONS_BLOOD, 10, BLOOD_ROOT, 1],
    ...
    MIDNIGHT_OIL: [NECROPLASM, 10, QUICKSILVER, 1], DEMONIC_ESSENCE: [DEMONPLASM, 10, SULFUR, 1],
...
def render_urn(st, page):
    ...
    if page == "Start":
        ...
        if ingr:
            ingr = ITEMS[ingr][1] + "x" + st.get("i_qty")
        else:
            ingr = "None"
        if cata:
            cata = ITEMS[cata][1] + "x" + st.get("c_qty")
        else:
            cata = "None"
        ...
        for i in MATS:
            if st.getQuestItemsCount(i):
                amt += 1
                html += "<tr><td height=45><img src=icon." + ITEMS[i][0] + " height=32 width=32></td><td width=180>" + \
                        ITEMS[i][1] + "</td><td><button value=X1 action=\"bypass -h Quest 373_SupplierOfReagents x_1_" + \
                        page[1] + "_" + str(
                    i) + "\" width=40 height=15 fore=sek.cbui92><button value=X10 action=\"bypass -h Quest 373_SupplierOfReagents x_2_" + \
                        page[1] + "_" + str(i) + "\" width=40 height=15 fore=sek.cbui92></td></tr>"
    ...
    def onEvent(self, event, st):
        ...
        if event == "30166-4.htm":
            ...
            if st.getInt("mixing"):
                temp = st.getInt("temp")
                if temp:
                    ingredient, catalyst, iq, cq = st.getInt("ingredient"), st.getInt("catalyst"), st.getInt(
                        "i_qty"), st.getInt("c_qty")
                    st.set("ingredient", "0")
                    st.set("i_qty", "0")
                    st.set("catalyst", "0")
                    st.set("c_qty", "0")
