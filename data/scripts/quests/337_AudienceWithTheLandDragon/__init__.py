    ...
    def onTalk(self, npc, player):
        ...
        if npcId == HELTON:
            if st.getInt("all") == 2:
                ...
                if st.getQuestItemsCount(MUSFEL_FANG) == 0 or st.getQuestItemsCount(
                        SECOND_FRAGMENT_OF_ABYSS_JEWEL) == 0:
        ...
        if st:
            ...
            if cond == 1:
                if npcId == HAMRUT and st.getQuestItemsCount(HAMRUT_LEG) == 0 and st.getInt("chakiris") == 0:
                    ...
                    st.playSound("ItemSound.quest_itemget")
                elif npcId == MARSH_STALKER and st.getQuestItemsCount(MARSH_STALKER_HORN) == 0 and st.getInt(
                        "kaiena") == 0:
                    st.giveItems(MARSH_STALKER_HORN, 1)
                    st.playSound("ItemSound.quest_itemget")
                elif npcId == MARSH_DRAKE and st.getQuestItemsCount(MARSH_DRAKE_TALONS) == 0 and st.getInt(
                        "kaiena") == 0:
                    st.giveItems(MARSH_DRAKE_TALONS, 1)
                    st.playSound("ItemSound.quest_itemget")
                elif npcId in (BLOODY_QUEEN, BLOODY_QUEEN2) and st.getQuestItemsCount(
                        REMAINS_OF_SACRIFICED) == 0 and st.getInt("orven") == 0:
                    for i in range(8):
                        st.addSpawn(SACRIFICE_OF_THE_SACRIFICED, 180000)
                elif npcId == SACRIFICE_OF_THE_SACRIFICED and st.getQuestItemsCount(
                        REMAINS_OF_SACRIFICED) == 0 and st.getInt("orven") == 0:
                    st.giveItems(REMAINS_OF_SACRIFICED, 1)
                    st.playSound("ItemSound.quest_itemget")
                elif npcId == HARIT_LIZARDMAN_SHAMAN and st.getRandom(5) == 0 and st.getQuestItemsCount(
                        TOTEM_OF_LAND_DRAGON) == 0 and st.getInt("kendra") == 0:
                    for i in range(3):
                        st.addSpawn(HARIT_LIZARDMAN_ZEALOT, 180000)
                elif npcId == HARIT_LIZARDMAN_ZEALOT and st.getQuestItemsCount(TOTEM_OF_LAND_DRAGON) == 0 and st.getInt(
                        "kendra") == 0:
                ...
                if npcId in (CAVE_MAIDEN, CAVE_KEEPER, CAVE_KEEPER1, CAVE_MAIDEN1) and st.getQuestItemsCount(
                        THIRD_FRAGMENT_OF_ABYSS_JEWEL) == 0 and st.getRandom(5) == 0:
