		...
		else if (event.startsWith("CE")) // Client Event
		{
			...
			case 40:
				...
				case 6:
					...
					break;
				case 7:
					if (player.getCurrentClass().isMage() && cl.getId() != 49
							&& st.getInt("lvl") < 7 && player.getCurrentClass().getLevel() == 1)
					{
				...
				case 19:
					if (st.getInt("lvl") < 19)
					{
						...
						if (race != 5 && player.getCurrentClass().getLevel() == 1
								&& cl.getLevel() == 1)
						{
				...
				case 35:
					...
					{
						int race = st.getPlayer().getRace().ordinal();
						if (race != 5 && player.getCurrentClass().getLevel() == 20
								&& cl.getLevel() == 20)
						{
							//st.playTutorialVoice("tutorial_voice_???");
							st.playSound("ItemSound.quest_tutorial");
