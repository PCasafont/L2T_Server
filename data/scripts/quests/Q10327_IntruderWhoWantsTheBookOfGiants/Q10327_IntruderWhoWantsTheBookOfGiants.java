...
public class Q10327_IntruderWhoWantsTheBookOfGiants extends Quest
{
	...
	public Q10327_IntruderWhoWantsTheBookOfGiants(int questId, String name, String descr)
	{
		...
		addFirstTalkId(this.guard);
		addFirstTalkId(this.book);
		addSkillSeeId(this.guard);
	}
		...
		if (npc.getNpcId() == this.pantheon)
		{
			...
			else if (event.equalsIgnoreCase("teleport"))
			{
				...
				st.set("guardId", String.valueOf(
						addSpawn(this.guard, -114710, 245457, -7968, 49152, false, 0, false, player.getObjectId())
								.getObjectId()));
				bookIds[0] =
						addSpawn(this.book, -113757, 244686, -7952, 0, false, 0, false, player.getObjectId()).getObjectId();
				bookIds[1] =
						addSpawn(this.book, -115672, 244683, -7952, 0, false, 0, false, player.getObjectId()).getObjectId();
				bookIds[2] =
						addSpawn(this.book, -114714, 245750, -7952, 0, false, 0, false, player.getObjectId()).getObjectId();
				bookIds[3] =
						addSpawn(this.book, -114706, 243605, -7952, 0, false, 0, false, player.getObjectId()).getObjectId();
	...
	public String onFirstTalk(L2Npc npc, final L2PcInstance player)
	{
		...
		else if (npc.getNpcId() == this.book)
		{
			if (st.getInt("cond") == 1 && st.getInt("giantsBookId") == npc.getObjectId())
			{
				htmltext = "33126-01.htm";
				st.set("cond", "2");
				st.giveItems(this.bookItem, 1);
				st.playSound("ItemSound.quest_middle");
				player.sendPacket(new ExShowScreenMessage(1032322, 0, true, 7000));
				final L2Npc intruder1 =
						addSpawn(this.intruder, -114835, 244966, -7976, 16072, false, 0, false, player.getObjectId());
				final L2Npc intruder2 =
						addSpawn(this.intruder, -114564, 244954, -7976, 16072, false, 0, false, player.getObjectId());
