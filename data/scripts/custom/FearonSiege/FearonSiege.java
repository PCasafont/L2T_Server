...
public class FearonSiege extends Quest
{
	...
	private static final String[] _dummyKainTexts = { "They... They are coming...", "They will arrive soon...", "No one will be safe...", "But... The Aden army will be ready to fight!", "Hurry up Leona Blackbird please...", "Fearon Village is in dangerous..." };
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		{
			_dummyKain.broadcastPacket(new CreatureSay(_dummyKain.getObjectId(), 1, _dummyKain.getName(), _dummyKainTexts[Rnd.get(_dummyKainTexts.length)]));
		}
		...
		else if (event.equalsIgnoreCase("enter_instance"))
		{
			if (_isEventOn && (_eventStatus == 1) && (_eventRound < 15) && (_instanceId != 0))
			{
				...
				if (((playerHwid != null) && !playerHwid.isEmpty() && !_hwids.contains(playerHwid)) || (_playerIds.contains(player.getObjectId()) && _hwids.contains(playerHwid)))
				{
					...
					player.sendPacket(new ExShowScreenMessage("Sarch the Protection Stone out of the Fearon Village!", 5000));
				}
				else
			...
			if ((chars != null) && !chars.isEmpty())
			{
				...
				else if ((fuckedCount > 5) && (fuckedCount <= 10)) //Summon Tree of Life
				{
					if ((_summonTreeHelper == null) || (_summonTreeHelper.isDecayed() && (_warriorMageSup.getCurrentMp() >= _summonTree.getMpConsume()))) //Be sure we don't spawn more than one
					{
						...
						ThreadPoolManager.getInstance().scheduleAi(new Runnable()
						{
							...
							public void run()
							{
								_summonTreeHelper = addSpawn(_summonTreeId, _warriorMageSup.getX(), _warriorMageSup.getY(), _warriorMageSup.getZ() + 20, 0, true, 30000, true, _instanceId);
				...
				else if (fuckedCount > 10) //Protective Gravity Core
				{
					if ((_summonGravityCore == null) || (_summonGravityCore.isDecayed() && (_warriorMageSup.getCurrentMp() >= _summonTree.getMpConsume()))) //Be sure we don't spawn more than one
					{
						...
						ThreadPoolManager.getInstance().scheduleAi(new Runnable()
						{
							...
							public void run()
							{
								_summonGravityCore = addSpawn(_gravityCoreId, _warriorMageSup.getX(), _warriorMageSup.getY(), _warriorMageSup.getZ() + 20, 0, false, 20000, true, _instanceId);
				...
				//Give Buffs
				{
					...
					if ((buffSkill != null) && (_warriorMageSup.getCurrentMp() >= buffSkill.getMpConsume()))
					{
						...
						_warriorMageSup.broadcastPacket(new CreatureSay(_warriorMageSup.getObjectId(), 1, _warriorMageSup.getName(), skillType + "s! Come close to me to receive the power of " + skillType + "s God!"));
		...
		else if (event.equalsIgnoreCase("stage_0_prepare_event"))
		{
			...
			Announcements.getInstance().announceToAll("The global Fearon Siege Instance now is available through Kain Van Halter located in Giran!");
			
			Announcements.getInstance().announceToAll("The instance will start on 10 minutes!");
			
			_protectionStone = addSpawn(_protectionStoneId, -79660, 244954, -3651 + 20, 0, false, 0, false, _instanceId);
		...
		{
			InstanceManager.getInstance().sendPacket(_instanceId, new ExShowScreenMessage("Stay close to the Protection Stone!", 5000));
		...
		else if (event.equalsIgnoreCase("stage_all_spawn_round"))
		{
			...
			InstanceManager.getInstance().sendPacket(_instanceId, new Earthquake(153581, 142081, -12741, _eventRound, 10));
			
			InstanceManager.getInstance().sendPacket(_instanceId, new ExShowScreenMessage(1, 0, 2, 0, 0, 0, 0, true, 5 * 1000, 0, "Round: " + _eventRound));
			
			if (_eventRound == 17)
				InstanceManager.getInstance().sendDelayedPacketToInstance(_instanceId, 6, new ExShowScreenMessage(1, 0, 2, 0, 0, 0, 0, true, 5 * 1000, 0, "The Protection Stone now can debuff the enemies!"));
			
			if (_eventRound == 15)
				InstanceManager.getInstance().sendDelayedPacketToInstance(_instanceId, 6, new ExShowScreenMessage(1, 0, 2, 0, 0, 0, 0, true, 5 * 1000, 0, "The entrance to the instance is now closed!"));
			
			L2Skill passiveSkill = SkillTable.getInstance().getInfo(_passiveSkillId, _eventRound < 20 ? _eventRound : 10);
			...
			if (_eventRound < 20)
			{
				for (int i = 0; i < 61; i++)
				{
					...
					L2Npc minion = addSpawn(_invadeMobs[Rnd.get(_invadeMobs.length)], -79660 + x, 244954 + y, -3651 + 20, -1, false, 0, true, _instanceId);
			...
			else if (_eventRound == 20)
			{
				...
				InstanceManager.getInstance().sendPacket(_instanceId, new CreatureSay(_bossMakkum.getObjectId(), 1, _bossMakkum.getName(), "It's the time to end with your lives!"));
			}
		}
		...
		if (Util.contains(_invadeMobs, npc.getNpcId()))
		{
			synchronized (_allMinions)
			{
				if (_allMinions.contains(npc))
				{
					...
					{
						InstanceManager.getInstance().sendPacket(_instanceId, new ExShowScreenMessage(1, 0, 2, 0, 0, 0, 0, true, 5 * 1000, 0, "Next round will start in 15 seconds!"));
		...
		else if (npc.getNpcId() == _makkumBossId)
		{
			if (_eventStatus == 4)
			{
				...
				for (Entry<Integer, Long> i : _rewardedInfo.entrySet())
				{
					...
					Log.warning(_qn + ": Rewarded in total :" + ItemTable.getInstance().getTemplate(i.getKey()).getName() + "(" + i.getValue() + ")");
	...
	public final String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isPet, L2Skill skill)
	{
		...
		if (npc.getNpcId() == _makkumBossId)
		{
			if ((_eventStatus == 1) && (npc.getCurrentHp() < (npc.getMaxHp() * 0.90)))
			{
				...
				InstanceManager.getInstance().sendPacket(_instanceId, new SpecialCamera(_protectionStone.getObjectId(), 1000, 0, 150, 0, 16000));
				
				InstanceManager.getInstance().sendPacket(_instanceId, new MagicSkillUse(_protectionStone, _portalEffect1.getId(), 1, _portalEffect1.getHitTime(), _portalEffect1.getReuseDelay()));
				...
				ThreadPoolManager.getInstance().scheduleAi(new Runnable()
				{
					...
					public void run()
					{
						InstanceManager.getInstance().sendPacket(_instanceId, new MagicSkillUse(_protectionStone, _portalEffect2.getId(), 1, _portalEffect2.getHitTime(), _portalEffect2.getReuseDelay()));
						
						for (int i = 0; i < 15; i++)
						{
							L2Npc guard = addSpawn(_warriorGuard, _protectionStone.getX(), _protectionStone.getY(), _protectionStone.getZ(), 0, true, 0, true, _instanceId);
						...
						ThreadPoolManager.getInstance().scheduleAi(new Runnable()
						{
							...
							public void run()
							{
								...
								InstanceManager.getInstance().sendDelayedPacketToInstance(_instanceId, 8, new ExShowScreenMessage(1, 0, 2, 0, 0, 0, 0, true, 5 * 1000, 0, "The Royal Army Guards from the Aden realm has been arrived!"));
			...
			else if ((_eventStatus == 2) && (npc.getCurrentHp() < (npc.getMaxHp() * 0.50)))
			{
				...
				ThreadPoolManager.getInstance().scheduleAi(new Runnable()
				{
					...
					public void run()
					{
						...
						_warriorLeona = addSpawn(_warriorLeonaId, _protectionStone.getX(), _protectionStone.getY(), _protectionStone.getZ() + 20, 0, true, 0, true, _instanceId);
						_warriorKain = addSpawn(_warriorKainId, _protectionStone.getX(), _protectionStone.getY(), _protectionStone.getZ() + 20, 0, true, 0, true, _instanceId);
						_warriorMageSup = addSpawn(_warriorMageSUpId, _protectionStone.getX(), _protectionStone.getY(), _protectionStone.getZ() + 20, 0, true, 0, true, _instanceId);
						
						_warriorLeona.broadcastPacket(new MagicSkillUse(_warriorLeona, _warriorsSpawnEffect.getId(), 1, _warriorsSpawnEffect.getHitTime(), _warriorsSpawnEffect.getReuseDelay()));
						_warriorKain.broadcastPacket(new MagicSkillUse(_warriorKain, _warriorsSpawnEffect.getId(), 1, _warriorsSpawnEffect.getHitTime(), _warriorsSpawnEffect.getReuseDelay()));
						_warriorMageSup.broadcastPacket(new MagicSkillUse(_warriorMageSup, _warriorsSpawnEffect.getId(), 1, _warriorsSpawnEffect.getHitTime(), _warriorsSpawnEffect.getReuseDelay()));
						...
						ThreadPoolManager.getInstance().scheduleAi(new Runnable()
						{
							...
							public void run()
							{
								...
								InstanceManager.getInstance().sendDelayedPacketToInstance(_instanceId, 3, new ExShowScreenMessage(1, 0, 2, 0, 0, 0, 0, true, 5 * 1000, 0, "The Royal Army Captains from the Aden realm has been arrived!"));
							}
						}, 4000);
