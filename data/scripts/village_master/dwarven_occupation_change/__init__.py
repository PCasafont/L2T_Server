...
class Quest(JQuest):
    ...
    def onAdvEvent(self, event, npc, player):
        ...
        if event in CLASSES.keys():
            prefix, intended_race, required_class, denial1, denial2, required_marks, required_level, new_class, reward = \
                CLASSES[event]
    ...
    def onTalk(Self, npc, player):
        ...
        if npcId in BH_NPCS:
            key = "BH"
        elif npcId in WS_NPCS:
            key = "WS"
        elif npcId in SCAV_NPCS:
            key = "SC"
        elif npcId in ARTI_NPCS:
            key = "AR"
        if key:
            prefix, intended_race, required_class, denial1, denial2, required_marks, required_level, new_class, reward = \
                CLASSES[key]
