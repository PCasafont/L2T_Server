...
public class CrystalCaverns extends Quest
{
	...
	private void Throw(L2Character effector, L2Character effected)
	{
		...
		if (Config.GEODATA > 0)
		{
			Location destiny = GeoData.getInstance()
					.moveCheck(effected.getX(), effected.getY(), effected.getZ(), x, y, z, effected.getInstanceId());
			x = destiny.getX();
			y = destiny.getY();
		}
	...
	protected int enterInstance(L2PcInstance player, String template, teleCoord teleto)
	{
		...
		else
		{
			...
			Log.info("Crystal Caverns started " + template + " Instance: " + instanceId + " created by player: " +
					player.getName());
			...
			if (player.getParty() == null)
			{
				...
				InstanceManager.getInstance().setInstanceTime(player.getObjectId(), INSTANCEID,
			...
			else
			{
				for (L2PcInstance partyMember : party.getPartyMembers())
				{
					...
					InstanceManager.getInstance().setInstanceTime(partyMember.getObjectId(), INSTANCEID,
							System.currentTimeMillis() + INSTANCEPENALTY);
	...
	protected void runHall(CCWorld world)
	{
		...
		{
			L2Npc mob =
					addSpawn(CGMOBS[Rnd.get(CGMOBS.length)], spawn[0], spawn[1], spawn[2], spawn[3], false, 0, false,
							world.instanceId);
	...
	protected void runSteamOracles(CCWorld world, int[][] oracleOrder)
	{
		...
		{
			world.oracles.put(addSpawn(oracle[0], oracle[1], oracle[2], oracle[3], oracle[4], false, 0, false,
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isPet, L2Skill skill)
	{
		if (npc.getNpcId() == TEARS)
		{
			...
			if (tmpworld instanceof CCWorld)
			{
				...
				{
					boolean notAOE = true;
					if (skill != null && (skill.getTargetType() == L2SkillTargetType.TARGET_AREA ||
							skill.getTargetType() == L2SkillTargetType.TARGET_FRONT_AREA ||
				...
				if (nowHp < maxHp * 0.4 && rand < 5)
				{
					...
					for (int i = 0; i < 10; i++)
					{
						L2Npc copy = addSpawn(TEARS_COPY, npc.getX(), npc.getY(), npc.getZ(), 0, false, 0, false,
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (tmpworld instanceof CCWorld)
		{
			...
			else if (event.equalsIgnoreCase("Timer2") || event.equalsIgnoreCase("Timer3") ||
					event.equalsIgnoreCase("Timer4") || event.equalsIgnoreCase("Timer5"))
			{
				teleto.x = 144653;
				teleto.y = 152606;
				teleto.z = -12126;
			...
			else if (event.equalsIgnoreCase("Timer21") || event.equalsIgnoreCase("Timer31") ||
					event.equalsIgnoreCase("Timer41") || event.equalsIgnoreCase("Timer51"))
			{
			...
			else if (event.equalsIgnoreCase("spawnGuards"))
			{
				...
				world.guards
						.add(addSpawn(KECHIGUARD, 153622, 149699, -12131, 56890, false, 0, false, world.instanceId));
				world.guards
						.add(addSpawn(KECHIGUARD, 153609, 149622, -12131, 64023, false, 0, false, world.instanceId));
				world.guards
						.add(addSpawn(KECHIGUARD, 153606, 149428, -12131, 64541, false, 0, false, world.instanceId));
				world.guards
						.add(addSpawn(KECHIGUARD, 153601, 149534, -12131, 64901, false, 0, false, world.instanceId));
				world.guards.add(addSpawn(KECHIGUARD, 153620, 149354, -12131, 1164, false, 0, false, world.instanceId));
				world.guards
						.add(addSpawn(KECHIGUARD, 153637, 149776, -12131, 61733, false, 0, false, world.instanceId));
				world.guards
						.add(addSpawn(KECHIGUARD, 153638, 149292, -12131, 64071, false, 0, false, world.instanceId));
				world.guards
						.add(addSpawn(KECHIGUARD, 153647, 149857, -12131, 59402, false, 0, false, world.instanceId));
				world.guards
						.add(addSpawn(KECHIGUARD, 153661, 149227, -12131, 65275, false, 0, false, world.instanceId));
			...
			else if (event.equalsIgnoreCase("spawn_oracle"))
			{
				...
				addSpawn(Rnd.get(10) < 5 ? 29116 : 29117, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), false,
						0, false, world.instanceId); // Baylor's Chest
			...
			else if (event.equalsIgnoreCase("baylor_alarm"))
			{
				if (world.alarm == null)
				{
					...
					world.alarm = addSpawn(ALARMID, spawnLoc[0], spawnLoc[1], spawnLoc[2], 10800, false, 0, false,
							world.instanceId);
					world.alarm.disableCoreAI(true);
					world.alarm.setIsImmobilized(true);
					world.alarm.broadcastPacket(new CreatureSay(world.alarm.getObjectId(), 1, world.alarm.getName(),
			...
			else if (event.equalsIgnoreCase("baylor_skill"))
			{
				...
				else
				{
					...
					if (nowHp < maxHp * 0.2 && world.raidStatus < 3 && npc.getFirstEffect(5224) == null &&
							npc.getFirstEffect(5225) == null)
					{
						if (nowHp < maxHp * 0.15 && world.raidStatus == 2)
						{
							npc.doCast(SkillTable.getInstance().getInfo(5225, 1));
							npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(),
					...
					else if (nowHp < maxHp * 0.3 && rand > 50 && npc.getFirstEffect(5225) == null &&
			...
			else if (event.equalsIgnoreCase("autoFood"))
			{
				...
				else
				{
					...
					{
						npc.broadcastPacket(
								new CreatureSay(npc.getObjectId(), 1, npc.getName(), "ah ... I am so hungry ..."));
					}
			...
			{
				CrystalGolem cryGolem = world.crystalGolems.get(npc);
				L2CharPosition newpos =
						new L2CharPosition(cryGolem.foodItem.getX(), cryGolem.foodItem.getY(), cryGolem.foodItem.getZ(),
								0);
	...
	public String onKill(L2Npc npc, L2PcInstance player, boolean isPet)
	{
		...
		if (tmpworld instanceof CCWorld)
		{
			...
			if (world.status == 2 && world.npcList1.containsKey(npc))
			{
				...
				world.status = 3;
				world.tears =
						addSpawn(TEARS, 144298, 154420, -11854, 32767, false, 0, false, world.instanceId); // Tears
				CrystalGolem crygolem1 = new CrystalGolem();
				CrystalGolem crygolem2 = new CrystalGolem();
				world.crystalGolems
						.put(addSpawn(CRYSTAL_GOLEM, 140547, 151670, -11813, 32767, false, 0, false, world.instanceId),
								crygolem1);
				world.crystalGolems
						.put(addSpawn(CRYSTAL_GOLEM, 141941, 151684, -11813, 63371, false, 0, false, world.instanceId),
								crygolem2);
			...
			else if (world.status == 7)
			{
				if (npc.getNpcId() == DOLPH)
				{
					...
					addTrap(DOOR_OPENING_TRAP[0], DOOR_OPENING_TRAP[1], DOOR_OPENING_TRAP[2], DOOR_OPENING_TRAP[3],
							DOOR_OPENING_TRAP[4], null, world.instanceId);
				}
			}
				...
				if (checkKillProgress(0, npc, world))
				{
					...
					switch (world.status)
					{
						...
						case 25:
							...
							L2Npc kechi =
									addSpawn(KECHI, 154069, 149525, -12158, 51165, false, 0, false, world.instanceId);
							startQuestTimer("checkKechiAttack", 1000, kechi, null);
							return "";
						default:
							Log.warning("CrystalCavern-SteamCorridor: status " + world.status +
									" error. OracleOrder not found in " + world.instanceId);
							return "";
					}
					...
					else
					{
						for (L2PcInstance partyMember : party.getPartyMembers())
						{
							...
							partyMember.destroyItemByItemId("Quest",
									rnd < 33 ? BOSS_CRYSTAL_1 : rnd < 67 ? BOSS_CRYSTAL_2 : BOSS_CRYSTAL_3, 1,
									partyMember, true);
