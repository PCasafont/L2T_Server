...
public class ConquerACastle extends L2AttackableAIScript
{
	...
	public String onSpellFinished(L2Npc npc, L2PcInstance player, L2Skill skill)
	{
		...
		if (wrld != null && wrld instanceof ConquerACastleWorld)
		{
			...
			if (npc.getNpcId() == _helperDwarf)
			{
				if (skill.getId() == 299)
				{
					world.golem = addSpawn(_helperGolem, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), true, 0, false, npc.getInstanceId());
					
					InstanceManager.getInstance().sendPacket(world.instanceId, new CreatureSay(npc.getObjectId(), 1, "Dwarf Soldier", "C'mon golem! Destroy these doors!"));
	...
	{
		L2DoorInstance door = (L2DoorInstance) getClosetDoor(world);
		if (door == null)
		{
			world.isGolemAttacking = false;
			world.golem.deleteMe();
			
			InstanceManager.getInstance().sendPacket(world.instanceId, new CreatureSay(world.dwarf.getObjectId(), 1, "Dwarf Soldier", "My golem can't see more doors, please guide me to the next position!"));
	...
	public final String onSpawn(L2Npc npc)
	{
		...
		if (npc.getNpcId() == _crystalOfProtection)
			...
			addSpawn(_helperDwarf, npc.getX(), npc.getY(), npc.getZ() + 50, npc.getHeading(), true, 0, false, npc.getInstanceId());
		...
		if (tmpworld instanceof ConquerACastleWorld)
		{
			...
			if (npc.getNpcId() == _crystalOfProtection)
			{
				...
				if ((world.status == 6 && world.castleId != 9 && world.castleId != 7) || (world.status == 7 && (world.castleId == 9 || world.castleId == 7)))
				{
					...
					InstanceManager.getInstance().sendPacket(world.instanceId, new CreatureSay(world.finalBoss.getObjectId(), 1, "Shilen's Protector", "It's the time to die fucking warriors!"));
					InstanceManager.getInstance().sendPacket(world.instanceId, new ExShowScreenMessage("All the seals has been destroyed, the boss is now available!", 6000));
				}
				else
				{
					String value = "";
					...
					InstanceManager.getInstance().sendPacket(world.instanceId, new ExShowScreenMessage("The " + value + " seal has been destroyed!", 6000));
				}
			}
			...
			{
				InstanceManager.getInstance().sendPacket(world.instanceId, new CreatureSay(npc.getObjectId(), 1, "Dwarf Soldier", "Please, protect my golem!"));
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (wrld != null && wrld instanceof ConquerACastleWorld)
		{
			...
			{
				//Spawn the boos
				world.finalBoss = addSpawn(_shilensProtector, _finalBossSpawn[world.castleId-1][0], _finalBossSpawn[world.castleId-1][1], _finalBossSpawn[world.castleId-1][2], _finalBossSpawn[world.castleId-1][3], false, 0, false, world.instanceId);
				world.finalBoss.setIsParalyzed(true);
				world.finalBoss.setIsInvul(true);
				world.finalBoss.startVisualEffect(VisualEffect.HOLD_2);
				
				InstanceManager.getInstance().sendDelayedPacketToInstance(world.instanceId, 15, new ExShowScreenMessage("Welcome to the castle of "+CastleManager.getInstance().getCastleById(world.castleId).getName()+"!", 6000));
				InstanceManager.getInstance().sendDelayedPacketToInstance(world.instanceId, 31, new ExShowScreenMessage("Infiltrate to the Castle and destroy the Crystals as fast you can!", 6000));
			...
			else if (event.startsWith("stage_all_dwarf_help"))
			{
				...
				{	
					InstanceManager.getInstance().sendPacket(world.instanceId, new CreatureSay(npc.getObjectId(), 1, "Dwarf Soldier", (player.getAppearance().getSex() ? "Mrs. " : "Mr. ") + player.getName() + "! I'll follow you!"));
				...
				{
					L2Object door = getClosetDoor(world);
					if (door == null)
					{	
						InstanceManager.getInstance().sendPacket(world.instanceId, new CreatureSay(npc.getObjectId(), 1, "Dwarf Soldier", (player.getAppearance().getSex() ? "Mrs. " : "Mr. ") + player.getName() + "! I don't see any door in my range, please guide me to the correct position!"));
						return "";
					}
					...
					InstanceManager.getInstance().sendPacket(world.instanceId, new CreatureSay(npc.getObjectId(), 1, "Dwarf Soldier", (player.getAppearance().getSex() ? "Mrs. " : "Mr. ") + player.getName() + "! I'll make my best golem, give me few minutes!"));
	...
	private static L2Object getClosetDoor(ConquerACastleWorld world)
	{
		...
		for (L2DoorInstance instanceDoor : InstanceManager.getInstance().getInstance(world.instanceId).getDoors())
		{
			...
			if (!world.dwarf.isInsideRadius(instanceDoor, _wildCannon.getCastRange()-100, true, true) || !GeoData.getInstance().canSeeTarget(world.dwarf, instanceDoor))
				continue;
			...
			if (instanceDoor.getDistanceSq(world.dwarf) < var)
			{
				var = instanceDoor.getDistanceSq(world.dwarf);
				door = instanceDoor;
			}
		}
	...
	private final synchronized void enterInstance(L2PcInstance player)
	{
		...
		else
		{
			...
			final int instanceId = InstanceManager.getInstance().createDynamicInstance(_qn + "/" + CastleManager.getInstance().getCastleById(castleId).getName() + ".xml");
