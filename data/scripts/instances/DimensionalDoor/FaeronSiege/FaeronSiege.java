	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (wrld != null && wrld instanceof FearonSiegeWorld)
		{
			...
			if (event.equalsIgnoreCase("stage_1_start"))
			{
				if (_debug)
					world._eventRound = 9;
				world._protectionStone = addSpawn(_protectionStoneId, -79660, 244954, -3651 + 20, 0, false, 0, false, world.instanceId);
				
				startQuestTimer("stone_ai", 3000, world._protectionStone, null);
				
				InstanceManager.getInstance().sendPacket(world.instanceId, new ExShowScreenMessage("Sarch the Protection Stone out of the Fearon Village!", 8000));
				InstanceManager.getInstance().sendDelayedPacketToInstance(world.instanceId, 10, new ExShowScreenMessage("Stay close to the Protection Stone!", 5000));
				...
				for (int i = 0; i < 61; i++)
				{
					...
					addSpawn(_dummyInvaderDoor, -79660 + x, 244954 + y, -3651 + 20, -1, false, 0, true, world.instanceId);
				}
			...
			else if (event.equalsIgnoreCase("stage_all_spawn_round"))
			{
				...
				InstanceManager.getInstance().sendPacket(world.instanceId, new Earthquake(153581, 142081, -12741, world._eventRound, 10));
				InstanceManager.getInstance().sendPacket(world.instanceId, new ExShowScreenMessage(1, 0, 2, 0, 0, 0, 0, true, 5 * 1000, 0, "Round: " + world._eventRound));
				
				if (world._eventRound == 7)
					InstanceManager.getInstance().sendDelayedPacketToInstance(world.instanceId, 6, new ExShowScreenMessage(1, 0, 2, 0, 0, 0, 0, true, 5 * 1000, 0, "The Protection Stone now can debuff the enemies!"));
				
				L2Skill passiveSkill = SkillTable.getInstance().getInfo(_passiveSkillId, world._eventRound < 10 ? world._eventRound * 2 : 10);
				if (world._eventRound < 10)
				{
					for (int i = 0; i < 61; i++)
					{
						...
						L2Npc minion = addSpawn(_invadeMobs[Rnd.get(_invadeMobs.length)], -79660 + x, 244954+ y, -3651 + 20, -1, false, 0, true, world.instanceId);
				...
				{
					//BossTime
					world._bossMakkum = addSpawn(_makkumBossId, -80015, 244904, -3677, 917, false, 0, true, world.instanceId);
					...
					InstanceManager.getInstance().sendPacket(world.instanceId, new CreatureSay(world._bossMakkum.getObjectId(), 1, world._bossMakkum.getName(), "It's the time to end with your lives!"));
				}
			}	
			else if (event.equalsIgnoreCase("stone_ai"))
			{
				...
				{	
					for (L2Character chara : chars)
					{
						...
						if (chara.isInsideRadius(world._protectionStone, _protectionSkill.getSkillRadius(), false, false))
						{
				...
				if (chars != null && !chars.isEmpty())
				{
					...
					else if (fuckedCount >= 4)	//Protective Gravity Core
					{
						if (world._summonGravityCore == null || (world._summonGravityCore.isDecayed() && world._warriorMageSup.getCurrentMp() >= _summonTree.getMpConsume()))	//Be sure we don't spawn more than one
						{
							...
							ThreadPoolManager.getInstance().scheduleAi(new Runnable()
							{
								public void run()
								{
									world._summonGravityCore = addSpawn(_gravityCoreId, world._warriorMageSup.getX(), world._warriorMageSup.getY(), world._warriorMageSup.getZ() + 20, 0, false, 20000, true, world.instanceId);
					...
					else	//Give Buffs
					{
						...
						if (buffSkill != null && world._warriorMageSup.getCurrentMp() >= buffSkill.getMpConsume())
						{
							...
							world._warriorMageSup.broadcastPacket(new CreatureSay(world._warriorMageSup.getObjectId(), 1, world._warriorMageSup.getName(), skillType+"s! Come close to me to receive the power of "+skillType+"s God!"));
							world._warriorMageSup.setTarget(world._warriorMageSup);
		...
		if (tmpWorld instanceof FearonSiegeWorld)
		{
			...
			if (npc == world._bossMakkum)
			{
				if (world.status == 0  && npc.getCurrentHp() < npc.getMaxHp() * 0.90)
				{
					...
					InstanceManager.getInstance().sendPacket(world.instanceId, new Earthquake(153581, 142081, -12741, world._eventRound, 10));
					InstanceManager.getInstance().sendPacket(world.instanceId, new SpecialCamera(world._protectionStone.getObjectId(), 1000, 0, 150, 0, 16000));
					InstanceManager.getInstance().sendPacket(world.instanceId, new MagicSkillUse(world._protectionStone, _portalEffect1.getId(), 1, _portalEffect1.getHitTime(), _portalEffect1.getReuseDelay()));
					
					ThreadPoolManager.getInstance().scheduleAi(new Runnable()
					{
						public void run()
						{
							InstanceManager.getInstance().sendPacket(world.instanceId, new MagicSkillUse(world._protectionStone, _portalEffect2.getId(), 1, _portalEffect2.getHitTime(), _portalEffect2.getReuseDelay()));
							
							for (int i = 0; i < 15; i++)
							{
								L2Npc guard = addSpawn(_warriorGuard, world._protectionStone.getX(), world._protectionStone.getY(), world._protectionStone.getZ(), 0, true, 0, true, world.instanceId);
								world._guardArmy.add(guard);
								
								guard.broadcastPacket(new MagicSkillUse(guard, _warriorsSpawnEffect.getId(), 1, _warriorsSpawnEffect.getHitTime(), _warriorsSpawnEffect.getReuseDelay()));
							}
							...
							ThreadPoolManager.getInstance().scheduleAi(new Runnable()
							{
								public void run()
								{
									...
									InstanceManager.getInstance().sendDelayedPacketToInstance(world.instanceId, 3, new ExShowScreenMessage(1, 0, 2, 0, 0, 0, 0, true, 5 * 1000, 0, "The Royal Army Guards from the Aden realm has been arrived!")); 
									
									L2Npc randomGuard = world._guardArmy.get(Rnd.get(world._guardArmy.size()));
									if (randomGuard != null)
										InstanceManager.getInstance().sendDelayedPacketToInstance(world.instanceId, 3, new CreatureSay(randomGuard.getObjectId(), 1, randomGuard.getName(), "Our Captains will arrive soon!"));
				...
				else if (world.status == 1 && npc.getCurrentHp() < npc.getMaxHp() * 0.50)
				{
					...
					ThreadPoolManager.getInstance().scheduleAi(new Runnable()
					{
						public void run()
						{
							...
							world._warriorLeona = addSpawn(_warriorLeonaId, world._protectionStone.getX(), world._protectionStone.getY(), world._protectionStone.getZ() + 20, 0, true, 0, true, world.instanceId);
							world._warriorKain = addSpawn(_warriorKainId, world._protectionStone.getX(), world._protectionStone.getY(), world._protectionStone.getZ() + 20, 0, true, 0, true, world.instanceId);
							world._warriorMageSup = addSpawn(_warriorMageSUpId, world._protectionStone.getX(), world._protectionStone.getY(), world._protectionStone.getZ() + 20, 0, true, 0, true, world.instanceId);
							
							world._warriorLeona.broadcastPacket(new MagicSkillUse(world._warriorLeona, _warriorsSpawnEffect.getId(), 1, _warriorsSpawnEffect.getHitTime(), _warriorsSpawnEffect.getReuseDelay()));
							world._warriorKain.broadcastPacket(new MagicSkillUse(world._warriorKain, _warriorsSpawnEffect.getId(), 1, _warriorsSpawnEffect.getHitTime(), _warriorsSpawnEffect.getReuseDelay()));
							world._warriorMageSup.broadcastPacket(new MagicSkillUse(world._warriorMageSup, _warriorsSpawnEffect.getId(), 1, _warriorsSpawnEffect.getHitTime(), _warriorsSpawnEffect.getReuseDelay()));
							
							L2Npc randomGuard = world._guardArmy.get(Rnd.get(world._guardArmy.size()));
							if (randomGuard != null)
								InstanceManager.getInstance().sendDelayedPacketToInstance(world.instanceId, 5, new CreatureSay(randomGuard.getObjectId(), 1, randomGuard.getName(), "They're here finally!"));
							...
							ThreadPoolManager.getInstance().scheduleAi(new Runnable()
							{
								public void run()
								{
									...
									InstanceManager.getInstance().sendDelayedPacketToInstance(world.instanceId, 3, new ExShowScreenMessage(1, 0, 2, 0, 0, 0, 0, true, 5 * 1000, 0, "The Royal Army Captains from the Aden realm has been arrived!"));
									
									InstanceManager.getInstance().sendDelayedPacketToInstance(world.instanceId, 3, new CreatureSay(world._warriorLeona.getObjectId(), 1, world._warriorLeona.getName(), "Keep pushing warriors!"));
									InstanceManager.getInstance().sendDelayedPacketToInstance(world.instanceId, 3, new CreatureSay(world._warriorKain.getObjectId(), 1, world._warriorKain.getName(), "C'mon!"));
			...
			{	
				synchronized(world._allMinions)
				{
					if (world._allMinions.contains(npc))
					{
						...
						{
							InstanceManager.getInstance().sendPacket(world.instanceId, new ExShowScreenMessage(1, 0, 2, 0, 0, 0, 0, true, 5 * 1000, 0, "Next round will start in 15 seconds!"));
				...
				if (player.isInParty())
				{
					for (L2PcInstance pMember : player.getParty().getPartyMembers())
					{
						...
						if (InstanceManager.getInstance().canGetUniqueReward(pMember, world._rewardedPlayers))
						{
							...
							pMember.addItem(_qn, DimensionalDoor.getDimensionalDoorRewardId(), Rnd.get(70, 80), player, true);
						}
						else
