...
public class RescueOfTheLastGiant extends L2AttackableAIScript
{
	...
	private static final int[][] _secondRoom2 = { { -108096, 209248 }, { -108314, 208699 }, { -107541, 208697 }, { -107776, 209248 }, { -108314, 209022 }, { -107542, 209024 }, { -108314, 208855 }, { -107926, 209248 }, { -108206, 209134 }, { -107541, 208857 }, { -107650, 209142 } };
	
	private static final int[][] _lastGuardSpawns = { { -108500, 211232 }, { -108500, 211596 }, { -107349, 211596 }, { -107349, 211232 } };
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (wrld != null && wrld instanceof RescueOfTheLastGiantWorld)
		{
			...
			if (event.equalsIgnoreCase("stage_1_start"))
			{
				...
				//Screen msg
				InstanceManager.getInstance().sendDelayedPacketToPlayer(world.instancedPlayer, 5, world.instanceId, new ExShowScreenMessage(10338012, 0, true, 4000));
				InstanceManager.getInstance().sendDelayedPacketToPlayer(world.instancedPlayer, 10, world.instanceId, new ExShowScreenMessage(10338013, 0, true, 4000));
			...
			else if (event.equalsIgnoreCase("stage_2_round_2"))
			{
				...
				synchronized (world.mobs)
				{
					...
					{
						L2Npc mob = addSpawn(world.specificMobClass, spawns[0], spawns[1], -10872, 49536, false, 0, false, world.instanceId);
						world.mobs.add(mob);
					}
			...
			else if (event.equalsIgnoreCase("stage_2_round_3"))
			{
				...
				L2Npc newNpc = addSpawn(world.specificMobClass, -107926, 209248, -10872, 49536, false, 0, false, world.instanceId);
			...
			else if (event.equalsIgnoreCase("stage_3_spawns"))
			{
				...
				InstanceManager.getInstance().sendDelayedPacketToPlayer(world.instancedPlayer, 5, world.instanceId, new ExShowScreenMessage(10338006, 0, true, 4000));
				InstanceManager.getInstance().sendDelayedPacketToPlayer(world.instancedPlayer, 15, world.instanceId, new ExShowScreenMessage(10338007, 0, true, 4000));
				InstanceManager.getInstance().sendDelayedPacketToPlayer(world.instancedPlayer, 20, world.instanceId, new ExShowScreenMessage(10338008, 0, true, 4000));
				InstanceManager.getInstance().sendDelayedPacketToPlayer(world.instancedPlayer, 25, world.instanceId, new ExShowScreenMessage(10338009, 0, true, 4000));
			}
			...
			else if (event.equalsIgnoreCase("releaseSeal"))
			{
				...
				else if (world.status == 6)
				{
					...
					world.instancedPlayer.addItem(_qn, DimensionalDoor.getDimensionalDoorRewardId(), Rnd.get(3 * DimensionalDoor.getDimensionalDoorRewardRate(), 8 * DimensionalDoor.getDimensionalDoorRewardRate()), player, true);
		...
		if (tmpWorld instanceof RescueOfTheLastGiantWorld)
		{
			...
			if (npc.getNpcId() == _bossId)
			{
				if (npc.getCurrentHp() < npc.getMaxHp() * 0.05 && !world.deviceSpawned) //5%
				{
					...
					//Spawn device
					L2Npc device = addSpawn(_controlDevice, -107790, 211409, -10872, 32768, false, 60000, false, world.instanceId);
					...
					{
						L2Npc mob = addSpawn(world.specificMobClass, spawn[0], spawn[1], -10872, 49536, false, 0, false, world.instanceId);
	...
	public String onKill(L2Npc npc, L2PcInstance player, boolean isPet)
	{
		...
		if (tmpworld instanceof RescueOfTheLastGiantWorld)
		{
			...
			else if (world.status > 4)
			{
				if (npc.getNpcId() == _bossId)
				{
					world.instancedPlayer.addItem(_qn, DimensionalDoor.getDimensionalDoorRewardId(), 1 * DimensionalDoor.getDimensionalDoorRewardRate(), player, true);
					InstanceManager.getInstance().finishInstance(world.instanceId, true);
