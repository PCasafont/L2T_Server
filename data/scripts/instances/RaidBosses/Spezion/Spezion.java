...
public class Spezion extends L2AttackableAIScript
{
	...
	private static final int 		_spezionHeadstone 	= 32945;
	...
	public Spezion(int questId, String name, String descr)
	{
		...
		addTalkId(_spezionHeadstone);
		addStartNpc(_spezionHeadstone);
	...
	public String onKill(L2Npc npc, L2PcInstance player, boolean isPet)
	{
		...
		if (tmpworld instanceof PrisonOfDarknessWorld)
		{
			...
			else if (npc == world.spezionBoss)
			{
				...
				InstanceManager.getInstance().setInstanceReuse(world.instanceId, template, world.isHardMode ? 24 * 60 : 4 * 60);
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (wrld != null && wrld instanceof PrisonOfDarknessWorld)
		{
			...
			else if (event.equalsIgnoreCase("stage_all_spawn_fake_spezions"))
			{
				...
				for (int i = 0; i < 3; i++)
				{
					L2Npc fakeSpezion = addSpawn(_fakeSpezion, world.spezionBoss.getX(), world.spezionBoss.getY(), world.spezionBoss.getZ(), 0, true, 0, false, world.instanceId);
			...
			else if (event.equalsIgnoreCase("stage_all_cannon"))
			{
				...
				if (!Util.checkIfInRange(800, npc, world.spezionBoss, false) && Util.checkIfInRange(1300, npc, world.spezionBoss, false) && GeoData.getInstance().canSeeTarget(npc, world.spezionBoss))
				{
	...
	private final synchronized void enterInstance(L2PcInstance player, int template_id)
	{
		...
		{
			if (!_debug && !InstanceManager.getInstance().checkInstanceConditions(player, template_id, Config.SPEZION_MIN_PLAYERS, 7, 92, 99))
			{	
				return;
			}
	...
	{
		new Spezion(-1, _qn, "instances/RaidBosses");
	}
}
