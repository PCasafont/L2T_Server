...
public class Freya extends L2AttackableAIScript
{
	...
	public Freya(int questId, String name, String descr)
	{
		...
		addTalkId(_jiniaId);
		addStartNpc(_jiniaId);
		addTalkId(_sirraId);
		addStartNpc(_sirraId);
		addFirstTalkId(_sirraId);
		addFirstTalkId(_jiniaInnerId);
		addFirstTalkId(_kegorInnerId);
				...
				switch (skill.getId())
				{
					case 6277: //Summon Spirits
						...
						for (int i = 0; i < 10; i++)
						{
							...
							L2Npc minion = addSpawn(world.ArchBreathId, npc.getX() + x, npc.getY() + y, npc.getZ() + 20, -1, false, 0, true, world.instanceId);
	...
	private final synchronized void enterInstance(L2PcInstance player, int template_id)
	{
		...
		else
		{
			...
			for (L2PcInstance enterPlayer : allPlayers)
			{
				...
				//Delete Frozen core
				enterPlayer.deleteAllItemsById(_frozenCore);
	...
	{
		for (int sp[] : _iceBreathingCenterRoom)
		{
	...
	{
		for (int sp[] : _iceKnightAroundRoom)
		{
	...
	{
		new Freya(-1, _qn, "instances/GrandBosses");
	}
}
