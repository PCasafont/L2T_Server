...
public class Trasken extends L2AttackableAIScript
{
	...
	private static final int _enterNpc = !Config.isServer(Config.DREAMS) ? 30537 : 80001; //Daichir
	...
	private static final int[][] _walkRoutes = { { 80800, -182273, -9880, 16544 }, { 81844, -181768, -9897, 4951 }, { 82457, -181777, -9899, 64543 }, { 82962, -182286, -9899, 52926 }, { 82957, -183324, -9897, 47472 }, { 82627, -183946, -9897, 40673 }, { 81887, -183622, -9895, 24724 }, { 80992, -181181, -9892, 14214 }, { 79823, -180464, -9887, 31456 }, { 79127, -181424, -9879, 37761 }, { 78644, -182568, -9898, 45995 }, { 79045, -183289, -9899, 54458 }, { 80491, -182208, -9886, 14486 } };
	
	private static final int[][] _earthWyrmTailSpawns = { { 81181, -180408, -9872, 39299 }, { 81211, -182104, -9901, 39299 }, { 82984, -182071, -9905, 39299 }, { 79560, -182129, -9905, 39299 }, { 80988, -183857, -9902, 39299 } };
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (wrld != null && wrld instanceof TraskenWorld)
		{
			...
			if (event.equalsIgnoreCase("stage_1_start"))
			{
				...
				{
					L2Npc teredor = addSpawn(_teredorId, 82366, -183533, -9888, 56178, false, 0, false, world.instanceId);
					teredor.setIsInvul(true);
				...
				//Spawn the tail
				world.Trasken = addSpawn(_earthWyrmTail, _earthWyrmTailSpawns[0][0], _earthWyrmTailSpawns[0][1], _earthWyrmTailSpawns[0][2], _earthWyrmTailSpawns[0][3], false, 0, false, world.instanceId);
				world.Trasken.setIsInvul(true);
				world.Trasken.setIsImmobilized(true);
				world.Trasken.setIsMortal(false);
				
				//Cast the earthquake
				InstanceManager.getInstance().sendPacket(world.instanceId, new Earthquake(world.Trasken.getX(), world.Trasken.getY(), world.Trasken.getZ(), 5, 5));
				//Custom Message
				InstanceManager.getInstance().sendPacket(world.instanceId, new ExShowScreenMessage("You should decrease the number of Tentacles in order to weak Earth Wyrm Trasken", 10000));
			}
			...
			else if (event.equalsIgnoreCase("stage_all_hiding_casted"))
			{
				...
				if (world.isTraskenTime)
				{
					...
					//TRASKEN
					world.Trasken = addSpawn(_earthWyrmTrasken, 82383, -183527, -9892, 26533, false, 0, false, world.instanceId);
				}
				else
				{
					//TAIL
					int[] tailSpawn = _earthWyrmTailSpawns[Rnd.get(_earthWyrmTailSpawns.length)];
					world.Trasken = addSpawn(_earthWyrmTail, tailSpawn[0], tailSpawn[1], tailSpawn[2], tailSpawn[3], false, 0, false, world.instanceId);
			...
			else if (event.equalsIgnoreCase("stage_all_haunting_casted"))
			{
				...
				if (world.isTraskenTime)
					InstanceManager.getInstance().sendPacket(world.instanceId, new ExShowScreenMessage("Focus all your power to Earth Wyrm trasken!", 10000)); //Custom Message
			}
			else if (event.equalsIgnoreCase("stage_last_spawn_larvas"))
			{
				for (int[] a : _larvaSpawns)
				{
					L2Npc larva = addSpawn(_larvas[Rnd.get(_larvas.length)], a[0], a[1], a[2], a[3], false, 0, false, world.instanceId);
					larva.setIsRunning(true);
				}
			}
			else if (event.equalsIgnoreCase("stage_last_stomach_glands_message"))
			{
				InstanceManager.getInstance().sendPacket(world.instanceId, new ExShowScreenMessage(1620001, 0, true, 5000)); //Acid is secreting from the Stomach Glands
			}
			else if (event.equalsIgnoreCase("stage_last_foes_message"))
			{
				...
				InstanceManager.getInstance().sendPacket(world.instanceId, new ExShowScreenMessage(1620003, 0, true, 5000)); //The heart is being protected by foes
			...
			{
				InstanceManager.getInstance().sendPacket(world.instanceId, new ExShowScreenMessage(1620006, 0, true, 5000)); //Eliminate those who protect the heart of the Earth Wyrm
			...
			switch (npc.getNpcId())
			{
				...
				case _earthWyrmTrasken:
					if (world.status == 1 && !npc.isInvul() && npc.getCurrentHp() < npc.getMaxHp() * 0.05)
					{
						...
						world.TraskenHeart = addSpawn(_earthWyrmHeart, 88285, -173758, -15965, 49151, false, 0, false, world.instanceId);
						...
						InstanceManager.getInstance().sendPacket(world.instanceId, new ExShowScreenMessage(1620005, 0, true, 5000)); //You were ingested by the Earth Wyrm
			...
			switch (npc.getNpcId())
			{
				...
				case _tentacleMedium:
					...
					if (world.mobCount == _tentaclesToKill && !world.isTeredorTime)
					{
						...
						//Custom Message
						InstanceManager.getInstance().sendPacket(world.instanceId, new ExShowScreenMessage("It's the time to kill Teredor's, search and destroy all of them!", 10000));
					}
					break;
				...
				case _stomachGland:
					...
					InstanceManager.getInstance().sendPacket(world.instanceId, new ExShowScreenMessage(1620002, 0, true, 5000)); //All Stomach Glands have been destroyed
				...
				case _earthWyrmHeart:
					...
					InstanceManager.getInstance().sendPacket(world.instanceId, new ExSendUIEventRemove());
					InstanceManager.getInstance().sendPacket(world.instanceId, new ExShowScreenMessage(1620011, 0, true, 5000)); //Heart of Earth Wyrm has been destroyed
	...
	private final synchronized void enterInstance(L2PcInstance player, int template_id)
	{
		...
		{
			if (!_debug && !InstanceManager.getInstance().checkInstanceConditions(player, template_id, Config.TRASKEN_MIN_PLAYERS, 100, 92, Config.MAX_LEVEL))
			{
				return;
			}
			...
			if (_debug)
				...
				allPlayers.addAll(Config.TRASKEN_MIN_PLAYERS > Config.MAX_MEMBERS_IN_PARTY ? player.getParty().getCommandChannel().getMembers() : player.getParty().getCommandChannel() != null ? player.getParty().getCommandChannel().getMembers() : player.getParty().getPartyMembers());
