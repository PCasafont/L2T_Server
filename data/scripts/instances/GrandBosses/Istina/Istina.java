...
public class Istina extends L2AttackableAIScript
{
    ...
    private static final int[] all_mobs = {29195, 29196, ballistaId, sealingEnergy};
    private static final int[] minionIds = {taklacanId, torumbaId, dopagen};
    private static final int[] templates = {169, 170};
    private static final int ZONE_BLUE_ID = 60021;
    private static final int ZONE_RED_ID = 60022;
    private static final int ZONE_GREEN_ID = 60020;
    private static final int manifestation_red_id = 14212;
    private static final int manifestation_blue_id = 14213;
    private static final int manifestarion_green_id = 14214;

    //Zones
    private static final L2ZoneType _ZONE_BLUE = ZoneManager.getInstance().getZoneById(ZONE_BLUE_ID);
    private static final L2ZoneType _ZONE_RED = ZoneManager.getInstance().getZoneById(ZONE_RED_ID);
    private static final L2ZoneType _ZONE_GREEN = ZoneManager.getInstance().getZoneById(ZONE_GREEN_ID);
    ...
    private static final L2Skill _manifestation_red = SkillTable.getInstance().getInfo(manifestation_red_id, 1);
    private static final L2Skill _manifestation_blue = SkillTable.getInstance().getInfo(manifestation_blue_id, 1);
    private static final L2Skill _manifestation_green = SkillTable.getInstance().getInfo(manifestarion_green_id, 1);
            ...
            else if (event.equalsIgnoreCase("stage_all_epic_sealing_energy_task"))
            {
                if (world.status < 4)
                {
                    ...
                    if (knownPlayers > 1 && knownChars - knownPlayers < 20 && world.Istina.isInCombat())
                    {
                        ...
                        {
                            L2Npc sealingEnergy = addSpawn(this.sealingEnergy, world.Istina.getX(), world.Istina.getY(),
    ...
    public String onEnterZone(L2Character character, L2ZoneType zone)
    {
        ...
        if (tmpWorld instanceof IstinaWorld)
        {
            ...
            switch (zone.getId())
            {
                case ZONE_GREEN_ID: //Green, center
                    switch (world.zone)
                    {
                        case USE_ZONE_BLUE:
                        case USE_ZONE_RED:
