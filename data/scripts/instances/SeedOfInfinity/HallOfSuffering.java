...
public class HallOfSuffering extends Quest
{
    ...
    protected void runTumors(HSWorld world)
    {
        ...
        L2Npc mob = addSpawn(TUMOR_ALIVE, TUMOR_SPAWNS[world.status][0], TUMOR_SPAWNS[world.status][1],
                TUMOR_SPAWNS[world.status][2], 0, false, 0, false, world.instanceId);
    ...
    protected void runTwins(HSWorld world)
    {
        world.status++;
        world.klodekus =
                addSpawn(TWIN_SPAWNS[0][0], TWIN_SPAWNS[0][1], TWIN_SPAWNS[0][2], TWIN_SPAWNS[0][3], 0, false, 0, false,
                        world.instanceId);
        world.klanikus =
                addSpawn(TWIN_SPAWNS[1][0], TWIN_SPAWNS[1][1], TWIN_SPAWNS[1][2], TWIN_SPAWNS[1][3], 0, false, 0, false,
                        world.instanceId);
    ...
    {
        String htmltext = HtmCache.getInstance()
                .getHtm(player.getHtmlPrefix(), Config.DATA_FOLDER + "scripts/instances/SeedOfInfinity/32530-10.htm");
                ...
                for (int objectId : tmpworld.allowed)
                {
                    ...
                    if (player != null && player.isOnline())
                    {
                        InstanceManager.getInstance()
                                .setInstanceTime(objectId, tmpworld.templateId, reenter.getTimeInMillis());
        ...
        if (tmpworld instanceof HSWorld)
        {
            ...
            else if (world.status == 6 && (npc.getNpcId() == KLODEKUS || npc.getNpcId() == KLANIKUS))
            {
                if (world.klanikus.isDead() && world.klodekus.isDead())
                {
                    ...
                    addSpawn(TEPIOS, TEPIOS_SPAWN[0], TEPIOS_SPAWN[1], TEPIOS_SPAWN[2], 0, false, 0, false,
                            world.instanceId);
                }
            }
        }
