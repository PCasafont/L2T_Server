...
public class FinalEmperialTomb extends Quest
{
	...
	//NPCs
	private static final int GUIDE = !Config.isServer(Config.DREAMS) ? 32011 : 80001;
	...
	private static final int[][] PORTRAIT_SPAWNS = { { 29048, -89381, -153981, -9168, 3368, -89378, -153968, -9168, 3368 }, { 29048, -86234, -152467, -9168, 37656, -86261, -152492, -9168, 37656 }, { 29049, -89342, -152479, -9168, -5152, -89311, -152491, -9168, -5152 }, { 29049, -86189, -153968, -9168, 29456, -86217, -153956, -9168, 29456 } };
	...
	protected int enterInstance(L2PcInstance player, String template, int[] coords)
	{
		...
		// teleport players
		for (L2PcInstance channelMember : Config.FRINTEZZA_MIN_PLAYERS > Config.MAX_MEMBERS_IN_PARTY ? player.getParty().getCommandChannel().getMembers() : player.getParty().getCommandChannel() != null ? player.getParty().getCommandChannel().getMembers() : player.getParty().getPartyMembers())
		{
					...
					else if ((_world.frintezza != null) && !_world.frintezza.isDead())
					{
						...
						for (FrintezzaSong element : FRINTEZZASONGLIST)
						{
							if (rnd < element.chance)
							{
								...
								broadCastPacket(_world, new MagicSkillUse(_world.frintezza, _world.frintezza, element.skill.getSkillId(), element.skill.getSkillLvl(), element.skill.getSkill().getHitTime(), 0, 0));
		...
		public void run()
		{
			switch (_status)
			{
				...
				case 13:
					...
					break;
				case 14:
					broadCastPacket(_world, new ExShowScreenMessage(NpcStringId.MOURNFUL_CHORALE_PRELUDE.getId(), 5000));
					broadCastPacket(_world, new SpecialCamera(_world.frintezza.getObjectId(), 120, 180, 45, 1500, 10000, 0, 0, 1, 0));
					broadCastPacket(_world, new MagicSkillUse(_world.frintezza, _world.frintezza, 5006, 1, 34000, 0, 0));
				...
				case 17:
					...
					broadCastPacket(_world, new MagicSkillUse(_world.scarletDummy, _world.overheadDummy, 5004, 1, 5800, 0, 0));
