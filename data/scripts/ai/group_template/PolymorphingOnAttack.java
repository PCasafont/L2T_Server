...
public class PolymorphingOnAttack extends L2AttackableAIScript
{
    ...
    static
    {
        ...
        MOBSPAWNS
                .put(21267,
                        new Integer[]{21270, 100, 100, -1}); //Cave Ant Larva -> Cave Ant Soldier (always polymorphs)
            ...
            if (tmp != null)
            {
                if (npc.getCurrentHp() <= npc.getMaxHp() * tmp[1] / 100.0 && Rnd.get(100) < tmp[2])
                {
                    if (tmp[3] >= 0)
                    {
                        ...
                        npc.broadcastPacket(new CreatureSay(npc.getObjectId(), Say2.ALL_NOT_RECORDED, npc
                                .getName(), text));
                    }
                    npc.deleteMe();
                    L2Attackable newNpc = (L2Attackable) addSpawn(tmp[0], npc.getX(), npc.getY(), npc.getZ() + 10, npc
