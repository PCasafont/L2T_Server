							...
							for (String b : a)
							{
								...
								if (!c[0].isEmpty())
								{
									if (Integer.valueOf(c[1]) > Rnd.get(100))
									{
										...
										npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 0, npc.getName(), " Player:  " + playerToReward.getName() + ", rewarded with: " + ItemTable.getInstance().getTemplate(itemId).getName() + "("+itemCount+")"));
										
										GmListTable.broadcastMessageToGMs(getName() + ": Player: " + playerToReward.getName() + ", rewarded with: " + ItemTable.getInstance().getTemplate(itemId).getName() + "("+itemCount+")") ;
									}	
								}
							}
						}
