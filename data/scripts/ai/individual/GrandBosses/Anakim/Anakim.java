	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (npcId == enterCubic || npcId == anakimCubic)
		{
			...
			else if (anakimStatus == GrandBossManager.getInstance().WAITING && npcId == anakimCubic)
			{
				...
				GrandBossManager.getInstance().addBoss((L2GrandBossInstance) anakimBoss);
			...
			Location enterLoc = npcId == enterCubic ? this.enterLoc : enterAnakimLoc;
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("spawn_remant"))
		{
			List<L2Spawn> spawns = SpawnTable.getInstance()
					.getSpecificSpawns("pre_anakim"); //Can be moved into a global script var, testing
			...
			if (npc == null)
			{
				for (int i = 0; i < 2; i++)
				{
					...
					{
						L2Npc remnant =
								addSpawn(this.remnant, randomSpawn.getX(), randomSpawn.getY(), randomSpawn.getZ(),
