...
public class Lindvior extends L2AttackableAIScript
{
	...
	private static final int[] _allMinionIds = { _lynDracoIds[0], _lynDracoIds[1], _lynDracoIds[2], _lynDracoIds[3], _lynDracoIds[4], _lynDracoIds[5], _cycloneId, _giantCycloneId };
	...
	public Lindvior(int questId, String name, String descr)
	{
		...
		//Unlock
		startQuestTimer("unlock_lindvior", GrandBossManager.getInstance().getUnlockTime(_secondFloorLindvior), null, null);
	}
	...
	public final String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (npcId == _npcEnterId)
		{
			...
			{
				GrandBossManager.getInstance().setBossStatus(_secondFloorLindvior, GrandBossManager.getInstance().WAITING);
			...
			if (_debug)
				...
				allPlayers.addAll(Config.LINDVIOR_MIN_PLAYERS > Config.MAX_MEMBERS_IN_PARTY || player.getParty().isInCommandChannel() ? player.getParty().getCommandChannel().getMembers() : player.getParty().getPartyMembers());
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("stage_1_start"))
		{
			...
			_bossZone.sendDelayedPacketToZone(5000, new ExShowScreenMessage(14211701, 0, true, 5000)); //You must activate the 4 Generators.
				...
				if (npc.getDisplayEffect() == 1) //Orange
				{
					...
					synchronized (_manageGenerators)
					{
						if (_manageGenerators.size() < 3)
							_bossZone.broadcastPacket(new ExShowScreenMessage(14211701, 0, true, 5000)); //You must activate the 4 Generators.
		...
		else if (event.equalsIgnoreCase("stage_2_end"))
		{
			...
			_bossLocation = new Location(_lindviorBoss.getX(), _lindviorBoss.getY(), _lindviorBoss.getZ(), _lindviorBoss.getHeading());
		...
		else if (event.equalsIgnoreCase("end_lindvior"))
		{
			...
			if (GrandBossManager.getInstance().getBossStatus(_secondFloorLindvior) != GrandBossManager.getInstance().DEAD)
				GrandBossManager.getInstance().setBossStatus(_secondFloorLindvior, GrandBossManager.getInstance().ALIVE);
		}
	...
	private void spawnMinions(L2Character npc, int minionId, int rad, int amount)
	{
		...
		if (mobCount < 70)
		{
			for (int i = 0; i < amount; i++)
			{
				...
				L2MonsterInstance minion = (L2MonsterInstance) addSpawn(minionId == -1 ? _lynDracoIds[Rnd.get(_lynDracoIds.length)] : minionId, npc.getX() + x, npc.getY() + y, npc.getZ() + 20, -1, false, 0, true, npc.getInstanceId());
					...
					synchronized (_manageGenerators)
					{
						if (_manageGenerators.containsKey(npc))
						{
							...
							else
							{
								...
								double generatorDamage = _manageGenerators.get(npc); //Get the current damage from this generator
								...
								else
								{
									if (calcDamage >= maxGeneratorDamage)
									{
										...
										_bossZone.broadcastPacket(new ExShowScreenMessage("$s1 has charged the cannon!".replace("$s1", attacker.getName()), 5000)); //$s1 has charged the cannon!
										...
										if (count == 4)
										{
											...
											GrandBossManager.getInstance().setBossStatus(_secondFloorLindvior, GrandBossManager.getInstance().FIGHTING);
											...
											_bossZone.sendDelayedPacketToZone(5000, new ExShowScreenMessage(14211708, 0, true, 5000)); //Lindvior has fallen from the sky!
		...
		else if (npc.getNpcId() == _firstFloorLindvior)
		{
			if (_bossStage == 3)
			{
				if (npc.getCurrentHp() < npc.getMaxHp() * 0.50) //50%
				{
					...
					_bossZone.broadcastPacket(new ExShowScreenMessage(14211705, 0, true, 5000)); //A fearsome power emanates from Lindvior!
	...
	private void landingLindvior()
	{
		switch (_bossStage)
		{
			...
			case 5:
			case 8:
				_lindviorBoss = addSpawn(_flyLindvior, _bossLocation.getX(), _bossLocation.getY(), _bossLocation.getZ(), _bossLocation.getHeading(), false, 0, true);
			...
			case 7:
			case 9:
				_lindviorBoss = addSpawn(_secondFloorLindvior, _bossLocation.getX(), _bossLocation.getY(), _bossLocation.getZ(), _bossLocation.getHeading(), false, 0, true);
