...
public class Lindvior extends L2AttackableAIScript
{
    ...
    public Lindvior(int questId, String name, String descr)
    {
        ...
        //Unlock
        startQuestTimer("unlock_lindvior", GrandBossManager.getInstance().getUnlockTime(secondFloorLindvior), null,
                null);
    }
        ...
        if (npcId == npcEnterId)
        {
            ...
            if (lindStatus == GrandBossManager.getInstance().ALIVE)
            {
                GrandBossManager.getInstance()
                        .setBossStatus(secondFloorLindvior, GrandBossManager.getInstance().WAITING);
    ...
    public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
    {
            ...
            if (bossStage == 1)
            {
                if (npc.getDisplayEffect() == 1) //Orange
                {
                    ...
                    synchronized (manageGenerators)
                    {
                        ...
                        {
                            bossZone.broadcastPacket(new ExShowScreenMessage(14211701, 0, true,
        ...
        else if (event.equalsIgnoreCase("stage_2_end"))
        {
            ...
            bossLocation = new Location(lindviorBoss.getX(), lindviorBoss.getY(), lindviorBoss.getZ(),
                    lindviorBoss.getHeading());
        ...
        else if (event.equalsIgnoreCase("end_lindvior"))
        {
            ...
            if (GrandBossManager.getInstance().getBossStatus(secondFloorLindvior) !=
                    GrandBossManager.getInstance().DEAD)
            {
                GrandBossManager.getInstance()
                        .setBossStatus(secondFloorLindvior, GrandBossManager.getInstance().ALIVE);
            }
        }
        ...
        if (mobCount < 70)
        {
            for (int i = 0; i < amount; i++)
            {
                ...
                L2MonsterInstance minion = (L2MonsterInstance) addSpawn(
                        minionId == -1 ? lynDracoIds[Rnd.get(lynDracoIds.length)] : minionId, npc.getX() + x,
                        ...
                        if (manageGenerators.containsKey(npc))
                        {
                            ...
                            else
                            {
                                ...
                                double generatorDamage =
                                        manageGenerators.get(npc); //Get the current damage from this generator
                                ...
                                else
                                {
                                    if (calcDamage >= maxGeneratorDamage)
                                    {
                                        ...
                                        if (count == 4)
                                        {
                                            ...
                                            GrandBossManager.getInstance().setBossStatus(secondFloorLindvior,
        ...
        else if (npc.getNpcId() == firstFloorLindvior)
        {
            if (bossStage == 3)
            {
                if (npc.getCurrentHp() < npc.getMaxHp() * 0.50) //50%
                {
                    ...
                    bossZone.broadcastPacket(new ExShowScreenMessage(14211705, 0, true,
    ...
    private void landingLindvior()
    {
        switch (bossStage)
        {
            ...
            case 5:
            case 8:
                lindviorBoss = addSpawn(flyLindvior, bossLocation.getX(), bossLocation.getY(), bossLocation.getZ(),
                        bossLocation.getHeading(), false, 0, true);
            ...
            case 9:
                lindviorBoss =
                        addSpawn(secondFloorLindvior, bossLocation.getX(), bossLocation.getY(), bossLocation.getZ(),
                                bossLocation.getHeading(), false, 0, true);
