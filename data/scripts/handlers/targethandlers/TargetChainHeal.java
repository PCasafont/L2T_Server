	...
	private final boolean isReachableTarget(final L2Character activeChar, final L2Character target)
	{
		...
		if (target instanceof L2Playable)
		{
			...
			if (pTarget.isPlayingEvent())
			{
				...
				if (pTarget.getEvent().getParticipantTeamId(pTarget.getObjectId()) != pTarget.getEvent().getParticipantTeamId(activeChar.getObjectId()))
					return false;
			}
			...
			if (activeChar instanceof L2PcInstance)
			{
				...
				if (player.isInSameParty(pTarget) || player.isInSameChannel(pTarget) || player.isInSameClan(pTarget) || player.isInSameAlly(pTarget))
					return true;
			}
