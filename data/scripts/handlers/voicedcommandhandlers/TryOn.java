	...
	public boolean useVoicedCommand(String command, L2PcInstance player, String params)
	{
		...
		if (!Config.isServer(Config.DREAMS))
		{
			...
			if (search.isEmpty())
			{
				...
				sb.append("<tr><td align=center><button action=\"bypass -h voice .tryon search $what page=" + page + "\" value=Search! width=120 height=20 back=L2UI_ct1.button_df fore=L2UI_ct1.button_df></td></tr>");
			}
			else
			{
				sb.append("<center><tr><td align=center>Searching for <font color=LEVEL>" + search + "</font></td></tr>");
				sb.append("<tr><td align=center><button action=\"bypass -h voice .tryon page=" + page + "\" value=\"Remove Search\" width=120 height=20 back=L2UI_ct1.button_df fore=L2UI_ct1.button_df></td></tr>");
		...
		{
			sb.append("<html><body><title>Ancient Merchant</title>Jolie: <br> Oh. You're wondering how a certain appearance looks like? which one is it?<br>");
		}
		...
		for (L2Item item : items)
		{
			if (Config.isServer(Config.DREAMS))
			{
				String content = HtmCache.getInstance().getHtm(null, "DonationManager/Costume_Preview_Table.htm");
				
				content = content.replace("%itemName%", item.getName());
				content = content.replace("%icon%", item.getIcon() == null ? "" : item.getIcon());
				content = content.replace("%itemId%", item.getItemId() + "");
				content = content.replace("%bypass%", bypass + " item=" + item.getItemId());
			...
			sb.append("<td FIXWIDTH=30><button action=\"" + bypass + " item=" + item.getItemId() + "\" value=\" \" width=32 height=32 back=\"" + item.getIcon() + "\" fore=\"" + item.getIcon() + "\"></td>");
		...
		if (Config.isServer(Config.DREAMS))
		{
			NpcHtmlMessage nhm = new NpcHtmlMessage(0, 986);
