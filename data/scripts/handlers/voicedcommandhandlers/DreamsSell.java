		...
		if (command.equalsIgnoreCase("sell"))
		{
			...
			if (!isSelling)
			{
				if (params.equals("start"))
				{
					...
					else
					{
						...
						player.sendMessage("The selling process couldn't be started. Make sure everything's well set up.");
					}
				}
				else if (params.startsWith("item"))
				{
					...
					String html = "<html><head><title>Magic Gem</title><body>Unknown:<br>Here, you can manage the ingredients that will be required in order to purchase your %name%.<br><center><tr><td>Your are editing <font color=\"6ab3dd\">%name%</font>.</tr></td>" + "<br>" + "Amount For Sale: %amount%<br>" + "<tr><td><edit var=text type=number width=130 height=11 length=26><br>" + "<button value=\"Modify Amount\" action=\"bypass -h voice .sell item " + index + " setAmount $text\" back=\"l2ui_ct1.button_df\" width=100 height=20 fore=\"l2ui_ct1.button_df\"></td></tr><br><br>" + "<br><center><font color=\"6ab3dd\">Ingredients</font></center><br>%prices%<br><br>" + "%addPrice%<br1>" + "<a action=\"bypass -h voice .sell\"><font color=\"acd6ed\">Back to the previous page.</font></a><br1>" + "</center></body></html>";
					...
					{
						String itemHtm = "<table><tr><td width=40><table bgcolor=000000 width=24><tr><td><button action=\"\" value=\" \" width=32 height=32 back=\"" + priceItem.getIcon() + "\" fore=\"" + priceItem.getIcon() + "\"></td></tr></table></td><td width=220><table bgcolor=131210 width=220><tr><td>[<font color=FFFFFF><a action=\"bypass -h voice .sell item " + index + " deletePrice " + priceItem.getItemId() + "\" value=\" \" width=32 height=32\">X</a></font>] <font color=FFFFFF><a action=\"\" value=\" \" width=32 height=32\">" + priceItem.getName() + "</a></font></td></tr><tr><td>" + item.getPriceItems().get(priceItem) + " " + priceItem.getName() + " will be required.</td></tr></table></td></tr></table><br>" + "<br><center><font color=\"6ab3dd\">Amount:</font></center><br1>" + "<edit var=amt" + priceItem.getItemId() + " type=number width=100 height=11 length=26><br> " + "<button value=\"Apply\" action=\"bypass -h voice .sell item " + index + " setPriceAmount " + priceItem.getItemId() + " $amt" + priceItem.getItemId() + "\" back=\"l2ui_ct1.button_df\" width=85 height=20 fore=\"l2ui_ct1.button_df\">" + "<button value=\"Delete\" action=\"bypass -h voice .sell item " + index + " deletePrice " + priceItem.getItemId() + "\" back=\"l2ui_ct1.button_df\" width=85 height=20 fore=\"l2ui_ct1.button_df\"><br><br>";
					...
					packet.replace("%addPrice%", "<a action=\"bypass -h voice .sell item " + index + " addPrice\"><font color=\"c2dceb\">Add another Ingredient.</font></a><br1>");
			...
			if (!isSelling)
			{
				...
				else if (params.startsWith("addItem"))
				{
					...
					{
						packet.replace("%addItem%", "<center>Choose the item to sell from your inventory.</center><br1>");
			...
			for (int i = 0; i < list.getItemCount(); i++)
			{
				...
				String newItemHtm = "<table><tr><td width=40><table bgcolor=000000 width=24><tr><td><button action=\"bypass -h voice .sell item " + i + "\" value=\" \" width=32 height=32 back=\"" + itemIcon + "\" fore=\"" + itemIcon + "\"></td></tr></table></td><td width=220><table bgcolor=131210 width=220><tr><td>[<font color=FFFFFF><a action=\"bypass -h voice .sell deleteItem " + item.getObjectId() + "\" value=\" \" width=32 height=32\">X</a></font>] <font color=FFFFFF><a action=\"bypass -h voice .sell item " + i + "\" value=\" \" width=32 height=32\">" + itemTemplate.getName() + "</a></font></td></tr><tr><td>Click me to set-up ingredients.</td></tr></table></td></tr></table><br>";
				@SuppressWarnings("unused")
				String itemHtm = item.getItem().getName() + "<br>" + "<tr>" + "<td>Price items: " + item.getPriceItems().size() + "</td>" + "<td><button value=\"Edit\" action=\"bypass -h voice .sell item " + i + "\" back=\"l2ui_ct1.button_df\" width=65 height=20 fore=\"l2ui_ct1.button_df\"></td>" + "<td><button value=\"Delete\" action=\"bypass -h voice .sell deleteItem " + item.getObjectId() + "\" back=\"l2ui_ct1.button_df\" width=65 height=20 fore=\"l2ui_ct1.button_df\"></td>" + "</tr>";
			...
			packet.replace("%addItem%", "<a action=\"bypass -h voice .sell addItem\"><font color=\"c2dceb\">Add another Item.</font></a><br1>");
			
			if (!isSelling)
				packet.replace("%actionButton%", "<a action=\"bypass -h voice .sell start\"><font color=\"acd6ed\">Set up the store NOW.</font></a>");
			else
				packet.replace("%actionButton%", "<a action=\"bypass -h voice .sell stop\"><font color=\"acd6ed\">Stop the store NOW.</font></a>");
