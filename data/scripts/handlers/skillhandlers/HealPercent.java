...
{
	private static final L2SkillType[] SKILL_IDS = { L2SkillType.HEAL_PERCENT, L2SkillType.MANAHEAL_PERCENT, L2SkillType.CPHEAL_PERCENT, L2SkillType.HPMPHEAL_PERCENT, L2SkillType.HPMPCPHEAL_PERCENT, L2SkillType.HPCPHEAL_PERCENT };
	...
	public void useSkill(L2Character activeChar, L2Skill skill, L2Object[] targets)
	{
		...
		if ((skill.getCritChance() != -1) && (skill.getTargetType() != L2SkillTargetType.TARGET_SELF) && !skill.isPotion() && Formulas.calcMCrit(activeChar.getMCriticalHit(activeChar, skill)))
		{
		...
		for (L2Character target : (L2Character[]) targets)
		{
			...
			if (hp)
			{
				...
				amount = Math.min(target.calcStat(Stats.GAIN_HP_LIMIT, target.getMaxHp(), null, null), target.getCurrentHp() + amount) - target.getCurrentHp();
				if (amount < 0)
					amount = 0;
