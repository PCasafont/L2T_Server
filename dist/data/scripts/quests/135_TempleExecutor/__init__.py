    ...
    def onTalk(self, npc, player):
        ...
        if id == State.COMPLETED:
            ...
            if cond == 0:
                ...
                if st.getQuestItemsCount(SONIN_CR) and st.getQuestItemsCount(PANO_CR) and st.getQuestItemsCount(
                        ALEX_CR):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if st.getInt("cond") == 3:
            if st.getQuestItemsCount(CARGO) < 10:
                ...
                st.giveItems(MAP, 1)
                if st.getQuestItemsCount(MAP) >= 10 and st.getQuestItemsCount(CARGO) >= 10 and st.getQuestItemsCount(
                        CRYSTAL) >= 10:
                    st.set("cond", "4")
                    st.playSound("ItemSound.quest_middle")
                else:
