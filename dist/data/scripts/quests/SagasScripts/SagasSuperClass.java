...
public class SagasSuperClass extends QuestJython {
	...
	public int[] Z = {};
	public String[] Text = {};
	HashMap<Npc, Integer> SpawnList = new HashMap<Npc, Integer>();
	
	int[] QuestClass[] =
			{{0x7f}, {0x80, 0x81}, {0x82}, {0x05}, {0x14}, {0x15}, {0x02}, {0x03}, {0x2e}, {0x30}, {0x33}, {0x34}, {0x08}, {0x17}, {0x24}, {0x09},
					{0x18}, {0x25}, {0x10}, {0x11}, {0x1e}, {0x0c}, {0x1b}, {0x28}, {0x0e}, {0x1c}, {0x29}, {0x0d}, {0x06}, {0x22}, {0x21}, {0x2b},
					{0x37}, {0x39}};
	...
	public void AutoChat(Npc npc, String text) {
	...
	public QuestState findRightState(Npc npc) {
		...
		QuestState st = null;
		if (SpawnList.containsKey(npc)) {
			player = World.getInstance().getPlayer(SpawnList.get(npc));
			if (player != null) {
	...
	public void giveHallishaMark(QuestState st2) {
		if (st2.getInt("spawned") == 0) {
			if (st2.getQuestItemsCount(Items[3]) >= 700) {
				...
				Npc Archon = st2.addSpawn(Mob[1], xx, yy, zz);
				AddSpawn(st2, Archon);
				st2.set("spawned", "1");
	...
	public QuestState findQuest(Player player) {
		...
		if (st != null) {
			if (qnu != 68) {
				if (player.getCurrentClass().getId() == QuestClass[qnu - 67][0]) {
					return st;
				}
			} else {
				for (int q = 0; q < 2; q++) {
					if (player.getCurrentClass().getId() == QuestClass[1][q]) {
						return st;
					}
				}
			}
	...
	public int getClassId(Player player) {
		if (player.getCurrentClass().getId() == 0x81) {
			return classid[1];
		}
		return classid[0];
	}
		...
		if (st != null) {
			if (event.equalsIgnoreCase("0-011.htm") || event.equalsIgnoreCase("0-012.htm") || event.equalsIgnoreCase("0-013.htm") ||
				...
				if (st.getInt("spawned") == 1) {
					...
					Npc Mob_1 = st.addSpawn(Mob[0], X[0], Y[0], Z[0]);
					st.set("spawned", "1");
				...
				if (st.getInt("Quest0") == 0) {
					Npc Mob_3 = st.addSpawn(Mob[2], X[1], Y[1], Z[1]);
					Npc Mob_2 = st.addSpawn(NPC[4], X[2], Y[2], Z[2]);
					AddSpawn(st, Mob_3);
					AddSpawn(st, Mob_2);
			...
			if (st.getState() == State.COMPLETED && npcId == NPC[0]) {
				...
				if (cond == 0) {
					...
					if (npcId == NPC[0]) {
						if (player.getLevel() >= 76) {
							htmltext = "0-09.htm";
							if (getClassId(player) < 131 ||
									getClassId(player) > 135) //in Kamael quests, npc wants to chat for a bit before changing class
							{
								st.exitQuest(false);
								st.set("cond", "0");
	...
	public String onSkillSee(Npc npc, Player player, Skill skill, WorldObject[] targets, boolean isPet) {
		if (SpawnList.containsKey(npc) && SpawnList.get(npc) != player.getObjectId()) {
			Player quest_player = (Player) World.getInstance().findObject(SpawnList.get(npc));
