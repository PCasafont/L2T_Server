        ...
        if npcId == Grey:
            ...
            if not bracelet_1 and not bracelet_2:
                ...
                if not st.getQuestItemsCount(Cyb_Req):
                    ...
                    htmltext = "11"
                    st.takeItems(3708, -1)
                elif st.getQuestItemsCount(3699) or st.getQuestItemsCount(3700) or st.getQuestItemsCount(
                        3701) or st.getQuestItemsCount(3702) or \
                        st.getQuestItemsCount(3703) or st.getQuestItemsCount(3704) or st.getQuestItemsCount(
                    3705) or st.getQuestItemsCount(3706):
                    htmltext = "06"
                else:
                    htmltext = "10"
        if htmltext.isdigit():
