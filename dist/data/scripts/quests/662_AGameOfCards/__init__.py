...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        if event == "Klump_AcceptQuest.htm":  # quest accepted
            ...
            htmltext = htmltext.replace("CARD1", CARD_VALUES[card1]).replace("CARD2", CARD_VALUES[card2]).replace(
                "CARD3", CARD_VALUES[card3]).replace("CARD4", CARD_VALUES[card4]).replace("CARD5", CARD_VALUES[card5])
            htmltext = htmltext.replace("LINK1", link1).replace("LINK2", link2).replace("LINK3", link3).replace("LINK4",
                                                                                                                link4).replace(
                "LINK5", link5).replace("PRIZE", prizestr)
            if prize:
            ...
            self.games[name][num - 1] = st.getRandom(
                14) + 1  # generate index of random card, except index 0, which means 'card is closed'
