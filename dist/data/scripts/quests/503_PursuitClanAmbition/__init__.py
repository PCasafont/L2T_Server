    ...
    def onEvent(self, event, st):
        ...
        # Events Martien
            ...
            try:
                members = st.getPlayer().getClan().getOnlineMembers(0)[0]
                for i in members:
                    pst = QuestManager.getInstance().getQuest(qn).newQuestState(
                        st.getPlayer().getClan().getClanMember(int(i)).getPlayerInstance())
            ...
            if cond == condition and random < chance:
                if len(itemList) > 1:
                    ...
                    if npcId == 27181:  # Imperial Gravekeeper
                        ...
                        npc.broadcastPacket(NpcSay(spawnedNpc.getObjectId(), 0, spawnedNpc.getNpcId(),
