...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        if event == "30091-5.htm" and cond == 4:
            if st.getQuestItemsCount(ORIHARUKON) >= 5 and st.getQuestItemsCount(
                    SILVER_NUGGET) >= 500 and st.getQuestItemsCount(THONS) >= 150:
    ...
    def onTalk(self, npc, player):
        ...
        if id == State.COMPLETED:
            ...
            elif npcId == 30091 and cond == 4 and st.getQuestItemsCount(ORIHARUKON) >= 5 and st.getQuestItemsCount(
                    SILVER_NUGGET) >= 500 and st.getQuestItemsCount(THONS) >= 150:
