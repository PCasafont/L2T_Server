from l2server.gameserver.instancemanager import FourSepulchersManager
    ...
    def onTalk(Self, npc, player):
        ...
        if npcId == NAMELESS_SPIRIT:
            if int(st.get("cond")) == 0:
                ...
                if st.getQuestItemsCount(GOBLETS[0]) >= 1 and st.getQuestItemsCount(
                        GOBLETS[1]) >= 1 and st.getQuestItemsCount(GOBLETS[2]) >= 1 and st.getQuestItemsCount(
                    GOBLETS[3]) >= 1:
            ...
            if st.getInt("cond") == 1:
                if st.getQuestItemsCount(GOBLETS[0]) + st.getQuestItemsCount(GOBLETS[1]) + st.getQuestItemsCount(
                        GOBLETS[2]) + st.getQuestItemsCount(GOBLETS[3]) == 1:
                    htmltext = "31452-1.htm"
                elif st.getQuestItemsCount(GOBLETS[0]) + st.getQuestItemsCount(GOBLETS[1]) + st.getQuestItemsCount(
                        GOBLETS[2]) + st.getQuestItemsCount(GOBLETS[3]) > 1:
                ...
                if st.getQuestItemsCount(SEALED_BOX) >= 1:
                    if st.getQuestItemsCount(GOBLETS[0]) >= 1 and st.getQuestItemsCount(
                        ...
                        if st.getQuestItemsCount(GOBLETS[0]) + st.getQuestItemsCount(
                                GOBLETS[1]) + st.getQuestItemsCount(GOBLETS[2]) + st.getQuestItemsCount(GOBLETS[3]) > 1:
...
for npcTalkId in [GHOST_OF_WIGOTH_1, GHOST_OF_WIGOTH_2, CONQ_SM, EMPER_SM, SAGES_SM, JUDGE_SM, GHOST_CHAMBERLAIN_1,
                  GHOST_CHAMBERLAIN_2]:
