...
public class SquadSkills extends Quest {
	...
	private static final int[] supportUnitCaptain =
			{36382, 36360, 36344, 36322, 36308, 36290, 36275, 36253, 36237, 36215, 36199, 36177, 36163, 36145, 36132, 36114, 36099, 36077, 36061,
					36039, 36025, 36007, 35992, 35970, 35954, 35932, 35918, 35900, 35885, 35863, 35849, 35831, 35818, 35800, 35785, 35763, 35749,
					35731, 35716, 35694, 35680, 35662};
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
		...
		if (event.equalsIgnoreCase("show_subPledges")) //show all the available subPledges for learn skills
		{
			...
			//Add the main clan
			pledgeInfo +=
					"<tr><td><a action=\"bypass -h Quest " + qn + " show_available_skills_0\">" + playerClan.getName() + " (Main Clan)</a></td></tr>";
			...
			for (SubPledge pledge : subPledges) {
				...
				pledgeInfo += "<tr><td><a action=\"bypass -h Quest " + qn + " show_available_skills_" + pledge.getId() + "\">" + pledge.getName() +
						"</a></td></tr>";
			}
		...
		} else if (event.startsWith("show_available_skills_")) //Show available skills for that pledge
		{
			...
			if (skillToLearn.isEmpty()) {
				...
				for (Entry<Integer, Integer> toLearn : skillToLearn.entrySet()) {
					...
					skillInfo +=
							"<tr><td><a action=\"bypass -h Quest " + qn + " try_learn_skill_" + toLearn.getKey() + "_" + toLearn.getValue() + "_" +
									pledgeType + "\">" + SkillTable.getInstance().getInfo(toLearn.getKey(), 1).getName() + " (Level: " +
			...
			return HtmCache.getInstance()
					.getHtm(null, Config.DATA_FOLDER + "scripts/custom/SquadSkills/skillList.html")
