...
public class SeedOfAnnihilation extends Quest {
	...
	public void loadSeedRegionData() {
		...
		// Reptilikon data
		regionsData[1] = new SeedRegion(new int[]{22757, 22758, 22759},
				new int[][]{{22754, 22755, 22756}},
				60007,
				new int[][]{{-179600, 186998, -10704, 11632}, {-179295, 186444, -10704, 11632}});

		// Cokrakon data
		regionsData[2] = new SeedRegion(new int[]{22763, 22764, 22765},
				new int[][]{{22760, 22760, 22761}, {22760, 22760, 22762}, {22761, 22761, 22760}, {22761, 22761, 22762}, {22762, 22762, 22760},
						{22762, 22762, 22761}},
				60008,
	...
	private void startEffectZonesControl() {
		for (int i = 0; i < regionsData.length; i++) {
			for (int j = 0; j < regionsData[i].af_spawns.length; j++) {
				regionsData[i].af_npcs[j] = addSpawn(ANNIHILATION_FURNACE,
						regionsData[i].af_spawns[j][0],
						regionsData[i].af_spawns[j][1],
						regionsData[i].af_spawns[j][2],
						regionsData[i].af_spawns[j][3],
						false,
						0);
