...
public class Baium extends L2AttackableAIScript {
	...
	public String onTalk(L2Npc npc, L2PcInstance player) {
		...
		if (npc.getNpcId() == vortex) {
			...
			if (baiumStatus == GrandBossManager.getInstance().DEAD) {
				...
				if (!debug) {
					...
					if (baiumStatus == GrandBossManager.getInstance().ALIVE &&
							!InstanceManager.getInstance().checkInstanceConditions(player, -1, Config.BAIUM_MIN_PLAYERS, 200, 76, maxLvl)) {
						return null;
					} else if (baiumStatus == GrandBossManager.getInstance().WAITING &&
							!InstanceManager.getInstance().checkInstanceConditions(player, -1, Config.BAIUM_MIN_PLAYERS, 200, 76, maxLvl)) {
						return null;
			...
			if (debug) {
				...
				allPlayers.addAll(Config.BAIUM_MIN_PLAYERS > Config.MAX_MEMBERS_IN_PARTY || player.getParty().isInCommandChannel() ?
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
		...
		if (event.equalsIgnoreCase("unlock_baium")) {
			...
			{
				firstAttacker =
						baiumBoss.getKnownList().getKnownPlayers().get(Rnd.get(baiumBoss.getKnownList().getKnownPlayers().size())); //if is empty...
			}
			...
			if (firstAttacker != null) {
				...
				baiumBoss.broadcastPacket(new CreatureSay(baiumBoss.getObjectId(),
						0,
						baiumBoss.getName(),
			...
			for (L2PcInstance players : bossZone.getPlayersInside()) {
				...
				bossZone.broadcastPacket(new ExShowScreenMessage(
						"Not even the gods themselves could touch me. But you, $s1, you dare challenge me?! Ignorant mortal!".replace("$1",
								players.getName()),
						4000));//1000521
			}
