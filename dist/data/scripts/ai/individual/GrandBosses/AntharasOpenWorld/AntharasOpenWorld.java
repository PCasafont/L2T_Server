...
public class AntharasOpenWorld extends L2AttackableAIScript
{
    ...
    private static final int[] allMobIds =
            {behemothDragon, taraskDragon, dragonBombers[0], dragonBombers[1], antharasId};
    ...
    public String onTalk(L2Npc npc, L2PcInstance player)
    {
        ...
        if (npc.getNpcId() == heartOfWarding)
        {
            ...
            if (anthyStatus == GrandBossManager.getInstance().ALIVE)
            {
                ...
                startQuestTimer("antharas_spawn_task_1", debug ? 60000 : Config.ANTHARAS_WAIT_TIME * 60000, null,
                        null);
            }
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
    {
        ...
        else if (event.equalsIgnoreCase("antharas_spawn_task_1"))
        {
            ...
            //Cameras
            bossZone.sendDelayedPacketToZone(16,
                    new SpecialCamera(antharasBoss.getObjectId(), 700, 13, -19, 0, 20000, 0, 0, 1, 0));
            bossZone.sendDelayedPacketToZone(3016,
                    new SpecialCamera(antharasBoss.getObjectId(), 700, 13, 0, 6000, 20000, 0, 0, 1, 0));
            bossZone.sendDelayedPacketToZone(13016,
                    new SpecialCamera(antharasBoss.getObjectId(), 3700, 0, -3, 0, 10000, 0, 0, 1, 0));
            bossZone.sendDelayedPacketToZone(13216,
                    new SpecialCamera(antharasBoss.getObjectId(), 1100, 0, -3, 22000, 30000, 0, 0, 1, 0));
            bossZone.sendDelayedPacketToZone(24016,
                    new SpecialCamera(antharasBoss.getObjectId(), 1100, 0, -3, 300, 7000, 0, 0, 1, 0));
        ...
        else if (event.equalsIgnoreCase("spawn_minion_task"))
        {
            if (antharasBoss != null && !antharasBoss.isDead())
            {
                ...
                for (int i = 0; i < minionsToSpawn.size(); i++)
                {
                    ...
                    int x = 175000;
                    int y = 112400;
                    int dt = (antharasBoss.getX() - x) * (antharasBoss.getX() - x) +
                            (antharasBoss.getY() - y) * (antharasBoss.getY() - y);

                    while (tried++ < 25 && notFound)
                    {
                        int rx = Rnd.get(175000, 179900);
                        int ry = Rnd.get(112400, 116000);
                        int rdt = (antharasBoss.getX() - rx) * (antharasBoss.getX() - rx) +
                                (antharasBoss.getY() - ry) * (antharasBoss.getY() - ry);

                        if (GeoData.getInstance()
                                .canSeeTarget(antharasBoss.getX(), antharasBoss.getY(), -7704, rx, ry, -7704))
                        {
                            if (rdt < dt)
                            {
                                x = rx;
        ...
        if (character instanceof L2PcInstance)
        {
            if (GrandBossManager.getInstance().getBossStatus(antharasId) == GrandBossManager.getInstance().WAITING)
            {
                character.sendPacket(new ExSendUIEvent(0, 0, (int) TimeUnit.MILLISECONDS
                        .toSeconds(LastAction + Config.ANTHARAS_WAIT_TIME * 60000 - System.currentTimeMillis()), 0,
                        "Antharas is coming..."));
            }
        }
        return null;
