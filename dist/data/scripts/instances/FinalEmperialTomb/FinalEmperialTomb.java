...
public class FinalEmperialTomb extends Quest {
	...
	// Doors/Walls/Zones
	private static final int[] FIRST_ROOM_DOORS = {17130051, 17130052, 17130053, 17130054, 17130055, 17130056, 17130057, 17130058};
	...
	private static final int[][] PORTRAIT_SPAWNS = {{29048, -89381, -153981, -9168, 3368, -89378, -153968, -9168, 3368},
	...
	private void load() {
		//int spawnCount = 0;
		try {
			File file = new File(Config.DATAPACK_ROOT + "/" + Config.DATA_FOLDER + "scripts/instances/FinalEmperialTomb/final_emperial_tomb.xml");
			if (!file.exists()) {
					...
					for (XmlNode d : n.getChildren()) {
						if (d.getName().equalsIgnoreCase("spawn")) {
							...
							if (!d.hasAttribute("flag")) {
								Log.severe("[Final Emperial Tomb] Missing flag in npc List npcId: " + npcId + ", skipping");
								continue;
							}
					...
					for (XmlNode d : n.getChildren()) {
						if (d.getName().equalsIgnoreCase("zone")) {
							...
							if (!d.hasAttribute("minZ")) {
								Log.severe("[Final Emperial Tomb] Missing minZ in spawnZones List id: " + id + ", skipping");
								continue;
							}
							int minz = d.getInt("minZ");

							if (!d.hasAttribute("maxZ")) {
								Log.severe("[Final Emperial Tomb] Missing maxZ in spawnZones List id: " + id + ", skipping");
								continue;
							}
			...
			Log.log(Level.WARNING, "[Final Emperial Tomb] Could not parse final_emperial_tomb.xml file: " + e.getMessage(), e);
		}
	...
	protected int enterInstance(L2PcInstance player, String template, int[] coords) {
		...
		if (!InstanceManager.getInstance().checkInstanceConditions(player, INSTANCEID, Config.FRINTEZZA_MIN_PLAYERS, 45, 80, maxLvl)) {
			return 0;
		}
		...
		Log.info("Final Emperial Tomb started " + template + " Instance: " + instanceId + " created by player: " + player.getName());
		// teleport players
						...
						for (int i = 0; i < spw.count; i++) {
							if (spawnZoneList.containsKey(spw.zone)) {
								...
								spawn(world,
										...
										point[0],
										point[1],
										GeoData.getInstance().getSpawnHeight(point[0], point[1], point[2], point[3], null),
										Rnd.get(65535),
	...
	private class DemonSpawnTask implements Runnable {
		...
		public void run() {
			...
			for (int i : world.portraits.values()) {
				...
				L2MonsterInstance demon = (L2MonsterInstance) addSpawn(PORTRAIT_SPAWNS[i][0] + 2,
						PORTRAIT_SPAWNS[i][5],
						PORTRAIT_SPAWNS[i][6],
						PORTRAIT_SPAWNS[i][7],
						PORTRAIT_SPAWNS[i][8],
		...
		public void run() {
			...
			switch (status) {
				case 0: // new song play
					if (world.isVideo) {
						world.songTask = ThreadPoolManager.getInstance().scheduleGeneral(new SongTask(world, 0), 1000);
						...
						for (FrintezzaSong element : FRINTEZZASONGLIST) {
							if (rnd < element.chance) {
								...
								broadCastPacket(world,
										new MagicSkillUse(world.frintezza,
												world.frintezza,
												element.skill.getSkillId(),
												element.skill.getSkillLvl(),
												element.skill.getSkill().getHitTime(),
												0,
												0));
								world.songEffectTask = ThreadPoolManager.getInstance()
										.scheduleGeneral(new SongTask(world, 1), element.skill.getSkill().getHitTime() - 10000);
								world.songTask = ThreadPoolManager.getInstance()
										.scheduleGeneral(new SongTask(world, 0), element.skill.getSkill().getHitTime());
								break;
							}
						}
					}
				...
				case 1: // Frintezza song effect
					...
					if (world.frintezza != null && !world.frintezza.isDead() && world.activeScarlet != null && !world.activeScarlet.isDead()) {
						...
						if (targetList.size() > 0) {
							world.frintezza.doCast(skill, targetList.get(0), targetList.toArray(new L2Character[targetList.size()]));
						}
					}
					break;
				case 2: // finish morph
		...
		public void run() {
			switch (status) {
				...
				case 2:
					...
					break;
				case 3:
					broadCastPacket(world, new SpecialCamera(world.overheadDummy.getObjectId(), 0, 75, -89, 0, 100, 0, 0, 1, 0));
					broadCastPacket(world, new SpecialCamera(world.overheadDummy.getObjectId(), 0, 75, -89, 0, 100, 0, 0, 1, 0));
					broadCastPacket(world, new SpecialCamera(world.overheadDummy.getObjectId(), 300, 90, -10, 6500, 7000, 0, 0, 1, 0));

					world.frintezza = (L2GrandBossInstance) addSpawn(FRINTEZZA, -87784, -155083, -9087, 16048, false, 0, false, world.instanceId);
					...
					for (int[] element : PORTRAIT_SPAWNS) {
						L2MonsterInstance demon = (L2MonsterInstance) addSpawn(element[0] + 2,
								element[5],
								element[6],
								element[7],
								element[8],
					...
					break;
				case 4:
					broadCastPacket(world, new SpecialCamera(world.frintezzaDummy.getObjectId(), 1800, 90, 8, 6500, 7000, 0, 0, 1, 0));
					ThreadPoolManager.getInstance().scheduleGeneral(new IntroTask(world, 5), 900);
					break;
				case 5:
					broadCastPacket(world, new SpecialCamera(world.frintezzaDummy.getObjectId(), 140, 90, 10, 2500, 4500, 0, 0, 1, 0));
					ThreadPoolManager.getInstance().scheduleGeneral(new IntroTask(world, 6), 4000);
					break;
				case 6:
					broadCastPacket(world, new SpecialCamera(world.frintezza.getObjectId(), 40, 75, -10, 0, 1000, 0, 0, 1, 0));
					broadCastPacket(world, new SpecialCamera(world.frintezza.getObjectId(), 40, 75, -10, 0, 12000, 0, 0, 1, 0));
				...
				case 10:
					...
					sendPacketX(new SpecialCamera(world.portraitDummy1.getObjectId(), 1000, 118, 0, 0, 1000, 0, 0, 1, 0),
							new SpecialCamera(world.portraitDummy3.getObjectId(), 1000, 62, 0, 0, 1000, 0, 0, 1, 0),
							-87784);
					sendPacketX(new SpecialCamera(world.portraitDummy1.getObjectId(), 1000, 118, 0, 0, 10000, 0, 0, 1, 0),
					...
					break;
				case 11:
					broadCastPacket(world, new SpecialCamera(world.frintezza.getObjectId(), 240, 90, 0, 0, 1000, 0, 0, 1, 0));
					broadCastPacket(world, new SpecialCamera(world.frintezza.getObjectId(), 240, 90, 25, 5500, 10000, 0, 0, 1, 0));
					...
					break;
				case 12:
					broadCastPacket(world, new SpecialCamera(world.frintezza.getObjectId(), 100, 195, 35, 0, 10000, 0, 0, 1, 0));
				...
				case 13:
					...
					break;
				case 14:
					broadCastPacket(world, new ExShowScreenMessage(NpcStringId.MOURNFUL_CHORALE_PRELUDE.getId(), 5000));
					broadCastPacket(world, new SpecialCamera(world.frintezza.getObjectId(), 120, 180, 45, 1500, 10000, 0, 0, 1, 0));
					broadCastPacket(world, new MagicSkillUse(world.frintezza, world.frintezza, 5006, 1, 34000, 0, 0));
					ThreadPoolManager.getInstance().scheduleGeneral(new IntroTask(world, 15), 1500);
					break;
				case 15:
					broadCastPacket(world, new SpecialCamera(world.frintezza.getObjectId(), 520, 135, 45, 8000, 10000, 0, 0, 1, 0));
					ThreadPoolManager.getInstance().scheduleGeneral(new IntroTask(world, 16), 7500);
					break;
				case 16:
					broadCastPacket(world, new SpecialCamera(world.frintezza.getObjectId(), 1500, 110, 25, 10000, 13000, 0, 0, 1, 0));
					ThreadPoolManager.getInstance().scheduleGeneral(new IntroTask(world, 17), 9500);
					break;
				case 17:
					broadCastPacket(world, new SpecialCamera(world.overheadDummy.getObjectId(), 930, 160, -20, 0, 1000, 0, 0, 1, 0));
					broadCastPacket(world, new SpecialCamera(world.overheadDummy.getObjectId(), 600, 180, -25, 0, 10000, 0, 0, 1, 0));
					broadCastPacket(world, new MagicSkillUse(world.scarletDummy, world.overheadDummy, 5004, 1, 5800, 0, 0));

					ThreadPoolManager.getInstance().scheduleGeneral(new IntroTask(world, 18), 5000);
					break;
				case 18:
					world.activeScarlet = (L2GrandBossInstance) addSpawn(29046, -87784, -153298, -9165, 16384, false, 0, false, world.instanceId);
					...
					broadCastPacket(world, new SpecialCamera(world.scarletDummy.getObjectId(), 800, 180, 10, 1000, 10000, 0, 0, 1, 0));
					ThreadPoolManager.getInstance().scheduleGeneral(new IntroTask(world, 19), 2100);
					break;
				case 19:
					broadCastPacket(world, new SpecialCamera(world.activeScarlet.getObjectId(), 300, 60, 8, 0, 10000, 0, 0, 1, 0));
					ThreadPoolManager.getInstance().scheduleGeneral(new IntroTask(world, 20), 2000);
					break;
				case 20:
					broadCastPacket(world, new SpecialCamera(world.activeScarlet.getObjectId(), 500, 90, 10, 3000, 5000, 0, 0, 1, 0));
				...
				case 24:
					stopPc();
					broadCastPacket(world, new SpecialCamera(world.frintezza.getObjectId(), 250, 120, 15, 0, 1000, 0, 0, 1, 0));
				...
				case 25:
					broadCastPacket(world, new MagicSkillUse(world.frintezza, world.frintezza, 5006, 1, 34000, 0, 0));
					broadCastPacket(world, new SpecialCamera(world.frintezza.getObjectId(), 500, 70, 15, 3000, 10000, 0, 0, 1, 0));
					ThreadPoolManager.getInstance().scheduleGeneral(new IntroTask(world, 26), 3000);
					break;
				case 26:
					broadCastPacket(world, new SpecialCamera(world.frintezza.getObjectId(), 2500, 90, 12, 6000, 10000, 0, 0, 1, 0));
				...
				case 27:
					...
					broadCastPacket(world, new SpecialCamera(world.activeScarlet.getObjectId(), 250, world.scarlet_a, 12, 0, 1000, 0, 0, 1, 0));
					broadCastPacket(world, new SpecialCamera(world.activeScarlet.getObjectId(), 250, world.scarlet_a, 12, 0, 10000, 0, 0, 1, 0));
					ThreadPoolManager.getInstance().scheduleGeneral(new IntroTask(world, 28), 500);
					break;
				case 28:
					world.activeScarlet.doDie(world.activeScarlet);
					broadCastPacket(world, new SpecialCamera(world.activeScarlet.getObjectId(), 450, world.scarlet_a, 14, 8000, 8000, 0, 0, 1, 0));
				...
				case 29:
					...
					break;
				case 30:
					world.activeScarlet = (L2GrandBossInstance) addSpawn(SCARLET2,
							world.scarlet_x,
							world.scarlet_y,
							world.scarlet_z,
							world.scarlet_h,
					...
					broadCastPacket(world, new SpecialCamera(world.activeScarlet.getObjectId(), 450, world.scarlet_a, 12, 500, 14000, 0, 0, 1, 0));
				...
				case 32:
					...
					break;
				case 33:
					broadCastPacket(world, new SpecialCamera(world.activeScarlet.getObjectId(), 300, world.scarlet_a - 180, 5, 0, 7000, 0, 0, 1, 0));
					broadCastPacket(world, new SpecialCamera(world.activeScarlet.getObjectId(), 200, world.scarlet_a, 85, 4000, 10000, 0, 0, 1, 0));
				...
				case 34:
					...
					break;
				case 35:
					broadCastPacket(world, new SpecialCamera(world.frintezza.getObjectId(), 100, 120, 5, 0, 7000, 0, 0, 1, 0));
					broadCastPacket(world, new SpecialCamera(world.frintezza.getObjectId(), 100, 90, 5, 5000, 15000, 0, 0, 1, 0));
					ThreadPoolManager.getInstance().scheduleGeneral(new IntroTask(world, 36), 7000);
					break;
				case 36:
					broadCastPacket(world, new SpecialCamera(world.frintezza.getObjectId(), 900, 90, 25, 7000, 10000, 0, 0, 1, 0));
	...
	private class StatusTask implements Runnable {
		...
		private void addAggroToMobs() {
			...
			if (target == null || target.getInstanceId() != world.instanceId || target.isDead() || target.isFakeDeath()) {
				for (int objId : world.allowed) {
