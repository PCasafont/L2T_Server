...
public class Nursery extends L2AttackableAIScript {
	...
	public Nursery(int questId, String name, String descr) {
		...
		addTalkId(tieId);
		addStartNpc(tieId);
		addFirstTalkId(tieId);
		
		for (int i : failedCreations) {
			addKillId(i);
		}
				...
				if (world.status == 0) {
					...
					if (buff != null) {
						...
						if (buff.getSkill() != null) {
							...
							if (skillId == energyRegenerationIds[0]) {
								pointsToGive = 40;
							} else if (skillId == energyRegenerationIds[1]) {
								pointsToGive = 60;
							} else {
								pointsToGive = 80;
							}
							...
							world.instancePlayer.sendPacket(new ExShowScreenMessage(1,
									-1,
									2,
									0,
									0,
									...
									2000,
									0,
									"Soldier Tie received " + pointsToGive + " pieces of bio-energy residue.")); //1811146
						}
					}
		...
		if (wrld != null && wrld instanceof NurseryWorld) {
			...
			if (event.equalsIgnoreCase("stage_1_start")) {
				...
				if (world.instancePlayer != null && world.instancePlayer.getInstanceId() == world.instanceId) {
					world.instancePlayer.sendPacket(new ExSendUIEvent(3,
							(int) (System.currentTimeMillis() - world.enterTime) / 1000,
							world.points * 60,
							1,
							2409));
					startQuestTimer("stage_all_screen_message", 1000, null, world.instancePlayer); //1sec
				...
				if (world.status == 2) {
					...
					if (world.points >= 1 && world.points <= 800) {
						shinyCoins += Rnd.get(2 * DimensionalDoor.getDimensionalDoorRewardRate(), 1 * DimensionalDoor.getDimensionalDoorRewardRate());
					} else if (world.points >= 801 && world.points <= 1600) {
						shinyCoins += Rnd.get(4 * DimensionalDoor.getDimensionalDoorRewardRate(), 2 * DimensionalDoor.getDimensionalDoorRewardRate());
					} else if (world.points >= 1601 && world.points <= 2000) {
						shinyCoins += Rnd.get(5 * DimensionalDoor.getDimensionalDoorRewardRate(), 3 * DimensionalDoor.getDimensionalDoorRewardRate());
					} else if (world.points >= 2001 && world.points <= 2400) {
						shinyCoins += Rnd.get(7 * DimensionalDoor.getDimensionalDoorRewardRate(), 4 * DimensionalDoor.getDimensionalDoorRewardRate());
					} else if (world.points >= 2401 && world.points <= 2800) {
						shinyCoins += Rnd.get(9 * DimensionalDoor.getDimensionalDoorRewardRate(), 4 * DimensionalDoor.getDimensionalDoorRewardRate());
					} else if (world.points >= 2801 && world.points <= 3200) {
						shinyCoins +=
								Rnd.get(11 * DimensionalDoor.getDimensionalDoorRewardRate(), 4 * DimensionalDoor.getDimensionalDoorRewardRate());
					} else if (world.points >= 3201 && world.points <= 3600) {
						shinyCoins +=
								Rnd.get(13 * DimensionalDoor.getDimensionalDoorRewardRate(), 4 * DimensionalDoor.getDimensionalDoorRewardRate());
					} else if (world.points >= 3601 && world.points <= 4000) {
						shinyCoins +=
								Rnd.get(15 * DimensionalDoor.getDimensionalDoorRewardRate(), 4 * DimensionalDoor.getDimensionalDoorRewardRate());
					} else if (world.points >= 4001) {
						shinyCoins +=
								Rnd.get(17 * DimensionalDoor.getDimensionalDoorRewardRate(), 4 * DimensionalDoor.getDimensionalDoorRewardRate());
					}
					
					if (world.points > 600) {
						world.instancePlayer.addItem(qn, DimensionalDoor.getDimensionalDoorRewardId(), shinyCoins, world.instancePlayer, true);
					}
	...
	public String onKill(Npc npc, Player player, boolean isPet) {
		...
		if (tmpworld instanceof NurseryWorld) {
			...
			if (npc.getNpcId() >= failedCreations[0] && npc.getNpcId() <= failedCreations[3]) {
				...
				if (!world.isMaguenSpawned && Rnd.get(100) > 90) {
					...
					if (world.instancePlayer.getFirstEffect(world.energyBuffId) == null) {
						world.energyBuffId = energyRegenerationIds[Rnd.get(energyRegenerationIds.length)];
				...
				if (world.isMaguenSpawned) {
					...
					world.instancePlayer.sendPacket(new ExShowScreenMessage(1,
							...
							2000,
							0,
							"Maguen gets surprised and gives " + world.leakedPoints + " pieces of bio-energy residue.")); //1811147
				}
			}
		}
		return "";
