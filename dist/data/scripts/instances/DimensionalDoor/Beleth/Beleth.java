	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
		...
		if (wrld != null && wrld instanceof belethWorld) {
			...
			if (event.equalsIgnoreCase("stage_1_open_door")) {
				...
				InstanceManager.getInstance()
						.sendPacket(world.instanceId,
								new PlaySound(1,
										"BS07_A",
										1,
										world.camera1.getObjectId(),
										world.camera1.getX(),
										world.camera1.getY(),
										world.camera1.getZ()));
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera1.getObjectId(), 400, 75, 5, 0, 2500, 0, 0, 1, 0));

				startQuestTimer("stage_1_intro_1_2", 2300, null, player);
			} else if (event.equalsIgnoreCase("stage_1_intro_1_2")) {
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera1.getObjectId(), 1800, -45, 10, 5000, 5000, 0, 0, 1, 0));

				startQuestTimer("stage_1_intro_1_3", 4900, null, player);
			} else if (event.equalsIgnoreCase("stage_1_intro_1_3")) {
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera1.getObjectId(), 1900, -45, 10, 5000, 5000, 0, 0, 1, 0));

				startQuestTimer("stage_1_intro_1_4", 4900, null, player);
			} else if (event.equalsIgnoreCase("stage_1_intro_1_4")) {
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera2.getObjectId(), 2200, 130, 0, 0, 1500, -20, 15, 1, 0));

				startQuestTimer("stage_1_intro_1_5", 1400, null, player);
			} else if (event.equalsIgnoreCase("stage_1_intro_1_5")) {
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera2.getObjectId(), 2300, 100, 0, 2000, 4500, 0, 10, 1, 0));
				...
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera4.getObjectId(), 1500, 210, 0, 0, 1500, 0, 0, 1, 0));
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera4.getObjectId(), 900, 255, 0, 5000, 6500, 0, 10, 1, 0));
				...
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera4.getObjectId(), 900, 255, 0, 0, 1500, 0, 10, 1, 0));

				startQuestTimer("stage_1_intro_1_9", 1000, null, player);
			} else if (event.equalsIgnoreCase("stage_1_intro_1_9")) {
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera4.getObjectId(), 1000, 255, 0, 7000, 17000, 0, 25, 1, 0));

				startQuestTimer("stage_1_intro_1_10", 3000, null, player);
			} else if (event.equalsIgnoreCase("stage_1_intro_1_10")) {
				world.belethBoss = addSpawn(realBelethId, 16321, 214211, -9352, 49369, false, 0, false, world.instanceId);

				startQuestTimer("stage_1_intro_1_11", 200, null, player);
			} else if (event.equalsIgnoreCase("stage_1_intro_1_11")) {
				InstanceManager.getInstance().sendPacket(world.instanceId, new SocialAction(world.belethBoss.getObjectId(), 1));
				for (int i = 0; i < 6; i++) {
				...
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.belethBoss.getObjectId(), 0, 270, -5, 0, 4000, 0, 0, 1, 0));

				startQuestTimer("stage_1_intro_1_13", 3500, null, player);
			} else if (event.equalsIgnoreCase("stage_1_intro_1_13")) {
				InstanceManager.getInstance().sendPacket(world.instanceId, new SocialAction(world.belethBoss.getObjectId(), 4));
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new MagicSkillUse(world.belethBoss, world.belethBoss, 5531, 1, 2000, 0, 0));
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.belethBoss.getObjectId(), 800, 270, 10, 3000, 6000, 0, 0, 1, 0));

				startQuestTimer("stage_1_intro_1_14", 5000, null, player);
			} else if (event.equalsIgnoreCase("stage_1_intro_1_14")) {
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera3.getObjectId(), 100, 270, 15, 0, 5000, 0, 0, 1, 0));
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera3.getObjectId(), 100, 270, 15, 0, 5000, 0, 0, 1, 0));
				...
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera3.getObjectId(), 100, 270, 15, 3000, 6000, 0, 5, 1, 0));
				...
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new MagicSkillUse(world.belethBoss, world.belethBoss, 5532, 1, 2000, 0, 0));

				startQuestTimer("stage_1_intro_1_18", 2000, null, player);
			} else if (event.equalsIgnoreCase("stage_1_intro_1_18")) {
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera3.getObjectId(), 700, 270, 20, 1500, 4000, 0, 0, 1, 0));
				...
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera3.getObjectId(), 5, 250, 5, 0, 13000, 20, 15, 1, 0));

				startQuestTimer("stage_1_intro_1_22", 1000, null, player);
			} else if (event.equalsIgnoreCase("stage_1_intro_1_22")) {
				InstanceManager.getInstance().sendPacket(world.instanceId, new SocialAction(world.belethBoss.getObjectId(), 3));

				startQuestTimer("stage_1_intro_1_23", 4000, null, player);
			} else if (event.equalsIgnoreCase("stage_1_intro_1_23")) {
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new MagicSkillUse(world.belethBoss, world.belethBoss, 5533, 1, 2000, 0, 0));
				...
				InstanceManager.getInstance()
						.sendPacket(world.instanceId,
								new PlaySound(1,
										"BS07_D",
										1,
										world.camera3.getObjectId(),
										world.camera3.getX(),
										world.camera3.getY(),
										world.camera3.getZ()));
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera3.getObjectId(), 400, 290, 25, 0, 10000, 0, 0, 1, 0));
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera3.getObjectId(), 400, 290, 25, 0, 10000, 0, 0, 1, 0));
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera3.getObjectId(), 400, 110, 25, 4000, 10000, 0, 0, 1, 0));
				InstanceManager.getInstance().sendPacket(world.instanceId, new SocialAction(world.belethBoss.getObjectId(), 5));

				startQuestTimer("stage_final_2", 4000, null, player);
			} else if (event.equalsIgnoreCase("stage_final_2")) {
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera1.getObjectId(), 400, 295, 25, 4000, 5000, 0, 0, 1, 0));

				startQuestTimer("stage_final_3", 4500, null, player);
			} else if (event.equalsIgnoreCase("stage_final_3")) {
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera1.getObjectId(), 400, 295, 10, 4000, 11000, 0, 25, 1, 0));

				startQuestTimer("stage_final_4", 9000, null, player);
			} else if (event.equalsIgnoreCase("stage_final_4")) {
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera1.getObjectId(), 250, 90, 25, 0, 1000, 0, 0, 1, 0));
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera1.getObjectId(), 250, 90, 25, 0, 10000, 0, 0, 1, 0));
				...
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera2.getObjectId(), 800, 180, 0, 0, 4000, 0, 10, 1, 0));
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new SpecialCamera(world.camera2.getObjectId(), 800, 180, 0, 0, 4000, 0, 10, 1, 0));
				...
				if (player.isInParty()) {
					for (L2PcInstance pMember : player.getParty().getPartyMembers()) {
						...
						if (InstanceManager.getInstance().canGetUniqueReward(pMember, world.rewardedPlayers)) {
							...
							pMember.addItem(qn, DimensionalDoor.getDimensionalDoorRewardId(), 40, player, true);
						} else {
