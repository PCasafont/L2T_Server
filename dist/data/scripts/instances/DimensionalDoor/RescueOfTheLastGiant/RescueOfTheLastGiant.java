...
public class RescueOfTheLastGiant extends L2AttackableAIScript {
	...
	private static final Skill powerHealSkill = SkillTable.getInstance().getInfo(14625, 1);
	
	private static final int[][] secondRoom1 = {{-107776, 209248}, {-108096, 209248}, {-107926, 209248}};
	
	private static final int[][] secondRoom2 =
			{{-108096, 209248}, {-108314, 208699}, {-107541, 208697}, {-107776, 209248}, {-108314, 209022}, {-107542, 209024}, {-108314, 208855},
					{-107926, 209248}, {-108206, 209134}, {-107541, 208857}, {-107650, 209142}};
	
	private static final int[][] lastGuardSpawns = {{-108500, 211232}, {-108500, 211596}, {-107349, 211596}, {-107349, 211232}};
		...
		if (wrld != null && wrld instanceof RescueOfTheLastGiantWorld) {
			...
			if (npc.getNpcId() == dummyNpc) {
				if (world.status == 0) {
					...
					startQuestTimer("stage_3_spawns", ScenePlayerDataTable.getInstance().getVideoDuration(46) + 2000, npc, null);
				}
			}
		}
		...
		if (wrld != null && wrld instanceof RescueOfTheLastGiantWorld) {
			...
			if (event.equalsIgnoreCase("stage_1_start")) {
				...
				InstanceManager.getInstance()
						.sendDelayedPacketToPlayer(world.instancedPlayer, 5, world.instanceId, new ExShowScreenMessage(10338012, 0, true, 4000));
				InstanceManager.getInstance()
						.sendDelayedPacketToPlayer(world.instancedPlayer, 10, world.instanceId, new ExShowScreenMessage(10338013, 0, true, 4000));
				...
				for (Npc power : InstanceManager.getInstance().getInstance(world.instanceId).getNpcs()) {
				...
				//The power
				Skill powerSkill = SkillTable.getInstance().getInfo(14700, 1);
				
				Npc power = addSpawn(80287, -107925, 208869, -10877, 8191, false, 10000, false, world.instanceId);
				power.setTarget(world.instancedPlayer);
				...
				synchronized (world.mobs) {
					for (int[] spawns : secondRoom2) {
						Npc mob = addSpawn(world.specificMobClass, spawns[0], spawns[1], -10872, 49536, false, 0, false, world.instanceId);
						world.mobs.add(mob);
					}
					
					//Send to attack
					for (Npc mobs : world.mobs) {
				...
				Npc newNpc = addSpawn(world.specificMobClass, -107926, 209248, -10872, 49536, false, 0, false, world.instanceId);
				...
				//Spawn FlyNpc
				Npc flyNpc = addSpawn(80289, -107926, 212489, -10824, 49536, false, 0, false, world.instanceId);
				...
				InstanceManager.getInstance()
						.sendDelayedPacketToPlayer(world.instancedPlayer, 5, world.instanceId, new ExShowScreenMessage(10338006, 0, true, 4000));
				InstanceManager.getInstance()
						.sendDelayedPacketToPlayer(world.instancedPlayer, 15, world.instanceId, new ExShowScreenMessage(10338007, 0, true, 4000));
				InstanceManager.getInstance()
						.sendDelayedPacketToPlayer(world.instancedPlayer, 20, world.instanceId, new ExShowScreenMessage(10338008, 0, true, 4000));
				InstanceManager.getInstance()
						.sendDelayedPacketToPlayer(world.instancedPlayer, 25, world.instanceId, new ExShowScreenMessage(10338009, 0, true, 4000));
				...
				if (world.status == 5) {
					...
					world.instancedPlayer.addItem(qn, DimensionalDoor.getDimensionalDoorRewardId(), 10, player, true);
				}
		...
		if (tmpWorld instanceof RescueOfTheLastGiantWorld) {
			...
			if (npc.getNpcId() == bossId) {
				if (npc.getCurrentHp() < npc.getMaxHp() * 0.05 && !world.deviceSpawned) //5%
				{
					...
					//Spawn device
					Npc device = addSpawn(controlDevice, -107790, 211409, -10872, 32768, false, 60000, false, world.instanceId);
					...
					for (int[] spawn : lastGuardSpawns) {
						Npc mob = addSpawn(world.specificMobClass, spawn[0], spawn[1], -10872, 49536, false, 0, false, world.instanceId);
	...
	public String onKill(Npc npc, Player player, boolean isPet) {
		...
		if (tmpworld instanceof RescueOfTheLastGiantWorld) {
			...
			} else if (world.status == 1) //Second room
			{
				...
				if (npc.getNpcId() == bossId) {
					world.instancedPlayer.addItem(qn,
							DimensionalDoor.getDimensionalDoorRewardId(),
							DimensionalDoor.getDimensionalDoorRewardRate(),
							player,
							true);
					InstanceManager.getInstance().finishInstance(world.instanceId, true);
	...
	private static void sendText(int inst, int npcid, int chatId, int npcstring1, int npcstring2) {
		for (Npc mob : InstanceManager.getInstance().getInstance(inst).getNpcs()) {
			if (mob != null && mob.getNpcId() == npcid) {
				...
				if (npcstring2 != 0) {
					mob.broadcastPacket(new NpcSay(mob.getObjectId(), chatId, mob.getTemplate().TemplateId, npcstring2));
				}
			}
		}
	}
	...
	private static void sendToAttack(Player player, Npc npc) {
	...
	private static void dropHerb(Npc mob, Player player) {
		for (int i = 0; i < 2; i++) {
			((MonsterInstance) mob).dropItem(player, herbs[i], 1);
		}
	}
