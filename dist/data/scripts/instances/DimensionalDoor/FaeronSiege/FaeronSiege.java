	...
	public final String onAdvEvent(String event, Npc npc, Player player) {
		...
		if (wrld != null && wrld instanceof FearonSiegeWorld) {
			...
			if (event.equalsIgnoreCase("stage_1_start")) {
				...
				world.protectionStone = addSpawn(protectionStoneId, -79660, 244954, -3651 + 20, 0, false, 0, false, world.instanceId);

				startQuestTimer("stone_ai", 3000, world.protectionStone, null);

				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new ExShowScreenMessage("Sarch the Protection Stone out of the Fearon Village!", 8000));
				InstanceManager.getInstance()
						.sendDelayedPacketToInstance(world.instanceId, 10, new ExShowScreenMessage("Stay close to the Protection Stone!", 5000));

				//Spawn dummy effects
				for (int i = 0; i < 61; i++) {
					int x = (int) (1200 * Math.cos(i * 0.618));
					int y = (int) (1200 * Math.sin(i * 0.618));
					addSpawn(dummyInvaderDoor, -79660 + x, 244954 + y, -3651 + 20, -1, false, 0, true, world.instanceId);
				}
				...
				InstanceManager.getInstance().sendPacket(world.instanceId, new Earthquake(153581, 142081, -12741, world.eventRound, 10));
				InstanceManager.getInstance()
						.sendPacket(world.instanceId, new ExShowScreenMessage(1, 0, 2, 0, 0, 0, 0, true, 5 * 1000, 0, "Round: " + world.eventRound));
				...
				if (world.eventRound < 10) {
					...
					//BossTime
					world.bossMakkum = addSpawn(makkumBossId, -80015, 244904, -3677, 917, false, 0, true, world.instanceId);
					...
					InstanceManager.getInstance()
							.sendPacket(world.instanceId,
									new CreatureSay(world.bossMakkum.getObjectId(),
											1,
											world.bossMakkum.getName(),
				...
				if (chars != null && !chars.isEmpty()) {
					...
					for (Player chara : chars) {
						...
						if (chara.isDead() && world.warriorMageSup.isInsideRadius(chara, resSkill.getCastRange(), false, false)) {
					...
					{
						if (world.summonGravityCore == null || world.summonGravityCore.isDecayed() &&
								world.warriorMageSup.getCurrentMp() >= summonTree.getMpConsume()) //Be sure we don't spawn more than one
						{
							world.warriorMageSup.broadcastPacket(new CreatureSay(world.warriorMageSup.getObjectId(),
									1,
									world.warriorMageSup.getName(),
							...
							ThreadPoolManager.getInstance().scheduleAi(new Runnable() {
								@Override
								public void run() {
									world.summonGravityCore = addSpawn(gravityCoreId,
											world.warriorMageSup.getX(),
											world.warriorMageSup.getY(),
											world.warriorMageSup.getZ() + 20,
											0,
											false,
											20000,
											true,
		...
		if (tmpWorld instanceof FearonSiegeWorld) {
			...
			if (npc == world.bossMakkum) {
				if (world.status == 0 && npc.getCurrentHp() < npc.getMaxHp() * 0.90) {
					...
					InstanceManager.getInstance().sendPacket(world.instanceId, new Earthquake(153581, 142081, -12741, world.eventRound, 10));
					InstanceManager.getInstance()
							.sendPacket(world.instanceId, new SpecialCamera(world.protectionStone.getObjectId(), 1000, 0, 150, 0, 16000));
					InstanceManager.getInstance()
							.sendPacket(world.instanceId,
									new MagicSkillUse(world.protectionStone,
											portalEffect1.getId(),
											1,
											portalEffect1.getHitTime(),
					...
					ThreadPoolManager.getInstance().scheduleAi(new Runnable() {
						...
						public void run() {
							...
							for (int i = 0; i < 15; i++) {
								...
								guard.broadcastPacket(new MagicSkillUse(guard,
										warriorsSpawnEffect.getId(),
										1,
								...
								public void run() {
									...
									if (randomGuard != null) {
										InstanceManager.getInstance()
												.sendDelayedPacketToInstance(world.instanceId,
														3,
														new CreatureSay(randomGuard.getObjectId(),
																1,
																randomGuard.getName(),
					...
					ThreadPoolManager.getInstance().scheduleAi(new Runnable() {
						...
						public void run() {
							...
							world.warriorLeona = addSpawn(warriorLeonaId,
									world.protectionStone.getX(),
									world.protectionStone.getY(),
									world.protectionStone.getZ() + 20,
									0,
									true,
									0,
									true,
									world.instanceId);
							world.warriorKain = addSpawn(warriorKainId,
									world.protectionStone.getX(),
									world.protectionStone.getY(),
									world.protectionStone.getZ() + 20,
									0,
									...
									true,
									world.instanceId);
							world.warriorMageSup = addSpawn(warriorMageSUpId,
									world.protectionStone.getX(),
									world.protectionStone.getY(),
									world.protectionStone.getZ() + 20,
									0,
							...
							world.warriorLeona.broadcastPacket(new MagicSkillUse(world.warriorLeona,
									warriorsSpawnEffect.getId(),
									1,
									warriorsSpawnEffect.getHitTime(),
									warriorsSpawnEffect.getReuseDelay()));
							world.warriorKain.broadcastPacket(new MagicSkillUse(world.warriorKain,
									warriorsSpawnEffect.getId(),
									1,
									warriorsSpawnEffect.getHitTime(),
									warriorsSpawnEffect.getReuseDelay()));
							world.warriorMageSup.broadcastPacket(new MagicSkillUse(world.warriorMageSup,
									warriorsSpawnEffect.getId(),
									1,
									warriorsSpawnEffect.getHitTime(),
									warriorsSpawnEffect.getReuseDelay()));
							...
							if (randomGuard != null) {
								InstanceManager.getInstance()
										.sendDelayedPacketToInstance(world.instanceId,
												5,
												new CreatureSay(randomGuard.getObjectId(), 1, randomGuard.getName(), "They're here finally!"));
							}
							...
							ThreadPoolManager.getInstance().scheduleAi(new Runnable() {
								...
								public void run() {
									...
									InstanceManager.getInstance()
											.sendDelayedPacketToInstance(world.instanceId,
													3,
													new CreatureSay(world.warriorLeona.getObjectId(),
															1,
															world.warriorLeona.getName(),
															"Keep pushing warriors!"));
									InstanceManager.getInstance()
											.sendDelayedPacketToInstance(world.instanceId,
													3,
													new CreatureSay(world.warriorKain.getObjectId(), 1, world.warriorKain.getName(), "C'mon!"));
				...
				if (player.isInParty()) {
					for (Player pMember : player.getParty().getPartyMembers()) {
						...
						if (InstanceManager.getInstance().canGetUniqueReward(pMember, world.rewardedPlayers)) {
							...
							pMember.addItem(qn, DimensionalDoor.getDimensionalDoorRewardId(), 75, player, true);
						} else {
