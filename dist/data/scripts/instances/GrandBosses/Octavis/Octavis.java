...
public class Octavis extends L2AttackableAIScript {
	...
	private static final String qn = "Octavis";

	//Ids
	private static final int lydia = 32892;
	private static final int lydiaInner = 33882;
	private static final int octavisCrystal = 37505;
	private static final int heroOfTheArena = 23089;
	private static final int octavisInfluenceDummy = 18984;
	private static final int[] templates = {180, 181};
	private static final int[] minionDoorIds = {26210101, 26210102, 26210103, 26210104, 26210105, 26210106};
	private static final int[] allMobs = {29209, 29211, 29212, 29210, 23086, 23088, 23146, 29191, 29193, 29194, 29192, 22928, 22930, 23144};
	...
	private static final Location[] enterCords =
			{new Location(210646, 118783, -9996), new Location(210898, 119152, -9996), new Location(210503, 119168, -9996),
					new Location(211026, 118601, -9996), new Location(210275, 118595, -9996)};

	//Spawns
	private static final int[][] gladiatorSpawns =
			{{206515, 122206, -9975, 49151}, {207869, 122211, -9975, 44315}, {208825, 121253, -9975, 37421}, {208822, 119898, -9975, 29865},
					{207868, 118945, -9976, 18939}, {206513, 118935, -9975, 12266}};

	private static final int[][] beastSpawns = {
			//x,y,z,heading
			{206638, 119234, -10015, 12291}, {207748, 119238, -10015, 20329}, {208549, 120014, -10015, 28508}, {208531, 121132, -10015, 36123},
			{207748, 121917, -10015, 45054}, {206633, 121917, -10015, 53212}};

	private static final int[][] curatorSpawns = {
			//x,y,z,heading
			{206911, 121248, -10008, 53085}, {207472, 121252, -10012, 45202}, {207865, 120855, -10012, 36893}, {207869, 120295, -10008, 28432},
			{207473, 119896, -10012, 20544}, {206911, 119898, -10008, 12461}, {206517, 120296, -10008, 3845}, {206507, 120860, -10008, 61612}};

	private static final int[][] heroOfTheArenaSpawns = {
			//x,y,z,heading
			{207579, 120506, -10015, 32767}, {207514, 120347, -10014, 25521}, {207419, 120251, -10014, 24175}, {207259, 120189, -10015, 16383},
			{207126, 120186, -10015, 16383}, {206965, 120252, -10015, 7744}, {206870, 120348, -10015, 7866}, {206803, 120508, -10015, 65188},
			{206811, 120642, -10015, 64987}, {206871, 120805, -10015, 56909}, {207126, 120960, -10015, 48007}, {207261, 120956, -10015, 49015},
			{207415, 120891, -10015, 41553}, {207514, 120798, -10015, 40429}, {207572, 120641, -10015, 32977}};

	//Others
	private static final int[][] walkRoutes = {{208006, 120929, -10008},
			//1
			{207546, 121391, -10008}, {206860, 121394, -10008}, {206374, 120902, -10008}, {206405, 120215, -10008}, {206858, 119745, -10008},
	...
	public Octavis(int questId, String name, String descr) {
		...
		addTalkId(lydia);
		addStartNpc(lydia);
		addFirstTalkId(lydiaInner);
		addTalkId(lydiaInner);
		addStartNpc(lydiaInner);
		...
		if (wrld != null && wrld instanceof OctavisWorld) {
			...
			if (npc.getNpcId() == world.firstOctavisId) {
				if (skill == heroArrowShaft) {
					...
					world.Dummy.doCast(heroInfluence1);
				}
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
		...
		if (wrld != null && wrld instanceof OctavisWorld) {
			...
			if (event.equalsIgnoreCase("stage_1_open_doors")) {
				...
				for (L2PcInstance target : world.octavisLions.getKnownList().getKnownPlayersInRadius(150)) {
					...
					world.octavisLions.doCast(lionsAttack);
				...
				world.octavisBoss.doCast(heroArrowShaft);
			} else if (event.equalsIgnoreCase("stage_2_spawnboss")) {
				world.status = 3;

				world.octavisBoss = addSpawn(world.secondOctavisId, 207187, 120576, -10008, 52, false, 0, false, world.instanceId);
				...
				InstanceManager.getInstance().sendDelayedPacketToInstance(world.instanceId, 5, new ExShowScreenMessage(1802377, 0, true, 5000));

				addSpawn(lydiaInner, 207194, 120574, -10010, 60699, false, 0, false, world.instanceId);
				...
				synchronized (world.rewardedPlayers) {
					if (InstanceManager.getInstance().canGetUniqueReward(player, world.rewardedPlayers)) {
						...
						player.addItem(qn, octavisCrystal, 1, npc, true);
					} else {
		...
		if (npc != null && npc.getNpcId() == lydia && Util.isDigit(event) && Util.contains(templates, Integer.valueOf(event))) {
			try {
		...
		if (tmpWorld instanceof OctavisWorld) {
			...
			if (npc.getNpcId() == world.octavisLionsId) {
				...
				if (world.status == 3) {
					...
					startQuestTimer("stage_last_spawnboss", ScenePlayerDataTable.getInstance().getVideoDuration(37) + 2000, npc, null);
				}
			} else if (npc.getNpcId() == world.lastOctavisId) {
				if (world.status == 6) {
					world.status = 7;
	...
	public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
		...
		if (tmpworld instanceof OctavisWorld) {
			...
			if (npc.getNpcId() == world.lastOctavisId) {
				...
				InstanceManager.getInstance().setInstanceReuse(world.instanceId, world.templateId, world.templateId == templates[0] ? false : true);
				InstanceManager.getInstance().finishInstance(world.instanceId, false);

				if (world.isHardMode) {
					startQuestTimer("stage_last_lydia_spawn", ScenePlayerDataTable.getInstance().getVideoDuration(38), npc, null);
				}
			}
		}
	...
	private final synchronized void enterInstance(L2PcInstance player, int template_id) {
		...
		if (world != null) {
			...
			if (!debug && !InstanceManager.getInstance().checkInstanceConditions(player, template_id, minPlayers, 49, 92, maxLevel)) {
				return;
			}
	...
	public static void main(String[] args) {
		new Octavis(-1, qn, "instances/GrandBosses");
	}
}
