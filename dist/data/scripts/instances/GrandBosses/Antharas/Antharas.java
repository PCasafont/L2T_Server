...
public class Antharas extends L2AttackableAIScript {
				...
				switch (world.status) {
					case 0:
						world.antharas.getAI()
								.setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, new L2CharPosition(180242, 114877, -7710, 130)); //Move to entrance
						break;
	...
	private final synchronized void enterInstance(Player player) {
		...
		if (world != null) {
			...
			if (!debug &&
					!InstanceManager.getInstance().checkInstanceConditions(player, template, Config.ANTHARAS_INSTANCED_MIN_PLAYERS, 35, 92, 99)) {
				return;
			}
			...
			if (debug) {
				...
				allPlayers.addAll(
						Config.ANTHARAS_INSTANCED_MIN_PLAYERS > Config.MAX_MEMBERS_IN_PARTY ? player.getParty().getCommandChannel().getMembers() :
			...
			log.debug(getName() + ": [" + template + "] instance started: " + instanceId + " created by player: " + player.getName());
