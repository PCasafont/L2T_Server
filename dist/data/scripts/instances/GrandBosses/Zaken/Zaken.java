...
public class Zaken extends L2AttackableAIScript {
	...
	private static final Location[] zakenSpawns =
			{new Location(54237, 218135, -3496), new Location(56288, 218087, -3496), new Location(55273, 219140, -3496),
					new Location(54232, 220184, -3496), new Location(56259, 220168, -3496), new Location(54250, 218122, -3224),
					new Location(56308, 218125, -3224), new Location(55243, 219064, -3224), new Location(54255, 220156, -3224),
					new Location(56255, 220161, -3224), new Location(54261, 218095, -2952), new Location(56258, 218086, -2952),
					new Location(55258, 219080, -2952), new Location(54292, 220096, -2952), new Location(56258, 220135, -2952)};
		...
		if (wrld != null && wrld instanceof ZakenWorld) {
			...
			if (event.equalsIgnoreCase("stage_0_start")) {
				...
				if (world.templateId == nightTimeEpic) {
					world.zakenBoss = addSpawn(world.zakenId,
							world.zakenLocation.getX(),
							world.zakenLocation.getY(),
					...
					InstanceManager.getInstance()
							.sendPacket(world.instanceId, new ExShowScreenMessage(1800866, 10000)); //The candles can lead you to Zaken. Destroy him
				}
				...
				if (npc.isInsideRadius(world.zakenLocation.getX(), world.zakenLocation.getY(), world.zakenLocation.getZ(), 1500, true, false)) {
					...
					if (world.blueCandlesCount == 4) {
						InstanceManager.getInstance()
								.sendPacket(world.instanceId, new ExShowScreenMessage(1800867, 5000)); //Who dares awaken the mighty Zaken?
					...
					for (L2ZoneType zakenRoom : zakenRooms) {
						if (world.barrelInfo.get(npc) == zakenRoom.getId()) {
							...
							for (int i = 1; i <= 3; i++) {
								randomSpawn = zakenRoom.getZone().getRandomPoint();
								addSpawn(world.zombieCaptainId,
										randomSpawn[0],
										randomSpawn[1],
										randomSpawn[2],
										Rnd.get(65000),
	...
	public String onKill(L2Npc npc, L2PcInstance attacker, boolean isPet) {
		...
		if (tmpWorld instanceof ZakenWorld) {
			...
			if (npc.getNpcId() == world.zakenId) {
				...
				InstanceManager.getInstance()
						.setInstanceReuse(world.instanceId,
								world.templateId,
	...
	private final synchronized void enterInstance(L2PcInstance player, int template_id) {
		...
		if (world != null) {
			...
			if (!debug && !InstanceManager.getInstance().checkInstanceConditions(player, template_id, minPlayers, maxPlayers, minLevel, maxLevel)) {
				return;
			}

			String template = template_id == dayTimeEasy ? "Zaken-daytime" : template_id == dayTimeHard ? "Zaken-daytime83" : "Zaken-nighttime";
