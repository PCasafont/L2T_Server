...
public class Tauti extends L2AttackableAIScript {
	...
	//Skills
	private static final Skill leapAtkUp = SkillTable.getInstance().getInfo(16036, 1);
	private static final Skill leapAtkDown = SkillTable.getInstance().getInfo(16037, 1);

	//Cords
	private static final Location[] enterCords =
			{new Location(-149190, 210051, -10202), new Location(-149187, 209811, -10202), new Location(-148795, 209815, -10202),
					new Location(-148789, 210081, -10202), new Location(-148995, 209877, -10202)};

	//Spawns
	private static final int[][] zahakExtremeCords =
			{{-147580, 213653, -10056, 53698}, {-148034, 213214, -10056, 61229}, {-148038, 212574, -10056, 4019}, {-147589, 212110, -10056, 12426},
					{-146941, 212114, -10056, 20521}, {-146488, 212574, -10056, 28677}, {-146490, 213216, -10056, 36842},
					{-146944, 213668, -10056, 44997}};

	private static final int[][] zahakEasyeCords =
			{{-147580, 213653, -10056, 53698}, {-148038, 212574, -10056, 4019}, {-146941, 212114, -10056, 20521}, {-146490, 213216, -10056, 36842}};
	...
	private class TautiWorld extends InstanceWorld {
		...
		private List<Npc> fakeTautis;

		public TautiWorld() {
			fakeTautis = new ArrayList<Npc>();
			isHardMode = false;
		}
	}
	...
	public Tauti(int questId, String name, String descr) {
		...
		addSpellFinishedId(fakeLeapTauti);

		for (int id : enterNpcs) {
			addTalkId(id);
			addStartNpc(id);
		}
	}
				...
				switch (skill.getId()) {
						...
						if (!world.isHardMode) {
							InstanceManager.getInstance()
									.sendPacket(world.instanceId,
											new ExShowScreenMessage(1801783, 0, true, 5000)); //You rat-like creatures! Taste my attack!
						}
						break;
					...
					case 15202: //Tauti Ultra Typhoon
						if (!world.isHardMode) {
							InstanceManager.getInstance()
									.sendPacket(world.instanceId,
											new ExShowScreenMessage(1801784, 0, true, 5000)); //Do you think you are safe outside? Feel my strength!
						}
						break;
				}
			} else if (npc.getNpcId() == fakeLeapTauti) {
				if (skill == leapAtkDown) {
					npc.deleteMe();
				}
			}
		}
	...
	public final String onAdvEvent(String event, Npc npc, Player player) {
		...
		if (wrld != null && wrld instanceof TautiWorld) {
			...
			if (event.equalsIgnoreCase("stage_1_open_doors")) {
				...
				if (world.status < 3) {
					...
					if (world.Tauti.isCastingNow()) {
						...
						if (world.Tauti.isInCombat()) {
							nextLeap += leapAtkUp.getHitTime();
							nextLeap += leapAtkDown.getHitTime();

							world.Tauti.setTarget(world.Tauti);
							world.Tauti.doCast(leapAtkUp);

							if (world.isHardMode) {
								world.fakeTautis.clear();
								for (int i = 1; i <= Rnd.get(1, 3); i++) {
									int randObjId = world.allowed.get(Rnd.get(world.allowed.size()));
									Player target = World.getInstance().getPlayer(randObjId);
									if (target == null) {
										continue;
									}

									Npc dummyTauti = addSpawn(fakeLeapTauti,
											target.getX(),
											target.getY(),
											target.getZ(),
											-1,
							...
							startQuestTimer("stage_all_leap_attack_down", leapAtkUp.getHitTime(), world.Tauti, null);
						}
					}
				...
				for (Npc fake : world.fakeTautis) {
					fake.setTarget(fake);
					fake.doCast(leapAtkDown);
				}
			} else if (event.equalsIgnoreCase("stage_all_leap_attack_down")) {
				if (world.status < 3) {
					world.Tauti.setTarget(world.Tauti);
					world.Tauti.doCast(leapAtkDown);
				}
				...
				for (Npc minion : InstanceManager.getInstance().getInstance(world.instanceId).getNpcs()) {
					if (minion == null) {
						continue;
					}

					if (minion.getNpcId() == zahakId) {
						minion.doDie(null);
					}
				}
			}
		...
		if (npc != null && Util.contains(enterNpcs, npc.getNpcId()) && Util.isDigit(event) && Util.contains(templates, Integer.valueOf(event))) {
			try {
		...
		if (tmpWorld instanceof TautiWorld) {
			...
			if (npc.getNpcId() == world.TautiId) {
				if (world.status == 0 && npc.getCurrentHp() < npc.getMaxHp() * 0.70 ||
					...
					InstanceManager.getInstance()
							.sendPacket(world.instanceId, new ExShowScreenMessage(1801649, 0, true, 5000)); //Jahak is infusing its Petra to Tauti.
				...
				} else if (world.status == 2 && npc.getCurrentHp() < npc.getMaxHp() * 0.10) //10%
				{
					...
					startQuestTimer("stage_2_spawn_axe", ScenePlayerDataTable.getInstance().getVideoDuration(71) + 2000, npc, null);
				}
			}
		}
	...
	public final String onTalk(Npc npc, Player player) {
		...
		if (npcId == enterNpcs[1] || npcId == enterNpcs[2]) {
			return "Easy.html";
		} else if (npcId == enterNpcs[0]) {
			return "Hard.html";
		}
	...
	private final synchronized void enterInstance(Player player, int template_id) {
		...
		if (world != null) {
			...
			if (!debug && !InstanceManager.getInstance().checkInstanceConditions(player, template_id, minPlayers, 35, 92, Config.MAX_LEVEL)) {
				return;
			}
