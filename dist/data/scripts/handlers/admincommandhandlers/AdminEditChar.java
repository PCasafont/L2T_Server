...
public class AdminEditChar implements IAdminCommandHandler {
	...
	private static final String[] ADMIN_COMMANDS = {"admin_edit_character", "admin_current_player", "admin_nokarma",
			// this is to remove karma from selected char...
			"admin_setkarma",
			// sets karma of target char to any amount. //setkarma <karma>
			"admin_setfame",
			// sets fame of target char to any amount. //setfame <fame>
			...
			// changes char name
			"admin_setsex",
			// changes characters' sex
			"admin_setcolor",
			// change charnames' color display
			"admin_settcolor",
			// change char title color
			"admin_setclass",
			// changes chars' classId
			"admin_setpk",
			// changes PK count
			"admin_setpvp",
			// changes PVP count
			"admin_fullfood",
			// fulfills a pet's food bar
	...
	private void listCharacters(Player activeChar, int page) {
		...
		{
			players = allPlayers.toArray(new Player[allPlayers.size()]);
		}
		...
		for (int i = charactersStart; i < charactersEnd; i++) {
			...
			StringUtil.append(replyMSG,
					...
					players[i].getName(),
					"</a></td><td width=110>",
					players[i].getCurrentClass().getName(),
					"</td><td width=40>",
	...
	private void gatherCharacterInfo(Player activeChar, Player player, String filename) {
		...
		adminReply.replace("%clan%",
				String.valueOf(player.getClan() != null ?
						"<a action=\"bypass -h admin_clan_info " + player.getObjectId() + "\">" + player.getClan().getName() + "</a>" : null));
		...
		adminReply.replace("%inst%",
				player.getInstanceId() > 0 ?
						"<tr><td>InstanceId:</td><td><a action=\"bypass -h admin_instance_spawns " + String.valueOf(player.getInstanceId()) + "\">" +
								String.valueOf(player.getInstanceId()) + "</a></td></tr>" : "");
	...
	private void setTargetKarma(Player activeChar, int newKarma) {
		...
		if (Config.DEBUG) {
			log.debug("[SET KARMA] [GM]" + activeChar.getName() + " Changed karma for " + player.getName() + " from (" + oldKarma + ") to (" +
					newKarma + ").");
		}
	}
	...
	private void adminModifyCharacter(Player activeChar, String modifications) {
		...
		//Common character information
		player.sendMessage(
				"Admin has changed your stats." + "  HP: " + hpval + "  MP: " + mpval + "  CP: " + cpval + "  PvP Flag: " + pvpflagval + " PvP/PK " +
		...
		activeChar.sendMessage(
				"Changed stats of " + player.getName() + "." + "  HP: " + hpval + "  MP: " + mpval + "  CP: " + cpval + "  PvP: " + pvpflagval +
						" / " + pvpkillsval);
		
		if (Config.DEBUG) {
			log.debug("[GM]" + activeChar.getName() + " changed stats of " + player.getName() + ". " + " HP: " + hpval + " MP: " + mpval + " CP: " +
					cpval + " PvP: " + pvpflagval + " / " + pvpkillsval);
		}
	...
	private void findDualbox(Player activeChar, int multibox) {
		...
		for (String dualboxIP : keys) {
			StringUtil.append(results,
					"<a action=\"bypass -h admin_find_ip " + dualboxIP + "\">" + dualboxIP + " (" + dualboxIPs.get(dualboxIP) + ")</a><br1>");
		}
	...
	private void findDualboxStrict(Player activeChar, int multibox) {
		...
		Map<IpPack, List<Player>> ipMap = new HashMap<IpPack, List<Player>>();
		...
		for (IpPack dualboxIP : keys) {
			StringUtil.append(results,
					"<a action=\"bypass -h admin_find_ip " + dualboxIP.ip + "\">" + dualboxIP.ip + " (" + dualboxIPs.get(dualboxIP) + ")</a><br1>");
		}
	...
	private void gatherPartyInfo(Player target, Player activeChar) {
		...
		for (Player member : target.getParty().getPartyMembers()) {
			...
			text.append(member.getLevel() + "</td><td width=130><a action=\"bypass -h admin_character_info " + member.getName() + "\">" +
					member.getName() + "</a>");
