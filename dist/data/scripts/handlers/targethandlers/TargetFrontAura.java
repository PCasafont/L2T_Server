...
public class TargetFrontAura implements ISkillTargetTypeHandler {
		...
		//synchronized (activeChar.getKnownList().getKnownObjects())
		{
			for (WorldObject obj : objs) {
				if (obj instanceof Attackable || obj instanceof Playable) {
					...
					if (src != null) {
						if (!((Creature) obj).isInFrontOf(activeChar)) {
							continue;
						}
						...
						if (obj instanceof Player) {
							@SuppressWarnings("unused") Player trg = (Player) obj;
	...
	public static void main(String[] args) {
		SkillTargetTypeHandler.getInstance().registerSkillTargetType(new TargetFrontAura());
	}
}
