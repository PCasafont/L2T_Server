			...
			if (clan != null) {
				...
				//synchronized (activeChar.getKnownList().getKnownObjects())
				{
					for (WorldObject newTarget : objs) {
						...
						if (((Player) newTarget).getPet() != null) {
							if (Util.checkIfInRange(radius, activeChar, ((Player) newTarget).getPet(), true)) {
								if (!((Player) newTarget).getPet().isDead() && player.checkPvpSkill(newTarget, skill) && onlyFirst == false) {
									targetList.add(((Player) newTarget).getPet());
								}
							}
						}
