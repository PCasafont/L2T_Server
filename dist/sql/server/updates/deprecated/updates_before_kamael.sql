...
ALTER TABLE `items`
  ADD `time_of_use` INT;
ALTER TABLE `items`
  ADD KEY `key_time_of_use` (`time_of_use`);

-- 051016-[1334].sql
ALTER TABLE `items`
  ADD COLUMN `custom_type1` INT DEFAULT 0;
ALTER TABLE `items`
  ADD COLUMN `custom_type2` INT DEFAULT 0;

-- 051103-[1438].sql
-- needed only if your charater tables doesn't contains clan_privs already
ALTER TABLE `characters`
  ADD `clan_privs` INT DEFAULT '0' NOT NULL;

-- 051104-[1447].sql
-- needed only if your charater tables doesn't contains 'wantspeace' already
ALTER TABLE `characters`
  ADD COLUMN `wantspeace` DECIMAL(1, 0) DEFAULT 0;

-- 051112-[1505].sql
-- needed only if your charater tables doesn't contains 'deletetime' already
ALTER TABLE `characters`
  MODIFY `deletetime` DECIMAL(20, 0) NOT NULL DEFAULT 0;

-- 051112-[1506].sql
-- needed only if your charater tables doesn't contains 'deleteclan' already
ALTER TABLE `characters`
  ADD COLUMN `deleteclan` DECIMAL(20, 0) NOT NULL DEFAULT 0;

-- 051129-[1670].sql
-- needed only if your clan_data tables doesn't contains 'crest_id' and 'ally_crest_id' already
ALTER TABLE `clan_data`
  ADD COLUMN `crest_id` INT DEFAULT 0;
ALTER TABLE `clan_data`
  ADD COLUMN `ally_crest_id` INT DEFAULT 0;

-- 051205-[1768].sql
-- Needed only if your character tables are needed to be preserved.
ALTER TABLE `character_hennas`
  ADD `class_index` INT(1) NOT NULL DEFAULT '0',
  DROP PRIMARY KEY,
  ADD PRIMARY KEY (`char_obj_id`, `slot`, `class_index`);
ALTER TABLE `character_quests`
  ADD `class_index` INT(1) NOT NULL DEFAULT '0',
  DROP PRIMARY KEY,
  ADD PRIMARY KEY (`char_id`, `name`, `var`, `class_index`);
ALTER TABLE `character_shortcuts`
  CHANGE `unknown` `class_index` INT(1) NOT NULL DEFAULT '0',
  DROP PRIMARY KEY,
  ADD PRIMARY KEY (`char_obj_id`, `slot`, `page`, `class_index`);
ALTER TABLE `character_skills`
  ADD `class_index` INT(1) NOT NULL DEFAULT '0',
  DROP PRIMARY KEY,
  ADD PRIMARY KEY (`char_obj_id`, `skill_id`, `class_index`);
ALTER TABLE `character_skills_save`
  ADD `class_index` INT(1) NOT NULL DEFAULT '0',
  DROP PRIMARY KEY,
  ADD PRIMARY KEY (`char_obj_id`, `skill_id`, `class_index`);
ALTER TABLE `characters`
  ADD `base_class` INT(2) NOT NULL DEFAULT '0';
...
-- 060215-[c4_1489].sql
ALTER TABLE `clan_data`
  ADD `crest_large_id` INT(11)
  AFTER `crest_id`;

-- 060215-[c4req_update].sql
ALTER TABLE `character_recipebook`
  ADD type INT NOT NULL DEFAULT 0;
UPDATE `character_recipebook`
SET type = 1;

-- 11012007_3477.sql
ALTER TABLE `clanhall`
  ADD paid INT(1) NOT NULL DEFAULT '0';
UPDATE `clanhall`
SET paid = 1
WHERE paidUntil > 0; -- 20060305-[1575].sql
-- add column onlinetime
ALTER TABLE `characters`
  ADD `onlinetime` DECIMAL(20, 0) DEFAULT '0' NOT NULL
  AFTER `online`;
...
-- 20060424b.sql
UPDATE `seven_signs`
SET dawn_red_stones         = dawn_red_stones + dusk_red_stones,
...
ALTER TABLE `character_friends`
  ADD COLUMN friend_id INT(11) DEFAULT 0 NOT NULL
  AFTER char_id;
-- get the friend_id
UPDATE `character_friends`
SET friend_id = (SELECT obj_Id
                 FROM characters
                 WHERE char_name = friend_name);

-- 20060527-[2012].sql
-- Alter `characters` table
ALTER TABLE `characters`
  ADD COLUMN in_jail DECIMAL(1, 0) DEFAULT 0;
ALTER TABLE `characters`
  ADD COLUMN jail_timer DECIMAL(20, 0) DEFAULT 0;
-- Insert data in table `zone`
INSERT INTO `zone` VALUES (1, 'Jail', 'GM Jail', -115600, -250700, -113500, -248200, 0, 0);

-- 20060712-[dp1896].sql
ALTER TABLE `global_tasks`
  CHANGE `last_activation` `last_activation` DECIMAL(20, 0) NOT NULL DEFAULT 0;

-- 20060920-[dp2090].sql
ALTER TABLE `characters`
  ADD power_grade DECIMAL(11, 0);

-- 20060925-[dp2103].sql
ALTER TABLE `characters`
  CHANGE power_grade power_grade DECIMAL(11, 0) NULL DEFAULT NULL;

-- 20061010-[dp2162].sql
ALTER TABLE `character_subclasses`
  CHANGE `exp` `exp` DECIMAL(20, 0) DEFAULT '0' NOT NULL;
ALTER TABLE `characters`
  CHANGE `exp` `exp` DECIMAL(20, 0) DEFAULT NULL;
ALTER TABLE `pets`
  CHANGE `exp` `exp` DECIMAL(20, 0) DEFAULT NULL;

-- 20061015-[dp2197].sql
ALTER TABLE `characters`
  ADD nobless DECIMAL(1, 0) DEFAULT '0' NOT NULL;

-- 20070501.sql
DELETE FROM `character_quests`
WHERE var = 'awaitSealedMStone';
...
-- update05152006.sql
ALTER TABLE `character_subclasses`
  ADD COLUMN class_index INT(1) NOT NULL DEFAULT 0
  AFTER level;

-- update06122007.sql
ALTER TABLE `items`
  ADD mana_left DECIMAL(3, 0) NOT NULL DEFAULT -1;

-- update09122007.sql
ALTER TABLE `characters`
  ADD COLUMN death_penalty_level INT(2) NOT NULL DEFAULT 0
  AFTER clan_create_expiry_time;
...
-- update17112007.sql
ALTER TABLE `character_skills_save`
  ADD buff_index INT(2) NOT NULL DEFAULT 0;

-- update20060522.sql
ALTER TABLE `seven_signs_status`
  MODIFY COLUMN dawn_stone_score DECIMAL(20, 0) NOT NULL DEFAULT 0,
  MODIFY COLUMN dusk_stone_score DECIMAL(20, 0) NOT NULL DEFAULT 0;
ALTER TABLE `seven_signs`
  MODIFY COLUMN ancient_adena_amount DECIMAL(20, 0) NOT NULL DEFAULT 0,
  MODIFY COLUMN contribution_score DECIMAL(20, 0) NOT NULL DEFAULT 0;

-- update20060607.sql
ALTER TABLE `characters`
  ADD COLUMN `isin7sdungeon` DECIMAL(1, 0) NOT NULL DEFAULT 0
  AFTER `deleteclan`;

-- update20061118.sql
ALTER TABLE `clan_data`
  ADD COLUMN reputation_score INT NOT NULL DEFAULT 0;

ALTER TABLE `characters`
  ADD COLUMN subpledge INT NOT NULL DEFAULT 0;

-- update20061124.sql
ALTER TABLE `characters`
  ADD COLUMN last_recom_date DECIMAL(20, 0) NOT NULL DEFAULT 0
  AFTER subpledge;

-- update20061126.sql
ALTER TABLE `character_skills_save`
  ADD COLUMN reuse_delay INT(8) NOT NULL DEFAULT 0
  AFTER effect_cur_time;
ALTER TABLE `character_skills_save`
  ADD COLUMN restore_type INT(1) NOT NULL DEFAULT 0
  AFTER reuse_delay;

-- update20061206.sql
ALTER TABLE `characters`
  ADD COLUMN lvl_joined_academy INT(1) NOT NULL DEFAULT 0
  AFTER last_recom_date;
ALTER TABLE `characters`
  ADD COLUMN apprentice INT(1) NOT NULL DEFAULT 0
  AFTER lvl_joined_academy;
ALTER TABLE `characters`
  ADD COLUMN sponsor INT(1) NOT NULL DEFAULT 0
  AFTER apprentice;

-- update20061207.sql
ALTER TABLE `character_skills`
  CHANGE `skill_name` `skill_name` VARCHAR(30);

-- update20061208.sql
ALTER TABLE `character_skills`
  CHANGE skill_name skill_name VARCHAR(35);

-- update20061230.sql
-- *** DANGER *** - This update must DROP & CREATE the `clanhall` & `auction` tables due to structure changes
ALTER TABLE `clan_data`
  ADD `auction_bid_at` INT NOT NULL DEFAULT '0';
ALTER TABLE `auction_bid`
  ADD `time_bid` DECIMAL(20, 0) NOT NULL DEFAULT '0';
ALTER TABLE `auction_bid`
  ADD `clan_name` VARCHAR(50) NOT NULL
  AFTER `bidderName`;

-- setting zones
DELETE FROM `zone`
WHERE type = 'Clan Hall';
ALTER TABLE `zone`
  ADD `z2` INT(11) NOT NULL DEFAULT '0'
  AFTER `z`;
...
-- update20070101.sql
ALTER TABLE `characters`
  ADD COLUMN varka_ketra_ally INT(1) NOT NULL DEFAULT 0
  AFTER sponsor;
...
INSERT INTO `repair_character_quests` SELECT
                                        `char_id`,
                                        `value`
                                      FROM `character_quests`
                                      WHERE `name` = '336_CoinOfMagic' AND `var` = 'cond';
UPDATE `character_quests`, repair_character_quests
SET
...
ALTER TABLE `raidboss_spawnlist`
  DROP respawn_delay;
ALTER TABLE `raidboss_spawnlist`
  ADD respawn_min_delay INT(11) NOT NULL DEFAULT '43200'
  AFTER heading; -- 12 (36-24) hours
ALTER TABLE `raidboss_spawnlist`
  ADD respawn_max_delay INT(11) NOT NULL DEFAULT '129600'
  AFTER respawn_min_delay; -- 36 hours
DELETE FROM `raidboss_spawnlist`
WHERE boss_id IN (25328, 25339, 25342, 25346, 25349); -- remove Shadow of Halisha and Hellman spawns (possible exploits)-- update20070303.sql
ALTER TABLE `clan_data`
