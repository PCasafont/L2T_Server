...
class Quest(JQuest):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if npcId == 20015:
            st.set("id", "0")
            if st.getInt("cond") == 1 and st.getQuestItemsCount(LUCKY_KEY) == 1 and st.getQuestItemsCount(
                    FAMILYS_ASHES) < 3:
                if st.getRandom(2) == 0:
            ...
            st.set("id", "0")
            if st.getInt("cond") == 1 and st.getQuestItemsCount(HUB_SCENT) == 1 and st.getQuestItemsCount(
                    HEART_OF_LUNACY) < 3:
                if st.getRandom(2) == 0:
