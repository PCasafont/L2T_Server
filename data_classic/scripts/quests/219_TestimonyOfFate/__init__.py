...
class Quest(JQuest):
    def __init__(self, id, name, descr):
        ...
        self.questItemIds = [ALDERS_SKULL1_ID, KAIRAS_INSTRUCTIONS_ID, REVELATIONS_MANUSCRIPT_ID, KAIRAS_LETTER1_ID,
                             KASANDRAS_REMAINS_ID,
                             BELLADONNA_ID, HERBALISM_TEXTBOOK_ID, IXIAS_LIST_ID, MEDUSA_ICHOR_ID, TYRANTS_BLOOD_ID,
                             M_SPIDER_FLUIDS_ID,
                             DEAD_SEEKER_DUNG_ID, NIGHTSHADE_ROOT_ID, ALDERS_SKULL2_ID, ALDERS_RECEIPT_ID,
                             KAIRAS_RECOMMEND_ID,
                             ARKENIAS_LETTER_ID, PALUS_CHARM_ID, THIFIELS_LETTER_ID, ARKENIAS_NOTE_ID,
                             RED_FAIRY_DUST_ID, TIMIRIRAN_SAP_ID,
                             PIXY_GARNET_ID, GRANDIS_SKULL_ID, KARUL_BUGBEAR_SKULL_ID, BREKA_OVERLORD_SKULL_ID,
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30476 and st.getInt("cond") == 0 and st.getInt("onlyone") == 0:
        ...
        elif npcId == 30476 and st.getInt("cond") >= 1 and (
                    st.getQuestItemsCount(METHEUS_FUNERAL_JAR_ID) or st.getQuestItemsCount(KASANDRAS_REMAINS_ID)):
            htmltext = "30476-07.htm"
        elif npcId == 30476 and st.getInt("cond") >= 1 and (
                    st.getQuestItemsCount(HERBALISM_TEXTBOOK_ID) or st.getQuestItemsCount(IXIAS_LIST_ID)):
            ...
            st.addSpawn(30613, 78977, 149036, -3597, 300000)
                    ...
                    st.getQuestItemsCount(ALDERS_SKULL2_ID) or st.getQuestItemsCount(ALDERS_RECEIPT_ID)):
        ...
        elif npcId == 30476 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                KAIRAS_INSTRUCTIONS_ID) and player.getLevel() < 37:
            ...
            st.takeItems(KAIRAS_INSTRUCTIONS_ID, 1)
                ...
                KAIRAS_RECOMMEND_ID) and player.getLevel() >= 38:
        ...
        elif npcId == 30614 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                METHEUS_FUNERAL_JAR_ID) and st.getQuestItemsCount(KASANDRAS_REMAINS_ID) == 0:
            htmltext = "30614-02.htm"
        elif npcId == 30614 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                METHEUS_FUNERAL_JAR_ID) == 0 and st.getQuestItemsCount(KASANDRAS_REMAINS_ID):
            st.set("cond", "5")
            htmltext = "30614-03.htm"
            st.giveItems(HERBALISM_TEXTBOOK_ID, 1)
            st.takeItems(KASANDRAS_REMAINS_ID, 1)
        elif npcId == 30614 and st.getInt("cond") >= 1 and (
                    st.getQuestItemsCount(HERBALISM_TEXTBOOK_ID) or st.getQuestItemsCount(IXIAS_LIST_ID)):
            ...
            st.takeItems(BELLADONNA_ID, 1)
                                    ...
                                    st.getQuestItemsCount(ALDERS_SKULL1_ID) or st.getQuestItemsCount(
                                    ALDERS_SKULL2_ID) or st.getQuestItemsCount(
                                ALDERS_RECEIPT_ID) or st.getQuestItemsCount(
                            REVELATIONS_MANUSCRIPT_ID) or st.getQuestItemsCount(
                        KAIRAS_INSTRUCTIONS_ID) or st.getQuestItemsCount(KAIRAS_RECOMMEND_ID)):
        ...
        elif npcId == 30463 and st.getInt("cond") >= 1 and st.getQuestItemsCount(IXIAS_LIST_ID) and (
                                    st.getQuestItemsCount(MEDUSA_ICHOR_ID) < 10 or st.getQuestItemsCount(
                                M_SPIDER_FLUIDS_ID) < 10 or st.getQuestItemsCount(
                            DEAD_SEEKER_DUNG_ID) < 10 or st.getQuestItemsCount(
                        TYRANTS_BLOOD_ID) < 10 or st.getQuestItemsCount(NIGHTSHADE_ROOT_ID) < 10):
            htmltext = "30463-02.htm"
        elif npcId == 30463 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                IXIAS_LIST_ID) and st.getQuestItemsCount(MEDUSA_ICHOR_ID) >= 10 and st.getQuestItemsCount(
            M_SPIDER_FLUIDS_ID) >= 10 and st.getQuestItemsCount(
            DEAD_SEEKER_DUNG_ID) >= 10 and st.getQuestItemsCount(TYRANTS_BLOOD_ID) >= 10 and st.getQuestItemsCount(
            NIGHTSHADE_ROOT_ID) >= 10:
        ...
        elif npcId == 30463 and st.getInt("cond") >= 1 and (
                                    st.getQuestItemsCount(ALDERS_SKULL1_ID) or st.getQuestItemsCount(
                                    ALDERS_SKULL2_ID) or st.getQuestItemsCount(
                                ALDERS_RECEIPT_ID) or st.getQuestItemsCount(
                            REVELATIONS_MANUSCRIPT_ID) or st.getQuestItemsCount(
        ...
        elif npcId == 30613 and st.getInt("cond") >= 1 and (
                    st.getQuestItemsCount(ALDERS_SKULL1_ID) or st.getQuestItemsCount(ALDERS_SKULL2_ID)):
        ...
        elif npcId == 30114 and st.getInt("cond") >= 1 and (
                        st.getQuestItemsCount(REVELATIONS_MANUSCRIPT_ID) or st.getQuestItemsCount(
                        KAIRAS_INSTRUCTIONS_ID) or st.getQuestItemsCount(KAIRAS_RECOMMEND_ID)):
        ...
        elif npcId == 30358 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                PALUS_CHARM_ID) and st.getQuestItemsCount(THIFIELS_LETTER_ID):
            htmltext = "30358-02.htm"
        elif npcId == 30358 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                PALUS_CHARM_ID) and st.getQuestItemsCount(ARKENIAS_NOTE_ID):
            htmltext = "30358-03.htm"
        elif npcId == 30358 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                PALUS_CHARM_ID) and st.getQuestItemsCount(ARKENIAS_LETTER_ID):
            htmltext = "30358-04.htm"
            st.giveItems(MARK_OF_FATE_ID, 1)
            st.giveItems(7562, 16)
            st.addExpAndSp(68273, 4556)
            st.giveItems(57, 12385)
            ...
            st.set("onlyone", "1")
        elif npcId == 30419 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                PALUS_CHARM_ID) and st.getQuestItemsCount(THIFIELS_LETTER_ID):
            htmltext = "30419-01.htm"
        elif npcId == 30419 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                PALUS_CHARM_ID) and st.getQuestItemsCount(ARKENIAS_NOTE_ID) and (
                        st.getQuestItemsCount(RED_FAIRY_DUST_ID) < 1 or st.getQuestItemsCount(TIMIRIRAN_SAP_ID) < 1):
            htmltext = "30419-03.htm"
        elif npcId == 30419 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                PALUS_CHARM_ID) and st.getQuestItemsCount(ARKENIAS_NOTE_ID) and (
                        st.getQuestItemsCount(RED_FAIRY_DUST_ID) >= 1 and st.getQuestItemsCount(TIMIRIRAN_SAP_ID) >= 1):
            htmltext = "30419-04.htm"
        elif npcId == 30419 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                PALUS_CHARM_ID) and st.getQuestItemsCount(ARKENIAS_LETTER_ID):
            htmltext = "30419-06.htm"
        elif npcId == 31845 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                PALUS_CHARM_ID) and st.getQuestItemsCount(ARKENIAS_NOTE_ID) and st.getQuestItemsCount(
            RED_FAIRY_DUST_ID) == 0 and st.getQuestItemsCount(PIXY_GARNET_ID) == 0:
            htmltext = "31845-01.htm"
        elif npcId == 31845 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                PALUS_CHARM_ID) and st.getQuestItemsCount(ARKENIAS_NOTE_ID) and st.getQuestItemsCount(
            RED_FAIRY_DUST_ID) == 0 and st.getQuestItemsCount(PIXY_GARNET_ID) and (
                                st.getQuestItemsCount(GRANDIS_SKULL_ID) < 10 or st.getQuestItemsCount(
                            KARUL_BUGBEAR_SKULL_ID) < 10 or st.getQuestItemsCount(
                        BREKA_OVERLORD_SKULL_ID) < 10 or st.getQuestItemsCount(LETO_OVERLORD_SKULL_ID) < 10):
            htmltext = "31845-03.htm"
        elif npcId == 31845 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                PALUS_CHARM_ID) and st.getQuestItemsCount(ARKENIAS_NOTE_ID) and st.getQuestItemsCount(
            RED_FAIRY_DUST_ID) == 0 and st.getQuestItemsCount(PIXY_GARNET_ID) and st.getQuestItemsCount(
            GRANDIS_SKULL_ID) >= 10 and st.getQuestItemsCount(
            KARUL_BUGBEAR_SKULL_ID) >= 10 and st.getQuestItemsCount(
            BREKA_OVERLORD_SKULL_ID) >= 10 and st.getQuestItemsCount(LETO_OVERLORD_SKULL_ID) >= 10:
            ...
            st.takeItems(LETO_OVERLORD_SKULL_ID, st.getQuestItemsCount(LETO_OVERLORD_SKULL_ID))
                ...
                PALUS_CHARM_ID) and st.getQuestItemsCount(ARKENIAS_NOTE_ID) and st.getQuestItemsCount(
            RED_FAIRY_DUST_ID) and st.getQuestItemsCount(PIXY_GARNET_ID) == 0:
            htmltext = "31845-05.htm"
        elif npcId == 31850 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                PALUS_CHARM_ID) and st.getQuestItemsCount(ARKENIAS_NOTE_ID) and st.getQuestItemsCount(
            TIMIRIRAN_SAP_ID) == 0 and st.getQuestItemsCount(TIMIRIRAN_SEED_ID) == 0:
            htmltext = "31850-01.htm"
        elif npcId == 31850 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                PALUS_CHARM_ID) and st.getQuestItemsCount(ARKENIAS_NOTE_ID) and st.getQuestItemsCount(
            TIMIRIRAN_SAP_ID) == 0 and st.getQuestItemsCount(TIMIRIRAN_SEED_ID) and st.getQuestItemsCount(
            BLACK_WILLOW_LEAF_ID) == 0:
            ...
            st.takeItems(TIMIRIRAN_SEED_ID, 1)
                ...
                PALUS_CHARM_ID) and st.getQuestItemsCount(ARKENIAS_NOTE_ID) and st.getQuestItemsCount(
            TIMIRIRAN_SAP_ID) and st.getQuestItemsCount(TIMIRIRAN_SEED_ID) == 0:
    ...
    def onKill(self, npc, player, isPet):
        ...
        npcId = npc.getNpcId()
        if npcId == 20144:
            if st.getInt("cond") >= 1 and st.getQuestItemsCount(METHEUS_FUNERAL_JAR_ID) and st.getQuestItemsCount(
                    KASANDRAS_REMAINS_ID) == 0:
                st.giveItems(KASANDRAS_REMAINS_ID, 1)
                st.takeItems(METHEUS_FUNERAL_JAR_ID, 1)
                st.set("cond", "3")
                st.playSound("Itemsound.quest_middle")
        elif npcId == 20158:
            if st.getInt("cond") >= 1 and st.getQuestItemsCount(IXIAS_LIST_ID) and st.getQuestItemsCount(
                    MEDUSA_ICHOR_ID) < 10:
                if st.getRandom(2) == 1:
                    if st.getQuestItemsCount(MEDUSA_ICHOR_ID) == 9:
                        st.giveItems(MEDUSA_ICHOR_ID, 1)
                        st.playSound("Itemsound.quest_middle")
                        if st.getQuestItemsCount(MEDUSA_ICHOR_ID) >= 10 and st.getQuestItemsCount(
                                M_SPIDER_FLUIDS_ID) >= 10 and st.getQuestItemsCount(
                            DEAD_SEEKER_DUNG_ID) >= 10 and st.getQuestItemsCount(
                            TYRANTS_BLOOD_ID) >= 10 and st.getQuestItemsCount(NIGHTSHADE_ROOT_ID) >= 10:
                            st.set("cond", "7")
                    else:
                        st.giveItems(MEDUSA_ICHOR_ID, 1)
            ...
            if st.getInt("cond") >= 1 and st.getQuestItemsCount(PALUS_CHARM_ID) and st.getQuestItemsCount(
                    ARKENIAS_NOTE_ID) and st.getQuestItemsCount(TIMIRIRAN_SAP_ID) == 0 and st.getQuestItemsCount(
                TIMIRIRAN_SEED_ID) and st.getQuestItemsCount(BLACK_WILLOW_LEAF_ID) == 0:
                st.giveItems(BLACK_WILLOW_LEAF_ID, 1)
                st.playSound("Itemsound.quest_middle")
        elif npcId == 20554:
            if st.getInt("cond") >= 1 and st.getQuestItemsCount(PALUS_CHARM_ID) and st.getQuestItemsCount(
                    ARKENIAS_NOTE_ID) and st.getQuestItemsCount(RED_FAIRY_DUST_ID) == 0 and st.getQuestItemsCount(
                PIXY_GARNET_ID) and st.getQuestItemsCount(GRANDIS_SKULL_ID) < 10:
            ...
            if st.getInt("cond") >= 1 and st.getQuestItemsCount(PALUS_CHARM_ID) and st.getQuestItemsCount(
                    ARKENIAS_NOTE_ID) and st.getQuestItemsCount(RED_FAIRY_DUST_ID) == 0 and st.getQuestItemsCount(
                PIXY_GARNET_ID) and st.getQuestItemsCount(KARUL_BUGBEAR_SKULL_ID) < 10:
            ...
            if st.getInt("cond") >= 1 and st.getQuestItemsCount(PALUS_CHARM_ID) and st.getQuestItemsCount(
                    ARKENIAS_NOTE_ID) and st.getQuestItemsCount(RED_FAIRY_DUST_ID) == 0 and st.getQuestItemsCount(
                PIXY_GARNET_ID) and st.getQuestItemsCount(BREKA_OVERLORD_SKULL_ID) < 10:
            ...
            if st.getInt("cond") >= 1 and st.getQuestItemsCount(PALUS_CHARM_ID) and st.getQuestItemsCount(
                    ARKENIAS_NOTE_ID) and st.getQuestItemsCount(RED_FAIRY_DUST_ID) == 0 and st.getQuestItemsCount(
                PIXY_GARNET_ID) and st.getQuestItemsCount(LETO_OVERLORD_SKULL_ID) < 10:
