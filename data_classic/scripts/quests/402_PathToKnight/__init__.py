...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        player = st.getPlayer()
        coin1, coin2, coin3, coin4, coin5, coin6 = st.getQuestItemsCount(COIN_OF_LORDS1), st.getQuestItemsCount(
            COIN_OF_LORDS2), st.getQuestItemsCount(COIN_OF_LORDS3), st.getQuestItemsCount(
            COIN_OF_LORDS4), st.getQuestItemsCount(COIN_OF_LORDS5), st.getQuestItemsCount(COIN_OF_LORDS6)
        guards_mark1, guards_mark2, guards_mark3 = st.getQuestItemsCount(GLUDIO_GUARDS_MARK1), st.getQuestItemsCount(
            GLUDIO_GUARDS_MARK2), st.getQuestItemsCount(GLUDIO_GUARDS_MARK3)
        church_mark1, church_mark2, church_mark3 = st.getQuestItemsCount(EINHASAD_CHURCH_MARK1), st.getQuestItemsCount(
            EINHASAD_CHURCH_MARK2), st.getQuestItemsCount(EINHASAD_CHURCH_MARK3)
    ...
    def onKill(self, npc, player, isPet):
        ...
        if st.getInt("cond"):
            ...
            if st.getQuestItemsCount(item_required) and st.getQuestItemsCount(item) < max and st.getRandom(
                    100) < chance:
                st.giveItems(item, 1)
