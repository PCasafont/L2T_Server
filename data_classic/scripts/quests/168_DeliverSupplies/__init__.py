...
class Quest(JQuest):
    def __init__(self, id, name, descr):
        ...
        self.questItemIds = [SENTRY_BLADE1_ID, OLD_BRONZE_SWORD_ID, JENNIES_LETTER_ID, SENTRY_BLADE2_ID,
                             SENTRY_BLADE3_ID]
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30349 and st.getInt("cond") == 0 and st.getInt("onlyone") == 0:
        ...
        elif npcId == 30349 and st.getInt("cond") == 2 and st.getQuestItemsCount(
                SENTRY_BLADE1_ID) == 1 and st.getQuestItemsCount(SENTRY_BLADE2_ID) == 1 and st.getQuestItemsCount(
            SENTRY_BLADE3_ID) == 1:
            htmltext = "30349-05.htm"
            st.takeItems(SENTRY_BLADE1_ID, 1)
            st.set("cond", "3")
            st.playSound("ItemSound.quest_middle")
        elif npcId == 30349 and st.getInt("cond") == 3 and st.getQuestItemsCount(SENTRY_BLADE1_ID) == 0 and (
                        st.getQuestItemsCount(SENTRY_BLADE2_ID) == 1 or st.getQuestItemsCount(SENTRY_BLADE3_ID) == 1):
            htmltext = "30349-07.htm"
        elif npcId == 30349 and st.getInt("cond") == 4 and st.getQuestItemsCount(
                OLD_BRONZE_SWORD_ID) == 2 and st.getInt("onlyone") == 0:
            if st.getInt("id") != 168:
                st.set("id", "168")
            ...
            if npcId == 30360 and st.getInt("cond") == 1 and st.getQuestItemsCount(JENNIES_LETTER_ID) == 1:
                ...
                st.set("cond", "2")
                st.playSound("ItemSound.quest_middle")
            elif npcId == 30360 and st.getInt("cond") == 2 and (
                            st.getQuestItemsCount(SENTRY_BLADE1_ID) + st.getQuestItemsCount(
                            SENTRY_BLADE2_ID) + st.getQuestItemsCount(SENTRY_BLADE3_ID)) > 0:
                htmltext = "30360-02.htm"
            elif npcId == 30355 and st.getInt("cond") == 3 and st.getQuestItemsCount(
                    SENTRY_BLADE2_ID) == 1 and st.getQuestItemsCount(SENTRY_BLADE1_ID) == 0:
            ...
            elif npcId == 30357 and st.getInt("cond") == 3 and st.getQuestItemsCount(
                    SENTRY_BLADE3_ID) == 1 and st.getQuestItemsCount(SENTRY_BLADE1_ID) == 0:
