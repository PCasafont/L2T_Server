...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30624 and st.getInt("cond") == 0:
            ...
            st.addExpAndSp(63537, 4360)
            st.giveItems(57, 11488)
        ...
        elif npcId == 30625 and st.getInt("cond") >= 5 and (
                                        st.getQuestItemsCount(ASCALONS_LETTER2) or st.getQuestItemsCount(
                                        WHITE_ROSE_INSIGNIA) or st.getQuestItemsCount(
                                    GROOTS_LETTER) or st.getQuestItemsCount(ASCALONS_LETTER3) or st.getQuestItemsCount(
                            MOUENS_ORDER1) or st.getQuestItemsCount(MOUENS_ORDER2) or st.getQuestItemsCount(
                    MOUENS_LETTER)):
        ...
        elif npcId == 30093 and st.getInt("step") >= 9 and (
                            st.getQuestItemsCount(ASCALONS_LETTER3) or st.getQuestItemsCount(
                            MOUENS_ORDER1) or st.getQuestItemsCount(MOUENS_ORDER2) or st.getQuestItemsCount(
                    MOUENS_LETTER)):
