    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30629:
            if st.getInt("cond") == 0 and st.getInt("onlyone") == 0:
                ...
                if st.getQuestItemsCount(TONE_OF_WATER) and st.getQuestItemsCount(
                        TONE_OF_FIRE) and st.getQuestItemsCount(TONE_OF_WIND) and st.getQuestItemsCount(TONE_OF_EARTH):
            ...
            if st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS) == 1 and st.getQuestItemsCount(
                    TONE_OF_WATER) == 0 and st.getQuestItemsCount(UNDINE_CHARM) == 0:
                htmltext = "30413-01.htm"
                st.giveItems(UNDINE_CHARM, 1)
                st.set("cond", "7")
            elif st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS) == 1 and st.getQuestItemsCount(
                    UNDINE_CHARM) == 1:
                if st.getQuestItemsCount(DAZZLING_DROP) < 20:
                    ...
                    TONE_OF_WATER) == 1 and st.getQuestItemsCount(UNDINE_CHARM) == 0:
                htmltext = "30413-04.htm"
        elif npcId == 30411:
            if st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS) == 1 and st.getQuestItemsCount(
                    TONE_OF_FIRE) == 0 and st.getQuestItemsCount(SALAMANDER_CHARM) == 0:
                ...
                if st.getQuestItemsCount(FLAME_CRYSTAL) < 5:
                    ...
                    TONE_OF_FIRE) == 1 and st.getQuestItemsCount(SALAMANDER_CHARM) == 0:
                htmltext = "30411-04.htm"
        elif npcId == 30412:
            if st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS) == 1 and st.getQuestItemsCount(
                    TONE_OF_WIND) == 0 and st.getQuestItemsCount(SYLPH_CHARM) == 0:
                htmltext = "30412-01.htm"
            elif st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS) == 1 and st.getQuestItemsCount(
                    SYLPH_CHARM) == 1:
                if st.getQuestItemsCount(HARPYS_FEATHER) + st.getQuestItemsCount(
                        WYRMS_WINGBONE) + st.getQuestItemsCount(WINDSUS_MANE) < 40:
                    ...
                    TONE_OF_WIND) == 1 and st.getQuestItemsCount(SYLPH_CHARM) == 0:
                htmltext = "30412-05.htm"
        elif npcId == 30409:
            if st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS) == 1 and st.getQuestItemsCount(
                    TONE_OF_EARTH) == 0 and st.getQuestItemsCount(SERPENT_CHARM) == 0:
                htmltext = "30409-01.htm"
            elif st.getInt("cond") and st.getQuestItemsCount(SCORE_OF_ELEMENTS) == 1 and st.getQuestItemsCount(
                    SERPENT_CHARM) == 1:
                if st.getQuestItemsCount(EN_MONSTEREYE_SHELL) + st.getQuestItemsCount(
                        EN_STONEGOLEM_POWDER) + st.getQuestItemsCount(EN_IRONGOLEM_SCRAP) < 30:
                    ...
                    TONE_OF_EARTH) == 1 and st.getQuestItemsCount(SERPENT_CHARM) == 0:
