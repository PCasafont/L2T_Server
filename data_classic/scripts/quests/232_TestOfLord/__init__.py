...
STATS = [["atubaStat", "nerugaStat", "urutuStat", "urutuDrop", "dudaStat", "gandiStat", "markantusStat"],
         ["cond", "phase"]]
...
class Quest(JQuest):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if item == MARSH_SPIDER_FEELER and int(st.get(var)) == value:
            ...
            if count == maxcount - 1:
                st.playSound("ItemSound.quest_middle")
                if newValue == 1 and st.getQuestItemsCount(RAGNA_ORC_HEAD) and st.getQuestItemsCount(
                        RAGNA_CHIEF_NOTICE):
                    st.set(var, "2")
                else:
