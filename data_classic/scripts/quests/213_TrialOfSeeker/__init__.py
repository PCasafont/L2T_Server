        ...
        if st.getQuestItemsCount(required) and count < maxqty:
            if st.getRandom(100) < chance:
                ...
                if count + 1 == maxqty:
                    st.playSound("Itemsound.quest_middle")
                    if cond == 4:
                        if st.getQuestItemsCount(OL_MAHUM_RUNESTONE_ID) + st.getQuestItemsCount(
                                TUREK_RUNESTONE_ID) + st.getQuestItemsCount(ANT_RUNESTONE_ID) + st.getQuestItemsCount(
                            TURAK_BUGBEAR_RUNESTONE_ID) == 4:
                            st.set("cond", str(cond + 1))
                        return
                    elif cond == 16:
                        if st.getQuestItemsCount(ABYSS_RUNESTONE1_ID) + st.getQuestItemsCount(
                                ABYSS_RUNESTONE2_ID) + st.getQuestItemsCount(
                            ABYSS_RUNESTONE3_ID) + st.getQuestItemsCount(ABYSS_RUNESTONE4_ID) == 4:
                            st.set("cond", str(cond + 1))
                        return
                    else:
                        st.set("cond", str(cond + 1))
