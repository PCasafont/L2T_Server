...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        if id == State.COMPLETED:
            ...
            if npcId == 30017 and st.getInt("cond") and st.getQuestItemsCount(
                    GALLINS_OAK_WAND) >= 1 and not HaveAllQuestItems(st):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if st.getInt("cond") >= 1 and st.getItemEquipped(
                Inventory.PAPERDOLL_RHAND) == GALLINS_OAK_WAND and not st.getQuestItemsCount(
