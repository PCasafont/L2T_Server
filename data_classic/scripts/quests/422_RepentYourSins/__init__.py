...
class Quest(JQuest):
    def __init__(self, id, name, descr):
        ...
        self.questItemIds = [SCAVENGER_WERERAT_SKULL, TUREK_WARHOUND_TAIL, TYRANT_KINGPIN_HEART,
                             TRISALIM_TARANTULAS_VENOM_SAC, MANUAL_OF_MANACLES,
        ...
        if npcId == 30981:  # Black Judge
            ...
            if condition >= 14:
                if st.getQuestItemsCount(MANUAL_OF_MANACLES) == 1:
                    if st.getQuestItemsCount(SILVER_NUGGET) < 10 or st.getQuestItemsCount(
                            STEEL) < 5 or st.getQuestItemsCount(ADAMANTINE_NUGGET) < 2 \
                            or st.getQuestItemsCount(COKES) < 10 or st.getQuestItemsCount(BLACKSMITHS_FRAME) < 1:
                        htmltext = "30300-02.htm"
                    elif st.getQuestItemsCount(SILVER_NUGGET) >= 10 and st.getQuestItemsCount(
                            STEEL) >= 5 and st.getQuestItemsCount(ADAMANTINE_NUGGET) >= 2 \
                ...
                elif st.getQuestItemsCount(PENITENTS_MANACLES1) or st.getQuestItemsCount(
                        PENITENTS_MANACLES) or st.getQuestItemsCount(PENITENTS_MANACLES2):
