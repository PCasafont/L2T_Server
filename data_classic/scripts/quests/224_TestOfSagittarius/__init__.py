...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        elif npcId == 30717 and step == 11 and st.getQuestItemsCount(STAKATO_CHITIN) and st.getQuestItemsCount(
                MITHRIL_CLIP) and st.getQuestItemsCount(ST_BOWSTRING) and st.getQuestItemsCount(MANASHENS_HORN):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if st.getInt("step") == step and st.getQuestItemsCount(itemid) < maxcount and random < chance:
            if dropcondition == 1:
                ...
                if st.getQuestItemsCount(MITHRIL_CLIP) and st.getQuestItemsCount(
                        ST_BOWSTRING) and st.getQuestItemsCount(MANASHENS_HORN):
                    giveMiddle(st, itemid, step)
                else:
                    giveNormal(st, itemid)
            elif dropcondition == 5:
                if st.getQuestItemsCount(MITHRIL_CLIP) and st.getQuestItemsCount(
                        ST_BOWSTRING) and st.getQuestItemsCount(STAKATO_CHITIN):
                    giveMiddle(st, itemid, step)
                elif st.getQuestItemsCount(MITHRIL_CLIP) and st.getQuestItemsCount(
                        MANASHENS_HORN) and st.getQuestItemsCount(STAKATO_CHITIN):
                    giveMiddle(st, itemid, step)
                elif st.getQuestItemsCount(ST_BOWSTRING) and st.getQuestItemsCount(
                        MANASHENS_HORN) and st.getQuestItemsCount(STAKATO_CHITIN):
                    giveMiddle(st, itemid, step)
                else:
                    giveNormal(st, itemid)
...
for mobId in [20230, 20232, 20233, 20234, 20269, 20270, 27090, 20551, 20563, 20577, 20578, 20579, 20580, 20581, 20582,
              20079, 20080, 20081, 20082, 20084, 20086, 20089, 20090]:
