...
class Quest(JQuest):
    ...
    def onAdvEvent(self, event, npc, player):
        ...
        # Everything bellow here is for condition 4 and 5
            ...
            if st.getQuestItemsCount(SCEPTER_OF_BREKA) and st.getQuestItemsCount(
                    SCEPTER_OF_ENKU) and st.getQuestItemsCount(SCEPTER_OF_VUKU) and st.getQuestItemsCount(
                SCEPTER_OF_TUREK) and st.getQuestItemsCount(SCEPTER_OF_TUNATH):
                st.set("cond", "5")
    ...
    def onTalk(self, npc, player):
        ...
        if id == State.CREATED:  # Check if is starting the quest
            ...
            # Condition Oriented code for optimal performance
                ...
                if npcId == 30514 and st.getQuestItemsCount(VOKIYANS_ORDER1) and (
                                    st.getQuestItemsCount(MANASHEN_SHARD) < 10 or st.getQuestItemsCount(
                                TYRANT_TALON) < 10 or st.getQuestItemsCount(GUARDIAN_BASILISK_FANG) < 10):
                    htmltext = "30514-06.htm"
                # Begin of the cross compatible code
                elif CROSS_COMPATIBLE:
                    if st.getQuestItemsCount(VOKIYANS_ORDER1) and st.getQuestItemsCount(
                            MANASHEN_SHARD) == 10 and st.getQuestItemsCount(
                        TYRANT_TALON) == 10 and st.getQuestItemsCount(GUARDIAN_BASILISK_FANG) == 10:
                        st.set("cond", "2")
                        ...
                        st.set("cond", "4")
                    elif st.getQuestItemsCount(NECKLACE_OF_AUTHORITY) and (st.getQuestItemsCount(CHIANTAS_ORDER2) or (
                                                st.getQuestItemsCount(CHIANTAS_ORDER1) and st.getQuestItemsCount(
                                                SCEPTER_OF_BREKA) and st.getQuestItemsCount(
                                            SCEPTER_OF_VUKU) and st.getQuestItemsCount(
                                        SCEPTER_OF_TUREK) and st.getQuestItemsCount(
                                    SCEPTER_OF_TUNATH) and st.getQuestItemsCount(SCEPTER_OF_ENKU))):
                        st.set("cond", "5")
                    elif st.getQuestItemsCount(NECKLACE_OF_AUTHORITY) and st.getQuestItemsCount(CHIANTAS_ORDER3):
                        if st.getQuestItemsCount(TAMLIN_ORC_SKULL) == 20 and st.getQuestItemsCount(
                                TIMAK_ORC_HEAD) == 20:
                            st.set("cond", "7")
                        else:
                            st.set("cond", "6")
                ...
                elif npcId == 30642 and st.getQuestItemsCount(NECKLACE_OF_AUTHORITY) and st.getQuestItemsCount(
                        VOKIYANS_ORDER2):
                ...
                elif npcId == 30515 and st.getQuestItemsCount(NECKLACE_OF_AUTHORITY) and st.getQuestItemsCount(
                        CHIANTAS_ORDER1):
                    htmltext = "30515-01.htm"
                elif npcId == 30501 and st.getQuestItemsCount(NECKLACE_OF_AUTHORITY) and st.getQuestItemsCount(
                        CHIANTAS_ORDER1):
                ...
                if npcId == 30642 and st.getQuestItemsCount(NECKLACE_OF_AUTHORITY):
                    if st.getQuestItemsCount(CHIANTAS_ORDER1) and st.getQuestItemsCount(
                            SCEPTER_OF_BREKA) and st.getQuestItemsCount(SCEPTER_OF_VUKU) and st.getQuestItemsCount(
                        SCEPTER_OF_TUREK) and st.getQuestItemsCount(SCEPTER_OF_TUNATH) and st.getQuestItemsCount(
                        SCEPTER_OF_ENKU):
                ...
                if npcId == 30642 and st.getQuestItemsCount(NECKLACE_OF_AUTHORITY) and st.getQuestItemsCount(
                        CHIANTAS_ORDER3):
                    htmltext = "30642-10.htm"
            elif cond == 7:  # The Punishment Ends
                if npcId == 30642 and st.getQuestItemsCount(TAMLIN_ORC_SKULL) == 20 and st.getQuestItemsCount(
                        TIMAK_ORC_HEAD) == 20:
                ...
                if npcId == 30571 and st.getQuestItemsCount(TANAPIS_ORDER1) and st.getQuestItemsCount(
                        SCEPTER_OF_TANTOS):
        ...
        if cond == 1 and npcId in DROPLIST_COND_1.keys():
            ...
            if st.getQuestItemsCount(required) and count < maxquanty:
                ...
                if count + 1 == maxquanty:  # Check if got enough number of items
                    if st.getQuestItemsCount(MANASHEN_SHARD) == maxquanty and st.getQuestItemsCount(
                            TYRANT_TALON) == maxquanty and st.getQuestItemsCount(GUARDIAN_BASILISK_FANG) == maxquanty:
                        st.set("cond", "2")
                        st.playSound("Itemsound.quest_middle")
                    else:
                        st.playSound("Itemsound.quest_itemget")
            ...
            if st.getQuestItemsCount(required) and count < maxquanty:
                if st.getRandom(100) < chance:
                    st.giveItems(item, 1)
                    if count + 1 == maxquanty:  # Check if got enough number of items
                        if st.getQuestItemsCount(TIMAK_ORC_HEAD) == maxquanty and st.getQuestItemsCount(
                                TAMLIN_ORC_SKULL) == maxquanty:
                            st.set("cond", "7")
                            st.playSound("Itemsound.quest_middle")
                        else:
                            st.playSound("Itemsound.quest_itemget")
