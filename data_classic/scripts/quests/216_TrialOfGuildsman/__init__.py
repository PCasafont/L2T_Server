...
class Quest(JQuest):
    def __init__(self, id, name, descr):
        ...
        self.questItemIds = [RP_JOURNEYMAN_RING_ID, ALLTRANS_INSTRUCTIONS_ID, RP_JOURNEYMAN_RING_ID,
                             VALKONS_RECOMMEND_ID, MANDRAGORA_BERRY_ID,
                             ALLTRANS_RECOMMEND1_ID, DUNINGS_KEY_ID, NORMANS_INSTRUCTIONS_ID, NORMANS_LIST_ID,
                             NORMANS_RECEIPT_ID, ALLTRANS_RECOMMEND2_ID,
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30103 and st.getInt("cond") == 0 and st.getInt("onlyone") == 0:
        ...
        elif npcId == 30283 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                VALKONS_RECOMMEND_ID) == 1 and st.getQuestItemsCount(MANDRAGORA_BERRY_ID) == 0:
            htmltext = "30283-01.htm"
            st.set("cond", "2")
            st.playSound("ItemSound.quest_middle")
        elif npcId == 30283 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                VALKONS_RECOMMEND_ID) == 1 and st.getQuestItemsCount(MANDRAGORA_BERRY_ID) == 1:
        ...
        elif npcId == 30210 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                ALLTRANS_INSTRUCTIONS_ID) == 1 and st.getQuestItemsCount(ALLTRANS_RECOMMEND1_ID) == 1:
            htmltext = "30210-01.htm"
        elif npcId == 30210 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                ALLTRANS_INSTRUCTIONS_ID) and st.getQuestItemsCount(NORMANS_INSTRUCTIONS_ID) and st.getQuestItemsCount(
            NORMANS_RECEIPT_ID):
            htmltext = "30210-05.htm"
        elif npcId == 30210 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                ALLTRANS_INSTRUCTIONS_ID) and st.getQuestItemsCount(NORMANS_INSTRUCTIONS_ID) and st.getQuestItemsCount(
            DUNINGS_INSTRUCTIONS_ID):
            ...
            htmltext = "30210-07.htm"
                ...
                ALLTRANS_INSTRUCTIONS_ID) and st.getQuestItemsCount(NORMANS_LIST_ID):
            if st.getQuestItemsCount(GRAY_BONE_POWDER_ID) >= 70 and st.getQuestItemsCount(
                    GRANITE_WHETSTONE_ID) >= 70 and st.getQuestItemsCount(
                RED_PIGMENT_ID) >= 70 and st.getQuestItemsCount(BRAIDED_YARN_ID) >= 70:
                ...
                NORMANS_INSTRUCTIONS_ID) == 0 and st.getQuestItemsCount(NORMANS_LIST_ID) == 0 and st.getQuestItemsCount(
            ALLTRANS_INSTRUCTIONS_ID) == 1 and (
                    st.getQuestItemsCount(JOURNEYMAN_GEM_ID) or st.getQuestItemsCount(JOURNEYMAN_RING_ID)):
            htmltext = "30210-13.htm"
        elif npcId == 30688 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                ALLTRANS_INSTRUCTIONS_ID) and st.getQuestItemsCount(NORMANS_INSTRUCTIONS_ID) and st.getQuestItemsCount(
            NORMANS_RECEIPT_ID):
            ...
            htmltext = "30688-04.htm"
                ...
                NORMANS_RECEIPT_ID) == 0 and st.getQuestItemsCount(
            DUNINGS_INSTRUCTIONS_ID) == 0 and st.getQuestItemsCount(DUNINGS_KEY_ID) == 0 and st.getQuestItemsCount(
            ALLTRANS_INSTRUCTIONS_ID) == 1:
            htmltext = "30688-01.htm"
        elif npcId == 30298 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                ALLTRANS_INSTRUCTIONS_ID) and st.getQuestItemsCount(ALLTRANS_RECOMMEND2_ID):
            if player.getLevel() < 35:
                htmltext = "30298-01.htm"
            else:
                htmltext = "30298-02.htm"
        elif npcId == 30298 and st.getInt("cond") >= 1 and st.getQuestItemsCount(
                ALLTRANS_INSTRUCTIONS_ID) and st.getQuestItemsCount(PINTERS_INSTRUCTIONS_ID):
            if st.getQuestItemsCount(AMBER_BEAD_ID) < 70:
                ...
                ALLTRANS_INSTRUCTIONS_ID) == 1 and st.getQuestItemsCount(PINTERS_INSTRUCTIONS_ID) == 0 and (
                    st.getQuestItemsCount(JOURNEYMAN_DECO_BEADS_ID) or st.getQuestItemsCount(JOURNEYMAN_RING_ID)):
    ...
    def onKill(self, npc, player, isPet):
        ...
        npcId = npc.getNpcId()
        if npcId == 20223:
            if st.getInt("cond") >= 1 and st.getQuestItemsCount(VALKONS_RECOMMEND_ID) == 1 and st.getQuestItemsCount(
                    MANDRAGORA_BERRY_ID) == 0:
            ...
            if st.getInt("cond") >= 1 and st.getQuestItemsCount(
                    ALLTRANS_INSTRUCTIONS_ID) == 1 and st.getQuestItemsCount(
                NORMANS_INSTRUCTIONS_ID) == 1 and st.getQuestItemsCount(DUNINGS_INSTRUCTIONS_ID) == 1:
            ...
            if st.getInt("cond") >= 1 and st.getQuestItemsCount(
                    ALLTRANS_INSTRUCTIONS_ID) == 1 and st.getQuestItemsCount(
                NORMANS_LIST_ID) == 1 and st.getQuestItemsCount(GRAY_BONE_POWDER_ID) < 70:
            ...
            if st.getInt("cond") >= 1 and st.getQuestItemsCount(
                    ALLTRANS_INSTRUCTIONS_ID) == 1 and st.getQuestItemsCount(
                NORMANS_LIST_ID) == 1 and st.getQuestItemsCount(GRANITE_WHETSTONE_ID) < 70:
            ...
            if st.getInt("cond") >= 1 and st.getQuestItemsCount(
                    ALLTRANS_INSTRUCTIONS_ID) == 1 and st.getQuestItemsCount(
                NORMANS_LIST_ID) == 1 and st.getQuestItemsCount(RED_PIGMENT_ID) < 70:
            ...
            if st.getInt("cond") >= 1 and st.getQuestItemsCount(
                    ALLTRANS_INSTRUCTIONS_ID) == 1 and st.getQuestItemsCount(
                NORMANS_LIST_ID) == 1 and st.getQuestItemsCount(BRAIDED_YARN_ID) < 70:
            ...
            if st.getInt("cond") >= 1 and st.getQuestItemsCount(
                    ALLTRANS_INSTRUCTIONS_ID) == 1 and st.getQuestItemsCount(PINTERS_INSTRUCTIONS_ID) == 1:
                if st.getQuestItemsCount(AMBER_BEAD_ID) < 70:
