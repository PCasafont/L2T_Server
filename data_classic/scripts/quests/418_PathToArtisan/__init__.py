...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        elif npcId == 30527 and st.getInt("cond") and st.getQuestItemsCount(SILVERYS_RING) == 1 and (
                    st.getQuestItemsCount(RATMAN_TOOTH) + st.getQuestItemsCount(BIG_RATMAN_TOOTH)) < 12:
            htmltext = "30527-07.htm"
        elif npcId == 30527 and st.getInt("cond") and st.getQuestItemsCount(
                SILVERYS_RING) == 1 and st.getQuestItemsCount(RATMAN_TOOTH) >= 10 and st.getQuestItemsCount(
            BIG_RATMAN_TOOTH) >= 2:
        ...
        elif npcId == 30317 and st.getInt("cond") and st.getQuestItemsCount(
                KLUTOS_LETTER) == 0 and st.getQuestItemsCount(FOOTPRINT) == 0 and st.getQuestItemsCount(
            PASS_1ST) and st.getQuestItemsCount(PASS_2ND) == 0 and st.getQuestItemsCount(SECRET_BOX2) == 0:
            htmltext = "30317-01.htm"
        elif npcId == 30317 and st.getInt("cond") and st.getQuestItemsCount(PASS_1ST) and (
                    st.getQuestItemsCount(KLUTOS_LETTER) or st.getQuestItemsCount(FOOTPRINT)):
            htmltext = "30317-08.htm"
        elif npcId == 30317 and st.getInt("cond") and st.getQuestItemsCount(PASS_1ST) and st.getQuestItemsCount(
                PASS_2ND) and st.getQuestItemsCount(SECRET_BOX2):
            htmltext = "30317-09.htm"
        elif npcId == 30298 and st.getInt("cond") and st.getQuestItemsCount(PASS_1ST) and st.getQuestItemsCount(
                KLUTOS_LETTER):
            htmltext = "30298-01.htm"
        elif npcId == 30298 and st.getInt("cond") and st.getQuestItemsCount(PASS_1ST) and st.getQuestItemsCount(
                FOOTPRINT) and st.getQuestItemsCount(SECRET_BOX1) == 0:
            htmltext = "30298-04.htm"
        elif npcId == 30298 and st.getInt("cond") and st.getQuestItemsCount(PASS_1ST) and st.getQuestItemsCount(
                FOOTPRINT) and st.getQuestItemsCount(SECRET_BOX1):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if npcId == 20389:
            st.set("id", "0")
            if st.getInt("cond") and st.getQuestItemsCount(SILVERYS_RING) == 1 and st.getQuestItemsCount(
                    RATMAN_TOOTH) < 10:
