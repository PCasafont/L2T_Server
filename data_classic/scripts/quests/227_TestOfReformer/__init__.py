...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        elif npcId == 30666 and st.getInt("cond") == 20 and st.getQuestItemsCount(
                KATARIS_LETTER) > 0 and st.getQuestItemsCount(KAKANS_LETTER) > 0 and st.getQuestItemsCount(
            NYAKURIS_LETTER) > 0 and st.getQuestItemsCount(RAMUSS_LETTER) > 0:
            st.giveItems(MARK_OF_REFORMER, 1)
            st.giveItems(7562, 60)
            st.giveItems(ADENA, 11324)
            st.giveItems(SHADOW_WEAPON_COUPON_CGRADE, 15)
            st.addExpAndSp(62642, 4298)
    ...
    def onKill(self, npc, player, isPet):
        ...
        npcId = npc.getNpcId()
        if npcId == 27099:
            if st.getInt("cond") == 1 and st.getQuestItemsCount(RIPPED_DIARY) < 7 and st.getQuestItemsCount(
                    BOOK_OF_REFORM) >= 1:
            ...
            if st.getInt("cond") == 18 and st.getQuestItemsCount(BONE_FRAGMENT4) == 0:
                ...
                st.playSound("ItemSound.quest_itemget")
                if st.getQuestItemsCount(BONE_FRAGMENT4) > 0 and st.getQuestItemsCount(
                        BONE_FRAGMENT5) > 0 and st.getQuestItemsCount(BONE_FRAGMENT6) > 0 and st.getQuestItemsCount(
                    BONE_FRAGMENT7) > 0 and st.getQuestItemsCount(BONE_FRAGMENT8) > 0:
                    st.playSound("ItemSound.quest_middle")
                    st.set("cond", "19")
        elif npcId == 20104:
