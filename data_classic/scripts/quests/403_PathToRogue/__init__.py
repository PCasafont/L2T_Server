...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30379 and st.getInt("cond") == 0:
            ...
            if st.getQuestItemsCount(HORSESHOE_OF_LIGHT) == 0 and HaveAllStolenItems(st):
                ...
                st.set("cond", "5")
            elif st.getQuestItemsCount(NETIS_BOW) and st.getQuestItemsCount(NETIS_DAGGER) and st.getQuestItemsCount(
                    WANTED_BILL) == 0:
            ...
            htmltext = "30425-01.htm"
                ...
                HORSESHOE_OF_LIGHT) == 0 and st.getQuestItemsCount(BEZIQUES_LETTER) == 0:
    ...
    def onKill(self, npc, player, isPet):
        ...
        npcId = npc.getNpcId()
        if st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == NETIS_BOW or st.getItemEquipped(
                Inventory.PAPERDOLL_RHAND) == NETIS_DAGGER:
            if npcId in (20035, 20042, 20045, 20051, 20054, 20060):
                st.set("id", "0")
                if st.getInt("cond") and st.getQuestItemsCount(SPATOIS_BONES) < 10 and st.getRandom(10) < DROP_CHANCE[
                    npcId]:
