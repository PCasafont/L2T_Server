...
class Quest(JQuest):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if st.getInt("cond") == 1:
            ...
            elif npcId in [LESSER_SUCCUBUS, LESSER_SUCCUBUS_TUREN, LESSER_SUCCUBUS_TILFO] and not st.getQuestItemsCount(
                    SUCCUBUS_BLOOD):
                st.giveItems(SUCCUBUS_BLOOD, 1)
            if st.getQuestItemsCount(ONYX_BEAST_EYE) and st.getQuestItemsCount(TAINT_STONE) and st.getQuestItemsCount(
                    SUCCUBUS_BLOOD):
                st.set("cond", "2")
