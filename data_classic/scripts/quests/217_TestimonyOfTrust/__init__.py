...
class Quest(JQuest):
    def __init__(self, id, name, descr):
        ...
        self.questItemIds = [SCROLL_OF_DARKELF_TRUST_ID, SCROLL_OF_ELF_TRUST_ID, SCROLL_OF_DWARF_TRUST_ID,
                             SCROLL_OF_ORC_TRUST_ID, BREATH_OF_WINDS_ID,
                             SEED_OF_VERDURE_ID, ORDER_OF_OZZY_ID, LETTER_TO_ELF_ID, ORDER_OF_CLAYTON_ID,
                             BASILISK_PLASMA_ID, STAKATO_ICHOR_ID, HONEY_DEW_ID,
                             LETTER_TO_DARKELF_ID, LETTER_OF_THIFIELL_ID, LETTER_TO_SERESIN_ID, LETTER_TO_ORC_ID,
                             LETTER_OF_MANAKIA_ID, LETTER_TO_MANAKIA_ID,
                             PARASITE_OF_LOTA_ID, LETTER_TO_DWARF_ID, LETTER_TO_NICHOLA_ID, HEART_OF_PORTA_ID,
                             ORDER_OF_NICHOLA_ID, RECOMMENDATION_OF_HOLLIN_ID,
    ...
    def onTalk(self, npc, player):
        ...
        if id == State.CREATED:  # Check if is starting the quest
            ...
            if cond == 1:  # Condition 1 take the letter to Hierarch Asterios (Elven Master)
                ...
                if npcId == 30154 and st.getQuestItemsCount(BREATH_OF_WINDS_ID) and st.getQuestItemsCount(
                        SEED_OF_VERDURE_ID):
                ...
                if npcId == 30464 and st.getQuestItemsCount(ORDER_OF_CLAYTON_ID) and st.getQuestItemsCount(
                        STAKATO_ICHOR_ID) and st.getQuestItemsCount(HONEY_DEW_ID) and st.getQuestItemsCount(
                    BASILISK_PLASMA_ID):
                    htmltext = "30464-03.htm"
                    st.set("cond", "8")
            elif cond == 8:  # Condition 8 take the Stakato ichor, honey dew and basilisk plasma to Thifiell
                if npcId == 30358 and st.getQuestItemsCount(ORDER_OF_CLAYTON_ID) and st.getQuestItemsCount(
                        STAKATO_ICHOR_ID) and st.getQuestItemsCount(HONEY_DEW_ID) and st.getQuestItemsCount(
                    BASILISK_PLASMA_ID):
                ...
                if npcId == 30191 and st.getQuestItemsCount(SCROLL_OF_ELF_TRUST_ID) and st.getQuestItemsCount(
                ...
                if npcId == 30621 and st.getQuestItemsCount(ORDER_OF_NICHOLA_ID) and st.getQuestItemsCount(
                        HEART_OF_PORTA_ID):
                ...
                if npcId == 30191 and st.getQuestItemsCount(SCROLL_OF_DWARF_TRUST_ID) and st.getQuestItemsCount(
                ...
                if npcId == 30191:
                    ...
                    st.giveItems(ADENA, 12610)
                    st.addExpAndSp(65914, 4639)
    ...
    def onKill(self, npc, player, isPet):
        ...
        npcId = npc.getNpcId()
        cond = st.getInt("cond")
        if cond == 2 and npcId in [20013, 20019, 20036,
                                   20044]:  # Condition 2 kill the Luel of Zephy and Aktea of the Woods
            ...
            if st.getQuestItemsCount(required) and count < maxqty:
                ...
                if count + 1 == maxqty:  # Check if got enough number of items
                    ...
                    if npcId in [20550, 20082, 20084, 20086, 20087, 20088, 20157, 20230, 20232, 20234]:
                        ...
                        if st.getQuestItemsCount(BASILISK_PLASMA_ID) and st.getQuestItemsCount(
                                HONEY_DEW_ID) and st.getQuestItemsCount(STAKATO_ICHOR_ID):
                            st.set("cond", "7")
                            st.playSound("Itemsound.quest_middle")
                        else:
                            st.playSound("Itemsound.quest_itemget")
