...
class Quest(JQuest):
    def __init__(self, id, name, descr):
        ...
        self.questItemIds = [CAMOMILE_CHARM_ID, CARDIENS_LETTER_ID, WATER_OF_LIFE_ID, MOONFLOWER_CHARM_ID,
                             HIERARCHS_LETTER_ID, STARDUST_ID,
                             PURE_MITHRIL_CUP_ID, THALIAS_INSTRUCTIONS_ID, ISAELS_LETTER_ID, TEARS_OF_UNICORN_ID,
                             GRAIL_DIAGRAM_ID, PUSHKINS_LIST_ID,
                             THALIAS_LETTER1_ID, ARKENIAS_CONTRACT_ID, ANDARIEL_SCRIPTURE_COPY_ID,
                             ARKENIAS_INSTRUCTIONS_ID, ADONIUS_LIST_ID,
                             THALIAS_LETTER2_ID, TALINS_SPEAR_BLADE_ID, TALINS_SPEAR_SHAFT_ID, TALINS_RUBY_ID,
                             TALINS_AQUAMARINE_ID, TALINS_AMETHYST_ID,
        ...
        if npcId == 30460:
            if st.getInt("cond") == 0:
                ...
                if st.getGameTicks() != st.getInt("id"):
                    ...
                    st.addExpAndSp(94341, 6295)
                    st.giveItems(57, 17114)
            ...
            if st.getQuestItemsCount(THALIAS_LETTER2_ID):
                ...
                if st.getQuestItemsCount(TALINS_SPEAR_BLADE_ID) and st.getQuestItemsCount(
                        TALINS_SPEAR_SHAFT_ID) and st.getQuestItemsCount(TALINS_RUBY_ID) \
                        and st.getQuestItemsCount(TALINS_AQUAMARINE_ID) and st.getQuestItemsCount(
                    TALINS_AMETHYST_ID) and st.getQuestItemsCount(TALINS_PERIDOT_ID):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if npcId == 20550:
            if st.getQuestItemsCount(PUSHKINS_LIST_ID) and st.getQuestItemsCount(PURE_MITHRIL_ORE_ID) < 10:
                if st.getRandom(100) < 50:
                    ...
                    if st.getQuestItemsCount(PURE_MITHRIL_ORE_ID) < 10:
                        ...
                        if st.getQuestItemsCount(WYRMS_TALON1_ID) >= 20 and st.getQuestItemsCount(
                                ANT_SOLDIER_ACID_ID) >= 20:
                            st.set("cond", "5")
        elif npcId == 20176:
            if st.getQuestItemsCount(PUSHKINS_LIST_ID) and st.getQuestItemsCount(WYRMS_TALON1_ID) < 20:
                if st.getRandom(100) < 50:
                    ...
                    if st.getQuestItemsCount(WYRMS_TALON1_ID) < 20:
                        ...
                        st.playSound("ItemSound.quest_middle")
                        if st.getQuestItemsCount(PURE_MITHRIL_ORE_ID) >= 10 and st.getQuestItemsCount(
                                ANT_SOLDIER_ACID_ID) >= 20:
                            st.set("cond", "5")
            ...
            if st.getQuestItemsCount(TALINS_SPEAR_ID) and st.getQuestItemsCount(
                    GRAIL_OF_PURITY_ID) and not st.getQuestItemsCount(TEARS_OF_UNICORN_ID):
            ...
            if st.getQuestItemsCount(ISAELS_INSTRUCTIONS_ID) and st.getRandom(100) < 50:
                for id in [TALINS_SPEAR_BLADE_ID, TALINS_SPEAR_SHAFT_ID, TALINS_RUBY_ID, TALINS_AQUAMARINE_ID,
                           TALINS_AMETHYST_ID]:
