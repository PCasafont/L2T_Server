...
class Quest(JQuest):
    def __init__(self, id, name, descr):
        ...
        self.questItemIds = [GEM_BOX1, STAR_DIAMOND, GOUPHS_CONTRACT, REEPS_CONTRACT, ELVEN_WINE, BRONPS_CONTRACT,
                             AQUAMARINE, CHRYSOBERYL, COAL_PIECE, BRONPS_DICE, BRONPS_LETTER, BERRY_TART, BAT_DIAGRAM]
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30523 and id == State.COMPLETED:
        ...
        elif npcId == 30523 and st.getInt("cond") == 1 and (
                            st.getQuestItemsCount(REEPS_CONTRACT) or st.getQuestItemsCount(
                            ELVEN_WINE) or st.getQuestItemsCount(
                        BRONPS_DICE) or st.getQuestItemsCount(BRONPS_CONTRACT)):
            ...
            st.giveItems(COAL_PIECE, 1)
                            ...
                            st.getQuestItemsCount(BRONPS_LETTER) or st.getQuestItemsCount(
                            COAL_PIECE) or st.getQuestItemsCount(
                        BERRY_TART) or st.getQuestItemsCount(BAT_DIAGRAM)):
            ...
            if npcId == 30516 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    GOUPHS_CONTRACT) and st.getQuestItemsCount(REEPS_CONTRACT) == 0:
                htmltext = "30516-01.htm"
                st.giveItems(REEPS_CONTRACT, 1)
                st.takeItems(GOUPHS_CONTRACT, 1)
            elif npcId == 30516 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    GOUPHS_CONTRACT) == 0 and st.getQuestItemsCount(REEPS_CONTRACT):
                htmltext = "30516-02.htm"
            elif npcId == 30516 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    GOUPHS_CONTRACT) == 0 and st.getQuestItemsCount(REEPS_CONTRACT) == 0:
                htmltext = "30516-03.htm"
            elif npcId == 30555 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    REEPS_CONTRACT) == 0 and st.getQuestItemsCount(ELVEN_WINE) == 0:
                htmltext = "30555-01.htm"
            elif npcId == 30555 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    REEPS_CONTRACT) and st.getQuestItemsCount(ELVEN_WINE) == 0:
                htmltext = "30555-02.htm"
                st.giveItems(ELVEN_WINE, 1)
                st.takeItems(REEPS_CONTRACT, 1)
            elif npcId == 30555 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    REEPS_CONTRACT) == 0 and st.getQuestItemsCount(ELVEN_WINE):
                htmltext = "30555-03.htm"
            elif npcId == 30555 and st.getInt("cond") == 1 and st.getQuestItemsCount(GEM_BOX1) == 1:
                htmltext = "30555-04.htm"
            elif npcId == 30555 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    GEM_BOX1) == 0 and st.getQuestItemsCount(REEPS_CONTRACT) == 0 and st.getQuestItemsCount(
                ELVEN_WINE) == 0:
                htmltext = "30555-05.htm"
            elif npcId == 30529 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    ELVEN_WINE) and st.getQuestItemsCount(BRONPS_DICE) == 0:
                htmltext = "30529-01.htm"
                st.giveItems(BRONPS_DICE, 1)
                st.takeItems(ELVEN_WINE, 1)
            elif npcId == 30529 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    ELVEN_WINE) == 0 and st.getQuestItemsCount(BRONPS_DICE):
                htmltext = "30529-02.htm"
            elif npcId == 30529 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    ELVEN_WINE) == 0 and st.getQuestItemsCount(BRONPS_DICE) == 0:
                htmltext = "30529-03.htm"
            elif npcId == 30526 and st.getInt("cond") == 1 and st.getQuestItemsCount(BRONPS_DICE):
                htmltext = "30526-01.htm"
            elif npcId == 30526 and st.getInt("cond") == 1 and st.getQuestItemsCount(BRONPS_CONTRACT) and (
                            st.getQuestItemsCount(AQUAMARINE) + st.getQuestItemsCount(CHRYSOBERYL) < 20):
            ...
            elif npcId == 30526 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    BERRY_TART) or st.getQuestItemsCount(BAT_DIAGRAM) or st.getQuestItemsCount(STAR_DIAMOND):
                htmltext = "30526-08.htm"
            elif npcId == 30521 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    BRONPS_LETTER) and st.getQuestItemsCount(BERRY_TART) == 0:
                htmltext = "30521-01.htm"
                st.giveItems(BERRY_TART, 1)
                st.takeItems(BRONPS_LETTER, 1)
            elif npcId == 30521 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    BRONPS_LETTER) == 0 and st.getQuestItemsCount(BERRY_TART):
                htmltext = "30521-02.htm"
            elif npcId == 30521 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    BRONPS_LETTER) == 0 and st.getQuestItemsCount(BERRY_TART) == 0:
                htmltext = "30521-03.htm"
            elif npcId == 30522 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    BAT_DIAGRAM) == 0 and st.getQuestItemsCount(BERRY_TART) and st.getQuestItemsCount(
                STAR_DIAMOND) == 0:
                htmltext = "30522-01.htm"
                st.giveItems(BAT_DIAGRAM, 1)
                st.takeItems(BERRY_TART, 1)
            elif npcId == 30522 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    BAT_DIAGRAM) and st.getQuestItemsCount(BERRY_TART) == 0 and st.getQuestItemsCount(
                STAR_DIAMOND) == 0:
                htmltext = "30522-02.htm"
            elif npcId == 30522 and st.getInt("cond") == 1 and st.getQuestItemsCount(
                    BAT_DIAGRAM) == 0 and st.getQuestItemsCount(BERRY_TART) == 0 and st.getQuestItemsCount(
                STAR_DIAMOND):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if npcId == 20323:
            ...
            if st.getInt("cond") == 1 and st.getQuestItemsCount(BAT_DIAGRAM) and st.getQuestItemsCount(
                    STAR_DIAMOND) == 0:
