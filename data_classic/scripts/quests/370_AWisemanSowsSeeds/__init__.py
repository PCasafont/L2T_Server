...
class Quest(JQuest):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if chance in range(1, 15) and st.getQuestItemsCount(CHAPTER_OF_FIRE) == 0:
            ...
            st.playSound("ItemSound.quest_itemget")
        if st.getQuestItemsCount(CHAPTER_OF_FIRE) and st.getQuestItemsCount(CHAPTER_OF_WATER) and st.getQuestItemsCount(
                CHAPTER_OF_WIND) and st.getQuestItemsCount(CHAPTER_OF_EARTH):
