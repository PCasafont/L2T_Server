...
import sys
from l2server.gameserver.model.actor import L2Attackable
...
# all stats
STATS = ["cond", "step", "Lara_Part", "Arcanas", "Beginner_Arcanas", "Belthus", "Brynthea", "Celestiel", "Camoniell",
         "Basilla", "Almors"]
...
DROPLIST_SUMMON = {
    27102: ["Almors", CRYSTAL_OF_PROGRESS1, CRYSTAL_OF_INPROGRESS1, CRYSTAL_OF_FOUL1, CRYSTAL_OF_DEFEAT1,
            CRYSTAL_OF_VICTORY1],  # Pako the Cat
    27103: ["Camoniell", CRYSTAL_OF_PROGRESS2, CRYSTAL_OF_INPROGRESS2, CRYSTAL_OF_FOUL2, CRYSTAL_OF_DEFEAT2,
            CRYSTAL_OF_VICTORY2],  # Mimi the Cat
    27104: ["Belthus", CRYSTAL_OF_PROGRESS3, CRYSTAL_OF_INPROGRESS3, CRYSTAL_OF_FOUL3, CRYSTAL_OF_DEFEAT3,
            CRYSTAL_OF_VICTORY3],  # Shadow Turen
    27105: ["Basilla", CRYSTAL_OF_PROGRESS4, CRYSTAL_OF_INPROGRESS4, CRYSTAL_OF_FOUL4, CRYSTAL_OF_DEFEAT4,
            CRYSTAL_OF_VICTORY4],  # Unicorn Racer
    27106: ["Celestiel", CRYSTAL_OF_PROGRESS5, CRYSTAL_OF_INPROGRESS5, CRYSTAL_OF_FOUL5, CRYSTAL_OF_DEFEAT5,
            CRYSTAL_OF_VICTORY5],  # Unicorn Phantasm
    27107: ["Brynthea", CRYSTAL_OF_PROGRESS6, CRYSTAL_OF_INPROGRESS6, CRYSTAL_OF_FOUL6, CRYSTAL_OF_DEFEAT6,
            CRYSTAL_OF_VICTORY6]  # Silhoutte Tilfo
}
...
class Quest(JQuest):
    ...
    def onAdvEvent(self, event, npc, player):
        ...
        if event == "30634-08.htm":  # start part for Galatea
            ...
            if st.getInt(
                    "Beginner_Arcanas"):  # if the player has more then one beginners' arcana he can start a fight against the masters summon
            ...
            if npcId == Galatea:  # Start and End Npc Galatea related stuff
                if step == 1:  # step 1 means just State.STARTED
                    ...
                    if Arcanas == 6:  # finished all battles... the player is able to earn the marks
                        ...
                        st.addExpAndSp(83224, 5711)
                        st.giveItems(57, 15048)
                        for var in STATS:
                            st.unset(var)
                        st.exitQuest(False)
                        ...
                        for item in [LARS_LIST1, LARS_LIST2, LARS_LIST3, LARS_LIST4, LARS_LIST5, ALMORS_ARCANA,
                                     BASILLIA_ARCANA, CAMONIELL_ARCANA, CELESTIEL_ARCANA, BELTHUS_ARCANA,
                                     BRYNTHEA_ARCANA]:
                            st.takeItems(item, -1)
                ...
                if step > 1:
                    if SummonerStat == 1:  # default, just able to start talk with the summoner
                        ...
                        st.set("Arcanas", str(
                            Arcanas + 1))  # set arcana stat +1, if its 6... quest is finished and he can earn the mark
                    elif SummonerStat == 7:  # you already won the battle against my summon
                        htmltext = str(npcId) + "-10.htm"
        return htmltext

    def onDeath(self, killer, deadPerson,
                st):  # if players summon dies, the crystal of defeat is given to the player and set stat to lose
