...
class Quest(JQuest):
    ...
    def onAdvEvent(self, event, npc, player):
        ...
        if event == "32041-02.htm":
            ...
            if self.isSpawned == 0:
                ...
                golem.broadcastPacket(NpcSay(golem.getObjectId(), 0, golem.getNpcId(),
                                             "You, " + st.getPlayer().getName() + ", you attacked Wendy. Prepare to die!"))
                golem.setRunning()
    ...
    def onFirstTalk(self, npc, player):  # atm custom, on retail it is when you walk to npcs radius
        ...
        if st:
            if npc.getNpcId() == Stones and st.getInt("cond") == 17:
                ...
                st.set("cond", "18")
                player.sendPacket(ExShowScreenMessage(
                    "The radio signal detector is responding. # A suspicious pile of stones catches your eye.", 4500))
    ...
    def onKill(self, npc, player, isPet):
        ...
        if st.getState() == State.STARTED and st.getInt("cond") == 10:
            if npcId == Guardian:
                npc.broadcastPacket(NpcSay(npc.getObjectId(), 0, npcId,
