...
class Quest(JQuest):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if npcId in [20223, 20154, 20155, 20156]:
            ...
            st.set("id", "0")
            if st.getInt("cond") and st.getQuestItemsCount(RING_OF_TESTIMONY1_ID) == 1 and st.getQuestItemsCount(
                    CRIMSON_MOSS_ID) < 10:
            ...
            st.set("id", "0")
            if st.getInt("cond") and st.getQuestItemsCount(RING_OF_TESTIMONY2_ID) == 1 and st.getQuestItemsCount(
                    STAKATO_SHELL_ID) < 20:
                ...
                if st.getQuestItemsCount(STAKATO_SHELL_ID) < 20:
                    ...
                    st.playSound("ItemSound.quest_middle")
                    if st.getQuestItemsCount(STAKATO_SHELL_ID) >= 20 and st.getQuestItemsCount(
                            INPICIO_SAC_ID) >= 10 and st.getQuestItemsCount(SPIDER_THORN_ID) >= 10:
                        st.set("cond", "8")
        elif npcId == 20231:
            st.set("id", "0")
            if st.getInt("cond") and st.getQuestItemsCount(RING_OF_TESTIMONY2_ID) == 1 and st.getQuestItemsCount(
