...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        if event == "1":
            ...
            if st.getQuestItemsCount(ZIMENFS_POTION) or st.getQuestItemsCount(CHALIS_AXE) or st.getQuestItemsCount(
                    BRONKS_INGOT):
            ...
            if st.getQuestItemsCount(ZIMENFS_PAY) or st.getQuestItemsCount(CHALIS_PAY) or st.getQuestItemsCount(
                    BRONKS_PAY):
                n = st.getRandom(3)
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30524 and cond == 0:
        ...
        elif npcId == 30519 and cond and ((st.getQuestItemsCount(CHALIS_AXE) + st.getQuestItemsCount(
                BRONKS_INGOT) + st.getQuestItemsCount(ZIMENFS_POTION)) == 1) and ((st.getInt("id") / 10) == 0):
            htmltext = "30519-05.htm"
        elif npcId == 30519 and cond and ((st.getQuestItemsCount(CHALIS_AXE) + st.getQuestItemsCount(
                BRONKS_INGOT) + st.getQuestItemsCount(ZIMENFS_POTION)) == 1) and ((st.getInt("id") / 10)):
            htmltext = "30519-08.htm"
        elif npcId == 30519 and cond and ((st.getQuestItemsCount(CHALIS_PAY) + st.getQuestItemsCount(
                BRONKS_PAY) + st.getQuestItemsCount(ZIMENFS_PAY)) == 1) and (st.getInt("id") < 50):
        ...
        elif npcId == 30519 and cond and (st.getQuestItemsCount(BEAR_PIC) or st.getQuestItemsCount(
                TARANTULA_PIC) or st.getQuestItemsCount(BEAD_PARCEL) or st.getQuestItemsCount(
            ROUTS_TP_SCROLL) or st.getQuestItemsCount(SUCCUBUS_UNDIES)):
        ...
        elif npcId == 30556 and cond and st.getQuestItemsCount(TARANTULA_PIC) == 1 and st.getQuestItemsCount(
                BEAD) >= 20:
            ...
            htmltext = "30556-06.htm"
                    ...
                    st.getQuestItemsCount(ROUTS_TP_SCROLL) or st.getQuestItemsCount(SUCCUBUS_UNDIES)):
