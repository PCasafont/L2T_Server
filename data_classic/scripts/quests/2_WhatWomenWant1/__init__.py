...
class Quest(JQuest):
    ...
    def onAdvEvent(self, event, npc, player):
        ...
        if event == "30223-04.htm":
            if st.getQuestItemsCount(ARUJIENS_LETTER1) == 0 and st.getQuestItemsCount(
                    ARUJIENS_LETTER2) == 0 and st.getQuestItemsCount(ARUJIENS_LETTER3) == 0:
            ...
            st.takeItems(ARUJIENS_LETTER3, -1)
            st.giveItems(ADENA, 185)
            st.addExpAndSp(425, 33)
            st.set("cond", "0")
            st.exitQuest(False)
        ...
        if npcId == ARUJIEN and id == State.CREATED:
            ...
            if npcId == MIRABEL and cond == 1:
                if st.getQuestItemsCount(ARUJIENS_LETTER1):
                    ...
                    st.set("id", "2")
                    st.playSound("ItemSound.quest_middle")
                elif st.getQuestItemsCount(ARUJIENS_LETTER2) or st.getQuestItemsCount(
                        ARUJIENS_LETTER3) or st.getQuestItemsCount(POETRY_BOOK) or st.getQuestItemsCount(
                    GREENIS_LETTER):
                ...
                if st.getQuestItemsCount(ARUJIENS_LETTER2):
                    ...
                    st.set("id", "3")
                    st.playSound("ItemSound.quest_middle")
                elif st.getQuestItemsCount(ARUJIENS_LETTER3) or st.getQuestItemsCount(
                        POETRY_BOOK) or st.getQuestItemsCount(GREENIS_LETTER):
            ...
            elif npcId == GREENIS and (st.getQuestItemsCount(ARUJIENS_LETTER1) or st.getQuestItemsCount(
                    ARUJIENS_LETTER2) or st.getQuestItemsCount(ARUJIENS_LETTER3)):
