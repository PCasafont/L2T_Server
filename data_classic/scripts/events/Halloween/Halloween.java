...
public class Halloween extends Quest
{
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
    {
        ...
        else if (event.equalsIgnoreCase("stage_all_spawn_round"))
        {
            ...
            if (round < 20)
            {
                for (int i = 0; i < 60; i++)
                {
                    ...
                    L2Npc minion =
                            addSpawn(minionId, 153569 + x, 142075 + y, -12742 + 20, -1, false, 0, false, instanceId);
        ...
        else if (event.equalsIgnoreCase("stage_all_start_help_process"))
        {
            ...
            //Chats
            InstanceManager.getInstance()
                    .sendPacket(instanceId, new CreatureSay(boss.getObjectId(), 1, boss.getName(), "Poor heros..!"));
        ...
        {
            //Door Effect 1
            dummy.broadcastPacket(new MagicSkillUse(dummy, dummy, 6798, 1, 500, 500, 500, instanceId));//6783
        ...
        {
            //Door Effect 2
            dummy.broadcastPacket(new MagicSkillUse(dummy, dummy, 6799, 1, 500, 500, 500, 0));
            ...
            kegor.broadcastPacket(new MagicSkillUse(kegor, kegor, 6463, 1, 500, 500, 500, 0));

            jinia.broadcastPacket(new MagicSkillUse(jinia, jinia, 6463, 1, 500, 500, 500, 0));
        ...
        {
            jinia.broadcastPacket(new MagicSkillUse(jinia, jinia, 6176, 1, 500, 500, 500, 0));
        ...
        else if (event.equalsIgnoreCase("enter_to_instance"))
        {
            ...
            InstanceManager.getInstance().cancelPlayerBuffs(player);
        }
        ...
        if (npc.getNpcId() == mountedRaidId)
        {
            if (npc.getCurrentHp() < npc.getMaxHp() * 0.50)    //50%
            {
                if (!npc.isInvul() && eventState == 2)
                {
                    ...
                    //Do cast some skill
                    boss.broadcastPacket(new MagicSkillUse(boss, boss, 6796, 1, 500, 500, 500, 0));
