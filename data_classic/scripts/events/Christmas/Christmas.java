...
public class Christmas extends Quest
{
	...
	public String onAttack(L2Npc npc, L2PcInstance player, int damage, boolean isPet, L2Skill skill)
	{
		...
		for (Map.Entry<Integer, invaderInfo> _info: _attackInfo.entrySet())
		{
			...
			if ((_info.getValue().getExternalIP().equalsIgnoreCase(player.getExternalIP()) && _info.getValue().getInternalIP().equalsIgnoreCase(player.getInternalIP())))
			{
				sameIPs++;
				...
				if (sameIPs > 1)
				{
					...
					npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc.getTemplate().TemplateId, "Dual Box? Die b1tch!"));
	...
	public String onFirstTalk(L2Npc npc, L2PcInstance player)
	{
		...
		{	
			...
			tb.append("<a action=\"bypass -h Quest Christmas teleport_to_fantasy\"><font color=c2dceb>Teleport to Fantasy Island.</font></a><br>");
		}
		
		tb.append("<font color=\"3D81A8\">Available Gifts:</font><br>");

		tb.append("<a action=\"bypass -h npc_"+npc.getObjectId()+"_multisell 9999\"><font color=c2dceb>View the event shop.</font></a><br>");
