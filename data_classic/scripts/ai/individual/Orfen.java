...
public class Orfen extends L2AttackableAIScript
{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("check_orfen_pos"))
		{
			if ((_IsTeleported && npc.getCurrentHp() > npc.getMaxHp() * 0.95) ||
					(!_Zone.isInsideZone(npc) && !_IsTeleported))
			{
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		if (npc.getNpcId() == ORFEN)
		{
			...
			long respawnTime = (long) GrandBossManager.getInstance().getRespawnTime(ORFEN) +
					Rnd.get(GrandBossManager.getInstance().getRandomRespawnTime(ORFEN));
