...
public class Core extends L2AttackableAIScript
{
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
    {
        ...
        {
            L2Attackable mob = (L2Attackable) addSpawn(npc.getNpcId(), npc.getX(), npc.getY(), npc.getZ(), npc
                    .getHeading(), false, 0);
            mob.setIsRaidMinion(true);
            Minions.add(mob);
        }
            ...
            if (_FirstAttacked)
            {
                if (Rnd.get(100) == 0)
                {
                    npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc
                            .getNpcId(), 1000003)); // Removing intruders.
                }
            }
            else
            {
                _FirstAttacked = true;
                npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc
                        .getNpcId(), 1000001)); // A non-permitted target has been discovered.
                npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc
                        .getNpcId(), 1000002)); // Intruder removal system initiated.
            }
        }
    ...
    public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
    {
        ...
        if (npcId == CORE)
        {
            ...
            long respawnTime = (long) GrandBossManager.getInstance().getRespawnTime(CORE) + Rnd
                    .get(GrandBossManager.getInstance().getRandomRespawnTime(CORE));
        ...
        else if (GrandBossManager.getInstance().getBossStatus(CORE) == ALIVE && Minions != null && Minions
                .contains(npc))
        {
            Minions.remove(npc);
