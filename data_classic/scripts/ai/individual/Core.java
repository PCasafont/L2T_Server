...
public class Core extends L2AttackableAIScript
{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		{
			L2Attackable mob =
					(L2Attackable) addSpawn(npc.getNpcId(), npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), false,
							0);
			mob.setIsRaidMinion(true);
			Minions.add(mob);
		}
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		...
		if (npcId == CORE)
		{
			...
			long respawnTime = (long) GrandBossManager.getInstance().getRespawnTime(CORE) +
					Rnd.get(GrandBossManager.getInstance().getRandomRespawnTime(CORE));
		...
		else if (GrandBossManager.getInstance().getBossStatus(CORE) == ALIVE && Minions != null &&
				Minions.contains(npc))
		{
			Minions.remove(npc);
