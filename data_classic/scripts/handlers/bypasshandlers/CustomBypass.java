...
public class CustomBypass implements IBypassHandler
{
	...
	public boolean useBypass(String command, L2PcInstance player, L2Npc target)
	{
		...
		if (command.startsWith("titlecolor"))
		{
			...
			int medals = 10;
			if (!val.equalsIgnoreCase("FFFF77") && inv.getItemByItemId(6393) != null && inv.getItemByItemId(6393).getCount() >= medals)
			{
				player.destroyItemByItemId("Change Title Color", 6393, medals, player, true);
				player.setTitleColor(val);
				player.broadcastUserInfo();
				player.sendPacket(new CreatureSay(target.getObjectId(), Say2.TELL, target.getName(), "Title color changed!"));
			}
			else if (val.equalsIgnoreCase("FFFF77"))
			{
				...
				player.sendPacket(new CreatureSay(target.getObjectId(), Say2.TELL, target.getName(), "You have not enough medals..."));
		}
		else if (command.startsWith("clanrep"))
		{
			int val = Integer.valueOf(command.substring(8));
			PcInventory inv = player.getInventory();
			if (player.getClan() != null && inv.getItemByItemId(6393) != null && inv.getItemByItemId(6393).getCount() >= 1)
			{
				inv.destroyItemByItemId("Clan Reputation", 6393, 1, player, this);
				player.getClan().addReputationScore(val, true);
				CreatureSay cs = new CreatureSay(target.getObjectId(), Say2.CLAN, target.getName(), "The clan member " + player.getName() + " has contributed with 1 Glittering Medal to add " + val + " reputation points to the clan!");
				player.getClan().broadcastCSToOnlineMembers(cs, player);
			}
			else
				player.sendPacket(new CreatureSay(target.getObjectId(), Say2.TELL, target.getName(), "You don't have clan or enough medals."));
		}
