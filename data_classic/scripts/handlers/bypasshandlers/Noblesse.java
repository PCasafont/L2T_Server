...
public class Noblesse implements IBypassHandler
{
	...
	public boolean useBypass(String command, L2PcInstance activeChar, L2Npc target)
	{
		...
		{
			activeChar.sendPacket(new CreatureSay(target.getObjectId(), Say2.TELL, target.getName(),
					"You are already a noble, don't make me waste my time."));
		...
		{
			activeChar.sendPacket(new CreatureSay(target.getObjectId(), Say2.TELL, target.getName(),
					"You must have at least a subclass to be noble."));
			activeChar.sendPacket(ActionFailed.STATIC_PACKET);
			return true;
		}
		...
		activeChar.sendPacket(new CreatureSay(target.getObjectId(), Say2.TELL, target.getName(),
				"I have just made you become a noble!"));
