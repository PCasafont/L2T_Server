...
public class ChainHeal implements ISkillHandler
{
    ...
    public void useSkill(L2Character activeChar, L2Skill skill, L2Object[] targets)
    {
        ...
        L2Character[] characters = getTargetsToHeal((L2Character[]) targets, (L2PcInstance) activeChar, skill
                .getSkillRadius());
    ...
    private static boolean canAddCharacter(L2PcInstance caster, L2Character target, int listZie, int skillRadius)
    {
        ...
        if (caster.isInOlympiadMode() || target.isDead() || target.isInvul() || !target
                .isInsideRadius(caster, skillRadius, false, false) || target.getMaxHp() == target
                .getCurrentHp() || !GeoData.getInstance().canSeeTarget(caster, target))
        {
            return false;
        }
        ...
        {
            if (targetPlayer.getEvent() != null && caster.getEvent()
                    .getParticipantTeamId(caster.getObjectId()) != targetPlayer.getEvent()
                    .getParticipantTeamId(targetPlayer.getObjectId()) || (caster.getEvent() != null && caster.getEvent()
                    .getConfig().isAllVsAll() && caster.getEvent().isState(EventState.STARTED)))
            {
                return false;
            }
            return true;
        ...
        if (caster.getClan() != null && targetPlayer.getClan() != null)
        {
            ...
            if (caster.getClan().getAllyId() > 0 && targetPlayer.getClan().getAllyId() > 0 && caster.getClan()
                    .getAllyId() == targetPlayer.getClan().getAllyId())
            {
                return true;
            }
        }
        ...
        if (caster.isInParty() && targetPlayer.isInParty())
        {
            ...
            if (caster.getParty().isInCommandChannel() && targetPlayer.getParty().isInCommandChannel() && caster
                    .getParty().getCommandChannel().getChannelLeader() == targetPlayer.getParty().getCommandChannel()
                    .getChannelLeader())
            {
                return true;
            }
