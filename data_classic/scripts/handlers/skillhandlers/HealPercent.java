...
public class HealPercent implements ISkillHandler
{
    ...
    public void useSkill(L2Character activeChar, L2Skill skill, L2Object[] targets)
    {
        ...
        if (skill.getTargetType() != SkillTargetType.TARGET_SELF && !skill.isPotion() && Formulas
                .calcMCrit(activeChar.getMCriticalHit(activeChar, skill)))
        {
        ...
        for (L2Character target : (L2Character[]) targets)
        {
            ...
            if ((target == null || target.isDead() || target.isInvul()) && skill.getId() != 1505 && skill
                    .getId() != 11560)
            {
                continue;
            }
            ...
            if (target != activeChar)
            {
                ...
                if (activeChar instanceof L2PcInstance && ((L2PcInstance) activeChar)
                        .getEvent() != null && ((L2PcInstance) activeChar).getEvent()
                        .isState(EventState.STARTED) && ((L2PcInstance) activeChar).getEvent().getConfig()
