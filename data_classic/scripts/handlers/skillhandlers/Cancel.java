...
{
    private static final L2SkillType[] SKILL_IDS = {L2SkillType.CANCEL,};
    ...
    public void useSkill(L2Character activeChar, L2Skill skill, L2Object[] targets)
    {
        ...
        for (L2Object obj : targets)
        {
            ...
            rate *= Formulas.calcEffectTypeProficiency(activeChar, target, L2AbnormalType.CANCEL) / Formulas
            ...
            {
                activeChar
                        .sendMessage("Your cancel effect has a base land rate of " + rate +
    ...
    private void generalBuffCancellation(L2Character activeChar, L2Abnormal[] effects, int maxNegate, double rate, boolean targetWasInOlys)
    {
        ...
        for (int i = 0; i < maxNegate; i++)
        {
            ...
            if (activeChar instanceof L2PcInstance && activeChar.getActingPlayer().isLandRates())
            {
                activeChar.sendMessage("Attempted to remove " + buffs.get(candidate).getSkill()
            ...
            if (Rnd.get(100) < rate)
            {
                ...
                {
                    buffs.get(candidate).getEffected().getActingPlayer()
                            .scheduleEffectRecovery(buffs.get(candidate), 60, targetWasInOlys);
                }
                if (activeChar instanceof L2PcInstance && activeChar.getActingPlayer().isLandRates())
                {
                    activeChar.sendMessage("Attempt to remove " + buffs.get(candidate).getSkill()
                            .getName() + " succeeded.");
                }
            }
