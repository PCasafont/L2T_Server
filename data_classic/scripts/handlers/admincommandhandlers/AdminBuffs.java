...
public class AdminBuffs implements IAdminCommandHandler
{
    ...
    public void showBuffs(L2PcInstance activeChar, L2Character target, int page)
    {
        ...
        final StringBuilder html = StringUtil
                .startAppend(500 + effects.length * 200,
        ...
        for (int i = start; i < end; i++)
        {
            ...
            if (e != null)
            {
                StringUtil.append(html, "<tr><td>", e.getSkill().getName(), "</td><td>", e.getSkill()
                                .isToggle() ? "toggle" : e.getDuration() - e
                                .getTime() + "s", "</td><td><button value=\"Remove\" action=\"bypass -h admin_stopbuff ",
                        Integer
                                .toString(target.getObjectId()), " ", String.valueOf(e.getSkill()
                                .getId()),
        ...
        StringUtil
                .append(html, "<br><center><button value=\"Remove All\" action=\"bypass -h admin_stopallbuffs ", Integer
                                .toString(target
                                        .getObjectId()),
        ...
        if (Config.GMAUDIT)
        {
            GMAudit.auditGMAction(activeChar.getName() + " [" + activeChar.getObjectId() + "]", "getbuffs", target
                    .getName() + " (" + Integer.toString(target.getObjectId()) + ")", "");
        }
    }
    ...
    private void removeBuff(L2PcInstance activeChar, int objId, int skillId)
    {
        ...
        if ((target != null) && (skillId > 0))
        {
            ...
            for (L2Abnormal e : effects)
            {
                ...
                {
                    e.exit();
                    activeChar.sendMessage("Removed " + e.getSkill().getName() + " level " + e.getSkill()
                            .getLevel() + " from " + target.getName() + " (" + objId + ")");
                }
            }
            showBuffs(activeChar, target, 1);
            if (Config.GMAUDIT)
            {
                GMAudit.auditGMAction(activeChar.getName() + " [" + activeChar.getObjectId() + "]", "stopbuff", target
                        .getName() + " (" + objId + ")", Integer.toString(skillId));
            }
        }
    }
    ...
    private void removeAllBuffs(L2PcInstance activeChar, int objId)
    {
        ...
        if (target != null)
        {
            ...
            if (Config.GMAUDIT)
            {
                GMAudit.auditGMAction(activeChar.getName() + " [" + activeChar
                        .getObjectId() + "]", "stopallbuffs", target.getName() + " (" + objId + ")", "");
            }
        }
    }
}