...
public class SqlToXml
{
	public static void races()
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				PreparedStatement st2 = con.prepareStatement("SELECT * FROM char_templates WHERE raceId = ? GROUP BY P_ATK ORDER BY classId");
				...
				while (rset2.next())
				{
					content += "\t\t<template type=\"" + (fighter ? "fighter" : "mage") + "\"" + " STR=\"" + rset2.getInt("STR") + "\"" + " CON=\"" + rset2.getInt("CON") + "\"" + " DEX=\"" + rset2.getInt("DEX") + "\"" + " INT=\"" + rset2.getInt("_INT") + "\"" + " WIT=\"" + rset2.getInt("WIT") + "\"" + " MEN=\"" + rset2.getInt("MEN") + "\"" + " basePAtk=\"" + rset2.getInt("P_ATK") + "\"" + " baseMAtk=\"" + rset2.getInt("M_ATK") + "\"" + " basePDef=\"" + rset2.getInt("P_DEF") + "\"" + " baseMDef=\"" + rset2.getInt("M_DEF") + "\"" + " basePAtkSpd=\"" + rset2.getInt("P_SPD") + "\"" + " baseMAtkSpd=\"" + rset2.getInt("M_SPD") + "\"" + " baseAccuracy=\"" + rset2.getInt("ACC") + "\"" + " baseCritical=\"" + rset2.getInt("CRITICAL") + "\"" + " baseEvasion=\"" + rset2.getInt("EVASION") + "\"" + " baseMoveSpd=\"" + rset2.getInt("MOVE_SPD") + "\"" + " mColRadius=\"" + rset2.getFloat("M_COL_R") + "\"" + " mColHeight=\"" + rset2.getFloat("M_COL_H") + "\"" + " fColRadius=\"" + rset2.getFloat("F_COL_R") + "\"" + " fColHeight=\"" + rset2.getFloat("F_COL_H") + "\">\r\n";
					...
					while (rset3.next())
					{
						content += "\t\t\t<creationItem id=\"" + rset3.getInt("itemId") + "\"" + " count=\"" + rset3.getInt("amount") + "\"" + " equipped=\"" + rset3.getString("equipped") + "\" /> <!-- " + ItemTable.getInstance().createDummyItem(rset3.getInt("itemId")).getName() + " -->\r\n";
					}
			...
			while (rset.next())
			{
				content += "\t<creationItem id=\"" + rset.getInt("itemId") + "\"" + " count=\"" + rset.getInt("amount") + "\"" + " equipped=\"" + rset.getString("equipped") + "\" /> <!-- " + ItemTable.getInstance().createDummyItem(rset.getInt("itemId")).getName() + " -->\r\n";
			}
	...
	public static void classes()
	{
		...
		try
		{
			...
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("SELECT * FROM char_templates c, lvlupgain l WHERE c.classId = l.classid");
			ResultSet rset = statement.executeQuery();
			
			while (rset.next())
			{
				content += "\t<class id=\"" + rset.getInt("c.classId") + "\"" + " name=\"" + rset.getString("className") + "\"" + " level=\"" + rset.getInt("class_lvl") + "\"" + " baseHp=\"" + rset.getInt("defaulthpbase") + "\"" + " hpAdd=\"" + rset.getInt("defaulthpadd") + "\"" + " hpMod=\"" + rset.getInt("defaulthpmod") + "\"" + " baseMp=\"" + rset.getInt("defaultmpbase") + "\"" + " mpAdd=\"" + rset.getInt("defaultmpadd") + "\"" + " mpMod=\"" + rset.getInt("defaultmpmod") + "\"" + " baseCp=\"" + rset.getInt("defaultcpbase") + "\"" + " cpAdd=\"" + rset.getInt("defaultcpadd") + "\"" + " cpMod=\"" + rset.getInt("defaultcpmod") + "\">\r\n";
				
				PreparedStatement st2 = con.prepareStatement("SELECT min(level), max(level), skill_trees.* FROM skill_trees WHERE class_id = ? GROUP BY min_level, sp, skill_id ORDER BY min_level, sp, skill_id, level");
				...
				while (rset2.next())
				{
					...
					content += "\" reqSp=\"" + rset2.getInt("sp") + "\"" + " minLevel=\"" + rset2.getInt("min_level") + "\"" + (rset2.getString("learned_by_npc").equals("false") ? " learnFromPanel=\"false\"" : "") + (rset2.getString("learned_by_fs").equals("true") ? " learnFromFS=\"true\"" : "") + (rset2.getString("is_transfer").equals("true") ? " isTransfer=\"true\"" : "") + (rset2.getString("is_autoget").equals("true") ? " autoGet=\"true\"" : "") + " /> <!-- " + SkillTable.getInstance().getInfo(rset2.getInt("skill_id"), rset2.getInt("level")).getName() + " -->\r\n";
				}
	...
	public static void shops()
	{
		...
		try
		{
			...
			while (rset.next())
			{
				content += "\t<shop id=\"" + rset.getString("shop_id") + "\"" + " npcId=\"" + (rset.getString("npc_id").equalsIgnoreCase("gm") ? -1 : rset.getInt("npc_id")) + "\">" + " <!-- " + (rset.getString("npc_id").equalsIgnoreCase("gm") ? "GM Shop" : NpcTable.getInstance().getTemplate(rset.getInt("npc_id")).getName()) + " -->\r\n";
				
				PreparedStatement st2 = con.prepareStatement("SELECT * FROM merchant_buylists WHERE shop_id = ? ORDER BY `order` ASC");
				st2.setInt(1, rset.getInt("shop_id"));
				ResultSet rset2 = st2.executeQuery();
				
				while (rset2.next())
				{
					content += "\t\t<item id=\"" + rset2.getInt("item_id") + "\"" + (rset2.getInt("price") > -1 ? " price=\"" + rset2.getInt("price") + "\"" : "") + (rset2.getInt("count") > -1 ? " count=\"" + rset2.getInt("count") + "\"" : "") + (rset2.getInt("time") > 0 ? " time=\"" + rset2.getInt("time") + "\"" : "") + " /> <!-- " + ItemTable.getInstance().getTemplate(rset2.getInt("item_id")).getName() + " -->\r\n";
				}
	...
	public static void customShops()
	{
		...
		try
		{
			...
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("SELECT * FROM custom_merchant_shopids ORDER BY npc_id ASC");
			ResultSet rset = statement.executeQuery();
			
			while (rset.next())
			{
				PreparedStatement st2 = con.prepareStatement("SELECT * FROM custom_merchant_buylists WHERE shop_id = ? ORDER BY `order` ASC");
				st2.setInt(1, rset.getInt("shop_id"));
				ResultSet rset2 = st2.executeQuery();
	...
	public static void enchantSkillGroups()
	{
		...
		try
		{
			...
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("SELECT * FROM enchant_skill_groups GROUP BY group_id ORDER BY group_id ASC");
			ResultSet rset = statement.executeQuery();
			...
			while (rset.next())
			{
				...
				PreparedStatement st2 = con.prepareStatement("SELECT *, min(level), max(level) FROM enchant_skill_groups WHERE group_id = ? GROUP BY adena ORDER BY level ASC");
				...
				while (rset2.next())
				{
					...
					content += (rset2.getInt("adena") > -1 ? " adena=\"" + rset2.getInt("adena") + "\"" : "") + (rset2.getInt("sp") > -1 ? " sp=\"" + rset2.getInt("sp") + "\"" : "") + ">\r\n";
		...
		try
		{
			...
			while (rset.next())
			{
				...
				for (int i = 0; i < skills.length; i += 2)
					content += "\t\t<skill id=\"" + skills[i] + "\" levels=\"" + skills[i + 1] + "\" /> <!-- " + SkillTable.getInstance().getInfo(Integer.valueOf(skills[i]), Integer.valueOf(skills[i + 1])).getName() + " -->\r\n";
				if (rset.getInt("enchant6skill") > 0)
					content += "\t\t<enchant6Skill id=\"" + rset.getInt("enchant6skill") + "\" /> <!-- " + SkillTable.getInstance().getInfo(rset.getInt("enchant6skill"), 1).getName() + " -->\r\n";
				if (rset.getInt("shield_skill_id") > 0)
					content += "\t\t<shieldSkill id=\"" + rset.getInt("shield_skill_id") + "\" /> <!-- " + SkillTable.getInstance().getInfo(rset.getInt("shield_skill_id"), 1).getName() + " -->\r\n";
				content += "\t</armorSet>\r\n";
			}
	...
	public static void henna()
	{
		...
		try
		{
			...
			while (rset.next())
			{
				content += "\t<henna symbolId=\"" + rset.getString("symbol_id") + "\"" + " dyeId=\"" + rset.getString("dye_id") + "\"" + " name=\"" + ItemTable.getInstance().getTemplate(rset.getInt("dye_id")).getName().replace('<', '(').replace('>', ')') + "\"" + " price=\"" + rset.getString("price") + "\"" + (rset.getInt("stat_STR") != 0 ? " STR=\"" + rset.getString("stat_STR") + "\"" : "") + (rset.getInt("stat_CON") != 0 ? " CON=\"" + rset.getString("stat_CON") + "\"" : "") + (rset.getInt("stat_DEX") != 0 ? " DEX=\"" + rset.getString("stat_DEX") + "\"" : "") + (rset.getInt("stat_INT") != 0 ? " INT=\"" + rset.getString("stat_INT") + "\"" : "") + (rset.getInt("stat_WIT") != 0 ? " WIT=\"" + rset.getString("stat_WIT") + "\"" : "") + (rset.getInt("stat_MEM") != 0 ? " MEN=\"" + rset.getString("stat_MEM") + "\"" : "") + ">\r\n";
				
				PreparedStatement st2 = con.prepareStatement("SELECT class_id FROM henna_trees WHERE symbol_id = ? ORDER BY `class_id` ASC");
				st2.setInt(1, rset.getInt("symbol_id"));
				ResultSet rset2 = st2.executeQuery();
				
				while (rset2.next())
				{
					content += "\t\t<allowedClass id=\"" + rset2.getInt("class_id") + "\" />" + " <!-- " + PlayerClassTable.getInstance().getClassNameById(rset2.getInt("class_id")) + " -->\r\n";
				}
	...
	public static void fortSpawns()
	{
		...
		try
		{
			...
			for (Fort fort : FortManager.getInstance().getForts())
			{
				...
				PreparedStatement statement = con.prepareStatement("SELECT * FROM fort_spawnlist WHERE fortId = ? AND spawnType = 0");
				statement.setInt(1, fort.getFortId());
				ResultSet rset = statement.executeQuery();
				while (rset.next())
					content += "\t<spawn npcId=\"" + rset.getInt("npcId") + "\" x=\"" + rset.getInt("x") + "\" y=\"" + rset.getInt("y") + "\" z=\"" + rset.getInt("z") + "\" heading=\"" + rset.getInt("heading") + "\" respawn=\"60\" />\r\n";
				...
				for (int i = 1; i < 5; i++)
				{
					...
					content += "\t\t<spawn npcId=\"" + npcId + "\" x=\"" + x + "\" y=\"" + y + "\" z=\"" + z + "\" heading=\"" + heading + "\" respawn=\"60\" />\r\n";
				}
				content += "\t</specificSpawnList>\r\n";
				...
				rset = statement.executeQuery();
				while (rset.next())
					content += "\t\t<spawn npcId=\"" + rset.getInt("npcId") + "\" x=\"" + rset.getInt("x") + "\" y=\"" + rset.getInt("y") + "\" z=\"" + rset.getInt("z") + "\" heading=\"" + rset.getInt("heading") + "\" respawn=\"" + rset.getInt("respawnDelay") + "\" />\r\n";
