...
public class CustomCommunityBoard
{
	...
	private void loadForumNews()
	{
		...
		try
		{
			...
			PreparedStatement st = con.prepareStatement("SELECT * FROM " + Config.FORUM_DB_NAME + ".forums_topics WHERE forum_id = 4 AND pinned = 0 AND approved = 1 ORDER BY start_date DESC LIMIT 0, 70");
			...
			while (rs.next())
			{
				...
				PreparedStatement st2 = con.prepareStatement("SELECT * FROM " + Config.FORUM_DB_NAME + ".forums_posts WHERE pid = ?");
	...
	private String getCustomCastleInfoBoard()
	{
		...
		for (Castle castle : CastleManager.getInstance().getCastles())
		{
			...
			sb.append("<table width=710 height=160 border=0><tr><td><table><tr><td><img src=\"Crest.pledge_crest_" + Config.SERVER_ID + "_" + castle.getCastleId() + "\" width=256 height=128></td></tr></table></td>");
			...
			if (clan != null)
			{
				...
				sb.append("<tr><td FIXWIDTH=110>Clan Leader:</td><td FIXWIDTH=360> " + clan.getLeaderName() + "</td></tr>");
			}
			else
	...
	public String getCustomGrandBossInfo()
	{
		...
		sb.append("<table width=750 bgcolor=999999><tr><td FIXWIDTH=20>#</td><td FIXWIDTH=50>Name</td><td FIXWIDTH=30>Level</td><td FIXWIDTH=150>Respawn</td><td FIXWIDTH=120>Status</td><td FIXWIDTH=20>Drop</td></tr></table>");
		...
		for (Object id : _bossIds)
		{
			...
			else if (status == GrandBossManager.getInstance().DEAD)
			{
				...
				String color = String.format("%02x%02x%02x", (int) ((1 - gradient) * 0x80) + 0x7f, (int) (gradient * 0x80), (int) (gradient * 0x80));
			...
			sb.append("<table border=0 cellspacing=0 cellpadding=2 width=750 height=17><tr><td FIXWIDTH=20>" + (pos + 1) + "</td><td FIXWIDTH=50>" + boss.getName() + "</td><td FIXWIDTH=30>" + boss.Level + "</td><td FIXWIDTH=150>" + bossRespawn + "</td><td FIXWIDTH=120>" + bossStatus + "</td><td FIXWIDTH=20><button value=\" \" width=16 height=16 action=\"bypass _bbscustom;info;drop;" + boss.NpcId + ";1\" fore=L2UI_CH3.aboutotpicon back=L2UI_CH3.aboutotpicon></td></tr></table>");
	...
	public void parseCmd(String command, L2PcInstance activeChar)
	{
		...
		switch (val)
		{
			...
			case "itemAuction":
				sendCommunityBoardPage(getCommunityPage("itemAuction").replace("%auctionInfo%", TenkaiAuctionManager.getInstance().getAuctionInfo(activeChar.getObjectId(), Integer.valueOf(st.nextToken()))), activeChar);
				break;
	...
	private String getGainakStatus(L2PcInstance pl, int pageToShow)
	{
		...
		{
			sb.append("<table><tr><td><img src=\"Crest.pledge_crest_%serverId%_" + (gainakSiegeZone.isActive() ? 51 : 50) + "\"  width=512 height=128></td></tr></table>");
			sb.append("<br><br>");
