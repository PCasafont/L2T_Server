...
public class FourSepulchersManager
{
	...
	protected TIntObjectHashMap<ArrayList<L2SepulcherMonsterInstance>> _dukeMobs = new TIntObjectHashMap<ArrayList<L2SepulcherMonsterInstance>>();
	...
	protected TIntObjectHashMap<ArrayList<L2SepulcherMonsterInstance>> _viscountMobs = new TIntObjectHashMap<ArrayList<L2SepulcherMonsterInstance>>();
	...
	private void loadMysteriousBox()
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement("SELECT id, npc_templateid, locx, locy, locz, heading, respawn_delay, key_npc_id FROM four_sepulchers_spawnlist Where spawntype = ? ORDER BY id");
			...
			while (rset.next())
			{
				...
				{
					Log.warning("FourSepulchersManager.LoadMysteriousBox: Data missing in NPC table for ID: " + rset.getInt("npc_templateid") + ".");
				}
			}
	...
	private void initKeyBoxSpawns()
	{
		...
		for (int keyNpcId : _keyBoxNpc.keys())
		{
			try
			{
				...
				{
					Log.warning("FourSepulchersManager.InitKeyBoxSpawns: Data missing in NPC table for ID: " + _keyBoxNpc.get(keyNpcId) + ".");
				}
			}
			catch (Exception e)
			{
	...
	private void loadPhysicalMonsters()
	{
		...
		try
		{
			...
			PreparedStatement statement2 = con.prepareStatement("SELECT id, npc_templateid, locx, locy, locz, heading, respawn_delay, key_npc_id FROM four_sepulchers_spawnlist Where key_npc_id = ? and spawntype = ? ORDER BY id");
			while (rset1.next())
			{
				...
				while (rset2.next())
				{
					...
					{
						Log.warning("FourSepulchersManager.LoadPhysicalMonsters: Data missing in NPC table for ID: " + rset2.getInt("npc_templateid") + ".");
					}
				}
		...
		try
		{
			...
			while (rset1.next())
			{
				...
				while (rset2.next())
				{
					...
					{
						Log.warning("FourSepulchersManager.LoadMagicalMonsters: Data missing in NPC table for ID: " + rset2.getInt("npc_templateid") + ".");
					}
				}
		...
		try
		{
			...
			while (rset1.next())
			{
				...
				while (rset2.next())
				{
					...
					{
						Log.warning("FourSepulchersManager.LoadDukeMonsters: Data missing in NPC table for ID: " + rset2.getInt("npc_templateid") + ".");
					}
				}
		...
		try
		{
			...
			while (rset1.next())
			{
				...
				while (rset2.next())
				{
					...
					{
						Log.warning("FourSepulchersManager.LoadEmperorsGraveMonsters: Data missing in NPC table for ID: " + rset2.getInt("npc_templateid") + ".");
					}
				}
	...
	protected void initLocationShadowSpawns()
	{
		...
		for (int i = 0; i <= 3; i++)
		{
			...
			{
				Log.warning("FourSepulchersManager.InitLocationShadowSpawns: Data missing in NPC table for ID: " + _shadowSpawnLoc[locNo][i][0] + ".");
			}
		}
	}
	...
	protected void initExecutionerSpawns()
	{
		...
		for (int keyNpcId : _victim.keys())
		{
			try
			{
				...
				{
					Log.warning("FourSepulchersManager.InitExecutionerSpawns: Data missing in NPC table for ID: " + _victim.get(keyNpcId) + ".");
				}
			}
			catch (Exception e)
			{
