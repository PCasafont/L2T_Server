...
public class ClanRecruitManager
{
	...
	private void load()
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement("SELECT clan_id, karma, introduction, large_introduction FROM clan_recruit_data");
			...
			statement = con.prepareStatement("SELECT applicant_id, clan_id, karma, application FROM clan_recruit_applicants");
	...
	public boolean addClan(L2Clan clan, int karma, String introduction, String largeIntroduction)
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("INSERT INTO clan_recruit_data (clan_id, karma, introduction, large_introduction) VALUES (?, ?, ?, ?)");
	...
	public void updateClan(L2Clan clan, int karma, String introduction, String largeIntroduction)
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("UPDATE clan_recruit_data SET karma = ?, introduction = ?, large_introduction = ? WHERE clan_id = ?");
	...
	public boolean addApplicant(L2PcInstance player, int clanId, String application)
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("INSERT INTO clan_recruit_applicants (applicant_id, clan_id, application) VALUES (?, ?, ?)");
	...
	public boolean removeApplicant(int playerId)
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("DELETE FROM clan_recruit_applicants WHERE applicant_id = ?");
	...
	public boolean addWaitingUser(L2PcInstance player, int karma)
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("INSERT INTO clan_recruit_applicants (applicant_id, karma) VALUES (?, ?)");
			statement.setInt(1, player.getObjectId());
			statement.setInt(2, karma);
			statement.execute();
			statement.close();
