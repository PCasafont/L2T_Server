...
public class Lottery
{
	...
	public void increasePrize(long count)
	{
		...
		{
			Log.log(Level.WARNING, "Lottery: Could not increase current lottery prize: " + e.getMessage(), e);
		}
		finally
		{
	...
	private class startLottery implements Runnable
	{
		...
		public void run()
		{
			...
			{
				Log.log(Level.WARNING, "Lottery: Could not restore lottery data: " + e.getMessage(), e);
			}
			finally
			{
				L2DatabaseFactory.close(con);
			...
			{
				Log.log(Level.WARNING, "Lottery: Could not store new lottery data: " + e.getMessage(), e);
			}
			finally
			{
				L2DatabaseFactory.close(con);
	...
	private class finishLottery implements Runnable
	{
		...
		public void run()
		{
			...
			if (Config.DEBUG)
			{
				Log.info("Lottery: The lucky numbers are " + luckynums[0] + ", " + luckynums[1] + ", " + luckynums[2] +
			...
			{
				Log.log(Level.WARNING, "Lottery: Could restore lottery data: " + e.getMessage(), e);
			}
			finally
			{
				L2DatabaseFactory.close(con);
			...
			if (count1 > 0)
			{
				// There are winners.
				sm = SystemMessage
						.getSystemMessage(SystemMessageId.AMOUNT_FOR_WINNER_S1_IS_S2_ADENA_WE_HAVE_S3_PRIZE_WINNER);
				sm.addNumber(getId());
			...
			{
				Log.log(Level.WARNING, "Lottery: Could not store finished lottery data: " + e.getMessage(), e);
			}
			finally
			{
				L2DatabaseFactory.close(con);
	...
	public long[] checkTicket(int id, int enchant, int type2)
	{
		...
		{
			Log.log(Level.WARNING, "Lottery: Could not check lottery ticket #" + id + ": " + e.getMessage(), e);
		}
		finally
		{
			L2DatabaseFactory.close(con);
