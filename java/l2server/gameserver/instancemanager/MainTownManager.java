...
public class MainTownManager
{
	...
	private void load()
	{
		...
		if (GlobalVariablesManager.getInstance().isVariableStored("MTRotationIndex"))
			_rotationIndex = Integer.parseInt(GlobalVariablesManager.getInstance().getStoredVariable("MTRotationIndex"));
		else
	...
	private void changeMainTown()
	{
		...
		Announcements.getInstance().announceToAll("The main town has changed! Now it's " + _currentMainTown.getName() + ".");
	}
	...
	public String getNextTownsInfo()
	{
		...
		for (int i = 0; i < 10; i++)
		{
			...
			if (town != prevTown)
			{
				...
				info += "- From <font color=LEVEL>" + from + "</font> to <font color=LEVEL>" + to + "</font> " + "the main town will be <font color=LEVEL>" + prevTown.getName() + "</font>.<br1>";
