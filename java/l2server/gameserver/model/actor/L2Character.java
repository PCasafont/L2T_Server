...
import l2server.gameserver.network.serverpackets.MagicSkillCanceld;
...
public abstract class L2Character extends L2Object
{
	...
	public void teleToLocation(int x, int y, int z, int heading, boolean allowRandomOffset)
	{
		...
		else
		{
			...
			if (player.isBot())
				player.getBotController().onTeleport();
		}
	...
	private void beginCast(L2Skill skill, boolean simultaneously, boolean second, L2Character target, L2Object[] targets)
	{
		...
		if (effectWhileCasting)
		{
			...
			if (!isBot() && skill.getItemConsumeId() > 0)
			{
		...
		if (this instanceof L2Playable && !isBot())
		{
			if (!effectWhileCasting)
			{
				L2PcInstance player = getActingPlayer();
	...
	protected boolean checkDoCastConditions(L2Skill skill)
	{
		...
		if (!isBot() && getCurrentMp() < getStat().getMpConsume(skill) + getStat().getMpInitialConsume(skill))
		{
		...
		if (!isBot() && skill.getItemConsumeId() > 0 && getInventory() != null)
		{
	...
	public void onMagicHitTimer(MagicUseTask mut)
	{
		...
		try
		{
			...
			if (mpConsume > 0)
			{
				if (!isBot() && mpConsume > getCurrentMp())
				{
					sendPacket(SystemMessage.getSystemMessage(SystemMessageId.NOT_ENOUGH_MP));
					abortCast();
					return;
