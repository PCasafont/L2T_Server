...
public class L2TeleporterInstance extends L2Npc
{
    ...
    public void onBypassFeedback(L2PcInstance player, String command)
    {
        ...
        if (player.getFirstEffect(6201) != null || player.getFirstEffect(6202) != null || player
                .getFirstEffect(6203) != null)
        {
        ...
        else if (command.startsWith("Chat"))
        {
            ...
            if (!Config.IS_CLASSIC)
            {
                ...
                else if (val.equalsIgnoreCase("1") && cal.get(Calendar.HOUR_OF_DAY) >= 20 && cal
                        .get(Calendar.HOUR_OF_DAY) <= 23 && (cal.get(Calendar.DAY_OF_WEEK) == 1 || cal
    ...
    protected void doTeleport(L2PcInstance player, int val)
    {
        ...
        if (list != null)
        {
            ...
            if (SiegeManager.getInstance()
                    .getSiege(list.getLocX(), list.getLocY(), list.getLocZ()) != null)
            {
                player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.NO_PORT_THAT_IS_IN_SIGE));
                return;
            }
            else if (TownManager.townHasCastleInSiege(list.getLocX(), list
                    .getLocY()) && (isInsideZone(L2Character.ZONE_TOWN) || getNpcId() > 40000))
            {
                player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.NO_PORT_THAT_IS_IN_SIGE));
                return;
            }
            ...
            else if (Config.isServer(Config.TENKAI) && getNpcId() == 40001 && player.getPvpFlag() > 0 && player
                    .getCurrentHp() * 100 / player.getMaxHp() < 60) //pvping?
            {
            ...
            {
                if (cal.get(Calendar.HOUR_OF_DAY) >= 20 && cal.get(Calendar.HOUR_OF_DAY) <= 23 && (cal
                        .get(Calendar.DAY_OF_WEEK) == 1 || cal.get(Calendar.DAY_OF_WEEK) == 7))
                {
                    price /= 2;
                }
            }

            if (Config.ALT_GAME_FREE_TELEPORT || player
                    .destroyItemByItemId("Teleport " + (list.getIsForNoble() ? " nobless" : ""), list
                            .getItemId(), price, this, true))
            {
                if (Config.DEBUG)
                {
                    Log.fine("Teleporting player " + player.getName() + " to new location: " + list
    ...
    {
        if (getNpcId() > 40000 || CastleManager.getInstance()
                .getCastleIndex(this) < 0) // Teleporter isn't on castle ground
        {
