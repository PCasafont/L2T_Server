...
public class L2FortManagerInstance extends L2MerchantInstance
{
			...
			else if (actualCommand.equalsIgnoreCase("functions"))
			{
				if (val.equalsIgnoreCase("tele"))
				{
					...
					if (getFort().getFunction(Fort.FUNC_TELEPORT) == null)
						...
						html.setFile(player.getHtmlPrefix(), "fortress/" + getNpcId() + "-t" + getFort().getFunction(Fort.FUNC_TELEPORT).getLvl() + ".htm");
					sendHtmlMessage(player, html);
				}
	...
	private void doTeleport(L2PcInstance player, int val)
	{
		...
		if (list != null)
		{
			...
			{
				if (Config.DEBUG)
					Log.warning("Teleporting player " + player.getName() + " for Fortress to new location: " + list.getLocX() + ":" + list.getLocY() + ":" + list.getLocZ());
