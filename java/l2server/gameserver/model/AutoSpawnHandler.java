...
public class AutoSpawnHandler
{
	...
	public boolean removeSpawn(AutoSpawnInstance spawnInst)
	{
		...
		try
		{
			...
			if (Config.DEBUG)
				Log.info("AutoSpawnHandler: Removed auto spawn for NPC ID " + spawnInst._npcId + " (Object ID = " + spawnInst._objectId + ").");
		}
		catch (Exception e)
		{
			Log.log(Level.WARNING, "AutoSpawnHandler: Could not auto spawn for NPC ID " + spawnInst._npcId + " (Object ID = " + spawnInst._objectId + "): " + e.getMessage(), e);
			return false;
		}
	...
	private class AutoSpawner implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				if (Config.DEBUG)
					Log.info("AutoSpawnHandler: Spawned NPC ID " + spawnInst.getNpcId() + " at " + x + ", " + y + ", " + z + " (Near " + nearestTown + ") for " + spawnInst.getRespawnDelay() / 60000 + " minute(s).");
			...
			{
				Log.log(Level.WARNING, "AutoSpawnHandler: An error occurred while initializing spawn instance (Object ID = " + _objectId + "): " + e.getMessage(), e);
			}
		}
	}
	...
	private class AutoDespawner implements Runnable
	{
		...
		public void run()
		{
			...
			{
				Log.log(Level.WARNING, "AutoSpawnHandler: An error occurred while despawning spawn (Object ID = " + _objectId + "): " + e.getMessage(), e);
			}
		}
	}
