...
public class L2TenkaiEventZone extends L2ZoneType
{
	...
	protected void onExit(L2Character character)
	{
		if (character instanceof L2PcInstance)
		{
			...
			if ((event != null) && event.getConfig().needsClosedArena() && player.getEvent().isState(EventState.STARTED) && (event.getConfig().getLocation().getId() == (getId() - BASE_ID)) && (!event.isType(EventType.VIP) || (event.getParticipantTeam(player.getObjectId()).getVIP().getObjectId() == player.getObjectId())) && (!event.isType(EventType.CaptureTheFlag) || (player.getCtfFlag() != null)))
