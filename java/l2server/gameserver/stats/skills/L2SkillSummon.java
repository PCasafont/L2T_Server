	...
	public boolean checkCondition(L2Character activeChar, L2Object target, boolean itemOrWeapon)
	{
		if (activeChar instanceof L2PcInstance)
		{
			...
			else // Normal summon
			{
				...
				{
					if (!player.getSummon(0).getTemplate().getName().equalsIgnoreCase("Tree of life"))
					{
	...
	public void useSkill(L2Character caster, L2Object[] targets)
	{
		...
		if (_isCubic)
		{
			...
			if (targets.length > 1 || (targets.length == 1 && targets[0] != activeChar)) // Not self cubic skill
			{
				for (L2Object obj: targets)
				{
					...
					if (player == activeChar)
						player.addCubic(_npcId, cubicSkillLevel, getPower(), _activationtime, _activationchance, _maxcount, _summonTotalLifeTime, false);
					else // given by other player
						player.addCubic(_npcId, cubicSkillLevel, getPower(), _activationtime, _activationchance, _maxcount, _summonTotalLifeTime, true);
					if (hasEffects())
			...
			else // Normal cubic skill
			{
				...
				activeChar.addCubic(_npcId, cubicSkillLevel, getPower(), _activationtime, _activationchance, _maxcount, _summonTotalLifeTime, false);
		...
		{
			Log.warning("Summon attempt for nonexisting NPC ID:"+_npcId+", skill ID:"+this.getId());
		...
		if (activeChar.isBot())
			activeChar.getBotController().onSummonSpawn(summon);
