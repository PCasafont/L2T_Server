    ...
    public boolean onActionTime()
    {
        ...
        for (L2Character target : player.getKnownList().getKnownCharactersInRadius(getSkill().getSkillRadius()))
        {
            ...
            else if (target.getActingPlayer() != null)
            {
                ...
                if (targetPlayer.isInParty() && targetPlayer.getParty() == player.getParty() || target.getActingPlayer()
                        .getClanId() != 0 && target.getActingPlayer().getClanId() == player.getClanId() || target
                        .getActingPlayer().getAllyId() != 0 && target.getActingPlayer().getAllyId() == player
                        .getAllyId())
                {
