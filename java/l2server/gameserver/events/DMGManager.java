...
public class DMGManager
{
	...
	public static String getRankingInfo()
	{
		...
		for (Entry<Integer, dmginfo> info : dmgIinfo.entrySet())
		{
			...
			sb.append("<table cellspacing=0 cellpadding=2 width=750 height=17><tr><td><img src=\"L2UI_CT1.PlayerStatusWnd_ClassMark_" + info.getKey() + "_Big\" width=32 height=32></td><td FIXWIDTH=200>" + className + "</td><td FIXWIDTH=250>" + actualrecord + "</td><td FIXWIDTH=250>" + actualowner + "</td></tr></table>");
	...
	public static class dmginfo
	{
		...
		{
			pl.sendPacket(new CreatureSay(36610, 2, eventname, "Congrats, you raised the $1 record with $2!".replace("$1", PlayerClassTable.getInstance().getClassNameById(pl.getClassId())).replace("$2", String.valueOf(dmg))));
	...
	public static void saveData()
	{
		...
		try
		{
			...
			for (Entry<Integer, dmginfo> info : dmgIinfo.entrySet())
			{
				...
				statement = con.prepareStatement("UPDATE `dmg_data` SET `newdmg`=?, `newname`=?, `playerid`=?, `externalIP`=?, `internalIP`=? WHERE `classid`=?");
	...
	public static void giveRewardsAndReset()
	{
		...
		for (Entry<Integer, dmginfo> info : dmgIinfo.entrySet())
		{
			if (info.getValue().getNewName() != null && !info.getValue().getNewName().equals(""))
			{
				...
				Message msg = new Message(-1, charid, false, eventname, "Congratulations, you was classified on Damage Ranking, here is your reward!", 0);
		...
		Announcements.getInstance().announceToAll("All players classified on Damage Ranking were rewarded and the ranking was restarted, more rewards the next week!");
	}
	...
	private static void truncateTable()
	{
		...
		try
		{
			...
			statement = con.prepareStatement("UPDATE dmg_data SET newdmg=0, playerid=0, newname=0, externalIP=0, internalIP=0 WHERE newdmg > 0");
	...
	public static void updateIP(L2PcInstance pl)
	{
		...
		for (Entry<Integer, dmginfo> info : dmgIinfo.entrySet())
		{
			...
			if (info.getValue().getNewName().equalsIgnoreCase(pl.getName()) && !info.getValue().getExternalIP().equalsIgnoreCase(pl.getExternalIP()))
			{
	...
	public static void load()
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement("SELECT `classid`, `newdmg`, `newname`, `playerid`, `externalIP`, `internalIP` FROM `dmg_data`");
			...
			while (rs.next())
			{
				...
				info.setdmg(rs.getInt("classid"), rs.getInt("newdmg"), rs.getString("newname"), rs.getInt("playerid"), rs.getString("externalIP"), rs.getString("internalIP"));
