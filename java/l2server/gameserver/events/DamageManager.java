...
public class DamageManager
{
	...
	private class DamageInfo
	{
		...
		{
			pl.sendPacket(new CreatureSay(36610, 2, "Damage Manager", "Congrats, you raised the $1 record with $2!".replace("$1", PlayerClassTable.getInstance().getClassNameById(pl.getClassId())).replace("$2", String.valueOf(dmg))));
	...
	public void saveData()
	{
		...
		try
		{
			...
			for (Entry<Integer, DamageInfo> info : dmgIinfo.entrySet())
			{
				if (info == null)
					continue;
				statement = con.prepareStatement("UPDATE `dmg_data` SET `newdmg`=?, `newname`=?, `playerid`=?, `hwId`=? WHERE `classid`=?");
	...
	private void load()
	{
		...
		try
		{
			...
			PreparedStatement statement = con.prepareStatement("SELECT `classid`, `newdmg`, `newname`, `playerid`, `hwId` FROM `dmg_data`");
