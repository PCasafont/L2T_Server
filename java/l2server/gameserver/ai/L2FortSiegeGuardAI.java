...
public class L2FortSiegeGuardAI extends L2CharacterAI implements Runnable
{
    ...
    private boolean autoAttackCondition(L2Character target)
    {
        ...
        if (target == null || target instanceof L2DefenderInstance || target instanceof L2NpcInstance ||
                ...
                .isAlikeDead() || target instanceof L2FortBallistaInstance ||
                target instanceof L2FortCommanderInstance || target instanceof L2Playable)
        {
            ...
            if (player == null || player.getClan() != null && player.getClan().getHasFort() == ((L2Npc) _actor)
                    .getFort().getFortId())
            {
                return false;
            }
    ...
    private void thinkAttack()
    {
        if (Config.DEBUG)
        {
            Log.info("L2FortSiegeGuardAI.thinkAttack(); timeout=" + (_attackTimeout - TimeController.getGameTicks()));
        }

        if (_attackTimeout < TimeController.getGameTicks())
        {
        ...
        if (attackTarget == null || attackTarget.isAlikeDead() || _attackTimeout < TimeController.getGameTicks())
        {
    ...
    private final void factionNotifyAndSupport()
    {
        ...
        for (L2Character cha : _actor.getKnownList().getKnownCharactersInRadius(1000))
        {
            ...
            {
                if (_selfAnalysis.hasHealOrResurrect && cha instanceof L2PcInstance && ((L2Npc) _actor).getFort()
    ...
    private void attackPrepare()
    {
        ...
        if (attackTarget instanceof L2PcInstance && sGuard.getFort().getSiege()
            ...
            for (L2Skill sk : skills)
            {
                        ...
                        .getCurrentMp() >= _actor.getStat().getMpConsume(sk) && !sk.isPassive())
                {
                    ...
                    if (sk.getSkillType() == L2SkillType.BUFF || sk.getSkillType() == L2SkillType.HEAL)
                    {
                        ...
                        {
                            L2Abnormal[] effects = _actor.getAllEffects();
