...
public class L2FortSiegeGuardAI extends L2CharacterAI implements Runnable
{
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		if (target == null || target instanceof L2DefenderInstance || target instanceof L2NpcInstance ||
				target instanceof L2DoorInstance || target.isAlikeDead() || target instanceof L2FortBallistaInstance ||
				target instanceof L2FortCommanderInstance || target instanceof L2Playable)
		{
			...
			if (player == null ||
					player.getClan() != null && player.getClan().getHasFort() == ((L2Npc) this.actor).getFort().getFortId())
			{
				return false;
			}
		}
	...
	private void thinkActive()
	{
		...
		if (this.actor.getWalkSpeed() >= 0)
		{
			if (this.actor instanceof L2DefenderInstance)
			{
				((L2DefenderInstance) this.actor).returnHome();
			}
			else
			{
				((L2FortCommanderInstance) this.actor).returnHome();
			}
		}
	}
	...
	private void thinkAttack()
	{
		if (Config.DEBUG)
		{
			Log.info("L2FortSiegeGuardAI.thinkAttack(); timeout=" + (this.attackTimeout - TimeController.getGameTicks()));
		}
		...
		for (L2Character cha : this.actor.getKnownList().getKnownCharactersInRadius(1000))
		{
			...
			if (!(cha instanceof L2Npc))
			{
				if (this.selfAnalysis.hasHealOrResurrect && cha instanceof L2PcInstance &&
						((L2Npc) this.actor).getFort().getSiege().checkIsDefender(((L2PcInstance) cha).getClan()))
				{
					// heal friends
	...
	private void attackPrepare()
	{
		...
		if (this.actor instanceof L2FortCommanderInstance)
		{
			sGuard = (L2FortCommanderInstance) this.actor;
		}
		else
		{
			sGuard = (L2DefenderInstance) this.actor;
		}
		L2Character attackTarget = getAttackTarget();
		...
		else if (this.actor.isMuted() && dist_2 > range * range)
		{
			// Temporary hack for preventing guards jumping off towers,
			// before replacing this with effective geodata checks and AI modification
			double dz = this.actor.getZ() - attackTarget.getZ();
