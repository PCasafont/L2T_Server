...
public class L2AWarriorAI extends L2APlayerAI
{
	...
	protected boolean interactWith(L2Character target)
	{
		...
		if (getIntention() == CtrlIntention.AI_INTENTION_IDLE || !hasAbnormalType(this.player, "possession"))
		{
			// On IDLE or if there's no totem on, renew basic Feral Cry (Bear)
			// If there's any totem, just ignore this check.
			feralCry = this.player.getKnownSkill(BEAR_CRY);
		}
		else if (this.player.getCurrentCp() < this.player.getMaxCp() * 0.8)
		{
			// Under first contact, switch to Puma
			feralCry = this.player.getKnownSkill(PUMA_CRY);
		}
		else if (this.player.getCurrentHp() < this.player.getMaxHp() * 0.99)
		{
			// PVP is getting serious, change to Ogre
			feralCry = this.player.getKnownSkill(OGRE_CRY);
		}
		else if (this.player.isInsideRadius(target, 1000, true, true))
		{
			// Target is fleeing, let's use Rabbit to catch him
			feralCry = this.player.getKnownSkill(RABBIT_CRY);
		}
		else if (target instanceof L2PcInstance && ((L2PcInstance) target).getClassId() == 146)
		{
			// Target is a healer, change to Hawk
			feralCry = this.player.getKnownSkill(HAWK_CRY);
		}

		if (feralCry != null && this.player.useMagic(feralCry, true, false))
		{
			return true;
		}
				...
				this.player.getTarget() instanceof L2Playable)
		{
			...
			if (target != null && 600 - this.player.getDistanceSq(target) > 100)
			{
				L2Skill skill = this.player.getKnownSkill(HURRICANE_RUSH);
