...
public class RequestExRemoveItemAttribute extends L2GameClientPacket
{
	...
	public void runImpl()
	{
		...
		L2ItemInstance targetItem = activeChar.getInventory().getItemByObjectId(this.objectId);
		...
		if (targetItem.getElementals() == null || targetItem.getElemental(this.element) == null)
		{
			return;
		}
		...
		if (activeChar.reduceAdena("RemoveElement", getPrice(targetItem), activeChar, true))
		{
			...
			{
				targetItem.getElemental(this.element).removeBonus(activeChar);
			}
			targetItem.clearElementAttr(this.element);
			...
			SystemMessage sm;
			byte realElement = targetItem.isArmor() ? Elementals.getOppositeElement(this.element) : this.element;
			...
			activeChar.sendPacket(new ExBaseAttributeCancelResult(targetItem.getObjectId(), this.element));
		}
		else
		{
