...
public final class RequestOustPledgeMember extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (activeChar.getName().equalsIgnoreCase(this.target))
		{
		...
		L2ClanMember member = clan.getClanMember(this.target);
		if (member == null)
		{
			Log.warning("Target (" + this.target + ") is not member of the clan");
			return;
		}
		...
		clan.broadcastToOnlineMembers(new PledgeShowMemberListDelete(this.target));
