...
public final class Say2 extends L2GameClientPacket
{
	...
	protected void readImpl()
	{
		this.text = readS();
		this.type = readD();
		this.target = this.type == TELL ? readS() : null;
	}
	...
	protected void runImpl()
	{
		if (Config.DEBUG)
		{
			Log.info("Say2: Msg Type = '" + this.type + "' Text = '" + this.text + "'.");
		}
		...
		if (this.type < 0 || this.type >= CHAT_NAMES.length)
		{
			Log.warning("Say2: Invalid type: " + this.type + " Player : " + activeChar.getName() + " text: " +
		...
		if (!activeChar.isGM() &&
				(this.text.indexOf(8) >= 0 && this.text.length() > 500 || this.text.indexOf(8) < 0 && this.text.length() > 105))
		{
		...
		if (!this.text.equalsIgnoreCase(".event") && activeChar.isPlayingEvent() &&
		...
		if (activeChar.isCursedWeaponEquipped() && (this.type == TRADE || this.type == SHOUT || this.type == GLOBAL))
		{
		...
		{
			if (this.type == ALL || this.type == SHOUT || this.type == TRADE || this.type == HERO_VOICE || this.type == GLOBAL)
			{
		...
		if (this.type == PETITION_PLAYER && activeChar.isGM())
		{
			this.type = PETITION_GM;
		}

		if (Config.LOG_CHAT)
		{
			if (this.type == CLAN && activeChar.getClan() != null)
			{
				this.target = activeChar.getClan().getName();
			}
			else if (this.type == ALLIANCE && activeChar.getClan() != null)
			{
				this.target = activeChar.getClan().getAllyName();
			}
			...
			try
			{
				...
				statement.setString(2, CHAT_NAMES[this.type]);
				statement.setString(3, activeChar.getName());
				statement.setString(4, this.target);
		...
		if (this.text.indexOf(8) >= 0 && !parseAndPublishItem(activeChar))
		{
			return;
		}
		...
		IChatHandler handler = ChatHandler.getInstance().getChatHandler(this.type);
		if (handler != null)
		{
			...
			handler.handleChat(this.type, activeChar, this.target, this.text);
		}
		else
		{
			Log.info("No handler registered for ChatType: " + this.type + " Player: " + getClient());
		}
	}
	...
	private boolean parseAndPublishItem(L2PcInstance owner)
	{
		...
		{
			int pos = this.text.indexOf("ID=", pos1);
			if (pos == -1)
			{
				return false;
			}
			...
			pos += 3;
			while (Character.isDigit(this.text.charAt(pos)))
			{
				result.append(this.text.charAt(pos++));
			}
			...
			pos1 = this.text.indexOf(8, pos) + 1;
