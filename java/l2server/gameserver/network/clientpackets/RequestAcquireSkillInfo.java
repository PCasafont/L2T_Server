...
public class RequestAcquireSkillInfo extends L2GameClientPacket
{
	...
	{
		if (this.id <= 0 || this.level <= 0) // minimal sanity check
		{
			return;
		}
		...
		final L2Skill skill = SkillTable.getInstance().getInfo(this.id, this.level);
		...
		if (skill == null)
		{
			if (Config.DEBUG)
			{
				Log.warning("skill id " + this.id + " level " + this.level + " is undefined. aquireSkillInfo failed.");
			}
		...
		if (this.skillType == 0)
		{
			if (trainer instanceof L2TransformManagerInstance)
			{
				...
				{
					if (s.getId() == this.id && s.getLevel() == this.level)
					{
						canteach = true;
						itemId = s.getItemId();
						break;
