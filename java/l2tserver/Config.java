...
import l2tserver.gameserver.util.FloodProtectorConfig;
import l2tserver.log.Log;
import l2tserver.util.L2Properties;
		...
		if (ServerMode.serverMode == ServerMode.MODE_GAMESERVER)
		{
			...
			try
			{
				...
				try
				{
					...
					LineNumberReader lnr = new LineNumberReader(new BufferedReader(new FileReader(new File(CONFIG_DIRECTORY + LANGUAGE_FILTER_FILE))));
					String line = null;
					while ((line = lnr.readLine()) != null)
					{
						if (line.trim().isEmpty() || line.startsWith("#"))
