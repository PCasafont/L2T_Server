...
public final class SendWareHouseWithDrawList extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			player.sendMessage("You cannot withdraw items from the warehouse while being involved in the Grand Olympiad!");
			return;
		}
		...
		for (WarehouseItem i : _items)
		{
			...
			final L2ItemInstance newItem = warehouse.transferItem(warehouse.getOwnerId() + "'s " + warehouse.getName(), i.getObjectId(), i.getCount(), player.getInventory(), player, manager);
			if (newItem == null)
			{
