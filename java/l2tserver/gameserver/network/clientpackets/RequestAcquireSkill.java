...
import l2tserver.gameserver.datatables.SubPledgeSkillTree.SubUnitSkill;
...
import l2tserver.gameserver.model.L2SquadTrainer;
import l2tserver.gameserver.model.L2TransformSkillLearn;
...
import l2tserver.gameserver.model.actor.instance.L2VillageMasterInstance;
...
import l2tserver.gameserver.network.serverpackets.AcquireSkillDone;
...
import l2tserver.gameserver.network.serverpackets.ShortCutRegister;
...
public class RequestAcquireSkill extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (_level < 1 || _level > 100 || _id < 1 || _id > 32000)
		{
			...
			Log.warning("Recived Wrong Packet Data in Aquired Skill - id: " + _id + " level: " + _level + " for "+player);
			return;
		}
		...
		switch (_skillType)
		{
			case 0:
			{
				...
				for (L2SkillLearn s : skills)
				{
					...
					L2Skill sk = SkillTable.getInstance().getInfo(s.getId(), s.getLevel());
					if (sk == null || sk.getId() != skill.getId()
							|| sk.getLevel() != skill.getLevel())
						continue;
			...
			case 5:
			{
				...
				//if (_skillType == 5)
				{
					...
					if (dual != null)
					{
						...
						if (skill.getLevelHash() > 1)
							oldSkill = SkillTable.getInstance().getInfo(skill.getId(), skill.getLevelHash() - 1);
						player.storeSkill(skill, oldSkill, dual.getClassIndex());
					}
				}
