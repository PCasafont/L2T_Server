...
import l2tserver.gameserver.model.entity.Message.SendBySystem;
import l2tserver.gameserver.network.serverpackets.ExPledgeRecruitApplyInfo;
...
public final class RequestPledgeRecruitApply extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (_enter == 1)
		{
			if (ClanRecruitManager.getInstance().addApplicant(activeChar, _clanId, _application))
			{
				...
				Message msg = new Message(clan.getLeaderId(), "Clan Application", "There's a new applicant for your clan! Check out the clan entry for further information.", SendBySystem.SYSTEM);
	...
	{
		return "RequestPledgeRecruitApply";
	}
}
