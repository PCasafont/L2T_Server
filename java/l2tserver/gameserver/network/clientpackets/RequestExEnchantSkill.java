...
import l2tserver.gameserver.network.serverpackets.ExEnchantSkillInfoDetail;
import l2tserver.gameserver.network.serverpackets.ExEnchantSkillResult;
	...
	protected void runImpl()
	{
		...
		if (player.getSp() >= requiredSp || _type == 2)
		{
			...
			if (_type == 2)
			{
				...
				if (Config.DEBUG)
					Log.fine("Learned skill ID: " + _skillId + " Level: " + _skillLvl + " for " + requiredSp + " SP, " + requireditems + " Adena.");
			...
			player.sendPacket(new ExEnchantSkillInfo(_skillId, currentSkill.getLevel(), currentSkill.getEnchantRouteId(), currentSkill.getEnchantLevel()));
			player.sendPacket(new ExEnchantSkillInfoDetail(_type, _skillId, currentSkill.getLevel(), currentSkill.getEnchantRouteId(), currentSkill.getEnchantLevel(), player));
	...
	private void logSkillEnchant(L2PcInstance player, L2Skill skill, L2ItemInstance spb, int rate)
	{
		if (Config.LOG_SKILL_ENCHANTS)
		{
			...
			try
			{
				...
				PreparedStatement statement = con.prepareStatement("INSERT INTO log_enchant_skills(player_id, skill_id, skill_level, spb_id, rate, time) VALUES(?,?,?,?,?,?)");
