...
import l2tserver.gameserver.LoginServerThread.SessionKey;
...
import l2tserver.gameserver.model.CharSelectInfoPackage;
...
import l2tserver.gameserver.security.SecondaryPasswordAuth;
...
import l2tserver.network.ReceivablePacket;
...
public final class L2GameClient extends MMOClient<MMOConnection<L2GameClient>> implements Runnable
{
	...
	private boolean offlineModeConditions(L2PcInstance player)
	{
		...
		else if (Config.OFFLINE_CRAFT_ENABLE && (player.isInCraftMode()
		...
		if (Config.OFFLINE_BUFFERS_ENABLE && !player.isInStoreMode() && player.isInsideZone(L2Character.ZONE_PEACE) && CustomOfflineBuffersManager.getInstance().setUpOfflineBuffer(player))
			canSetShop = true;
