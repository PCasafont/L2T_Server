...
import static l2tserver.gameserver.datatables.MultiSell.PAGE_SIZE;
	...
	protected void writeImpl()
	{
		...
		for (TradeItem item : _player.getCustomSellList().getItems())
		{
			if (item.getAppearance() > 0)
			{
				...
				getClient().sendPacket(new CreatureSay(_player.getObjectId(), Say2.TELL, _player.getName(), "WARNING: The " + item.getItem().getName() + " has appearance of " + app.getName() + "!"));
			}
