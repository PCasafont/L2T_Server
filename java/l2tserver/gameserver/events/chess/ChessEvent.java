package l2tserver.gameserver.events.chess;

import l2tserver.gameserver.datatables.SkillTable;
import l2tserver.gameserver.model.L2Abnormal;
import l2tserver.gameserver.model.L2Skill;
import l2tserver.gameserver.model.actor.instance.L2ChessPieceInstance;
...
public class ChessEvent
{
	...
	private static void rewardPlayer(L2PcInstance player)
	{
		...
		npcHtmlMessage.setHtml("<html><head><title>Chess Event</title></head><body>You won the game, congratulations!</body></html>");
	...
	public static synchronized boolean addWaitingParticipant(L2PcInstance playerInstance, int sideId)
	{
		...
		while (i < 8 && !trobat)
		{
			if (_waitingPlayers[i][0] == null && _waitingPlayers[i][1] == null) trobat = true;
			i++;
		}
	...
	public static void onLogout(L2PcInstance playerInstance)
	{
		...
		{
			if (isState(ChessState.STARTED) && isPlayerParticipant(playerInstance.getObjectId())) for (L2Abnormal effect : playerInstance.getAllEffects()) if (effect != null) effect.exit();
	...
	public static synchronized void onBypass(String command, L2PcInstance playerInstance)
	{
		...
		if (command.startsWith("wait") && getParticipantSideId(playerInstance.getObjectId()) == -1) {
			...
			{
				String html = "<html><head><title>Instanced Events</title></head><body>Now you are at the waiting list<br>";
		...
		else if (command.equals("remove_wait"))
		{
			...
			npcHtmlMessage.setHtml("<html><head><title>Instanced Events</title></head><body>You have been deleted from the waiting list.</body></html>");
			playerInstance.sendPacket(npcHtmlMessage);
		}
		else if (command.startsWith("challenge"))
		{
			...
			npcHtmlMessage1.setHtml("<html><head><title>Instanced Events</title></head><body>You have challenged the player " + target.getName() + ".</body></html>");
			playerInstance.sendPacket(npcHtmlMessage1);
			
			NpcHtmlMessage npcHtmlMessage2 = new NpcHtmlMessage(0);
			
			npcHtmlMessage2.setHtml("<html><head><title>Instanced Events</title></head><body>" + playerInstance.getName() + " has challenged you for a game. Do you accept?<br>" +
		...
		else if (command.equals("accept"))
		{
			...
			npcHtmlMessage.setHtml("<html><head><title>Instanced Events</title></head><body>Your game is about to start. Get ready!</body></html>");
			playerInstance.sendPacket(npcHtmlMessage);
		...
		else if (command.equals("reject"))
		{
			...
			npcHtmlMessage1.setHtml("<html><head><title>Instanced Events</title></head><body>You have rejected " + target.getName() + "'s challenge.</body></html>");
			playerInstance.sendPacket(npcHtmlMessage1);
			
			NpcHtmlMessage npcHtmlMessage2 = new NpcHtmlMessage(0);
			
			npcHtmlMessage2.setHtml("<html><head><title>Instanced Events</title></head><body>" + playerInstance.getName() + " has rejected your challenge.</body></html>");
			target.sendPacket(npcHtmlMessage2);
		}
	}
	...
	public static boolean isPlayerWaiting(int playerObjectId)
	{
		for (int i = 0; i < 8; i++)
			if ((_waitingPlayers[i][0] != null && _waitingPlayers[i][0].getObjectId() == playerObjectId) ||
					(_waitingPlayers[i][1] != null && _waitingPlayers[i][1].getObjectId() == playerObjectId)) return true;
		return false;
	}
	...
	public static boolean isPlayerWaitingLeader(int playerObjectId)
	{
		for (int i = 0; i < 8; i++)
		{
			if (_waitingPlayerSideIds[i] != -1 &&
					_waitingPlayers[i][_waitingPlayerSideIds[i]] != null &&
	...
	public static boolean isPlayerChallenged(int playerObjectId)
	{
		for (int i = 0; i < 8; i++)
			if (_waitingPlayers[i][0] != null && _waitingPlayers[i][1] != null && _waitingPlayerSideIds[i] != -1 &&
					_waitingPlayers[i][_waitingPlayerSideIds[i]].getObjectId() == playerObjectId) return true;
		return false;
	}
	...
	{
		if (side == 0) return _board;
		else
		{
			L2ChessPieceInstance[][] board = new L2ChessPieceInstance[8][8];
			for (int i = 0; i < 8; i++) for (int j = 0; j < 8; j++) board[i][j] = _board[7-i][7-j];
			return board;
		}
	}
