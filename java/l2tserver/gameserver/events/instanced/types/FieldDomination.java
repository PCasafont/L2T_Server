	...
	public void convertFlag(L2EventFlagInstance flag, EventTeam team, L2PcInstance player)
	{
		...
		try
		{
			...
			if (team == null)
			{
				...
				sendToAllParticipants(getParticipantTeam(player.getObjectId()).getName() + " team's member " + player.getName() + " has neutralized a flag that was taken by the " + flag.getTeam().getName() + " team!");
			}
			else
			{
