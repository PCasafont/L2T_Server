...
public class TeamSurvival extends EventInstance
{
	...
	public void onKill(L2Character killerCharacter, L2PcInstance killedPlayerInstance)
	{
		...
		if (killerTeam != null)
		{
			...
			{
				killerCharacter.sendPacket(new CreatureSay(0, Say2.TELL, "Instanced Events", "You have been disqualified for killing a member of your own team!"));
		...
		if (!team.isAlive())
		{
			...
			team.cleanMe();
			if ((_config.getLocation().getTeamCount() != 4 && (!_teams[0].isAlive() || !_teams[1].isAlive())) || (_config.getLocation().getTeamCount() == 4 &&
