package l2tserver.gameserver.events.instanced.types;
...
import l2tserver.gameserver.events.instanced.EventConfig;
...
public class StalkedStalkers extends EventInstance
{
	...
	public void calculateRewards()
	{
		...
		Announcements.getInstance().announceToAll("The event has ended. The player " + sorted.get(0).getName() + " won with " + sorted.get(0).getEventPoints() + " points");
		return;
	}
	...
	public void onKill(L2Character killerCharacter, L2PcInstance killedPlayerInstance)
	{
		...
		else
		{
			...
			{
				killerPlayerInstance.sendPacket(new CreatureSay(0, Say2.TELL, "Instanced Events", "Your stupidity has tired me. Get out from here!"));
	...
	public void stalkerMessage(L2PcInstance stalker, int level)
	{
		...
		switch (level)
		{
			...
			case -2:
				...
				break;
			case -1:
				hint = "Ok, let's begin: You have to kill someone. Who? Watch out for the hints that I will be sending to you!";
				break;
			case 0:
			...
			case 3:
				if (player.getActiveClass() != player.getBaseClass())
					hint = "Hint 3: Tour target's current class is " + PlayerClassTable.getInstance().getClassNameById(player.getActiveClass()) + ", " +
					"and its base class is " + PlayerClassTable.getInstance().getClassNameById(player.getBaseClass()) + ".";
				else
					hint = "Hint 3: Tour target's class is " + PlayerClassTable.getInstance().getClassNameById(player.getBaseClass()) + ".";
				break;
			case 4:
