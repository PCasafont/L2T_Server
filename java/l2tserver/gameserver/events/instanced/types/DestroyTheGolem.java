...
public class DestroyTheGolem extends EventInstance
{
	...
	public void onGolemDestroyed(L2PcInstance player, EventTeam team)
	{
		...
		sendToAllParticipants(getParticipantTeam(player.getObjectId()).getName() + " team's member " + player.getName() + " has destroyed the " + team.getName() + " team's golem!");
		
		CreatureSay cs = new CreatureSay(player.getObjectId(), Say2.TELL, player.getName(), "I have destroyed the " + team.getName() + " team's golem!");
	...
	private void spawnGolem(EventTeam team)
	{
		...
		{
			Log.warning("Golem Engine[spawnGolem(" + team.getName() + ")]: exception: " + e.getStackTrace());
		}
	}
	...
	class UnspawnGolemsTask implements Runnable
	{
		...
		public void run()
		{
			DestroyTheGolem.this.unspawnGolems();
		}
	}
}
