...
public class KingOfTheHill extends EventInstance
{
	...
	public void calculateRewards()
	{
		...
		Announcements.getInstance().announceToAll("The event has ended. The player " + sorted.get(0).getName() + " won being on the highest place during the most time!");
		return;
	}
	...
	public String getRunningInfo(L2PcInstance player)
	{
		...
		if (_teams[0].getParticipatedPlayerCount() > 0)
		{
			...
			{
				if (participant != null)
					html += EventsManager.getInstance().getPlayerString(participant, player) + ": " + (participant.getZ() - _config.getLocation().getGlobalZ()) + "<br>";
			}
			if (html.length() > 4)
