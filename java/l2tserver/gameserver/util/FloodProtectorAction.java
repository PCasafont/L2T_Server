 ...
 */
package l2tserver.gameserver.util;
...
public final class FloodProtectorAction
{
	 ...
	 */
	private volatile int _nextGameTick = TimeController.getGameTicks();
	/**
	 * Request counter.
	 */
	...
	public synchronized boolean tryPerformAction(final String command)
	{
		...
		if (curTick < _nextGameTick || _punishmentInProgress)
		{
			...
			{
				log(" called command ", command, " ~", String.valueOf((_config.FLOOD_PROTECTION_INTERVAL - (_nextGameTick - curTick)) * TimeController.MILLIS_IN_TICK), " ms after previous command");
				_logged = true;
			}
		...
		if (_count.get() > 0)
		{
			...
			{
				log(" issued ", String.valueOf(_count), " extra requests within ~", String.valueOf(_config.FLOOD_PROTECTION_INTERVAL * TimeController.MILLIS_IN_TICK), " ms");
			}
		}
