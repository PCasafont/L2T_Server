package l2tserver.gameserver.communitybbs.Manager;
...
import l2tserver.gameserver.network.serverpackets.ShowBoard;
...
public class CustomCommunityBoard
{
	...
	private void loadForumNews()
	{
		...
		try
		{
			...
			PreparedStatement st = con.prepareStatement("SELECT * FROM "+Config.FORUM_DB_NAME+".topics WHERE forum_id = 4 AND pinned = 0 AND approved = 1 ORDER BY start_date DESC LIMIT 0, 70");
			...
			while (rs.next())
			{
				...
				PreparedStatement st2 = con.prepareStatement("SELECT * FROM "+Config.FORUM_DB_NAME+".posts WHERE pid = ?");
				st2.setInt(1, rs.getInt("topic_firstpost"));
				
				ResultSet rs2 = st2.executeQuery();
				rs2.next();
				
				String postUrl = "url http://www."+Config.FORUM_DB_NAME.replace("_board", "")+".com/board/index.php?/topic/"+tid+"-"+titleSeo+"/";
				cmboard_info.add(new cmboard(rs2.getString("post_date"), rs.getString("title").replace("&#39;", "'"), postUrl));
	...
	private void buildCmPage()
	{
		...
		htmltext += "<center><table><tr><td><img src=\"icon.etc_alphabet_n_i00\" width=32 height=32></td><td><img src=\"icon.etc_alphabet_e_i00\" width=32 height=32></td><td><img src=\"icon.etc_alphabet_w_i00\" width=32 height=32></td><td><img src=\"icon.etc_alphabet_s_i00\" width=32 height=32></td></tr></table></center>";
		htmltext += "<br><br><br>";
		...
		for (cmboard info : cmboard_info)
		{
			...
			htmltext += "<td><table width=375><tr><td><font color=\"LEVEL\"><a action=\""+info.getUrl()+"\">" + info.getTitle() + "</a></font> ("+info.getDate()+")</td></tr></table></td>";
	...
	private String getCustomCastleInfoBoard()
	{
		...
		for (Castle castle : CastleManager.getInstance().getCastles())
		{
			...
			sb.append("<table width=710 border=0 bgcolor=999999><tr><td align=center FIXWIDTH=710>" + castle.getName() + "</td></tr></table>");
			sb.append("<table width=710 height=160 border=0><tr><td><table><tr><td><img src=\"Crest.crest_" + Config.SERVER_ID + "_" + castle.getCastleId() + "\" width=256 height=128></td></tr></table></td>");
			...
			if (clan != null)
			{
				...
				sb.append("<tr><td FIXWIDTH=110>Clan Leader:</td><td FIXWIDTH=360> " + clan.getLeader().getName() + "</td></tr>");
			}
			else
			...
			if (defenderListSize > 0)
			{
				...
				for (L2SiegeClan siegeClan : defenderClanList)
				{
					...
					defenderClans += ClanTable.getInstance().getClan(siegeClan.getClanId()).getName() + (defenderListSize > 1 ? ", " : "");
					defenderListSize--;
				}
			...
			if (attackerListSize > 0)
			{
				...
				for (L2SiegeClan siegeClan : attackerClanList)
				{
					...
					attackerClans += ClanTable.getInstance().getClan(siegeClan.getClanId()).getName() + (attackerListSize > 1 ? ", " : "");
					attackerListSize--;
				}
	...
	public String createPages(int pageToShow, int maxPages, String url, String extraUrl)
	{
		...
		for (int x = 0; x < maxPages; x++)
		{
			int pagenr = x + 1;
			if (pageToShow == x)
				sb.append("<td><button value=" + String.valueOf(pagenr) + " width=30 height=30 action=\"bypass " + url + x + extraUrl + "\" fore=\"L2UI_CT1.Tab_DF_Tab_Selected\" back=\"L2UI_CT1.Tab_DF_Tab_Selected\"></td>");
			else
				sb.append("<td><button value=" + String.valueOf(pagenr) + " width=30 height=30 action=\"bypass " + url + x + extraUrl + "\" fore=\"L2UI_CT1.Tab_DF_Tab_Unselected\" back=\"L2UI_CT1.Tab_DF_Tab_Selected\"></td>");
		}
	...
	{
		Map<Object, Integer> raidIds = new HashMap<Object, Integer>();
		for (L2RaidBossInstance raid : BossManager.getInstance().getBosses().values())
			raidIds.put(raid.getNpcId(), raid.getLevel());
		raidIds = sortByValue(raidIds, true);
		_raidIds.addAll(raidIds.keySet());
		Map<Object, Integer> bossIds = new HashMap<Object, Integer>();
		...
		try
		{
			...
			while (rset.next())
			{
				...
				if (boss != null)
					bossIds.put(rset.getInt("boss_id"), (int)NpcTable.getInstance().getTemplate(rset.getInt("boss_id")).Level);
			}
	...
	public String getCustomGrandBossInfo()
	{
		...
		for (Object id : _bossIds)
		{
			...
			if (status == GrandBossManager.getInstance().FIGHTING)
				...
				bossStatus = "<font color=FF0000>" + String.valueOf(new Date((respawnTime))) + "</font>";
			...
			String bossRespawn = (spawnTime / 3600000) + " + random " + (randomSpawnTime / 3600000) + "h";
			...
			sb.append("<table border=0 cellspacing=0 cellpadding=2 width=750 height=17><tr><td FIXWIDTH=20>" + pos + "</td><td FIXWIDTH=60>" + boss.getName() + "</td><td FIXWIDTH=40>" + boss.Level + "</td><td FIXWIDTH=130>"+bossRespawn+"</td><td FIXWIDTH=120>" + bossStatus + "</td><td FIXWIDTH=20><button value=\" \" width=16 height=16 action=\"bypass _bbscustom;info;drop;" + boss.NpcId + ";1\" fore=L2UI_CH3.aboutotpicon back=L2UI_CH3.aboutotpicon></td></tr></table>");
	...
	public String getCustomRBInfo(int pageToShow, boolean isGM)
	{
		...
		sb.append("<center>");
		sb.append("<table width=750 bgcolor=999999><tr><td FIXWIDTH=20>#</td><td FIXWIDTH=140>Name</td><td FIXWIDTH=40>Level</td><td FIXWIDTH=50>Respawn</td><td FIXWIDTH=120>Status</td><td FIXWIDTH=20>Drop</td></tr></table>");
		...
		for (int i = pageStart; i < pageEnd; i++)
		{
			...
			if (boss == null || boss.getSpawn().getRespawnDelay() < 3600)
				continue;
			...
			String howIs = isInCombat ? "<font color=LEVEL>Under Attack</font>" : !boss.isDead() ? "<font color=99FF00>ALIVE</font>" : "<font color=FF0000>" + String.valueOf(new Date(respawn)) + "</font>";
			...
			sb.append("<table border=0 cellspacing=0 cellpadding=2 width=750 height=17><tr><td FIXWIDTH=20>" + i + "</td><td FIXWIDTH=140>" + nameString + bossIsAggro + "</td><td FIXWIDTH=40>" + npc.Level + "</td><td FIXWIDTH=50>" + (boss.getSpawn().getRespawnDelay() / 3600000) + "-" + ((boss.getSpawn().getRespawnDelay() + boss.getSpawn().getRandomRespawnDelay()) / 3600000) + " H</td><td FIXWIDTH=120>" + howIs + "</td><td FIXWIDTH=20><button value=\" \" width=16 height=16 action=\"bypass _bbscustom;info;drop;" + npc.NpcId + ";1\" fore=L2UI_CH3.aboutotpicon back=L2UI_CH3.aboutotpicon></td></tr></table>");
			sb.append("<img src=\"L2UI.Squaregray\" width=740 height=1>");
		}
		...
		switch (val)
		{
			case "action":
					switch(String.valueOf(st.nextToken()))
					{
						...
						case "showRadar":
							activeChar.getRadar().addMarker(Integer.valueOf(command.split(" ")[1]), Integer.valueOf(command.split(" ")[2]), Integer.valueOf(command.split(" ")[3]));
							break;
						...
						case "searchDrop":
							...
							if (Util.isDigit(itemName))
							{
								...
								sendCommunityBoardPage(getCommunityPage("searchDrop").replace("%result%", SearchDropManager.getInstance().getDrops(activeChar, Integer.valueOf(itemName), isSpoil, page)), activeChar);
							}
							else	
								sendCommunityBoardPage(getCommunityPage("searchDrop").replace("%result%", SearchDropManager.getInstance().searchPossiblesResults(activeChar, itemName, isSpoil)), activeChar);
							break;
						...
						case "worldBuff":
							switch(String.valueOf(st.nextToken()))
							{
								...
								{	
									...
									if (skillId != 0)
										CustomOfflineBuffersManager.getInstance().addBuffToBuffer(activeChar, skillId, price);
									break;
								}
								...
								case "bufferInfo":
								{
									...
									if (playerId != 0)
										CustomOfflineBuffersManager.getInstance().getSpecificBufferInfo(activeChar, playerId);
									break;
								}
						...
						case "voice":
							...
							IVoicedCommandHandler handler = VoicedCommandHandler.getInstance().getVoicedCommandHandler(voicedCommand);
							if (handler == null)
								return;
						...
						case "bid":
							...
							if (bidId != 0 && bidAmount != 0)
								CustomAuctionManager.getInstance().tryToBid(activeChar, bidId, bidAmount, coin);
							break;
			...
			case "gainak":
				sendCommunityBoardPage(getCommunityPage("Gainak").replace("%gainak%", getGainakStatus(activeChar, Integer.valueOf(st.nextToken()))), activeChar);
				break;
				
			case "gmEvent":
				sendCommunityBoardPage(getCommunityPage("GMEvent").replace("%info%", GMEventManager.getInstance().getCustomEventPanel(activeChar, Integer.valueOf(st.nextToken()))), activeChar);
				break;
			
			case "worldBuffers":
				sendCommunityBoardPage(getCommunityPage("WorldBuffers").replace("%buffers%", CustomOfflineBuffersManager.getInstance().getOfflineBuffersPage(Integer.valueOf(st.nextToken()))), activeChar);
				break;
			...
			case "itemAuction":
				sendCommunityBoardPage(getCommunityPage("itemAuction").replace("%auctionInfo%", CustomAuctionManager.getInstance().getAuctionInfo(activeChar.getObjectId(), Integer.valueOf(st.nextToken()))), activeChar);
				break;
			...
			case "info":
				...
				switch (toSend)
				{
					case "drop": // player, npcId, page
						sendDropPage(activeChar, Integer.valueOf(st.nextToken()), Integer.valueOf(st.nextToken()), null);
						break;
			...
			case "rankings":
				...
				sendCommunityBoardPage(getCommunityPage("rankings").replaceFirst("%ranking%", getRankingInfo(rankingType, activeChar)), activeChar);
				break;
			...
			case "buyPanel":
				sendCommunityBoardPage(getCustomBuyPage(Integer.valueOf(st.nextToken()), Integer.valueOf(st.nextToken())), activeChar);
				break;
	...
	private String getGainakStatus(L2PcInstance pl, int pageToShow)
	{
		...
		{
			sb.append("<table><tr><td><img src=\"Crest.crest_%serverId%_"+(gainakSiegeZone.isActive() ? 51 : 50)+"\"  width=512 height=128></td></tr></table>");
			sb.append("<br><br>");
			...
			{	
				...
				if (maxPages > 1)
					sb.append("<center>" + createPages(pageToShow, maxPages, "_bbscustom;gainak;", "") + "</center><br>");
				sb.append("<table width=600 bgcolor=999999><tr><td FIXWIDTH=100 align=center>Name</td><td FIXWIDTH=100 align=center>Clan</td><td FIXWIDTH=100 align=center>Ally</td></tr></table>");
				...
				for (int i = pageStart; i < pageEnd; i++)
				{
					...
					sb.append(""+color+"<table width=600><tr><td FIXWIDTH=100 align=center> " + player.getName() + " </td><td FIXWIDTH=100 align=center>"+ clanName +"</td><td FIXWIDTH=100 align=center>" + allyName + "</td></tr></table>"+(color.length() > 0 ? "</font>" : color)+"");
					sb.append("<table width=600><tr><td><img src=\"L2UI.Squaregray\" width=600 height=1></td></tr></table>");
				}
			}
			else
				sb.append("<table width=600><tr><td align=center><font color=LEVEL>Gainak is in peace!</font></td></tr></table>");
		}
		return sb.toString();
	}
	...
	public void sendDropPage(L2PcInstance pl, int npcId, int page, L2Npc npc)
	{
		...
		String replyMSG = "<html>"+(template.getName().length() > 0 ? "<title>"+template.getName()+"</title>" : "")+"<body>";
		...
		for (Entry<L2DropData, Float> drop : drops.entrySet())
		{
			...
			DropChances chances = NpcTable.getInstance().calculateRewardChances(template, pl, drop.getKey(), drop.getValue(), 0, false, npc);
			
			replyMSG += "<table><tr><td width=40><img src=\""+chances.icon+"\" width=32 height=32></td><td width=220><table><tr><td width=220>"+chances.itemName;
			if (chances.max > 1)
			{
		...
		if (!spoilDrops.isEmpty())
		{
			...
			for (Entry<L2DropData, Float> drop : spoilDrops.entrySet())
			{
				...
				DropChances chances = NpcTable.getInstance().calculateRewardChances(template, pl, drop.getKey(), drop.getValue(), 0, true, npc);
		...
		if (page > 1)
		{
			replyMSG += "<td width=120><a action=\"bypass _bbscustom;info;drop;" + npcId + ";" + (page - 1) + "\">Prev Page</a></td>";
		...
		if (hasMore)
		{
			...
			replyMSG += "<td width=100>Page " + page + "</td><td width=70><a action=\"bypass _bbscustom;info;drop;" + npcId + ";" + (page + 1) + "\">Next Page</a></td>";
		}
	...
	private String getCustomPlayerPanelInfo(L2PcInstance pl)
	{
		...
		a = a.replace("%refuseXp%", "<button value=" + isNoExp + " width=90 height=24 action=\"bypass _bbscustom;action;voice;noexp\" fore=L2UI_CT1.Button_DF_Calculator back=L2UI_CT1.Button_DF_Calculator_Over>");
		a = a.replace("%blockrequests%", "<button value=" + isRefusingRequests + " width=90 height=24 action=\"bypass_bbscustom;action;voice;blockrequests;\" fore=L2UI_CT1.Button_DF_Calculator back=L2UI_CT1.Button_DF_Calculator_Over>");
		a = a.replace("%refuseBuffs%", "<button value=" + isRefusingBuffs + " width=90 height=24 action=\"bypass_bbscustom;action;voice;refusebuff;\" fore=L2UI_CT1.Button_DF_Calculator back=L2UI_CT1.Button_DF_Calculator_Over>");
		a = a.replace("%blockpms%", "<button value=" + isRefusalPms + " width=90 height=24 action=\"bypass_bbscustom;action;voice;blockpms;\" fore=L2UI_CT1.Button_DF_Calculator back=L2UI_CT1.Button_DF_Calculator_Over>");
	...
	private String getCustomBuyPage(int pageToShow, int type)
	{
		...
		sb.append("<center>");
		sb.append("<table width=750 bgcolor=999999><tr><td FIXWIDTH=25>#</td><td FIXWIDTH=150>Name</td><td FIXWIDTH=90>Shop Type</td><td FIXWIDTH=110>Message</td></tr></table>");
		...
		for (int i = pageStart; i < pageEnd; i++)
		{
			...
			sb.append("<table border=0 cellspacing=0 cellpadding=2 width=750 height=17><tr><td FIXWIDTH=25>" + i + "</td><td FIXWIDTH=150><a action=\"bypass _bbscustom;action;trade;" + shop.getName() + "\">" + shop.getName() + "</a></td><td FIXWIDTH=90>" + shop.getShopNameType() + "</td><td FIXWIDTH=110>" + title + "</td></tr></table>");
			sb.append("<img src=\"L2UI.Squaregray\" width=740 height=1>");
		}
	...
	private void sendNormalChatWindow(L2PcInstance pl, String path)
	{
		...
		{	
			...
			htmlPage.replace("%lotteryMultiplier%", Config.CUSTOM_LOTTERY_REWARD_MULTIPLIER > 1 ? "x"+String.valueOf(Config.CUSTOM_LOTTERY_REWARD_MULTIPLIER) : "");
	...
	private void sendCommunityBoardPage(String html, L2PcInstance player)
	{
		...
		{
			Log.warning("CustomCommunityBoard: sendCommunityBoardPage this html exceeds the max html size supported by the client, requestor: " + player.getName() + " html size: " + html.length());
		}
	}
	...
	private String getRankingInfo(String rankingType, L2PcInstance player)
	{
		...
		Map<Object, Integer> allPlayers = new HashMap<Object, Integer>();
		for (L2PcInstance pl : L2World.getInstance().getAllPlayers().values())
		{
			...
			allPlayers.put((isClanReputation || isClanWarPoints) ? pl.getClan().getName() : pl.getName(), isOlympiadPoints ? Olympiad.getInstance().getNobleInfo(pl.getObjectId()).getPoints() : 
				isClanWarPoints ? getWarPoints(pl.getClan()) :
					isRaidPoints ? RaidBossPointsManager.getInstance().getPointsByOwnerId(pl.getObjectId()) :
						isClanReputation ? pl.getClan().getReputationScore() :
							isReputation ? pl.getReputation() :
								isFameRanking ? pl.getFame() :
									isPvPRanking ? pl.getPvpKills() :
										pl.getPkKills());
		}
		...
		StringBuilder sb = new StringBuilder();
		sb.append("<table bgcolor=999999 width=750><tr><td FIXWIDTH=40>Position #</td><td FIXWIDTH=100>Name</td><td FIXWIDTH=70>Count</td></tr></table>");
		
		String name = null;
		int counter = 1;
		for (Entry<Object, Integer> info : allPlayers.entrySet())
		{
			...
			if (counter <= 20)
			{
				...
				{
					playerIsInTop = true;
					sb.append("<table width=750><tr><td FIXWIDTH=40><font color=LEVEL>"+counter+"</font></td><td FIXWIDTH=100><font color=LEVEL>"+name+"</font></td><td FIXWIDTH=70><font color=LEVEL>"+info.getValue()+"</font></td></tr></table>");
				}
				else
					sb.append("<table width=750><tr><td FIXWIDTH=40>"+counter+"</td><td FIXWIDTH=100>"+name+"</td><td FIXWIDTH=70>"+info.getValue()+"</td></tr></table>");
		...
		if (!playerIsInTop)
		{
			...
			if (counter <= allPlayers.size())	//Player are on the list
				sb.append("<table width=750><tr><td FIXWIDTH=40><font color=LEVEL>"+counter+"</font></td><td FIXWIDTH=100><font color=LEVEL>"+name+"</font></td><td FIXWIDTH=70><font color=LEVEL>"+allPlayers.get(name)+"</font></td></tr></table>");
			else
	...
	private final Map<Object, Integer> sortByValue(Map<Object, Integer> unsortMap, final boolean ascending)
	{
		final List<Entry<Object, Integer>> list = new LinkedList<Entry<Object, Integer>>(unsortMap.entrySet());
		Collections.sort(list, new Comparator<Entry<Object, Integer>>()
		{
			@Override
			public int compare(Entry<Object, Integer> e1, Entry<Object, Integer> e2)
			{
				if (ascending)
		...
		final Map<Object, Integer> sortedMap = new LinkedHashMap<Object, Integer>();
