						...
						for (XmlNode subNode : classNode.getChildren())
						{
							if (subNode.getName().equalsIgnoreCase("skill"))
							{
								...
								for (String mls : levels)
								{
									...
									L2SkillLearn sl = new L2SkillLearn(skillId, skillLevel, reqSp, minLevel, minDualLevel, learnFromPanel, learnFromFS, isTransfer, autoGet);
							...
							else if (subNode.getName().equalsIgnoreCase("skillReplacement"))
							{
								...
								for (long hash : skills.keySet())
								{
									...
									if (sl.getId() == skillId)
									{
										...
										cl.addSkill(SkillTable.getSkillHashCode(replacedBy, sl.getLevel()), new L2SkillLearn(replacedBy, sl.getLevel(), sl.getSpCost(), sl.getMinLevel(), sl.getMinDualLevel(), sl.isLearnedFromPanel(), sl.isLearnedByFS(), sl.isTransferSkill(), sl.isAutoGetSkill()));
									}
								}
							}
									...
									if (cl.getParent() != null && cl.getParent().getSkills().containsKey(reqHash))
									{
										...
										if (!cl.getSkills().containsKey(reqHash))
										{
											...
											for (int lv = 2; cl.getParent().getSkills().containsKey(SkillTable.getSkillHashCode(reqSkillId, lv)); lv++)
												cl.addSkill(SkillTable.getSkillHashCode(reqSkillId, lv), new L2SkillLearn(reqSkillId, lv, 1, 85, 0, true, false, false, false));
										}
										else
					...
					else if (classNode.getName().equalsIgnoreCase("skill"))
					{
						...
						for (String mls : levels)
						{
							...
							for (int PlayerClass : _classes.keySet())
								_classes.get(PlayerClass).addSkill(hash, new L2SkillLearn(skillId, skillLevel, reqSp, minLevel, minDualLevel, learnFromPanel, learnFromFS, isTransfer, autoGet));
							_minSkillLevels.put(hash, minLevel);
						}
					}
				}
