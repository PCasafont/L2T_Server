...
import l2tserver.gameserver.stats.ItemParser;
		...
		if (process.equalsIgnoreCase("loot"))
		{
			...
			if (reference instanceof L2Attackable && ((L2Attackable) reference).isRaid()) // loot privilege for raids
			{
				...
				if (!Config.AUTO_LOOT_RAIDS && protectDrop)
				{
					...
					itemLootShedule = ThreadPoolManager.getInstance().scheduleGeneral(new ResetOwner(item), Config.LOOT_RAIDS_PRIVILEGE_INTERVAL * 1000);
