 ...
 */
package l2tserver.gameserver.model.itemauction;
...
public final class ItemAuctionInstance
{
	...
	public ItemAuctionInstance(final int instanceId, final AtomicInteger auctionIds, final XmlNode node) throws Exception
	{
		...
		Log.info("L2ItemAuctionInstance: Loaded " + _items.size() + " item(s) and registered " + _auctions.size() + " auction(s) for instance " + _instanceId + ".");
	...
	final void checkAndSetCurrentAndNextAuction()
	{
		...
		if (currentAuction != null && currentAuction.getAuctionState() != ItemAuctionState.FINISHED)
		{
			...
			Log.info("L2ItemAuctionInstance: Schedule current auction " + currentAuction.getAuctionId() + " for instance " + _instanceId);
		}
		else
		{
			...
			Log.info("L2ItemAuctionInstance: Schedule next auction " + nextAuction.getAuctionId() + " on " + DATE_FORMAT.format(new Date(nextAuction.getStartingTime())) + " for instance " + _instanceId);
		}
	}
		...
		private final void runImpl() throws Exception
		{
			...
			switch (state)
			{
				case CREATED:
				{
					...
					Log.fine("L2ItemAuctionInstance: Auction " + _auction.getAuctionId() + " has started for instance " + _auction.getInstanceId());
					checkAndSetCurrentAndNextAuction();
					break;
				}
	...
	final void onAuctionFinished(final ItemAuction auction)
	{
		...
		if (bid != null)
		{
			...
			if (player != null)
			{
				...
				Log.fine("L2ItemAuctionInstance: Auction " + auction.getAuctionId() + " has finished. Highest bid by " + player.getName() + " for instance " + _instanceId);
			}
			else
			{
				...
				Log.fine("L2ItemAuctionInstance: Auction " + auction.getAuctionId() + " has finished. Highest bid by " + CharNameTable.getInstance().getNameById(bid.getPlayerObjId()) + " for instance " + _instanceId);
			}
		...
		{
			Log.fine("L2ItemAuctionInstance: Auction " + auction.getAuctionId() + " has finished. There have not been any bid for instance " + _instanceId);
		}
	}
