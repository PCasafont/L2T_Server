...
import l2tserver.gameserver.network.serverpackets.ExAskModifyPartyLooting;
import l2tserver.gameserver.network.serverpackets.ExCloseMPCC;
import l2tserver.gameserver.network.serverpackets.ExOpenMPCC;
...
import l2tserver.gameserver.network.serverpackets.ExSetPartyLooting;
...
import l2tserver.gameserver.network.serverpackets.PartyMemberPosition;
import l2tserver.gameserver.network.serverpackets.PartySmallWindowAdd;
import l2tserver.gameserver.network.serverpackets.PartySmallWindowAll;
import l2tserver.gameserver.network.serverpackets.PartySmallWindowDelete;
...
public class L2Party
{
	 ...
	 * @see l2tserver.gameserver.model.actor.instance.L2PcInstance#isRequestExpired()
	 */
				...
				if (validMembers.contains(member) && penalty < 1)
				{
					...
					if (!member.isDead())
					{
						...
						if (member instanceof L2PcInstance)
						{
							...
							if (
									(pcMember.getLevel() >= target.getLevel() && (pcMember.getLevel() - target.getLevel()) < 11) ||
	...
	public void think()
	{
			...
			if (healer.getClassId() == 146)
			{
				for (L2Character enemy : enemies)
				{
					if (enemy.getTarget() == healer && (enemy.isAttackingNow()
							|| (enemy.getLastSkillCast() != null
							&& enemy.getLastSkillCast().isOffensive()))
		...
		for (L2Character enemy : enemies)
		{
			...
			if (!(enemy instanceof L2PcInstance))
			{
				if (_target == null && enemy.isInCombat()
						&& enemy.getDistanceSq(leader) < closest)
				{
