...
public class L2EventZone extends L2ZoneType
{
	...
	/**
	 * @see l2tserver.gameserver.model.zone.L2ZoneType#onDieInside(l2tserver.L2Character.gameserver.model.actor.L2Character)
	 */
	@Override
	...
	/**
	 * @see l2tserver.gameserver.model.zone.L2ZoneType#onReviveInside(l2tserver.L2Character.gameserver.model.actor.L2Character)
	 */
	@Override
	...
	class OutOfEventZoneTask implements Runnable
	{
		...
		public void run()
		{
			if (!isInsideZone(_player) && _player.isInEvent())
			{
				if (getDistanceToZone(_player) > 500 || getZone().getHighZ() < _player.getZ() || getZone().getLowZ() > _player.getZ())
				{
					if (_delay > 0)
					{
						if (!_warned)
						{
							_player.sendPacket(new CreatureSay(0, Say2.TELL, "Instanced Events", "You left the event zone. If you don't return in 10 seconds your character will die!"));
							_warned = true;
						}
						else if (_delay <= 5)
							_player.sendPacket(new CreatureSay(0, Say2.TELL, "Instanced Events", _delay + " seconds to return."));
