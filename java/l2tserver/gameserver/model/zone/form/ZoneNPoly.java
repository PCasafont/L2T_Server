 ...
 */
package l2tserver.gameserver.model.zone.form;

import java.awt.Color;

import l2tserver.gameserver.GeoEngine;
import l2tserver.gameserver.model.zone.L2ZoneForm;
import l2tserver.gameserver.network.serverpackets.ExServerPrimitive;
...
public class ZoneNPoly extends L2ZoneForm
{
	...
	public double getDistanceToZone(int x, int y)
	{
		...
		double u = ((x - _x[_x.length - 1]) * (_x[0] - _x[_x.length - 1]) + (y - _y[_y.length - 1]) * (_y[0] - _y[_y.length - 1])) /
		...
		{
			u = ((x - _x[_x.length - 1]) * (_x[0] - _x[_x.length - 1]) + (y - _y[_y.length - 1]) * (_y[0] - _y[_y.length - 1])) /
			(Math.pow(_x[0] - _x[_x.length - 1], 2) + Math.pow(_y[0] - _y[_y.length - 1], 2));
			if (u > 0 && u < 1)
