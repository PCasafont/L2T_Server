...
public class ClanHall
{
			...
			boolean paid)
	{
		...
		_ownerId = ownerId;
		if (Config.DEBUG)
			Log.warning("Init Owner : " + _ownerId);
		_lease = lease;
		_desc = desc;
		_location = location;
		...
		public void run()
		{
			try
			{
						...
						&& ClanTable.getInstance().getClan(getOwnerId()).getWarehouse().getItemByItemId(Config.CH_BID_ITEMID).getCount() >= getLease())
				{
					...
					ClanTable.getInstance().getClan(getOwnerId()).getWarehouse().destroyItemByItemId("CH_rental_fee", Config.CH_BID_ITEMID, getLease(), null, null);
					if (Config.DEBUG)
						Log.warning("deducted " + getLease() + " adena from " + getName() + " owner's cwh for ClanHall _paidUntil: " + _paidUntil);
					ThreadPoolManager.getInstance().scheduleGeneral(new FeeTask(), _paidUntil - _time);
					_paid = true;
					updateDb();
				}
