...
public class Auction
{
	...
	private void loadBid()
	{
		...
		try
		{
			...
			statement = con.prepareStatement("SELECT bidderId, bidderName, maxBid, clan_name, time_bid FROM clanhall_auction_bid WHERE auctionId = ? ORDER BY maxBid DESC");
			statement.setInt(1, getId());
			rs = statement.executeQuery();
	...
	private void returnItem(String clanName, long quantity, boolean penalty)
	{
		...
		final long limit = MAX_ADENA - (clan.getWarehouse().getItemByItemId(Config.CH_BID_ITEMID) != null ? clan.getWarehouse().getItemByItemId(Config.CH_BID_ITEMID).getCount() : 0);
	...
	private boolean takeItem(L2PcInstance bidder, long quantity)
	{
		...
		if (Config.CH_BID_ITEMID == 57)
			...
			bidder.sendMessage("There are not enough " + ItemTable.getInstance().getTemplate(Config.CH_BID_ITEMID).getName() + "s in the clan hall warehouse.");
		return false;
	}
	...
	private void updateInDB(L2PcInstance bidder, long bid)
	{
		...
		try
		{
			...
			{
				statement = con.prepareStatement("UPDATE clanhall_auction_bid SET bidderId=?, bidderName=?, maxBid=?, time_bid=? WHERE auctionId=? AND bidderId=?");
			...
			{
				statement = con.prepareStatement("INSERT INTO clanhall_auction_bid (id, auctionId, bidderId, bidderName, maxBid, clan_name, time_bid) VALUES (?, ?, ?, ?, ?, ?, ?)");
	...
	public void confirmAuction()
	{
		...
		try
		{
			...
			statement = con.prepareStatement("INSERT INTO clanhall_auction (id, sellerId, sellerName, sellerClanName, itemType, itemId, itemObjectId, itemName, itemQuantity, startingBid, currentBid, endDate) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
			statement.setInt(1, getId());
