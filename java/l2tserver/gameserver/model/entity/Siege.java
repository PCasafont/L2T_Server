...
import l2tserver.gameserver.instancemanager.SiegeGuardManager;
...
import l2tserver.gameserver.model.actor.instance.L2ControlTowerInstance;
...
import l2tserver.gameserver.network.serverpackets.SiegeInfo;
...
public class Siege implements Siegable
{
		...
		public void run()
		{
			...
			try
			{
				if (_castleInst.getSiege().getIsInProgress())
				{
					...
					ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleAddReputationPointsToMembers(_castleInst), 5 * 60 * 1000);
				}
				else
				{
					_castleInst.getSiege().endSiege();
	...
	public void startSiege()
	{
		if (!getIsInProgress())
		{
			...
			_controlTowerCount = SpawnTable.getInstance().getSpecificSpawns(getCastle().getName()+"_control_tower").size();
			_flameTowerCount = SpawnTable.getInstance().getSpecificSpawns(getCastle().getName()+"_flame_tower").size();;
	...
	private void saveSiegeClan(L2Clan clan, byte typeId, boolean isUpdateRegistration)
	{
		...
		{
			Log.log(Level.WARNING, "Exception: saveSiegeClan(L2Clan clan, int typeId, boolean isUpdateRegistration): " + e.getMessage(), e);
		}
		finally
		{
			L2DatabaseFactory.close(con);
