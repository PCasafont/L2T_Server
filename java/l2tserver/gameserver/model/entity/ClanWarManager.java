...
public class ClanWarManager
{
	...
	public final void load()
	{
		...
		try
		{
			...
			statement = con.prepareStatement("SELECT clan1, clan2, start_time, end_time, delete_time, clan1_score, clan2_score, clan1_war_declarator, clan2_war_declarator, clan1_deaths_for_war, clan1_shown_score, clan2_shown_score, loserId, winnerId FROM clan_wars");
			...
			while (rset.next())
			{
				...
				if (warstate != null)
				{
					...
					ClanWar war = new ClanWar(rset.getInt("clan1"), rset.getInt("clan2"), rset.getInt("clan1_score"), rset.getInt("clan2_score"), rset.getInt("clan1_war_declarator"), rset.getInt("clan2_war_declarator"), rset.getInt("clan1_deaths_for_war"), rset.getInt("clan1_shown_score"), rset.getInt("clan2_shown_score"), warstate, rset.getInt("loserId"), rset.getInt("winnerId"), rset.getLong("start_time"), rset.getLong("end_time"), rset.getLong("delete_time"));
					_wars.add(war);
	...
	public static class ClanWar
	{
		...
		public void saveData()
		{
			...
			{
				con = L2DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con.prepareStatement("UPDATE clan_wars SET clan1_score=?, clan2_score=?, clan1_deaths_for_war=?, clan1_shown_score=?, clan2_shown_score=? WHERE clan1=? AND clan2=?");
		...
		public void setWarDeclarator(int charId)
		{
			...
			{
				con = L2DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con.prepareStatement("UPDATE clan_wars set clan2_war_declarator=? WHERE clan1=? AND clan2=?");
		...
		private void calculateOutcome()
		{
			...
			if (!getTie())
			{
				...
				{
					con = L2DatabaseFactory.getInstance().getConnection();
					PreparedStatement statement = con.prepareStatement("UPDATE clan_wars set loserId=?, winnerId=? WHERE clan1=? AND clan2=?");
