...
import l2tserver.gameserver.instancemanager.SpawnDataManager;
import l2tserver.gameserver.instancemanager.SpawnDataManager.DbSpawnData;
...
public class L2Spawn
{
	...
	public L2Spawn(L2NpcTemplate template) throws SecurityException, ClassNotFoundException, NoSuchMethodException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException
	{
		if (template == null)
			throw new IllegalArgumentException("Null template!");
		
		// Create the generic constructor of L2NpcInstance managed by this L2Spawn
		Class<?>[] parameters = {int.class, Class.forName("l2tserver.gameserver.templates.chars.L2NpcTemplate")};
	...
	private void initializeNpc()
	{
		...
		if (Config.DEBUG)
			Log.finest("spawned Mob ID: " + _npc.getNpcId() + " ,at: " + _npc.getX() + " x, " + _npc.getY() + " y, " + _npc.getZ() + " z");
	}
	...
	private final Class<?> getClassFor(final String templateType, final boolean custom)
	{
		...
		try
		{
			instanceClass = Class.forName("l2tserver.gameserver.model.actor.instance." + (custom ? "custom.": "") + templateType + "Instance");
		}
