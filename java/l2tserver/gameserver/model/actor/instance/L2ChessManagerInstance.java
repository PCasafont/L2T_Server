...
public class L2ChessManagerInstance extends L2NpcInstance
{
	...
	public void onBypassFeedback(L2PcInstance playerInstance, String command)
	{
		...
		else if (command.startsWith("challenge "))
		{
			...
			if (receptor != null && !receptor.isChessChallengeRequest() && receptor.getObjectId() != playerInstance.getObjectId())
			{
		...
		else if ((command.startsWith("movements") || command.startsWith("moveto") || command.startsWith("attack")) && ChessEvent.isState(ChessState.STARTED) && !ChessEvent.pieceMoving && ChessEvent.turn == side)
		{
			...
			for (int i = 0; i < 8; i++)
				for (int j = 0; j < 8; j++)
					if (ChessEvent.getBoard(side)[i][j] != null && ChessEvent.getBoard(side)[i][j].getObjectId() == pieceId)
						...
						for (int j = 7; j >= 0; j--)
						{
							...
							for (int i = 0; i < 8; i++)
							{
								...
								if (piece.moveType(i, j, true) == 4)
									square = "<font color=\"00FF00\"><a action=\"bypass -h npc_" + getObjectId() + "_enrocar_" + piece.getSide().getPiece(7, 0).getObjectId() + "_" + i + "_" + j + "_dreta\">[]</a></font>";
								else if (piece.moveType(i, j, true) == 3)
									square = "<font color=\"00FF00\"><a action=\"bypass -h npc_" + getObjectId() + "_enrocar_" + piece.getSide().getPiece(0, 0).getObjectId() + "_" + i + "_" + j + "_esquerra\">[]</a></font>";
								else if (piece.moveType(i, j, true) == 2)
									square = "<font color=\"FF0000\"><a action=\"bypass -h npc_" + getObjectId() + "_attack_" + piece.getObjectId() + "_" + ChessEvent.getBoard(side)[i][j].getObjectId() + "_" + i + "_" + j + "\">[]</a></font>";
								else if (piece.moveType(i, j, true) == 1)
									square = "<font color=\"00FF00\"><a action=\"bypass -h npc_" + getObjectId() + "_moveto_" + piece.getObjectId() + "_" + i + "_" + j + "\">[]</a></font>";
								else square = "[]";
	...
	public void showChatWindow(L2PcInstance playerInstance, int val)
	{
		...
		if (ChessEvent.isState(ChessState.PARTICIPATING))
		{
			...
			if (!ChessEvent.isPlayerWaiting(playerInstance.getObjectId()))
			{
				...
				if (htmContent != null)
				{
					...
					for (int i = 0; i < 8; i++)
					{
						if (ChessEvent.getWaitingPlayers()[i][0] != null && ChessEvent.getWaitingPlayers()[i][1] == null)
						{
							player = ChessEvent.getWaitingPlayers()[i][0];
							color = "White";
						}
						else if (ChessEvent.getWaitingPlayers()[i][1] != null && ChessEvent.getWaitingPlayers()[i][0] == null)
						{
						...
						if (player != null)
						{
							list += "<tr><td>" + player.getName() + "</td><td>(" + color + ")</td><td><a action=\"bypass -h npc_" + getObjectId() + "_challenge" + i + "\">Retar!</a></td></tr>";
						}
						player = null;
					}
			...
			else if (ChessEvent.isPlayerChallenged(playerInstance.getObjectId()))
			{
				...
				if (htmContent != null)
				{
					...
					for (int i = 0; i < 8; i++)
					{
						if (ChessEvent.getWaitingPlayers()[i][1] != null && ChessEvent.getWaitingPlayers()[i][0] != null && ChessEvent.getWaitingPlayers()[i][0].getObjectId() == playerInstance.getObjectId())
							player = ChessEvent.getWaitingPlayers()[i][1];
						else if (ChessEvent.getWaitingPlayers()[i][0] != null && ChessEvent.getWaitingPlayers()[i][1] != null && ChessEvent.getWaitingPlayers()[i][1].getObjectId() == playerInstance.getObjectId())
							player = ChessEvent.getWaitingPlayers()[i][0];
					}
		...
		{
			NpcHtmlMessage npcHtmlMessage = new NpcHtmlMessage(getObjectId());
			if (ChessEvent.getSide(0).getPlayer() == null || ChessEvent.getSide(0).getPlayer().isOnline() || ChessEvent.getSide(1).getPlayer() == null || ChessEvent.getSide(1).getPlayer().isOnline())
			{
				//TODO: check him to be around the board!!
				npcHtmlMessage.setHtml("<html><head><title>Instanced Events</title></head><body>The current game has been interrupted due to some of the participants leaving.</body></html>");
					...
					for (int j = 7; j >= 0; j--)
					{
						...
						for (int i = 0; i < 8; i++)
						{
							...
							if (ChessEvent.getBoard(side)[i][j] == null)
								...
								square = "<a action=\"bypass -h npc_" + getObjectId() + "_movements_" + ChessEvent.getBoard(side)[i][j].getObjectId() + "\">[]</a>";
							else square = "<font color=\"FF0000\">[]</font>";
			...
			{
				npcHtmlMessage.setHtml("<html><head><title>Instanced Events</title></head><body>There is a started game already.<br>" +
						"<a action=\"bypass -h npc_" + getObjectId() + "_observe\">Observe Match</a></body></html>");
				playerInstance.sendPacket(npcHtmlMessage);
			}
		}
