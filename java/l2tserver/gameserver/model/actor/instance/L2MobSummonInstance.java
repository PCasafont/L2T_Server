...
public class L2MobSummonInstance extends L2SummonInstance
{
	...
	public void onAction(L2PcInstance player, boolean interact)
	{
		...
		{
			if (Config.DEBUG) Log.fine("new target selected:"+getObjectId());
	...
	public void showMobWindow(L2PcInstance player)
	{
		...
		if (getTemplate().getSkills() != null)
		{
			...
			for (L2Skill skill : getTemplate().getSkills().values())
			{
				if (skill.isActive())
				{
					...
					html += "<td><center>";
					if (skill.getId() < 1000)
						html += "<button action=\"bypass -h MobSummon UseSkill " + skill.getId() + " " + skill.getLevelHash() + "\" back=icon.skill0" + skill.getId() + " fore=icon.skill0" + skill.getId() + " width=32 height=32>";
					else
						html += "<button action=\"bypass -h MobSummon UseSkill " + skill.getId() + " " + skill.getLevelHash() + "\" back=icon.skill" + skill.getId() + " fore=icon.skill" + skill.getId() + " width=32 height=32>";
					html += "</center></td>";
					skills[i % 2] = skill;
					i++;
					if (i % 2 == 0)
					{
						html += "</tr><tr>";
						for (int j = 0; j < 2; j++)
							html += "<td><center><a action=\"bypass -h MobSummon UseSkill " + skill.getId() + " " + skill.getLevelHash() + "\">" + skills[j].getName() + "</a></center></td>";
						html += "</tr>";
					}
				}
			}
		...
		if (i == 0)
			...
			html += "</table>";
		html += "<br><button value=\"Exchange\" action=\"bypass -h MobSummon Exchange\" width=120 height=30 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">";
	...
	public void exchange(L2MobSummonInstance mob)
	{
		if (isInsideRadius(mob, 1500, true, false))
		{
			...
			getControlItem().updateDatabase(true);
	...
	public boolean useMagic(L2Skill skill, boolean forceUse, boolean dontMove)
	{
		...
		if (isSkillDisabled(skill.getId())
				|| (isAllSkillsDisabled() && !skill.canBeUsedWhenDisabled()))
		{
			if (getOwner() != null)
			{
		...
		if (skill.isOffensive())
		{
			...
			else
			{
				if (!target.isAttackable()
						&& getOwner() != null
						&& (!(target instanceof L2MobSummonInstance))
						&& (!getOwner().getAccessLevel().allowPeaceAttack()))
				{
					return false;
				}
