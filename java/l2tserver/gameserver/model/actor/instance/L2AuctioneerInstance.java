...
import l2tserver.gameserver.model.entity.Auction;
import l2tserver.gameserver.model.entity.Auction.Bidder;
		...
		else if (condition == COND_REGULAR)
		{
			...
			else if (actualCommand.equalsIgnoreCase("bid1"))
			{
				...
				try
				{
					...
					html.replace("%PLEDGE_ADENA%", String.valueOf(player.getClan().getWarehouse().getItemByItemId(Config.CH_BID_ITEMID) != null ? player.getClan().getWarehouse().getItemByItemId(Config.CH_BID_ITEMID).getCount() : 0));
			...
			else if (actualCommand.equalsIgnoreCase("sale"))
			{
				...
				html.replace("%ITEM_NAME%", getCurrencyName());
				html.replace("%AGIT_DEPOSIT%", String.valueOf(ClanHallManager.getInstance().getClanHallByOwner(player.getClan()).getLease()));
				html.replace("%AGIT_PLEDGE_ADENA%", String.valueOf(player.getClan().getWarehouse().getItemByItemId(Config.CH_BID_ITEMID) != null ? player.getClan().getWarehouse().getItemByItemId(Config.CH_BID_ITEMID).getCount() : 0));
				html.replace("%AGIT_LINK_BACK%", "bypass -h npc_"+getObjectId()+"_selectedItems");
				html.replace("%AGIT_CURRENCY%", getCurrencyName());
				html.replace("%objectId%", String.valueOf(getObjectId()));
				player.sendPacket(html);
