...
import l2tserver.gameserver.events.chess.ChessEventSide;
...
public final class L2ChessPieceInstance extends L2MonsterInstance
{
					...
					for (int j = 7; j >= 0; j--)
					{
						...
						for (int i = 0; i < 8; i++)
						{
							...
							if (moveType(i, j, true) == 4)
								square = "<font color=\"00FF00\"><a action=\"bypass -h npc_%objectId%_enrocar_" + getSide().getPiece(7, 0).getObjectId() + "_" + i + "_" + j + "_dreta\">[]</a></font>";
							else if (moveType(i, j, true) == 3)
								square = "<font color=\"00FF00\"><a action=\"bypass -h npc_%objectId%_enrocar_" + getSide().getPiece(0, 0).getObjectId() + "_" + i + "_" + j + "_esquerra\">[]</a></font>";
	...
	{
		int side = 1;
		if ((command.startsWith("moveto") || command.startsWith("attack") || command.startsWith("enrocar")) && ChessEvent.isState(ChessState.STARTED) && !ChessEvent.pieceMoving)
		{
