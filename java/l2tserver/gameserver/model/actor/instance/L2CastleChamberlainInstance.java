...
import l2tserver.gameserver.network.serverpackets.ExShowDominionRegistry;
...
public class L2CastleChamberlainInstance extends L2MerchantInstance
{
		...
		else if (condition == COND_OWNER)
		{
			...
			else if (actualCommand.equalsIgnoreCase("functions"))
			{
				...
				else if (val.equalsIgnoreCase("support"))
				{
					...
					{
						html.setFile(player.getHtmlPrefix(), "chamberlain/support"
	...
	private void doTeleport(L2PcInstance player, int val)
	{
		...
		if (list != null)
		{
			...
			{
				if (Config.DEBUG)
					Log.warning("Teleporting player " + player.getName()
