...
import l2tserver.gameserver.model.L2RecipeInstance;
import l2tserver.gameserver.model.L2RecipeList;
import l2tserver.gameserver.model.L2RecipeStatInstance;
...
import l2tserver.gameserver.network.serverpackets.RecipeBookItemList;
import l2tserver.gameserver.network.serverpackets.RecipeItemMakeInfo;
import l2tserver.gameserver.network.serverpackets.RecipeShopItemInfo;
...
public class RecipeController
{
	...
	private void loadFromXML() throws SAXException, IOException, ParserConfigurationException
	{
		...
		if (file.exists())
		{
			...
			for (XmlNode n : doc.getChildren())
			{
				...
				{
					recipesFile: for (XmlNode d : n.getChildren())
					{
						if (d.getName().equalsIgnoreCase("item"))
						{
							...
							for (XmlNode c : d.getChildren())
							{
								if (c.getName().equalsIgnoreCase("statUse"))
								{
									...
									{
										Log.severe("Error in StatUse parameter for recipe item id: " + id + ", skipping");
								...
								else if (c.getName().equalsIgnoreCase("altStatChange"))
								{
									...
									{
										Log.severe("Error in AltStatChange parameter for recipe item id: " + id + ", skipping");
										continue recipesFile;
									}
								}
	...
	private static class RecipeItemMaker implements Runnable
	{
		...
		public void run()
		{
			...
			if (Config.ALT_GAME_CREATION && !_items.isEmpty())
			{
				...
				if (!_items.isEmpty())
				{
					...
					_delay = (int) (Config.ALT_GAME_CREATION_SPEED * _player.getMReuseRate(_skill) * TimeController.TICKS_PER_SECOND / Config.RATE_CONSUMABLE_COST)
		...
		private void finishCrafting()
		{
			...
			int chance = (int) (_recipeList.getSuccessRate() + (_player.getLUC() * 0.2));
		...
		private void rewardPlayer()
		{
			...
			if (Config.ALT_GAME_CREATION)
			{
				...
				_player.addExpAndSp((int) _player.calcStat(Stats.EXP_RATE, _exp * Config.ALT_GAME_CREATION_XP_RATE
						* Config.ALT_GAME_CREATION_SPEED, null, null), (int) _player.calcStat(Stats.SP_RATE, _sp
