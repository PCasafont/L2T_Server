 ...
 */
public class NewbieHelperAI extends CreatureAI implements Runnable {
	private List<Integer> alreadyBuffed = new ArrayList<>();

	public NewbieHelperAI(Creature creature) {
		super(creature);
	...
	public void run() {
		...
		for (Creature activeChar : npc.getKnownList().getKnownCharacters()) {
			if (activeChar == null || !(activeChar instanceof Playable) ||
