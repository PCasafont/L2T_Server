 ...
 * @author Pere
 */
public final class ChessPieceInstance extends MonsterInstance {
	...
	public ChessPieceInstance(int objectId, NpcTemplate template) {
	...
	public void showChatWindow(Player playerInstance, int val) {
		...
		if (ChessEvent.isState(ChessState.STARTED)) {
			if (playerInstance.isGM() ||
					getSide().getPlayer().getObjectId() == playerInstance.getObjectId() && ChessEvent.turn == getSide().getId()) {
						...
						for (int i = 0; i < 8; i++) {
							...
							if (moveType(i, j, true) == 4) {
								square = "<font color=\"00FF00\"><a action=\"bypass -h npc_%objectId%_enrocar_" +
										getSide().getPiece(7, 0).getObjectId() + "_" + i + "_" + j + "_dreta\">[]</a></font>";
							} else if (moveType(i, j, true) == 3) {
								square = "<font color=\"00FF00\"><a action=\"bypass -h npc_%objectId%_enrocar_" +
										getSide().getPiece(0, 0).getObjectId() + "_" + i + "_" + j + "_esquerra\">[]</a></font>";
							} else if (moveType(i, j, true) == 2) {
								square = "<font color=\"FF0000\"><a action=\"bypass -h npc_%objectId%_attack_" +
										ChessEvent.getBoard(getSide().getId())[i][j].getObjectId() + "_" + i + "_" + j + "\">[]</a></font>";
	...
	public void move(int i, int j, ChessPieceInstance enemy) {
		...
		if (enemy != null) {
			while (getAI().getIntention() != CtrlIntention.AI_INTENTION_MOVE_TO &&
					(Math.abs(getX() - (rPosY * 45 - 60165 + moveY)) > 30 || Math.abs(getY() - (rPosX * 45 - 59752 + moveX)) > 30)) {
		...
		while (getAI().getIntention() != CtrlIntention.AI_INTENTION_MOVE_TO &&
				(Math.abs(getX() - (rPosY * 45 - 60165 + moveY)) > 5 || Math.abs(getY() - (rPosX * 45 - 59752 + moveX)) > 5)) {
			getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO,
					new L2CharPosition(rPosY * 45 - 60165 + moveY, rPosX * 45 - 59752 + moveX, -1932, 0));
			while (getAI().getIntention() == CtrlIntention.AI_INTENTION_MOVE_TO) {
				sleep();
		...
		if (enemy != null && enemy.getType() == 6) {
			...
			for (int k = 0; k < 8; k++) {
				for (int l = 0; l < 8; l++) {
					ChessPieceInstance enemySaving = getSide().getPiece(l, k);
	...
	public int moveType(int i, int j, boolean checkKing) {
		...
		switch (getType()) {
			case 1:
				if (i == posX && j == posY + 1 && estaBuit(i, j)) {
					canMove = true;
				}
				if (Math.abs(i - posX) == 1 && j == posY + 1 && hiHaEnemic(i, j)) {
					canMove = true;
				}
				if (firstMove && i == posX && j == posY + 2 && !anyBetween(posX, posY, i, j) && estaBuit(i, j)) {
					canMove = true;
				}
				break;
			case 2:
				if ((i == posX || j == posY) && !anyBetween(posX, posY, i, j)) {
					canMove = true;
				}
				break;
			case 3:
				if (Math.abs(i - posX) == 1 && Math.abs(j - posY) == 2 || Math.abs(i - posX) == 2 && Math.abs(j - posY) == 1) {
					canMove = true;
				}
				break;
			case 4:
				if (Math.abs(i - posX) == Math.abs(j - posY) && !anyBetween(posX, posY, i, j)) {
					canMove = true;
				}
				break;
			case 5:
				if ((i == posX || j == posY || Math.abs(i - posX) == Math.abs(j - posY)) && !anyBetween(posX, posY, i, j)) {
					canMove = true;
				}
				break;
			case 6:
				if ((Math.abs(i - posX) == 1 || Math.abs(j - posY) == 1) && Math.abs(i - posX) + Math.abs(j - posY) <= 2) {
					canMove = true;
				}
				if (firstMove && j == posY && i == posX - 2 && getSide().getPiece(0, 0) != null && !getSide().getPiece(0, 0).hasMoved() &&
						!anyBetween(posX, posY, 0, 0)) {
					canMove = true;
					toReturn = 3;
				}
				if (firstMove && j == posY && i == posX + 2 && getSide().getPiece(7, 0) != null && !getSide().getPiece(7, 0).hasMoved() &&
						!anyBetween(posX, posY, 7, 0)) {
					canMove = true;
					toReturn = 4;
				}
				break;
		...
		if (canMove) {
			if (hiHaAmic(i, j) || getSide().hasWon() &&
					(!hiHaEnemic(i, j) || getSide().getPiece(i, j).getObjectId() != getSide().getEnemy().getKing().getObjectId())) {
				return 0;
			}
			if (checkKing) {
				ChessPieceInstance piece = getSide().getPiece(i, j);
				int oPosX = posX;
				int oPosY = posY;
				posX = i;
				posY = j;
	...
	public boolean canKillTheKing() {
		ChessPieceInstance king = getSide().getEnemy().getKing();
		return moveType(7 - king.getPosX(), 7 - king.getPosY(), false) == 2;
	}
	
	private void sayMovement(ChessPieceInstance victim) {
		...
		CreatureSay cs1 = new CreatureSay(getSide().getPlayer().getObjectId(), Say2.TELL, "->" + getSide().getEnemy().getPlayer().getName(), message);
		getSide().getPlayer().sendPacket(cs1);
		CreatureSay cs2 = new CreatureSay(getSide().getPlayer().getObjectId(), Say2.TELL, getSide().getPlayer().getName(), message);
	...
	@Override
	public boolean isAutoAttackable(Creature character) {
		return character instanceof ChessPieceInstance;
	}
	...
	public void onSpawn() {
		...
		getSide().setPiece(posX, posY, this);
		super.onSpawn();
	}
