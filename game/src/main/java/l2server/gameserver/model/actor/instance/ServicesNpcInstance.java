...
public final class ServicesNpcInstance extends NpcInstance {
	...
	public void onBypassFeedback(Player player, String command) {
		...
		if (command.startsWith("1stClass")) {
			...
			if (Config.DEBUG) {
				log.debug("ClassMaster activated");
			}
					...
					if (player.getSubClasses().isEmpty()) {
						...
						for (Iterator<SubClass> subList = iterSubClasses(player); subList.hasNext(); ) {
							...
							if (subClassId == player.getActiveClass()) {
								...
								content += "<a action=\"bypass -h npc_" + getObjectId() + "_Subclass 5 " + subClass.getClassIndex() + "\">" +
			...
			if (currTime > reuseTime) {
				...
				GlobalVariablesManager.getInstance().storeVariable(varFormat, Long.toString(604800000 + System.currentTimeMillis()));
			} else {
	...
	private List<Integer> getAvailableSubClasses(Player player) {
	...
	private void showRemoveChat(Player player) {
	...
	public void showPledgeSkillList(Player player) {
		if (Config.DEBUG) {
			log.debug("PledgeSkillList activated on: " + getObjectId());
		}
	...
	public void assignSubPledgeLeader(Player player, String clanName, String leaderName) {
		if (Config.DEBUG) {
			log.debug(player.getObjectId() + " (" + player.getName() + ") requested to assign sub clan" + clanName + "leader " + "(" + leaderName +
					")");
		}
	...
	public void createSubPledge(Player player, String clanName, String leaderName, int pledgeType, int minClanLvl) {
		if (Config.DEBUG) {
			log.debug(player.getObjectId() + " (" + player.getName() + ") requested sub clan creation from " + getObjectId() + " (" + getName() + ")");
		}
	...
	public void changeClanLeader(Player player, String target) {
		if (Config.DEBUG) {
			log.debug(player.getObjectId() + " (" + player.getName() + ") requested change a clan leader from " + getObjectId() + " (" + getName() +
					")");
		}
	...
	public void recoverClan(Player player, int clanId) {
		if (Config.DEBUG) {
			log.debug(player.getObjectId() + " (" + player.getName() + ") requested recover a clan from " + getObjectId() + " (" + getName() + ")");
		}
	...
	public void dissolveClan(Player player, int clanId) {
		if (Config.DEBUG) {
			log.debug(player.getObjectId() + " (" + player.getName() + ") requested dissolve a clan from " + getObjectId() + " (" + getName() + ")");
		}
	...
	private void showRetrieveWindow(Player player, WarehouseListType itemtype, byte sortorder) {
		player.sendPacket(ActionFailed.STATIC_PACKET);
	...
	private void showWithdrawWindowClan(Player player, WarehouseListType itemtype, byte sortorder) {
		player.sendPacket(ActionFailed.STATIC_PACKET);
