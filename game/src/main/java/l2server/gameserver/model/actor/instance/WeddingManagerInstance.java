...
public class WeddingManagerInstance extends Npc {
	/**
	...
	public void onBypassFeedback(Player player, String command) {
		...
		if (player.getPartnerId() == 0) {
			...
			Player ptarget = World.getInstance().getPlayer(player.getPartnerId());
			...
			if (ptarget == null || !ptarget.isOnline()) {
				...
				if (player.isMarried()) {
					...
					// Fireworks
					Skill skill = SkillTable.FrequentSkill.LARGE_FIREWORK.getSkill();
					if (skill != null) {
					...
					Announcements.getInstance()
							.announceToAll("Congratulations to " + player.getName() + " and " + ptarget.getName() + "! They have been married.");
	 ...
	 * @return boolean
	 */
	private boolean wearsFormalWear(Player p1, Player p2) {
		if (Config.L2JMOD_WEDDING_FORMALWEAR) {
			Item fw1 = p1.getChestArmorInstance();
			Item fw2 = p2.getChestArmorInstance();
