...
public class TeleporterInstance extends Npc {
	private static Logger log = LoggerFactory.getLogger(TeleporterInstance.class.getName());
		...
		if (player.getFirstEffect(6201) != null || player.getFirstEffect(6202) != null || player.getFirstEffect(6203) != null) {
			...
			if (!Config.IS_CLASSIC) {
				if (val.equalsIgnoreCase("1") && player.getLevel() < 41) {
					...
					return;
				} else if (val.equalsIgnoreCase("1") && cal.get(Calendar.HOUR_OF_DAY) >= 20 && cal.get(Calendar.HOUR_OF_DAY) <= 23 &&
	...
	protected void doTeleport(Player player, int val) {
		...
		if (list != null) {
			...
			if (Config.ALT_GAME_FREE_TELEPORT ||
					player.destroyItemByItemId("Teleport " + (list.getIsForNoble() ? " nobless" : ""), list.getItemId(), price, this, true)) {
				if (Config.DEBUG) {
					log.debug("Teleporting player " + player.getName() + " to new location: " + list.getLocX() + ":" + list.getLocY() + ":" +
							list.getLocZ());
				}
