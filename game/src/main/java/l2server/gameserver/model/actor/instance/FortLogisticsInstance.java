...
public class FortLogisticsInstance extends MerchantInstance {
	...
	private static final int[] SUPPLY_BOX_IDS =
			{35665, 35697, 35734, 35766, 35803, 35834, 35866, 35903, 35935, 35973, 36010, 36042, 36080, 36117, 36148, 36180, 36218, 36256, 36293,
					36325, 36363};
			...
			if (player.getClan() != null && getFort().getOwnerClan() != null && player.getClan() == getFort().getOwnerClan() &&
				...
				if (getFort().getSiege().getIsInProgress()) {
					...
					if (level > 0) {
						...
						// spawn box
						NpcTemplate BoxTemplate = NpcTable.getInstance().getTemplate(SUPPLY_BOX_IDS[level - 1]);
						...
						box.setHeading(0);
						//World.getInstance().storeObject(box);
