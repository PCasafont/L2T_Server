...
public class InformerNpcInstance extends NpcInstance {
	...
	private String getContent(int val) {
		...
		try {
			...
			if (rset.next()) {
				content += "<br><center>" + rset.getString("name") + "</center><br>" + rset.getString("html") + "<br>" + "<center>%subLinks%<br>" +
						"<button value=\"Back\" action=\"bypass -h npc_%objectId%_Chat " + rset.getInt("parent_id") +
			...
			log.warn("Failed to close Statement/ResultSet in Informer NPC: " + e.getMessage());
		} finally {
	...
	private String getSubLinks(int val) {
		...
		try {
			...
			PreparedStatement statement = con.prepareStatement(
					"SELECT id, name FROM l2tenkai_web.features WHERE parent_id = ? AND exclude_" + LoginServerThread.getInstance().getServerName() +
							" = 0");
			...
			while (rset.next()) {
				...
				content += "<td><button value=\"" + rset.getString("name") + "\" action=\"bypass -h npc_%objectId%_Chat " + rset.getInt("id") +
