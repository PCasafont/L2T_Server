			...
			if (actualCommand.equalsIgnoreCase("auction")) {
				...
				try {
					...
					try {
						...
						Auction a = new Auction(player.getClan().getHasHideout(),
								player.getClan(),
								days * 86400000L,
								bid,
				...
				if (Config.DEBUG) {
					log.warn("bidding show successful");
				}
					...
					if (a != null) {
						...
						html.replace("%AGIT_AUCTION_REMAIN%",
								String.valueOf((a.getEndDate() - System.currentTimeMillis()) / 3600000) + " hours " +
										String.valueOf((a.getEndDate() - System.currentTimeMillis()) / 60000 % 60) + " minutes");
						html.replace("%AGIT_AUCTION_MINBID%", String.valueOf(a.getStartingBid()));
				...
				try {
					...
					html.replace("%PLEDGE_ADENA%",
							String.valueOf(player.getClan().getWarehouse().getItemByItemId(Config.CH_BID_ITEMID) != null ?
				...
				if (Config.DEBUG) {
					log.warn("cmd list: auction test started");
				}
				...
				for (Auction a : auctions) {
					...
					if (ClanHallManager.getInstance().getClanHallById(a.getItemId()) == null) {
						log.warn("Auction set for non existing clan hall! Id: " + a.getId() + ", item name: " + a.getItemName());
						continue;
					}
					...
					items += "<tr>" + "<td>" + ClanHallManager.getInstance().getClanHallById(a.getItemId()).getLocation() + "</td>" +
							"<td><a action=\"bypass -h npc_" + getObjectId() + "_bidding " + a.getId() + "\">" + a.getItemName() + "</a></td>" +
							"<td>" + format.format(a.getEndDate()) + "</td>" + "<td>" + a.getStartingBid() + "</td>" + "</tr>";
				}
				...
				if (Config.DEBUG) {
					log.warn("cmd bidlist: auction test started");
				}
				
				String biders = "";
				Map<Integer, Bidder> bidders = ClanHallAuctionManager.getInstance().getAuction(auctionId).getBidders();
				for (Bidder b : bidders.values()) {
					biders += "<tr>" + "<td>" + b.getClanName() + "</td><td>" + b.getName() + "</td><td>" + b.getTimeBid().get(Calendar.YEAR) + "/" +
							(b.getTimeBid().get(Calendar.MONTH) + 1) + "/" + b.getTimeBid().get(Calendar.DATE) + "</td><td>" + b.getBid() + "</td>" +
							"</tr>";
				}
				...
				long bid = ClanHallAuctionManager.getInstance()
						.getAuction(player.getClan().getAuctionBiddedAt())
						.getBidders()
						.get(player.getClanId())
						.getBid();
				...
				html.replace("%ITEM_NAME%", getCurrencyName());
				html.replace("%AGIT_DEPOSIT%", String.valueOf(ClanHallManager.getInstance().getClanHallByOwner(player.getClan()).getLease()));
				html.replace("%AGIT_PLEDGE_ADENA%",
						String.valueOf(player.getClan().getWarehouse().getItemByItemId(Config.CH_BID_ITEMID) != null ?
								player.getClan().getWarehouse().getItemByItemId(Config.CH_BID_ITEMID).getCount() : 0));
				html.replace("%AGIT_LINK_BACK%", "bypass -h npc_" + getObjectId() + "_selectedItems");
				html.replace("%AGIT_CURRENCY%", getCurrencyName());
