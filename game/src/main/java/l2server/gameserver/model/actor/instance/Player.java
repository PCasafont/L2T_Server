...
public class Player extends Playable {
	...
	private boolean checkUseMagicConditions(Skill skill, boolean forceUse, boolean dontMove) {
		...
		if (skill.isOffensive()) {
			...
			if (target != this && !(target instanceof NpcInstance) && !isInDuel() && target instanceof Player && !((Player) target).isInDuel() &&
					!target.isAttackable() && !getAccessLevel().allowPeaceAttack()) {
	...
	public void addCubic(int id,
		...
		CubicInstance cubic = new CubicInstance(this, id, level, (int) matk, activationtime, activationchance, maxcount, totalLifetime, givenByOther);
	...
	public void enteredNoLanding(int delay) {
		dismountTask = ThreadPoolManager.getInstance().scheduleGeneral(new Dismount(), delay * 1000);
	}
	...
	public void storeCharFriendMemos() {
		...
		for (Map.Entry<Integer, String> friendMemo : friendMemo.entrySet()) {
			try {
				...
				log.warn("Could not update character(" + getObjectId() + ") friend(" + friendMemo.getKey() + ") memo: " + e.getMessage(), e);
			} finally {
				DatabasePool.close(con);
			}
		}
		for (Map.Entry<Integer, String> blockMemo : blockMemo.entrySet()) {
			try {
				...
				log.warn("Could not update character(" + getObjectId() + ") block(" + blockMemo.getKey() + ") memo: " + e.getMessage(), e);
			} finally {
				DatabasePool.close(con);
			}
		}
