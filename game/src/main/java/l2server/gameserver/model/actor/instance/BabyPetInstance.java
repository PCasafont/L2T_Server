...
public final class BabyPetInstance extends PetInstance {
	...
	public BabyPetInstance(int objectId, NpcTemplate template, Player owner, Item control) {
		super(objectId, template, owner, control);
		setInstanceType(InstanceType.L2BabyPetInstance);
	}
	
	public BabyPetInstance(int objectId, NpcTemplate template, Player owner, Item control, byte level) {
	...
	private class CastTask implements Runnable {
		private final BabyPetInstance baby;
		private List<Skill> currentBuffs = new ArrayList<>();
		...
		public void run() {
			...
			if (owner != null && !owner.isDead() && !owner.isInvul() && !baby.isCastingNow() && !baby.isBetrayed() && !baby.isMuted() &&
					!baby.isOutOfControl() && bufferMode && baby.getAI().getIntention() != CtrlIntention.AI_INTENTION_CAST) {
				Skill skill = null;
				...
				if (baby.getFirstEffect(BUFF_CONTROL) == null) // Buff Control is not active
				{
					...
					if (buffs != null && !buffs.isEmpty()) {
						for (SkillHolder i : buffs) {
							...
							if (skill.getTargetType() == SkillTargetType.TARGET_SELF && baby.getFirstEffect(skill) != null) {
								continue;
							}
					...
					if (!currentBuffs.isEmpty()) {
						Abnormal[] effects = owner.getAllEffects();
						Iterator<Skill> iter;
						Skill currentSkill;
