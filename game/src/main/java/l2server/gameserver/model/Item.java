 ...
 */
public final class Item extends WorldObject implements ItemInstanceInfo {
	private static Logger log = LoggerFactory.getLogger(Item.class.getName());
	/**
	 ...
	 */
	public Item(int objectId, ItemTemplate item, long time) {
		super(objectId);
		setInstanceType(InstanceType.L2ItemInstance);
	...
	/**
	 * Remove a Item from the world and send server->client GetItem packets.<BR><BR>
	 * <p>
	...
	public boolean isEquipable() {
		return !(item.getBodyPart() == 0 || item.getItemType() == EtcItemType.LURE);
	}
	...
	public boolean isEquipped() {
		return loc == ItemLocation.EQUIPPED || loc == ItemLocation.PET_EQUIP;
	}
	...
	/**
	 * Returns the characteristics of the EtcItemTemplate
	 *
	 * @return EtcItemTemplate
	 */
	public EtcItemTemplate getEtcItem() {
		if (item instanceof EtcItemTemplate) {
			return (EtcItemTemplate) item;
		}
		return null;
	}
	
	/**
	 * Returns the characteristics of the WeaponTemplate
	 *
	 * @return WeaponTemplate
	 */
	public WeaponTemplate getWeaponItem() {
	...
	/**
	 * Returns the characteristics of the ArmorTemplate
	 *
	 * @return ArmorTemplate
	 */
	public ArmorTemplate getArmorItem() {
		if (item instanceof ArmorTemplate) {
			return (ArmorTemplate) item;
		}
		return null;
	}
	...
	public boolean isAvailable(Player player, boolean allowAdena, boolean allowNonTradeable) {
		return !isEquipped() // Not equipped
				&& getItem().getType2() != ItemTemplate.TYPE2_QUEST // Not Quest Item
				&& (getItem().getType2() != ItemTemplate.TYPE2_MONEY || getItem().getType1() != ItemTemplate.TYPE1_SHIELD_ARMOR)
	...
	public void removeEnsoulEffects() {
		...
		try {
			...
			log.error("Could not remove ensoul effect for item: " + this + " from DB:", e);
		} finally {
			DatabasePool.close(con);
		}
	}
	...
	public void removeAugmentation() {
		...
		try {
			...
			log.error("Could not remove augmentation for item: " + this + " from DB:", e);
		} finally {
			DatabasePool.close(con);
		}
	}
	...
	public void restoreAttributes() {
		...
		try {
			...
			log.error("Could not restore augmentation and elemental data for item " + this + " from DB: " + e.getMessage(), e);
		} finally {
			DatabasePool.close(con);
		}
	}
	...
	private void updateItemEnsoulEffects(Connection pooledCon) {
		Connection con = null;
		try {
			con = pooledCon == null ? DatabasePool.getInstance().getConnection() : pooledCon;
			...
			log.error("Could not update ensoul effects for item: " + this + " from DB:", e);
		} finally {
			if (pooledCon == null) {
				DatabasePool.close(con);
			}
	...
	private void updateItemAttributes(Connection pooledCon) {
		...
		try {
			...
			log.error("Could not update atributes for item: " + this + " from DB:", e);
		} finally {
			if (pooledCon == null) {
				DatabasePool.close(con);
			}
	...
	private void updateItemElements(Connection pooledCon) {
		...
		try {
			...
			log.error("Could not update elementals for item: " + this + " from DB:", e);
		} finally {
			if (pooledCon == null) {
				DatabasePool.close(con);
			}
	...
	public void clearElementAttr(byte element) {
		...
		try {
			...
			log.error("Could not remove elemental enchant for item: " + this + " from DB:", e);
		} finally {
			DatabasePool.close(con);
		}
	}
	...
	public void decreaseMana(boolean resetConsumingMana, int count) {
		...
		final Player player = World.getInstance().getPlayer(getOwnerId());
		if (player != null) {
			SystemMessage sm;
			switch (mana) {
				case 10:
			...
			if (mana == 0) // The life time has expired
			{
				...
				// unequip
				if (isEquipped()) {
					Item[] unequiped = player.getInventory().unEquipItemInSlotAndRecord(getLocationSlot());
	 ...
	 * ItemTemplate/ArmorTemplate/WeaponTemplate, but may add additional
	...
	public static Item restoreFromDb(int ownerId, ResultSet rs) {
		...
		try {
			...
			if (temp instanceof Item) {
				World.getInstance().removeObject(temp);
		...
		if (item == null) {
			log.error("Item item_id=" + item_id + " not known, object_id=" + objectId);
			return null;
		}
		inst = new Item(objectId, item, time);
	...
	/**
	 * Init a dropped Item and add it in the world as a visible object.<BR><BR>
	 * <p>
	 * <B><U> Actions</U> :</B><BR><BR>
	 * <li>Set the x,y,z position of the Item dropped and update its worldregion </li>
	 * <li>Add the Item dropped to visibleObjects of its WorldRegion</li>
	 * <li>Add the Item dropped in the world as a <B>visible</B> object</li><BR><BR>
	 * <p>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : This method DOESN'T ADD the object to allObjects of World </B></FONT><BR><BR>
	 * <p>
	 * <B><U> Assert </U> :</B><BR><BR>
	...
	public class ItemDropTask implements Runnable {
		...
		public final void run() {
			...
			synchronized (itm) {
				// Set the x,y,z position of the Item dropped and update its worldregion
				itm.setIsVisible(true);
				itm.getPosition().setWorldPosition(x, y, z);
				itm.getPosition().setWorldRegion(World.getInstance().getRegion(getPosition().getWorldPosition()));
				
				// Add the Item dropped to visibleObjects of its WorldRegion
			}
			...
			World.getInstance().addVisibleObject(itm, itm.getPosition().getWorldRegion());
	...
	private void updateInDb() {
		...
		try {
			...
			log.error("Could not update item " + this + " (owner id " + ownerId + ") in DB: Reason: " + e.getMessage(), e);
		} finally {
			DatabasePool.close(con);
		}
	}
	...
	private void insertIntoDb() {
		...
		try {
			...
			log.error("Could not insert item " + this + " into DB: Reason: " + e.getMessage(), e);
		} finally {
			DatabasePool.close(con);
		}
	}
	...
	private void removeFromDb() {
		...
		try {
			...
			log.error("Could not delete item " + this + " in DB: " + e.getMessage(), e);
		} finally {
			DatabasePool.close(con);
		}
	}
	...
	public void endOfLife() {
		Player player = World.getInstance().getPlayer(getOwnerId());
		if (player != null) {
			if (isEquipped()) {
				Item[] unequiped = player.getInventory().unEquipItemInSlotAndRecord(getLocationSlot());
				InventoryUpdate iu = new InventoryUpdate();
	...
	public static class ScheduleLifeTimeTask implements Runnable {
		private final Item limitedItem;
		
		public ScheduleLifeTimeTask(Item item) {
			limitedItem = item;
		}
	...
	/* (non-Javadoc)
	 * @see l2server.gameserver.model.WorldObject#decayMe()
	 */
	@Override
	public void decayMe() {
		if (Config.SAVE_DROPPED_ITEM) {
	...
	public boolean isValuable() {
		return getItem().getItemGrade() >= ItemTemplate.CRYSTAL_S || (isAugmented() || isPvp());
	}
	...
	public int getStoneType() {
		if (getName().contains("Weapon") ||
				getItem().getStandardItem() > -1 && ItemTable.getInstance().getTemplate(getItem().getStandardItem()) instanceof WeaponTemplate) {
			return ItemTemplate.TYPE2_WEAPON;
		}
		if (getName().contains("Armor")) {
			return ItemTemplate.TYPE2_SHIELD_ARMOR;
		}
		if (getName().contains("Accessory")) {
			return ItemTemplate.TYPE2_ACCESSORY;
		}
		return -1;
	}
}
