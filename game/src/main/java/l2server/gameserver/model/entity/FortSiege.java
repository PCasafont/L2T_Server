...
public class FortSiege implements Siegable {
	...
	public void killedCommander(FortCommanderInstance instance) {
		if (commanders != null && getFort() != null && commanders.size() != 0) {
			...
			if (spawn != null) {
				...
				log.warn("FortSiege.killedCommander(): killed commander, but commander not registered for fortress. NpcId: " + instance.getNpcId() +
	...
	public void checkAutoTask() {
		...
		if (delay < 0) {
			...
			// no attackers - waiting for suspicious merchant spawn
			{
				...
				{
					ThreadPoolManager.getInstance().executeTask(new ScheduleSuspiciousMerchantSpawn());
					siegeStartTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(3600), delay - 3600000);
				}
				if (delay > 600000) // more than 10 min, spawn suspicious merchant
				{
					ThreadPoolManager.getInstance().executeTask(new ScheduleSuspiciousMerchantSpawn());
					siegeStartTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(600), delay - 600000);
				} else if (delay > 300000) // more than 5 min
				{
					siegeStartTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(300), delay - 300000);
				} else if (delay > 60000) //more than 1 min
				{
					siegeStartTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(60), delay - 60000);
				} else
				// lower than 1 min, set to 1 min
				{
					siegeStartTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(60), 0);
				}
	...
	public void startAutoTask(boolean setTime) {
		...
		siegeStartTask = ThreadPoolManager.getInstance().scheduleGeneral(new ScheduleStartSiegeTask(300), 0);
	}
