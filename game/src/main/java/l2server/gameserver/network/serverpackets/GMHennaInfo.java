...
public class GMHennaInfo extends L2GameServerPacket {
	...
	private final HennaTemplate[] hennas = new HennaTemplate[4];
	private int count;
	
	public GMHennaInfo(Player activeChar) {
		this.activeChar = activeChar;
		
		int j = 0;
		for (int i = 0; i < 3; i++) {
			HennaTemplate h = activeChar.getHenna(i + 1);
			if (h != null) {
				hennas[j++] = h;
			}
		}
	...
	protected final void writeImpl() {
		...
		HennaTemplate specialDye = activeChar.getHenna(4);
		if (specialDye != null) {
