...
public class CustomOfflineBuffersManager {
		...
		synchronized (customBufferTable) {
			...
			for (Entry<Integer, BufferTable> i : customBufferTable.entrySet()) {
				...
				sb.append("<table width=750><tr><td FIXWIDTH=50>" + buffer.getName() + "</td><td FIXWIDTH=60>" +
						...
						")</td><td FIXWIDTH=35 align=center>" + Math.round(buffer.getCurrentMp() * 100 / buffer.getMaxMp()) +
						"%</td><td FIXWIDTH=80 align=center>" + (buffTable.getDescription() == null ? "" : buffTable.getDescription()) +
						"</td><td FIXWIDTH=30 align=center><button value=\"" + buffTable.getBuffs().size() +
						"\" width=50 height=16 action=\"bypass _bbscustom;action;worldBuff;bufferInfo;" + buffTable.getPlayerId() +
			...
			for (Entry<Integer, Long> i : buffTable.getBuffs().entrySet()) {
				...
				if (buffInfo != null) {
					sb.append("<table width=300 " + (loc % 2 == 0 ? "bgcolor=131210" : "") + " border=0><tr><td FIXWIDTH=32><img src=\"" +
							...
							"\" width=32 height=32></td><td FIXWIDTH=130><a action=\"bypass _bbscustom;action;worldBuff;getBuff;" + playerId + ";" +
							skillId + "\">" + buffInfo.getName() + "</a></td><td FIXWIDTH=35 align=center>" + buffInfo.getLevelHash() +
	...
	private boolean canGetBuffs(Player player) {
		...
		if (player.isInCombat() || player.getPvpFlag() > 0 || player.getInstanceId() != 0 || player.isInDuel() || player.isFakeDeath() ||
				player.isOutOfControl() || player.isInOlympiadMode() || AttackStanceTaskManager.getInstance().getAttackStanceTask(player)) {
	...
	public void offlineBuffPannel(Player player) {
		...
		for (Skill sk : buffSkills) {
			...
			sb.append("<tr><td><img src=\"" + getCorrectSkillIcon(sk.getName(), sk.getId()) + "\" width=32 height=32></td><td  FIXWIDTH=150>" +
					sk.getName() + "</td></tr>");
			...
			if (buffTable != null && buffTable.getBuffs().containsKey(sk.getId())) {
				...
				sb.append("<tr><td>Price:</td><td><edit var=\"addBuff" + sk.getId() +
						"\" width=100 type=number length=14></td><td><button action=\"bypass _bbscustom;action;worldBuff;addBuff;" + sk.getId() +
						"; $addBuff" + sk.getId() + "\" value=Add! width=60 height=20 back=L2UI_ct1.button_df fore=L2UI_ct1.button_df></td></tr>");
			}
		...
		synchronized (customBufferTable) {
			...
			if (buffTable != null) {
				if (buffTable.getBuffs().size() > 0 && buffTable.getCointId() != 0) {
					...
					Util.logToFile("The player: " + player.getName() + " set one offline(" + buffTable.getDescription() + ") buffer with " +
							ItemTable.getInstance().getTemplate(buffTable.getCointId()).getName() + " as a price!", "OfflineBuffers", true);
					return true;
				}
			}
		}
