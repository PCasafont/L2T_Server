 ...
 * @author Pere
 */
public class ChessManagerInstance extends NpcInstance {
	public ChessManagerInstance(int objectId, NpcTemplate template) {
	...
	public void onBypassFeedback(Player playerInstance, String command) {
		...
		if (command.equals("observe")) {
			...
			Player receptor = World.getInstance().getPlayer(command.substring(10));
						...
						for (int j = 7; j >= 0; j--) {
							...
							for (int i = 0; i < 8; i++) {
								...
								if (piece.moveType(i, j, true) == 4) {
									square = "<font color=\"00FF00\"><a action=\"bypass -h npc_" + getObjectId() + "_enrocar_" +
											piece.getSide().getPiece(7, 0).getObjectId() + "_" + i + "_" + j + "_dreta\">[]</a></font>";
								} else if (piece.moveType(i, j, true) == 3) {
									square = "<font color=\"00FF00\"><a action=\"bypass -h npc_" + getObjectId() + "_enrocar_" +
											piece.getSide().getPiece(0, 0).getObjectId() + "_" + i + "_" + j + "_esquerra\">[]</a></font>";
								} else if (piece.moveType(i, j, true) == 2) {
									square = "<font color=\"FF0000\"><a action=\"bypass -h npc_" + getObjectId() + "_attack_" + piece.getObjectId() +
											"_" + ChessEvent.getBoard(side)[i][j].getObjectId() + "_" + i + "_" + j + "\">[]</a></font>";
								} else if (piece.moveType(i, j, true) == 1) {
									square = "<font color=\"00FF00\"><a action=\"bypass -h npc_" + getObjectId() + "_moveto_" + piece.getObjectId() +
											"_" + i + "_" + j + "\">[]</a></font>";
								} else {
									square = "[]";
								}
			...
			if (!ChessEvent.isPlayerWaiting(playerInstance.getObjectId())) {
				...
				if (htmContent != null) {
					...
					for (int i = 0; i < 8; i++) {
						...
						if (player != null) {
							list += "<tr><td>" + player.getName() + "</td><td>(" + color + ")</td><td><a action=\"bypass -h npc_" + getObjectId() +
									"_challenge" + i + "\">Retar!</a></td></tr>";
						}
						player = null;
					}
				...
				if (htmContent != null) {
					...
					for (int i = 0; i < 8; i++) {
						if (ChessEvent.getWaitingPlayers()[i][1] != null && ChessEvent.getWaitingPlayers()[i][0] != null &&
								ChessEvent.getWaitingPlayers()[i][0].getObjectId() == playerInstance.getObjectId()) {
							player = ChessEvent.getWaitingPlayers()[i][1];
						} else if (ChessEvent.getWaitingPlayers()[i][0] != null && ChessEvent.getWaitingPlayers()[i][1] != null &&
					...
					for (int j = 7; j >= 0; j--) {
						...
						for (int i = 0; i < 8; i++) {
							...
							if (ChessEvent.getBoard(side)[i][j] == null) {
								...
								square =
										"<a action=\"bypass -h npc_" + getObjectId() + "_movements_" + ChessEvent.getBoard(side)[i][j].getObjectId() +
												"\">[]</a>";
							} else {
