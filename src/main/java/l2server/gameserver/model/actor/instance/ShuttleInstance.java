...
public class ShuttleInstance extends Vehicle {
	...
	public ShuttleInstance(int objectId, CreatureTemplate t, int shuttleId) {
		super(objectId, t);
	...
	public boolean moveToNextRoutePoint() {
		...
		List<Player> passengersToOust = new ArrayList<>();
		for (Player toOust : passengers) {
			passengersToOust.add(toOust);
		}
		ThreadPoolManager.getInstance().scheduleGeneral(new GoTask(passengersToOust, current, next), current.getTime() * 1000L);
		
		return true;
	}
	
	public boolean addPassenger(Player player, int x, int y, int z) {
	...
	public void oustPlayer(Player player, int x, int y, int z) {
	...
	private class GoTask implements Runnable {
		private List<Player> playersToOust;
		private ShuttleStop previous, current;
		private boolean doorClosed = false;
		
		public GoTask(List<Player> playersToOust, ShuttleStop previous, ShuttleStop current) {
		...
		public void run() {
			...
			if (!doorClosed) {
				...
				for (Player player : playersToOust) {
					if (ShuttleInstance.this.passengers.contains(player)) {
						player.teleToLocation(pos.getX(), pos.getY(), pos.getZ());
						ShuttleInstance.this.oustPlayer(player, pos.getX(), pos.getY(), pos.getZ());
					}
				}
				previous.closeDoor();
				doorClosed = true;
	...
	public class ShuttleStop {
		...
		public void moveTo() {
			...
			broadcastPacket(new ExShuttleMove(ShuttleInstance.this));
		}
