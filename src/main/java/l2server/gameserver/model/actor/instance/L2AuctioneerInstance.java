			...
			if (actualCommand.equalsIgnoreCase("auction")) {
				...
				for (Auction a : auctions) {
					...
					if (ClanHallManager.getInstance().getClanHallById(a.getItemId()) == null) {
						Log.warning("Auction set for non existing clan hall! Id: " + a.getId() + ", item name: " + a.getItemName());
						continue;
					}
				...
				if (player.getClan() != null && player.getClan().getHasHideout() == 0 && player.getClan().getAuctionBiddedAt() > 0) {
					...
					if (ClanHallManager.getInstance().getClanHallById(ItemId) != null) {
						...
						Log.warning("Clan Hall ID NULL : " + ItemId + " Can be caused by concurent write in ClanHallManager");
					}
