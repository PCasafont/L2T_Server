...
public class NpcBufferInstance extends NpcInstance {
	private static final int[] buffs = {14779, 14780, 14781, 14782, 14783, 14784, 14785, 14786, 14787, 14788, 14789, 14790};
	...
	public void onBypassFeedback(Player player, String command) {
		...
		if (command.startsWith("BuffMe")) {
			...
			Skill skill = SkillTable.getInstance().getInfo(buffId, buffLevel);
			if (skill != null) {
	...
	private static void giveBuff(Player player, int skillId) {
		if (player == null) {
			return;
		}
	...
	public static void buff(Playable character) {
		...
		if (character instanceof Player) {
			...
			if (!player.isMageClass()) {
				...
				if (shield != null && shield.getItem().getType1() == ItemTemplate.TYPE1_SHIELD_ARMOR) {
					type = 0;
				} else {
					type = 1;
				}
	...
	public static void giveBasicBuffs(Player player) {
		int type = 2; //Mage
