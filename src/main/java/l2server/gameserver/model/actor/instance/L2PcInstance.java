...
public class L2PcInstance extends L2Playable {
	 ...
	 */
	public static L2PcInstance create(int objectId,
	...
	/**
	 * Return the newbie rewards state of the L2PcInstance.<BR><BR>
	 */
	...
	/**
	 * Return the deleteTimer of the L2PcInstance.<BR><BR>
	 */
	...
	/**
	 * Set the deleteTimer of the L2PcInstance.<BR><BR>
	 */
	...
	public void checkIfWeaponIsAllowed() {
		...
		for (L2Abnormal currenteffect : getAllEffects()) {
			...
			if (!effectSkill.isOffensive() &&
					!(effectSkill.getTargetType() == L2SkillTargetType.TARGET_PARTY && effectSkill.getSkillType() == L2SkillType.BUFF)) {
	...
	public int giveAvailableSkills(boolean forceAll) {
		...
		while (skills.length > unLearnable) {
			...
			for (L2SkillLearn s : skills) {
				...
				if (sk == null || (sk.getId() == L2Skill.SKILL_DIVINE_INSPIRATION || sk.getId() == L2Skill.SKILL_DIVINE_EXPANSION) &&
	...
	/**
	 * Return the PcInventory Inventory of the L2PcInstance contained in inventory.<BR><BR>
	 */
	@Override
	...
	public void setProtection(boolean protect) {
		if (Config.DEVELOPER && (protect || protectEndTime > 0)) {
			Log.warning(getName() + ": Protection " +
	...
	public void setTeleportProtection(boolean protect) {
		if (Config.DEVELOPER && (protect || teleportProtectEndTime > 0)) {
			Log.warning(getName() + ": Tele Protection " +
		...
		switch (skill.getSkillType()) {
			...
			case SUMMON: {
				...
				if (!getSummons().isEmpty() &&
						(getMaxSummonPoints() == 0 || getSpentSummonPoints() + ((L2SkillSummon) skill).getSummonPoints() > getMaxSummonPoints())) {
					canSummon = false;
				}
	 ...
	 * <p>
	 * <FONT COLOR=#FF0000><B> <U>Caution</U> : This method DOESN'T SEND current HP and MP to all L2PcInstance of the statusListener</B></FONT><BR><BR>
	 */
	@Override
	public void broadcastStatusUpdate(L2Character causer, StatusUpdateDisplay display) {
		...
		if (party != null && (needCpUpdate || needHpUpdate || needMpUpdate(352))) {
			if (Config.DEBUG) {
				Log.fine("Send status for party window of " + getObjectId() + " (" + getName() + ") to his party. CP: " + getCurrentCp() + " HP: " +
	 ...
	 * Others L2PcInstance in the detection area of the L2PcInstance are identified in <B>knownPlayers</B>.
	...
	/**
	 * Return the createList object of the L2PcInstance.<BR><BR>
	 */
	...
	/**
	 * Return the clan object of the L2PcInstance.<BR><BR>
	 */
	...
	/**
	 * Set the party object of the L2PcInstance (without joining it).<BR><BR>
	 */
	...
	/**
	 * Return the party object of the L2PcInstance.<BR><BR>
	 */
	@Override
	...
	private static L2PcInstance restore(int objectId) {
		...
		try {
			...
			if (rset.next()) {
				...
				// Add the L2PcInstance object in allObjects
	 ...
	 * <p>
	 * <B><U> Concept</U> :</B><BR><BR>
	 * All skills own by a L2PcInstance are identified in <B>skills</B><BR><BR>
	 * <p>
	 * <B><U> Actions</U> :</B><BR><BR>
	 ...
	 * <p>
	 * <B><U> Concept</U> :</B><BR><BR>
	 * All skills own by a L2Character are identified in <B>skills</B><BR><BR>
	 * <p>
	 * <B><U> Actions</U> :</B><BR><BR>
	...
	public boolean useMagic(L2Skill skill, boolean forceUse, boolean dontMove) {
		...
		if (!canCastNow(skill)) {
			...
			if (currentSkill != null &&
					(skill.getId() == currentSkill.getSkillId() || currentSkill.getSkill().getSkillType() == L2SkillType.CLASS_CHANGE)) {
				sendPacket(ActionFailed.STATIC_PACKET);
				return false;
			}
	...
	private boolean checkUseMagicConditions(L2Skill skill, boolean forceUse, boolean dontMove) {
		...
		if (target instanceof L2DoorInstance) {
			boolean isCastle = ((L2DoorInstance) target).getCastle() != null && ((L2DoorInstance) target).getCastle().getCastleId() > 0 &&
					((L2DoorInstance) target).getCastle().getSiege().getIsInProgress() && ((L2DoorInstance) target).getIsShowHp();
			boolean isFort = ((L2DoorInstance) target).getFort() != null && ((L2DoorInstance) target).getFort().getFortId() > 0 &&
					((L2DoorInstance) target).getFort().getSiege().getIsInProgress() && !((L2DoorInstance) target).getIsShowHp();
		...
		if (skill.isOffensive()) {
			...
			if (target != this && !(target instanceof L2NpcInstance) && !isInDuel() && target instanceof L2PcInstance &&
					!((L2PcInstance) target).isInDuel() && !target.isAttackable() && !getAccessLevel().allowPeaceAttack()) {
		...
		if (skill.getSkillBehavior() != L2SkillBehaviorType.ATTACK && !skill.isOffensive() && target instanceof L2MonsterInstance && !forceUse &&
				!skill.isNeutral()) {
	...
	public void addCubic(int id,
		...
		L2CubicInstance cubic =
				new L2CubicInstance(this, id, level, (int) matk, activationtime, activationchance, maxcount, totalLifetime, givenByOther);
	...
	public void confirmUseMobSummonItem(L2ItemInstance item) {
		...
		if (mobId == 0) {
			...
			if (target instanceof L2MonsterInstance && !(target instanceof L2ArmyMonsterInstance) && !(target instanceof L2ChessPieceInstance) &&
					!(target instanceof L2EventGolemInstance) && !((L2MonsterInstance) target).isRaid() &&
				...
				forceIsCasting(TimeController.getGameTicks() + 20000 / TimeController.MILLIS_IN_TICK);
					...
					(((L2MonsterInstance) target).getCollisionHeight() >= 50 || ((L2MonsterInstance) target).getCollisionRadius() >= 30)) {
	...
	class MobPetSummonFinalizer implements Runnable {
		...
		public void run() {
			...
			setIsCastingNow(false);
			L2MobSummonInstance summon = new L2MobSummonInstance(IdFactory.getInstance().getNextId(), npcTemplate, L2PcInstance.this, item);
	...
	public boolean isCastingProtected() {
		return isCastingNow() && getLastSkillCast() != null && (getLastSkillCast().getTargetType() == L2SkillTargetType.TARGET_HOLY ||
				getLastSkillCast().getTargetType() == L2SkillTargetType.TARGET_FLAGPOLE || getLastSkillCast().getId() == 1050);
	}
	...
	public boolean isAlly(L2Character target) {
		return target instanceof L2PcInstance &&
				(getParty() != null && target.getParty() == getParty() || getClanId() != 0 && ((L2PcInstance) target).getClanId() == getClanId() ||
	...
	public boolean isEnemy(L2Character target) {
		return /*(target instanceof L2MonsterInstance && target.isInCombat())
				...
				(getAllyId() == 0 || ((L2PcInstance) target).getAllyId() != getAllyId()) && !target.isInsideZone(L2Character.ZONE_SIEGE) &&
	...
	public class CaptchaTask implements Runnable {
		...
		@Override
		public void run() {
			if (/*times == 0 ||*/getTarget() instanceof L2Attackable || isFishing()) {
				//times++;
				setBotLevel(0);
