...
public class L2SummonInstance extends L2Summon {
	...
	public L2SummonInstance(int objectId, L2NpcTemplate template, L2PcInstance owner, L2Skill skill) {
		...
		if (Config.DEBUG && itemConsumeCount != 0) {
			Log.warning("L2SummonInstance: Item Consume ID: " + itemConsumeId + ", Count: " + itemConsumeCount + ", Rate: " + itemConsumeSteps +
					" times.");
		}
		if (Config.DEBUG) {
	...
	static class SummonLifetime implements Runnable {
		...
		public void run() {
			if (Config.DEBUG) {
				Log.warning("L2SummonInstance: " + summon.getTemplate().Name + " (" + activeChar.getName() + ") run task.");
			}
	...
	public void unSummon(L2PcInstance owner) {
		...
		if (summonLifeTask != null) {
			...
			if (summonPrice != null) {
				for (L2Abnormal e : owner.getAllEffects()) {
