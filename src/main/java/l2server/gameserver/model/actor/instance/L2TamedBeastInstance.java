...
public final class L2TamedBeastInstance extends L2FeedableBeastInstance {
	...
	public void onOwnerGotAttacked(L2Character attacker) {
		...
		// for lower HP ranges, heal or recharge the owner with 1 skill use per attack.
			...
			for (L2Skill skill : skills.values()) {
				// if the skill is a buff, check if the owner has it already [  owner.getEffect(L2Skill skill) ]
				if (Rnd.get(5) < chance && (skill.getSkillType() == L2SkillType.HEAL || skill.getSkillType() == L2SkillType.BALANCE_LIFE ||
						skill.getSkillType() == L2SkillType.HEAL_PERCENT || skill.getSkillType() == L2SkillType.HEAL_STATIC ||
						skill.getSkillType() == L2SkillType.COMBATPOINTHEAL || skill.getSkillType() == L2SkillType.MANAHEAL ||
