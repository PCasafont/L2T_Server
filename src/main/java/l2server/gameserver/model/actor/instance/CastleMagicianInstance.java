 ...
 */
public class CastleMagicianInstance extends NpcInstance implements L2SquadTrainer {
	...
	public void onBypassFeedback(Player player, String command) {
		if (command.startsWith("Chat")) {
			...
			if (player.getClan() != null) {
				...
				if (clanLeader.getFirstEffect(AbnormalType.CLAN_GATE) != null) {
			...
			if (!player.isClanLeader() && tendency != 0) {
				...
				if (player.getInventory().getItemByItemId(cloakId) == null) {
					Item cloak = player.getInventory().addItem("Members Cloak", cloakId, 1, player, this);
	...
	/* (non-Javadoc)
	 * @see l2server.gameserver.model.actor.L2SquadTrainer#showSubUnitSkillList(l2server.gameserver.model.actor.instance.Player)
	 */
	@Override
