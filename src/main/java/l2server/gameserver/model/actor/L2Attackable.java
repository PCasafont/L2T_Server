			...
			//synchronized (getAggroList())
			{
				...
				for (AggroInfo info : getAggroList().values()) {
					...
					if (attacker instanceof L2SummonInstance ||
							attacker instanceof L2PetInstance && ((L2PetInstance) attacker).getPetLevelData().getOwnerExpTaken() > 0) {
							...
							if (!attacker.isDead()) {
								...
								if (attacker instanceof L2PcInstance) {
									...
									if (pcAttacker.getSkillLevelHash(467) > 0) {
										L2Skill skill = SkillTable.getInstance().getInfo(467, attacker.getSkillLevelHash(467));
