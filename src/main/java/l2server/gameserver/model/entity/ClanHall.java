		...
		try {
			...
			while (rs.next()) {
				functions.put(rs.getInt("type"),
						new ClanHallFunction(rs.getInt("type"),
								rs.getInt("lvl"),
								rs.getInt("lease"),
								0,
								rs.getLong("rate"),
			...
			try {
				...
				if (ClanTable.getInstance().getClan(getOwnerId()).getWarehouse().getItemByItemId(Config.CH_BID_ITEMID) != null &&
					...
					if (time > paidUntil + chRate) {
						...
						updateDb();
						SystemMessage sm =
								SystemMessage.getSystemMessage(SystemMessageId.PAYMENT_FOR_YOUR_CLAN_HALL_HAS_NOT_BEEN_MADE_PLEASE_MAKE_PAYMENT_TO_YOUR_CLAN_WAREHOUSE_BY_S1_TOMORROW);
