...
public class Auction {
		...
		try {
			...
			while (rs.next()) {
				...
				bidders.put(rs.getInt("bidderId"),
						new Bidder(rs.getString("bidderName"), rs.getString("clan_name"), rs.getLong("maxBid"), rs.getLong("time_bid")));
			}
	...
	private boolean takeItem(Player bidder, long quantity) {
		...
		if (Config.CH_BID_ITEMID == 57) {
			...
			bidder.sendMessage(
					"There are not enough " + ItemTable.getInstance().getTemplate(Config.CH_BID_ITEMID).getName() + "s in the clan hall warehouse.");
		}
		return false;
	}
	...
	private void updateInDB(Player bidder, long bid) {
		...
		try {
			...
			if (bidders.get(highestBidderId) == null) {
				bidders.put(highestBidderId,
						new Bidder(highestBidderName, bidder.getClan().getName(), bid, Calendar.getInstance().getTimeInMillis()));
			} else {
