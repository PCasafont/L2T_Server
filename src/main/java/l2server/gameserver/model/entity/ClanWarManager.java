...
public class ClanWarManager {
		...
		try {
			...
			while (rset.next()) {
				...
				if (warstate != null) {
					...
					ClanWar war = new ClanWar(rset.getInt("clan1"),
							rset.getInt("clan2"),
							rset.getInt("clan1_score"),
							rset.getInt("clan2_score"),
							rset.getInt("clan1_war_declarator"),
							rset.getInt("clan2_war_declarator"),
							rset.getInt("clan1_deaths_for_war"),
							rset.getInt("clan1_shown_score"),
							rset.getInt("clan2_shown_score"),
							warstate,
							rset.getInt("loserId"),
							rset.getInt("winnerId"),
							rset.getLong("start_time"),
							rset.getLong("end_time"),
							rset.getLong("delete_time"));
					wars.add(war);
	...
	public static class ClanWar {
		...
		public ClanWar(int clanId1,
		               int clanId2,
		               int clanScore1,
		               int clanScore2,
		               int clanWarDeclarator1,
		               int clanWarDeclarator2,
		               int clan1DeathsForWar,
		               int clan1ShownScore,
		               int clan2ShownScore,
		               WarState warState,
		               int loserId,
			...
			task = ThreadPoolManager.getInstance().scheduleGeneral(() -> {
				if (warState == WarState.DECLARED) {
					...
					clan1.broadcastMessageToOnlineMembers(
							"Clan war against " + clan2.getName() + " has been stopped. Didn't accept declaration!"); // TODO: System message.
				}
		...
		public int getElapsedTime() {
			switch (warState) {
				case DECLARED:
					return Config.PREPARE_MUTUAL_WAR_PERIOD * 3600 - (int) ((startTime - System.currentTimeMillis()) / 1000);
				case STARTED:
