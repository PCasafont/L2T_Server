 ...
 * @author Pere, Soul
 */
public class AWarriorAI extends APlayerAI {
	...
	public AWarriorAI(Creature creature) {
		super(creature);
	}
	...
	protected boolean interactWith(Creature target) {
		...
		} else if (target instanceof Player && ((Player) target).getClassId() == 146) {
		...
		if (player.getCurrentMp() > player.getMaxMp() * 0.7 || player.getCurrentHp() < player.getMaxHp() * 0.5 ||
			...
			if (target != null && 600 - player.getDistanceSq(target) > 100) {
				Skill skill = player.getKnownSkill(HURRICANE_RUSH);
