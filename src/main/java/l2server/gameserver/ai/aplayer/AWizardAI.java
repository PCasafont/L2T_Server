	...
	protected boolean interactWith(Creature target) {
		...
		//		|| player.getTarget() instanceof Playable)
		{
			for (Skill skill : player.getAllSkills()) {
				if (!skill.isOffensive() || skill.getTargetType() != SkillTargetType.TARGET_ONE || skill.getHitTime() > 5000) {
					continue;
				}
			...
			for (Skill skill : player.getAllSkills()) {
				...
				Skill magic = SkillTable.getInstance().getInfo(skill.getId() + stance, skill.getLevelHash());
				if (magic.getPower() > 150 && magic.getTargetType() == SkillTargetType.TARGET_ONE && player.useMagic(magic, true, false)) {
					break;
				}
			}
		}
