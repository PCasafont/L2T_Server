...
public class FortSiegeGuardAI extends CreatureAI implements Runnable {
	...
	private boolean autoAttackCondition(Creature target) {
		...
		if (target == null || target instanceof DefenderInstance || target instanceof NpcInstance || target instanceof DoorInstance ||
				target.isAlikeDead() || target instanceof FortBallistaInstance || target instanceof FortCommanderInstance ||
			...
			if (player == null || player.getClan() != null && player.getClan().getHasFort() == ((Npc) actor).getFort().getFortId()) {
				return false;
			}
		}
	...
	private void thinkActive() {
		...
		if (actor.getWalkSpeed() >= 0) {
			if (actor instanceof DefenderInstance) {
				((DefenderInstance) actor).returnHome();
			} else {
				((FortCommanderInstance) actor).returnHome();
			}
		}
	}
	...
	private void thinkAttack() {
		if (Config.DEBUG) {
			log.info("FortSiegeGuardAI.thinkAttack(); timeout=" + (attackTimeout - TimeController.getGameTicks()));
		}
		...
		for (Creature cha : actor.getKnownList().getKnownCharactersInRadius(1000)) {
			...
			if (!(cha instanceof Npc)) {
				if (selfAnalysis.hasHealOrResurrect && cha instanceof Player &&
						((Npc) actor).getFort().getSiege().checkIsDefender(((Player) cha).getClan())) {
					// heal friends
	...
	private void attackPrepare() {
		...
		if (actor instanceof FortCommanderInstance) {
			sGuard = (FortCommanderInstance) actor;
		} else {
			sGuard = (DefenderInstance) actor;
		}
		Creature attackTarget = getAttackTarget();
