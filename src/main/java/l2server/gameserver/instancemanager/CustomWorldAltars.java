...
public class CustomWorldAltars {
	...
	private class WorldAltarsInfo {
		...
		private void respawnAltar() {
			...
			spawnTask = ThreadPoolManager.getInstance().scheduleGeneral(new AltarRespawn(), (baseRespawnMinutes + randomSpawnTime) * 60000);
		...
		private void spawnBoss() {
			...
			bossNpc = NpcUtil.addSpawn(rndBoss,
					currentSpawn.getSpawn().getX() + Rnd.get(300),
					currentSpawn.getSpawn().getY() + Rnd.get(300),
					currentSpawn.getSpawn().getZ() + 50,
					currentSpawn.getSpawn().getHeading(),
					false,
					0,
					true,
					0);
			...
			altarNpc.broadcastPacket(new ExShowScreenMessage(
					altarNpc.getTemplate().Name + "'s seal has been broken and " + bossNpc.getTemplate().Name + " has been liberated!", 5000));
		}
			...
			public void run() {
				synchronized (spawnInfo) {
					...
					if (randomSpawn != null) {
						...
						altarNpc = NpcUtil.addSpawn(altarId,
								currentSpawn.getSpawn().getX(),
								currentSpawn.getSpawn().getY(),
								currentSpawn.getSpawn().getZ(),
								currentSpawn.getSpawn().getHeading(),
								false,
								0,
								true,
						...
						Announcements.getInstance()
								.announceToAll(
										"World Altars: The altar " + altarNpc.getName() + " has respawned in " + currentSpawn.getSpawnName() + "!");
					}
				}
			}
		}
	...
	public String getAltarsInfo(boolean isGM) {
		...
		for (WorldAltarsInfo i : altarsList) {
			sb.append("<table width=710 border=0 bgcolor=" + (i.isCursed() ? "FF6F79" : "999999") + "><tr><td align=center FIXWIDTH=710>" +
					i.getAltarName() + "</td></tr></table>");
			sb.append("<table width=710 height=150 border=0><tr><td><table><tr><td><img src=\"Crest.pledge_crest_" + Config.SERVER_ID + "_" +
					i.getAltarImageId() + "\" width=256 height=128></td></tr></table></td>");
			
			sb.append("<td FIXWIDTH=450><table width=450 border=0>");
			if (i.getCurrentSpawn() != null) {
				sb.append("<tr><td>Spawn Location:</td><td><a action=\"" +
						(isGM ? "bypass -h admin_move_to " : "bypass _bbscustom;action;showRadar; ") + "" + i.getCurrentSpawn().getSpawn().getX() +
						" " + i.getCurrentSpawn().getSpawn().getY() + " " + i.getCurrentSpawn().getSpawn().getZ() + "\"> " +
						i.getCurrentSpawn().getSpawnName() + "</a></td></tr>");
			...
			sb.append("<tr><td FIXWIDTH=150>Altar Respawn:</td><td FIXWIDTH=300> " + baseRespawnMinutes + " minute(s) + " + randomRespawnMinutes +
					" random.</td></tr>");
