...
public class SkillSummon extends Skill {
	...
	public boolean checkCondition(Creature activeChar, WorldObject target, boolean itemOrWeapon) {
		if (activeChar instanceof Player) {
			...
			} else if (!(this instanceof SkillTrap))// Normal summon
			{
	...
	public void useSkill(Creature caster, WorldObject[] targets) {
		...
		if (isCubic) {
			...
			if (targets.length > 1 || targets.length == 1 && targets[0] != activeChar) // Not self cubic skill
			{
				for (WorldObject obj : targets) {
					...
					if (cubicMastery == 0 && !player.getCubics().isEmpty()) {
						...
						for (CubicInstance c : player.getCubics().values()) {
							c.stopAction();
							c = null;
						}
					...
					if (player.getCubics().containsKey(npcId)) {
						CubicInstance cubic = player.getCubic(npcId);
						cubic.stopAction();
					...
					if (hasEffects()) {
						getEffects(player, player, new Env((byte) 0, Item.CHARGED_NONE));
					}
					player.broadcastUserInfo();
				}
				return;
			...
			// Normal cubic skill
			{
				...
				if (activeChar.getCubics().containsKey(npcId)) {
					CubicInstance cubic = activeChar.getCubic(npcId);
					cubic.stopAction();
					cubic.cancelDisappear();
				...
				if (activeChar.getCubics().size() > cubicMastery) {
					if (Config.DEBUG) {
						log.debug("player can't summon any more cubics. ignore summon skill");
					}
				...
				if (hasEffects()) {
					getEffects(activeChar, activeChar, new Env((byte) 0, Item.CHARGED_NONE));
				}
		...
		if (getId() == 10532) // Clone Attack
		{
			...
			int px = target.getX();
			int py = target.getY();
			double ph = Util.convertHeadingToDegree(((Creature) target).getHeading());
		...
		for (int i = 0; i < summonAmount; i++) {
			...
			if (summonTemplate == null) {
				log.warn("Summon attempt for nonexisting NPC ID:" + npcId + ", skill ID:" + getId());
			...
			if (summonTemplate.Type.equalsIgnoreCase("L2SiegeSummon")) {
				...
				summon = new MerchantSummonInstance(IdFactory.getInstance().getNextId(), summonTemplate, activeChar, this);
			} else if (summonTemplate.Name.equalsIgnoreCase("Incarnation")) {
				summon = new CloneInstance(IdFactory.getInstance().getNextId(), summonTemplate, activeChar, this);
			} else {
			...
			if (!(summon instanceof MerchantSummonInstance) && !(summon instanceof CloneInstance)) {
			...
			if (summon instanceof CloneInstance) {
