...
public class SqlToXml {
	public static void races() {
		...
		try {
			...
			while (rset.next()) {
				...
				while (rset2.next()) {
					content += "\t\t<template type=\"" + (fighter ? "Fighter" : "mage") + "\"" + " STR=\"" + rset2.getInt("STR") + "\"" + " CON=\"" +
							rset2.getInt("CON") + "\"" + " DEX=\"" + rset2.getInt("DEX") + "\"" + " INT=\"" + rset2.getInt("_INT") + "\"" +
							" WIT=\"" + rset2.getInt("WIT") + "\"" + " MEN=\"" + rset2.getInt("MEN") + "\"" + " basePAtk=\"" + rset2.getInt("P_ATK") +
							"\"" + " baseMAtk=\"" + rset2.getInt("M_ATK") + "\"" + " basePDef=\"" + rset2.getInt("P_DEF") + "\"" + " baseMDef=\"" +
							rset2.getInt("M_DEF") + "\"" + " basePAtkSpd=\"" + rset2.getInt("P_SPD") + "\"" + " baseMAtkSpd=\"" +
							rset2.getInt("M_SPD") + "\"" + " baseAccuracy=\"" + rset2.getInt("ACC") + "\"" + " baseCritical=\"" +
							rset2.getInt("CRITICAL") + "\"" + " baseEvasion=\"" + rset2.getInt("EVASION") + "\"" + " baseMoveSpd=\"" +
							rset2.getInt("MOVE_SPD") + "\"" + " mColRadius=\"" + rset2.getFloat("M_COL_R") + "\"" + " mColHeight=\"" +
							rset2.getFloat("M_COL_H") + "\"" + " fColRadius=\"" + rset2.getFloat("F_COL_R") + "\"" + " fColHeight=\"" +
							rset2.getFloat("F_COL_H") + "\">\r\n";
					...
					while (rset3.next()) {
						content += "\t\t\t<creationItem id=\"" + rset3.getInt("itemId") + "\"" + " count=\"" + rset3.getInt("amount") + "\"" +
								" equipped=\"" + rset3.getString("equipped") + "\" /> <!-- " +
			...
			while (rset.next()) {
				content += "\t<creationItem id=\"" + rset.getInt("itemId") + "\"" + " count=\"" + rset.getInt("amount") + "\"" + " equipped=\"" +
						rset.getString("equipped") + "\" /> <!-- " + ItemTable.getInstance().createDummyItem(rset.getInt("itemId")).getName() +
						" -->\r\n";
			}
	...
	public static void classes() {
		...
		try {
			...
			while (rset.next()) {
				content += "\t<class id=\"" + rset.getInt("c.classId") + "\"" + " name=\"" + rset.getString("className") + "\"" + " level=\"" +
						rset.getInt("class_lvl") + "\"" + " baseHp=\"" + rset.getInt("defaulthpbase") + "\"" + " hpAdd=\"" +
						rset.getInt("defaulthpadd") + "\"" + " hpMod=\"" + rset.getInt("defaulthpmod") + "\"" + " baseMp=\"" +
						rset.getInt("defaultmpbase") + "\"" + " mpAdd=\"" + rset.getInt("defaultmpadd") + "\"" + " mpMod=\"" +
						rset.getInt("defaultmpmod") + "\"" + " baseCp=\"" + rset.getInt("defaultcpbase") + "\"" + " cpAdd=\"" +
						rset.getInt("defaultcpadd") + "\"" + " cpMod=\"" + rset.getInt("defaultcpmod") + "\">\r\n";
				...
				while (rset2.next()) {
					...
					content += "\" reqSp=\"" + rset2.getInt("sp") + "\"" + " minLevel=\"" + rset2.getInt("min_level") + "\"" +
							...
							SkillTable.getInstance().getInfo(rset2.getInt("skill_id"), rset2.getInt("level")).getName() + " -->\r\n";
				}
		...
		try {
			...
			for (SpawnLine l1 : lines) {
				...
				for (SpawnLine l2 : lines) {
					...
					double distance =
							Math.sqrt((l2.Point.X - l1.Point.X) * (l2.Point.X - l1.Point.X) + (l2.Point.Y - l1.Point.Y) * (l2.Point.Y - l1.Point.Y));
			...
			while (rset.next()) {
				...
				for (int i = 0; i < skills.length; i += 2) {
					content += "\t\t<skill id=\"" + skills[i] + "\" levels=\"" + skills[i + 1] + "\" /> <!-- " +
							SkillTable.getInstance().getInfo(Integer.valueOf(skills[i]), Integer.valueOf(skills[i + 1])).getName() + " -->\r\n";
				}
	...
	public static void henna() {
		...
		try {
			...
			while (rset.next()) {
				content += "\t<henna symbolId=\"" + rset.getString("symbol_id") + "\"" + " dyeId=\"" + rset.getString("dye_id") + "\"" + " name=\"" +
						ItemTable.getInstance().getTemplate(rset.getInt("dye_id")).getName().replace('<', '(').replace('>', ')') + "\"" +
	...
	public static void fortSpawns() {
		...
		try {
			...
			for (Fort fort : FortManager.getInstance().getForts()) {
				...
				ResultSet rset = statement.executeQuery();
				while (rset.next()) {
					content += "\t<spawn npcId=\"" + rset.getInt("npcId") + "\" x=\"" + rset.getInt("x") + "\" y=\"" + rset.getInt("y") + "\" z=\"" +
							rset.getInt("z") + "\" heading=\"" + rset.getInt("heading") + "\" respawn=\"60\" />\r\n";
				}
				...
				while (rset.next()) {
					content +=
							"\t\t<spawn npcId=\"" + rset.getInt("npcId") + "\" x=\"" + rset.getInt("x") + "\" y=\"" + rset.getInt("y") + "\" z=\"" +
									rset.getInt("z") + "\" heading=\"" + rset.getInt("heading") + "\" respawn=\"60\" />\r\n";
				}
				...
				for (int i = 1; i < 5; i++) {
					String spawnParams = siegeSettings.getProperty(fort.getName().replace(" ", "") + "Commander" + i, "");
					...
					content += "\t\t<spawn npcId=\"" + npcId + "\" x=\"" + x + "\" y=\"" + y + "\" z=\"" + z + "\" heading=\"" + heading +
							"\" respawn=\"60\" />\r\n";
				}
				content += "\t</specificSpawnList>\r\n";
				...
				while (rset.next()) {
					content +=
							"\t\t<spawn npcId=\"" + rset.getInt("npcId") + "\" x=\"" + rset.getInt("x") + "\" y=\"" + rset.getInt("y") + "\" z=\"" +
									rset.getInt("z") + "\" heading=\"" + rset.getInt("heading") + "\" respawn=\"" + rset.getInt("respawnDelay") +
									"\" />\r\n";
				}
