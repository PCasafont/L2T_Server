	...
	public static void giveRewardsAndReset() {
		...
		for (Entry<Integer, dmginfo> info : dmgIinfo.entrySet()) {
			if (info.getValue().getNewName() != null && !info.getValue().getNewName().equals("")) {
				...
				Message msg =
						new Message(-1, charid, false, eventname, "Congratulations, you was classified on Damage Ranking, here is your reward!", 0);
	...
	public static void load() {
		...
		try {
			...
			PreparedStatement statement =
					con.prepareStatement("SELECT `classid`, `newdmg`, `newname`, `playerid`, `externalIP`, `internalIP` FROM `dmg_data`");
			...
			while (rs.next()) {
				...
				info.setdmg(rs.getInt("classid"),
						rs.getInt("newdmg"),
						rs.getString("newname"),
						rs.getInt("playerid"),
						rs.getString("externalIP"),
						rs.getString("internalIP"));
