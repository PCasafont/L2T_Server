...
public final class RequestPostAttachment extends L2GameClientPacket {
	...
	public void runImpl() {
		...
		for (Item item : attachments.getItems()) {
			...
			if (msg.getSenderId() > -1 && item.getOwnerId() != msg.getSenderId()) {
				Util.handleIllegalPlayerAction(activeChar,
						"Player " + activeChar.getName() + " tried to get wrong item (ownerId != senderId) from attachment!",
			...
			if (!item.getLocation().equals(ItemLocation.MAIL)) {
				Util.handleIllegalPlayerAction(activeChar,
						"Player " + activeChar.getName() + " tried to get wrong item (Location != MAIL) from attachment!",
						Config.DEFAULT_PUNISH);
				return;
			}
		...
		Util.logToFile(activeChar.getName() + " is retrieving items from Mail[" + msg.getId() + "].",
				"Logs/Mails/" + activeChar.getName() + "_Retrieve_Mails",
				"txt",
				true,
				true);
		...
		for (Item item : attachments.getItems()) {
			...
			if (msg.getSenderId() > -1 && item.getOwnerId() != msg.getSenderId()) {
				Util.logToFile("- " + activeChar.getName() + " could not retrieve " + item.getName() + " [" + item.getCount() + "] - MailSenderId[" +
								msg.getSenderId() + "] differs from ItemOwnerId[" + item.getOwnerId() + "].",
						"Logs/Mails/" + activeChar.getName() + "_Retrieve_Mails",
						"txt",
						true,
						false);
			...
			final Item newItem = attachments.transferItem(attachments.getName() + " from " + msg.getSenderName(),
					...
					null);
			if (newItem == null) {
				Util.logToFile("- " + activeChar.getName() + " could not retrieve " + item.getName() + " [" + item.getCount() +
						"] - Item was NULL after transfer.", "Logs/Mails/" + activeChar.getName() + "_Retrieve_Mails", "txt", true, false);
				return;
			}
			...
			Util.logToFile("- " + activeChar.getName() + " retrieved " + item.getName() + "[" + newItem.getCount() + "].",
					"Logs/Mails/" + activeChar.getName() + "_Retrieve_Mails",
					"txt",
					true,
					false);
