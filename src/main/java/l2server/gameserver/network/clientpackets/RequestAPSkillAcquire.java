...
public final class RequestAPSkillAcquire extends L2GameClientPacket {
	...
	protected void runImpl() {
		...
		for (int skillId : abilities.keys()) {
			...
			if (level > ability.getMaxLevel() || level > remainingPoints || counts[ability.getType() - 1] < ability.getReqPoints() ||
					ability.getReqSkill() > 0 &&
							(!abilities.containsKey(ability.getReqSkill()) || abilities.get(ability.getReqSkill()) < ability.getReqSkillLvl())) {
				sendPacket(new ExAcquireAPSkillList(getClient().getActiveChar(), false));
				return;
			}
