				...
				for (XmlNode npcNode : doc.getChildren()) {
					if (npcNode.getName().equalsIgnoreCase("npc")) {
						...
						for (XmlNode propertyNode : npcNode.getChildren()) {
							...
							if (propertyNode.getName().equalsIgnoreCase("spoilDrop") || propertyNode.getName().equalsIgnoreCase("itemDrop")) {
		...
		try {
			for (int i = 0; i < 1000; i++) {
				...
				String fileName = ("00000" + i * 1000).substring(String.valueOf(i * 1000).length()) + "-" +
						("00000" + ((i + 1) * 1000 - 1)).substring(String.valueOf((i + 1) * 1000 - 1).length()) + ".xml";
				BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(
						Config.DATAPACK_ROOT + "/" + Config.DATA_FOLDER + "npcs/" + fileName), "UTF-8"));
				...
				for (int j = i * 1000; j < (i + 1) * 1000; j++) {
					...
					String baseNode =
							"\t<npc" + t.getXmlNpcId() + t.getXmlTemplateId() + t.getXmlName() + t.getXmlServerSideName() + t.getXmlTitle() +
									t.getXmlServerSideTitle() + t.getXmlLevel() + t.getXmlType() + t.getXmlInteractionDistance() +
									t.getXmlAttackRange() + t.getXmlMaxHp() + t.getXmlHpReg() + t.getXmlMaxMp() + t.getXmlMpReg() + t.getXmlSTR() +
									t.getXmlCON() + t.getXmlDEX() + t.getXmlINT() + t.getXmlWIT() + t.getXmlMEN() + t.getXmlExp() + t.getXmlSp() +
									t.getXmlPAtk() + t.getXmlMAtk() + t.getXmlPDef() + t.getXmlMDef() + t.getXmlPAtkSpd() + t.getXmlMAtkSpd() +
									t.getXmlCritical() + t.getXmlMCritical() + t.getXmlWalkSpd() + t.getXmlRunSpd() + t.getXmlRandomWalk() +
									t.getXmlAggessive() + t.getXmlAggroRange() + t.getXmlCanSeeThroughSilentMove() + t.getXmlRHand() +
									t.getXmlLHand() + t.getXmlExtraDropGroup() + t.getXmlCollisionRadius() + t.getXmlCollisionHeight() +
									t.getXmlTargetable() + t.getXmlShowName() + t.getXmlIsLethalImmune() + t.getXmlIsDebuffImmune() +
									t.getXmlCanBeChampion() + t.getXmlIsNonTalking() + t.getXmlElemAtk() + t.getXmlElemRes() + t.getXmlAIType() +
									t.getXmlSkillChance() + t.getXmlPrimaryAttack() + t.getXmlCanMove() + t.getXmlMinRangeSkill() +
									t.getXmlMinRangeChance() + t.getXmlMaxRangeSkill() + t.getXmlMaxRangeChance() + t.getXmlSoulshots() +
									t.getXmlSpiritshots() + t.getXmlSSChance() + t.getXmlSpSChance() + t.getXmlIsChaos() + t.getXmlClan() +
									t.getXmlClanRange() + t.getXmlEnemy() + t.getXmlEnemyRange() + t.getXmlDodge() + t.getXmlMinSocial1() +
									t.getXmlMaxSocial1() + t.getXmlMinSocial2() + t.getXmlMaxSocial2() + ">\r\n";
					...
					if (t.getMinionData() != null) {
						for (L2MinionData md : t.getMinionData()) {
							...
							out.write("\t\t<minion npcId=\"" + md.getMinionId() + "\" min=\"" + md.getAmountMin() + "\" max=\"" + md.getAmountMax() +
									"\" />" + comment + "\r\n");
						}
					}
					...
					for (SpawnData s : t.getSpawns()) {
						out.write("\t\t<spawn x=\"" + s.X + "\" y=\"" + s.Y + "\" z=\"" + s.Z + "\" " + "heading=\"" + s.Heading + "\" respawn=\"" +
								s.Respawn + "\"");
