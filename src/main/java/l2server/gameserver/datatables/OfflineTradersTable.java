...
public class OfflineTradersTable {
	...
	private static final String SAVE_PRICES = "INSERT INTO character_offline_trade_item_prices (`charId`,`item`,`priceId`,`count`) VALUES (?,?,?,?)";
	...
	private static final String LOAD_OFFLINE_PRICES = "SELECT * FROM character_offline_trade_item_prices WHERE charId = ? AND item = ?";
	...
	public static void storeOffliners() {
		...
		try {
			...
			boolean checkInactiveStores =
					Config.isServer(Config.TENKAI) && System.currentTimeMillis() - Server.dateTimeServerStarted.getTimeInMillis() > 36000000;
			for (L2PcInstance pc : L2World.getInstance().getAllPlayers().values()) {
				try {
					...
					Log.log(Level.WARNING,
							"OfflineTradersTable[storeTradeItems()]: Error while saving offline trader: " + pc.getObjectId() + " " + e,
							e);
				}
			}
			stm.close();
				...
				while (rs.next()) {
					...
					try {
						...
						switch (type) {
							...
							case L2PcInstance.STORE_PRIVATE_CUSTOM_SELL:
								while (items.next()) {
									TradeItem item = player.getCustomSellList().addItem(items.getInt(2), items.getLong(3));
									if (item == null) {
										throw new NullPointerException();
									}
