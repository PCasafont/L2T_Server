...
public class EnchantItemTable implements Reloadable {
		...
		public final boolean isValid(L2ItemInstance enchantItem) {
			...
			switch (type2) {
				...
				case L2Item.TYPE2_ACCESSORY:
					...
					if (targetType != EnchantTargetType.ARMOR ||
							Config.ENCHANT_MAX_JEWELRY > 0 && enchantItem.getEnchantLevel() >= Config.ENCHANT_MAX_JEWELRY) {
						return false;
					}
					break;
	...
	private final Map<Integer, EnchantScroll> scrolls = new HashMap<>();
	private final Map<Integer, EnchantSupportItem> supports = new HashMap<>();
	...
	public boolean reload() {
		...
		for (XmlNode d : doc.getChildren()) {
			if (d.getName().equalsIgnoreCase("enchantScroll")) {
				...
				scrolls.put(id, new EnchantScroll(targetType, isBlessed, isCrystal, isSafe, grade, maxLevel, extraChance, onlyOnItem));
